...
model IdealHeatingCoolingWinter
  ...
  extends
    OCTExamples.ThermalZones_EnergyPlus_Examples.SmallOffice.Unconditioned;
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant THeaSet[5](each k(
      final unit="K",
      displayUnit="degC") = 293.15) "Set point temperature for heating"
    annotation (Placement(transformation(extent={{-120,70},{-100,90}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant THeaCoo[5](each k(
      final unit="K",
  ...
  Buildings.Controls.OBC.CDL.Continuous.MultiSum QHea_flow(nin=5)
    "Total heat flow rate"
    annotation (Placement(transformation(extent={{-30,90},{-10,110}})));
  Modelica.Blocks.Continuous.Integrator EHea
    "Heating energy"
    annotation (Placement(transformation(extent={{0,90},{20,110}})));
  Buildings.Controls.OBC.CDL.Continuous.MultiSum QCoo_flow(nin=5)
    "Total heat flow rate"
...
equation
  ...
  annotation (
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/EnergyPlus/Examples/SmallOffice/IdealHeatingCoolingWinter.mos" "Simulate and plot"),
    experiment(
      StartTime=432000,
      StopTime=864000,
      Tolerance=1e-06),
