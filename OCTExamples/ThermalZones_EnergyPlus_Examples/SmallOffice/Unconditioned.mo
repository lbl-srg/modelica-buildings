...
model Unconditioned
  ...
  Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(filNam=weaName,
      computeWetBulbTemperature=false) "Weather data reader"
    annotation (Placement(transformation(extent={{-80,40},{-60,60}})));
  Buildings.BoundaryConditions.WeatherData.Bus weaBus "Weather data bus"
    annotation (Placement(transformation(extent={{-50,40},{-30,60}})));
  OCTExamples.ThermalZones_EnergyPlus_Examples.SmallOffice.BaseClasses.Floor flo(
    redeclare package Medium = Medium,
    nor(T_start=275.15),
    wes(T_start=275.15),
    eas(T_start=275.15),
    sou(T_start=275.15),
    cor(T_start=275.15)) "One floor of the office building"
