...
model ThermalZone "Thermal zone model"
  ...
  final parameter Buildings.HeatTransfer.Data.Solids.Plywood matFur(x=0.15,
      nStaRef=5) "Material for furniture"
    annotation (Placement(transformation(extent={{160,120},{180,140}})));
  final parameter Buildings.HeatTransfer.Data.Solids.Concrete matCon(
    x=0.1,
    k=1.311,
    c=836,
    nStaRef=5) "Concrete"
    annotation (Placement(transformation(extent={{160,98},{180,118}})));
  final parameter Buildings.HeatTransfer.Data.Solids.Plywood matWoo(
    x=0.01,
    k=0.11,
    d=544,
    nStaRef=1) "Wood for exterior construction"
    annotation (Placement(transformation(extent={{160,78},{180,98}})));
  final parameter Buildings.HeatTransfer.Data.Solids.Generic matIns(
    x=0.087,
    k=0.049,
    c=836.8,
    d=265,
  ...
  final parameter Buildings.HeatTransfer.Data.Solids.GypsumBoard matGyp(
    x=0.0127,
    k=0.16,
    c=830,
    d=784,
  ...
  final parameter Buildings.HeatTransfer.Data.Solids.GypsumBoard matGyp2(
    x=0.025,
    k=0.16,
    c=830,
    d=784,
  ...
  OCTExamples.ScalableBenchmarks.BuildingVAV.BaseClasses.IntLoad intLoad(table=[0,
        0.1; ((gainFactor - 0.5) + 8)*3600,1.0; ((gainFactor - 0.5) + 18)*3600,
        0.1; 24*3600,0.1]) "Internal load schedule"
    annotation (Placement(transformation(extent={{-90,114},{-76,128}})));
  Buildings.Fluid.Sources.MassFlowSource_WeatherData souExf(
    redeclare package Medium = MediumA,
    use_m_flow_in=false,
  ...
  Buildings.Fluid.Sources.MassFlowSource_WeatherData souInf2(
    redeclare package Medium = MediumA,
    use_m_flow_in=false,
    m_flow=VInf_flow*1.2,
...
protected
  Buildings.Fluid.Sensors.EnthalpyFlowRate senEntFloSupAir(redeclare package
      Medium = MediumA, m_flow_nominal=0.2)
    "Supply air sensible enthalpy flow rate"
    annotation (Placement(transformation(extent={{-28,-8},{-12,8}})));
  Buildings.Fluid.Sensors.EnthalpyFlowRate senEntFloRetAir(redeclare package
      Medium = MediumA, m_flow_nominal=0.2)
    "Return air sensible enthalpy flow rate"
    annotation (Placement(transformation(extent={{-32,6},{-46,20}})));
  Buildings.Controls.OBC.CDL.Continuous.Max PHea "Heating power"
    annotation (Placement(transformation(extent={{80,94},{92,106}})));
  Buildings.Controls.OBC.CDL.Continuous.Min PCoo "Cooling power"
