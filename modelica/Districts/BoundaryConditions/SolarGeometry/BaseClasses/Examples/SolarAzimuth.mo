...
model SolarAzimuth "Test model for zenith angle"
  ...
  parameter Modelica.SIunits.Angle lat= 41.98*Modelica.Constants.pi/180
    "Latitude";
  Districts.BoundaryConditions.SolarGeometry.BaseClasses.SolarHourAngle solHouAng
    "Solar hour angle"
    annotation (Placement(transformation(extent={{20,-20},{40,0}})));
  Districts.BoundaryConditions.SolarGeometry.BaseClasses.ZenithAngle zen(lat=lat)
    "Zenith angle"
    annotation (Placement(transformation(extent={{60,40},{80,60}})));
  Districts.BoundaryConditions.SolarGeometry.BaseClasses.Declination decAng
    "Declination angle"
    annotation (Placement(transformation(extent={{-20,40},{0,60}})));
  Districts.BoundaryConditions.SolarGeometry.BaseClasses.SolarAzimuth solAzi(lat=lat)
    "Solar azimuth"
...
equation
  ...
  annotation (
    Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},{150,
            100}}), graphics),
    __Dymola_Commands(file="modelica://Districts/Resources/Scripts/Dymola/BoundaryConditions/SolarGeometry/BaseClasses/Examples/SolarAzimuth.mos"
        "Simulate and plot"));
end SolarAzimuth;
