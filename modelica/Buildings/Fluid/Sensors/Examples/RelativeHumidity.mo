...
model RelativeHumidity "Test model for relative humidity sensor"
 ...
 package Medium = Buildings.Media.PerfectGases.MoistAir "Medium model"
  ...
  Buildings.Fluid.Sensors.RelativeHumidity senRelHum(redeclare package Medium
      = Medium) annotation (Placement(transformation(extent={{20,40},{40,60}})));
...
equation
  ...
  connect(dp.port_a, massFlowRate.ports[1]) annotation (Line(
      points={{-5.55112e-16,20},{-10,20}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(senRelHum.port, dp.port_b) annotation (Line(
      points={{30,40},{30,20},{20,20}},
      color={0,127,255},
      smooth=Smooth.None));
