...
model DryEffectivenessNTU
  "Heat exchanger with effectiveness - NTU relation and not moisture condensation"
...
initial equation
  ...
  cp1_nominal = Medium1.specificHeatCapacityCp(
      Medium1.setState_pTX(Medium1.p_default, T_a1_nominal, Medium1.X_default));
  cp2_nominal = Medium2.specificHeatCapacityCp(
      Medium2.setState_pTX(Medium2.p_default, T_a2_nominal, Medium2.X_default));
  ...
  NTU_nominal  = if (eps_nominal > 0 and eps_nominal < 1) then
         Buildings.Fluid.HeatExchangers.BaseClasses.ntu_epsilonZ(eps=eps_nominal, Z=Z_nominal, flowRegime=flowRegime_nominal) else
         0;
...
<pre>
  Q_flow = QMax_flow * eps(NTU, Z, flowRegime),
</pre>
where
...
<p>
For a heat exchanger with humidity condensation, use
<a href=\"modelica://Buildings.Fluid.HeatExchangers.WetEffectivenessNTU\">
Buildings.Fluid.HeatExchangers.WetEffectivenessNTU</a>,
