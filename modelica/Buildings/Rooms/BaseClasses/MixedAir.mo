...
model MixedAir "Model for room air that is completely mixed"
  ...
  // Port definitions
  parameter Integer nPorts=0 "Number of fluid ports of this model" annotation (
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a glaUns[NConExtWin] if
    ...
    annotation (Placement(transformation(extent={{232,110},{252,130}}, rotation
          =0)));
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a glaSha[NConExtWin] if
    ...
    annotation (Placement(transformation(extent={{232,70},{252,90}}, rotation=0)));
  ...
  Buildings.HeatTransfer.Convection.Interior convConExt[NConExt](
    ...
    each conMod=conMod,
    each hFixed=hFixed) if haveConExt "Convective heat transfer"
    annotation (Placement(transformation(extent={{120,210},{100,230}})));
  Buildings.HeatTransfer.Convection.Interior convConExtWin[NConExtWin](
    final A=AConExtWinOpa,
    final til=datConExtWin.til,
    each conMod=conMod,
    each hFixed=hFixed) if haveConExtWin "Convective heat transfer"
  ...
  HeatTransfer.Windows.InteriorHeatTransfer convConWin[NConExtWin](
    ...
    final tauIRSha_glass=tauIRSha_glass) if haveConExtWin
  ...
  Buildings.HeatTransfer.Convection.Interior convConPar_a[nConPar](
    ...
    each conMod=conMod,
    each hFixed=hFixed) if haveConPar "Convective heat transfer"
  ...
  Buildings.HeatTransfer.Convection.Interior convConBou[nConBou](
    ...
    each conMod=conMod,
    each hFixed=hFixed) if haveConBou "Convective heat transfer"
    annotation (Placement(transformation(extent={{120,-170},{100,-150}})));
  Buildings.HeatTransfer.Convection.Interior convSurBou[nSurBou](
    final A=ASurBou,
    final til=surBou.til,
    each conMod=conMod,
    each hFixed=hFixed) if haveSurBou "Convective heat transfer"
    annotation (Placement(transformation(extent={{122,-230},{102,-210}})));
protected
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector theConConExt(final
      m=nConExt) if haveConExt
    "Thermal collector to convert from vector to scalar connector" annotation (
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector theConConExtWin(
    ...
    "Thermal collector to convert from vector to scalar connector" annotation (
      Placement(transformation(
        ...
        rotation=270,
        origin={48,180})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector theConConWin(final
      m=nConExtWin) if haveConExtWin
    "Thermal collector to convert from vector to scalar connector" annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector theConConPar_b(
    ...
    "Thermal collector to convert from vector to scalar connector" annotation (
      Placement(transformation(
        ...
        rotation=270,
        origin={50,-100})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector theConConBou(final
      m=nConBou) if haveConBou
    "Thermal collector to convert from vector to scalar connector" annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={50,-160})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector theConSurBou(final
      m=nSurBou) if haveSurBou
    "Thermal collector to convert from vector to scalar connector" annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
...
public
  ...
  final parameter Modelica.SIunits.TransmissionCoefficient tauIRSha_air[
  ...
  final parameter Boolean haveExteriorShade[NConExtWin]={datConExtWin[i].glaSys.haveExteriorShade
      for i in 1:NConExtWin}
    "Set to true if window has exterior shade (at surface a)"
    annotation (Dialog(group="Shading"));
  final parameter Boolean haveInteriorShade[NConExtWin]={datConExtWin[i].glaSys.haveInteriorShade
      for i in 1:NConExtWin}
  ...
  SolarRadiationExchange solRadExc(
    ...
    final tauGla=tauGlaSol) if haveConExtWin "Solar radiative heat exchange"
  ...
  Modelica.Blocks.Interfaces.RealInput QAbsSolSha_flow[NConExtWin](final unit=
        "W", quantity="Power") if haveConExtWin
  ...
  Modelica.Blocks.Interfaces.RealOutput HOutConExtWin[NConExtWin](unit="W/m2")
    if haveConExtWin "Outgoing solar radiation that strikes window" annotation
    (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={-200,-250})));
...
equation
  ...
  connect(theConSurBou.port_b, vol.heatPort) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  connect(vol.heatPort, heaPorAir) annotation (Line(
      points={{10,-200},{20,-200},{20,-90},{-150,-90},{-150,0},{-200,0},{-200,
          5.55112e-16},{-240,5.55112e-16}},
      color={191,0,0},
      smooth=Smooth.None));
  connect(convConWin.JOutUns, JOutUns) annotation (Line(
      points={{119,126},{196,126},{196,160},{250,160}},
      color={0,127,0},
      smooth=Smooth.None));
  connect(JInUns, convConWin.JInUns) annotation (Line(
      points={{248,144},{200,144},{200,124},{119,124}},
      color={0,0,0},
      pattern=LinePattern.None,
      smooth=Smooth.None));
  connect(convConWin.JOutSha, JOutSha) annotation (Line(
      points={{119,112},{200,112},{200,60},{250,60}},
      color={0,127,0},
      smooth=Smooth.None));
  connect(convConWin.JInSha, JInSha) annotation (Line(
  ...
  connect(conPar_a, irRadExc.conPar_a) annotation (Line(
      points={{242,-60},{160,-60},{160,60},{-60,60},{-60,8},{-80,8},{-80,8},{-80,
          7.5},{-79.9167,7.5}},
      color={191,0,0},
      smooth=Smooth.None));
  ...
  connect(irRadGai.uSha, uSha) annotation (Line(
      points={{-100.833,-22.5},{-120,-22.5},{-120,148},{-220,148},{-220,180},{-260,
          180}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(conExtWin, irRadGai.conExtWin) annotation (Line(
      points={{240,180},{160,180},{160,60},{-60,60},{-60,-22},{-70,-22},{-70,-22.5},
          {-80,-22.5}},
      color={191,0,0},
      smooth=Smooth.None));
  ...
  connect(theConConWin.port_b, vol.heatPort) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  connect(glaUns, convConWin.glaUns) annotation (Line(
  ...
  connect(conExtWinFra, solRadExc.conExtWinFra) annotation (Line(
      points={{242,5.55112e-16},{214,0},{160,0},{160,60},{-60,60},{-60,50},{-79.9167,
          50}},
      color={191,0,0},
      smooth=Smooth.None));
  ...
  connect(conExtWin, solRadExc.conExtWin) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  connect(QAbsSolSha_flow, convConWin.QAbs_flow) annotation (Line(
  ...
  connect(uSha, radTem.uSha) annotation (Line(
      points={{-260,180},{-220,180},{-220,148},{-120,148},{-120,-62.5},{-100.833,
          -62.5}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(conExtWin, radTem.conExtWin) annotation (Line(
      points={{240,180},{130,180},{130,180},{160,180},{160,60},{-60,60},{-60,-62.5},
          {-80,-62.5}},
      color={191,0,0},
      smooth=Smooth.None));
  ...
  connect(radTem.TRad, radiationAdapter.TRad) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(radiationAdapter.rad, heaPorRad) annotation (Line(
