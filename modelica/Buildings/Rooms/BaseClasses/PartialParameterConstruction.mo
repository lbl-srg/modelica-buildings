...
record PartialParameterConstruction "Partial record for constructions"
  ...
  replaceable parameter Buildings.HeatTransfer.Data.OpaqueConstructions.Generic
    layers "Material properties of opaque construction" annotation (
    Dialog(group="Opaque construction"),
    Evaluate=true,
    choicesAllMatching=true,
    Placement(transformation(extent={{146,258},{166,278}})));
  ...
  final parameter Integer nLay(
    min=1,
    fixed=true) = layers.nLay "Number of layers";
  final parameter Integer nSta[nLay](min=1) = {layers.material[i].nSta for i
     in 1:nLay} "Number of states" annotation (Evaluate=true);
