within Buildings.Utilities.IO.WeatherData.BaseClasses.Examples;
model ConvertRadiation "Test model for convert radiation"
  import Buildings;

public
  Buildings.Utilities.IO.WeatherData.BaseClasses.ConvertRadiation cheGloRad
    "Check global horizontal radiation"
    annotation (Placement(transformation(extent={{20,20},{40,40}})));
  Buildings.Utilities.IO.WeatherData.BaseClasses.ConvertRadiation cheDifRad
...
equation
  ...
  connect(simTim.y, timCon.simTim) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  annotation (Diagram(graphics), Commands(file="ConvertRadiation.mos" "run"));
