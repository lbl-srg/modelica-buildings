...
model VectorFunctions "Test model for functions that take a vector as argument"
  ...
  Districts.Utilities.Math.Min minVec(
                   nin=3)
  ...
  Districts.Utilities.Math.Max maxVec(
                   nin=3)
    annotation (Placement(transformation(extent={{-20,-60},{0,-40}})));
  Districts.Utilities.Math.Average aveVec(
                       nin=3)
...
equation
  ...
  connect(sine2.y, aveVec.u[3]) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  annotation ( __Dymola_Commands(file="modelica://Districts/Resources/Scripts/Dymola/Utilities/Math/Examples/VectorFunctions.mos"
        "Simulate and plot"),
