...
model Guideline36_newMulVAV
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.VAV.Controller_new
    ...
    yFanMin=yFanMin,
    VPriSysMax_flow=VPriSysMax_flow,
    peaSysPop=1.2*sum({0.05*AFlo[i] for i in 1:numZon})) "AHU controller"
    annotation (Placement(transformation(extent={{-140,540},{-60,668}})));
  Buildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.VAV.SetPoints.OutdoorAirFlow.Zone
    zonOutAirSet[numZon](
    AFlo=AFlo,
    have_occSen=false,
    have_winSen=false,
    desZonPop={0.05*AFlo[i] for i in 1:numZon},
    minZonPriFlo=minZonPriFlo)
    "Zone level calculation of the minimum outdoor airflow setpoint"
    annotation (Placement(transformation(extent={{-260,610},{-240,630}})));
  Buildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.VAV.SetPoints.OutdoorAirFlow.SumZone
    ...
    annotation (Placement(transformation(extent={{-200,610},{-180,630}})));
...
equation
  ...
  connect(TZonSet[1].yOpeMod, conVAVWes.uOpeMod) annotation (Line(points={{82,303},
        ...
        color={255,127,0}));
  connect(zonToSys.ySumDesZonPop, conAHU1.sumDesZonPop) annotation (Line(points
        ={{-178,629},{-174,629},{-174,637.778},{-144,637.778}}, color={0,0,127}));
  connect(zonToSys.VSumDesPopBreZon_flow, conAHU1.VSumDesPopBreZon_flow)
    annotation (Line(points={{-178,626},{-172,626},{-172,632.444},{-144,632.444}},
        color={0,0,127}));
  connect(zonToSys.VSumDesAreBreZon_flow, conAHU1.VSumDesAreBreZon_flow)
    annotation (Line(points={{-178,623},{-170,623},{-170,627.111},{-144,627.111}},
        color={0,0,127}));
  connect(zonToSys.yDesSysVenEff, conAHU1.uDesSysVenEff) annotation (Line(
        points={{-178,620},{-168,620},{-168,621.778},{-144,621.778}}, color={0,
          0,127}));
  connect(zonToSys.VSumUncOutAir_flow, conAHU1.VSumUncOutAir_flow) annotation (
      Line(points={{-178,617},{-168,617},{-168,616.444},{-144,616.444}}, color=
          {0,0,127}));
  connect(zonToSys.VSumSysPriAir_flow, conAHU1.VSumSysPriAir_flow) annotation (
      Line(points={{-178,611},{-168,611},{-168,611.111},{-144,611.111}}, color=
          {0,0,127}));
  connect(zonToSys.uOutAirFra_max, conAHU1.uOutAirFra_max) annotation (Line(
        points={{-178,614},{-162,614},{-162,605.778},{-144,605.778}}, color={0,
          0,127}));
  connect(zonOutAirSet.yDesZonPeaOcc, zonToSys.uDesZonPeaOcc) annotation (Line(
        points={{-238,629},{-220,629},{-220,628},{-202,628}}, color={0,0,127}));
  connect(zonOutAirSet.VDesPopBreZon_flow, zonToSys.VDesPopBreZon_flow)
    annotation (Line(points={{-238,626},{-202,626}}, color={0,0,127}));
  connect(zonOutAirSet.VDesAreBreZon_flow, zonToSys.VDesAreBreZon_flow)
    annotation (Line(points={{-238,623},{-220,623},{-220,624},{-202,624}},
        color={0,0,127}));
  connect(zonOutAirSet.yDesPriOutAirFra, zonToSys.uDesPriOutAirFra) annotation
    (Line(points={{-238,620},{-220,620},{-220,618},{-202,618}}, color={0,0,127}));
  connect(zonOutAirSet.VUncOutAir_flow, zonToSys.VUncOutAir_flow) annotation (
      Line(points={{-238,617},{-220,617},{-220,616},{-202,616}}, color={0,0,127}));
  connect(zonOutAirSet.yPriOutAirFra, zonToSys.uPriOutAirFra)
    annotation (Line(points={{-238,614},{-202,614}}, color={0,0,127}));
  connect(zonOutAirSet.VPriAir_flow, zonToSys.VPriAir_flow) annotation (Line(
        points={{-238,611},{-220,611},{-220,612},{-202,612}}, color={0,0,127}));
  connect(conAHU1.yAveOutAirFraPlu, zonToSys.yAveOutAirFraPlu) annotation (Line(
        points={{-56,614.667},{-40,614.667},{-40,520},{-218,520},{-218,622},{
          -202,622}}, color={0,0,127}));
  connect(conAHU1.VDesUncOutAir_flow, reaRep1.u) annotation (Line(points={{-56,
          625.333},{-40,625.333},{-40,620},{-22,620}}, color={0,0,127}));
  connect(reaRep1.y, zonOutAirSet.VUncOut_flow_nominal) annotation (Line(points
        ={{2,620},{10,620},{10,516},{-270,516},{-270,611},{-262,611}}, color={0,
          0,127}));
  annotation (
