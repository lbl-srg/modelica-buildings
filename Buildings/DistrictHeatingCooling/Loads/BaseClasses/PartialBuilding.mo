...
partial model PartialBuilding "Partial class for building model"
  ...
  parameter Integer nCooLoa = 1
    ...
    annotation(Evaluate=true);
  parameter Buildings.DistrictHeatingCooling.Loads.Types.ModelType heaLoaTyp[nHeaLoa]=
    fill(Buildings.DistrictHeatingCooling.Loads.Types.ModelType.HeatPort, nHeaLoa)
    "Type of heating load model"
    annotation(Evaluate=true);
  parameter Buildings.DistrictHeatingCooling.Loads.Types.ModelType cooLoaTyp[nCooLoa]=
    fill(Buildings.DistrictHeatingCooling.Loads.Types.ModelType.HeatPort, nCooLoa)
  ...
  Buildings.DistrictHeatingCooling.Loads.BaseClasses.FirstOrderODE heaLoaO[nHeaLoaO](
    ...
    each TOutHea_nominal=268.15) if nHeaLoaO > 0 "ODE model computing the temperature of heating load"
  ...
  Buildings.DistrictHeatingCooling.Loads.BaseClasses.FirstOrderODE cooLoaO[nCooLoaO](
...
protected
  parameter Integer nHeaLoaH = Modelica.Math.BooleanVectors.countTrue(
    {el==Buildings.DistrictHeatingCooling.Loads.Types.ModelType.HeatPort
                                                                       for el in heaLoaTyp})
  ...
  parameter Integer nHeaLoaT = Modelica.Math.BooleanVectors.countTrue(
    {el==Buildings.DistrictHeatingCooling.Loads.Types.ModelType.PrescribedT
                                                                          for el in heaLoaTyp})
  ...
  parameter Integer nHeaLoaO = Modelica.Math.BooleanVectors.countTrue(
    {el==Buildings.DistrictHeatingCooling.Loads.Types.ModelType.ODE
                                                                  for el in heaLoaTyp})
  ...
  connect(heaFloHeaLoaO.port_b, THeaLoaO.port)
    ...
    for i in 1:nHeaLoaO loop
      ...
      connect(heaFloHeaLoaO[i].Q_flow, Q_flowHeaAct[heaLoaO_idx[i]])
        annotation (Line(points={{-270,110},{-270,120},{-250,120},{-250,210},{310,210}}, color={0,0,127}));
    end for;
  end if;
  if nHeaLoaT > 0 then
    ...
    for i in 1:nHeaLoaT loop
      ...
      connect(heaFloHeaLoaT[i].Q_flow, Q_flowHeaAct[heaLoaT_idx[i]])
        annotation (Line(points={{-270,60},{-270,70},{-250,70},{-250,210},{310,210}}, color={0,0,127}));
    end for;
  end if;
  if nHeaLoaH > 0 then
    // Connection to I/O variables
    for i in 1:nHeaLoaH loop
      ...
      connect(heaFloHeaLoaH[i].Q_flow, Q_flowHeaAct[heaLoaH_idx[i]])
        annotation (Line(points={{-270,160},{-270,210},{310,210}}, color={0,0,127}));
    end for;
  end if;
  if nCooLoaO > 0 then
