...
model IdealHeaterCooler "Model of ideal heater or cooler"
  extends Modelica.Blocks.Icons.Block;

  parameter Modelica.SIunits.HeatFlowRate Q_flow_nominal "Maximum heat flow rate (positive for heating; negative for cooling)";
  ...
  parameter Real k(
    min=100*Buildings.Controls.OBC.CDL.Constants.eps) = 1 "Gain of controller"
    annotation (
      Dialog(group="Controller"));
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TMea "Measured temperature" annotation (
     Placement(transformation(
        ...
        rotation=90,
        origin={0,-120})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput Q_flow(final unit = "W") "Heat flow rate"
...
protected
  Buildings.Controls.OBC.CDL.Continuous.Gain gai(final k=Q_flow_nominal)
