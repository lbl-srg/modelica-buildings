...
model OneZoneEquipmentScheduleNonSampledOutputVariable
  ...
  extends Buildings.ThermalZones.EnergyPlus.Validation.OneZoneEquipmentScheduleNonSampled;
  ...
  Utilities.Diagnostics.AssertEquality assEqu(threShold=1E-6, message="EnergyPlus did not synchronize the output variable correctly")
    "Assertion to test whether the schedule and the EnergyPlus output variable are correctly synchronized"
    annotation (Placement(transformation(extent={{30,40},{50,60}})));
  Controls.OBC.CDL.Continuous.Gain gai(k=500) "Gain for internal heat gain"
...
equation
  ...
  connect(equEle.y, assEqu.u1) annotation (Line(points={{21,80},{24,80},{24,56},
          {28,56}}, color={0,0,127}));
  connect(assEqu.u2, gai.y)
    annotation (Line(points={{28,44},{-18,44}}, color={0,0,127}));
  connect(gai.u, intLoaFra.y) annotation (Line(points={{-42,44},{-50,44},{-50,80},
          {-58,80}}, color={0,0,127}));
...
First implementation.
 ...
 __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/EnergyPlus/Validation/OneZoneEquipmentScheduleNonSampledOutputVariable.mos"
        "Simulate and plot"),
experiment(
      StopTime=172800,
      Tolerance=1e-06));
