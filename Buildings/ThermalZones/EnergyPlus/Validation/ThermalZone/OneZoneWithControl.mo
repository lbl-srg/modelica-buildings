...
model OneZoneWithControl
  ...
  Fluid.Sources.Boundary_pT pAtm(redeclare package Medium = Medium, nPorts=1)
    "Boundary condition"
...
equation
  ...
  connect(zon.qGai_flow,gai.y)
    annotation (Line(points={{18,110},{-19,110}},
                                               color={0,0,127}));
  ...
  connect(building.weaBus, freshAir.weaBus) annotation (Line(
      points={{-60,30},{-50,30},{-50,30.2},{-40,30.2}},
      color={255,204,51},
      thickness=0.5));
  connect(duc.port_a, zon.ports[1])
    annotation (Line(points={{10,60},{37,60},{37,80.9}}, color={0,127,255}));
  connect(freshAir.ports[1], zon.ports[2]) annotation (Line(points={{-20,30},{39,
          30},{39,80.9}},     color={0,127,255}));
  connect(fan.port_a, zon.ports[3]) annotation (Line(points={{40,-20},{20,-20},{
          20,10},{41,10},{41,80.9}},     color={0,127,255}));
  connect(hea.port_b, zon.ports[4])
    annotation (Line(points={{100,-20},{112,-20},{112,30},{43,30},{43,80.9}},
                                                           color={0,127,255}));
