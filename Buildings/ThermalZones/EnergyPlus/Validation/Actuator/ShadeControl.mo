...
model ShadeControl
  ...
  Buildings.Controls.OBC.Shade.Shade_H shaH(HHigh=200, HLow=10)
...
equation
  connect(qIntGai.y, zonNor.qGai_flow) annotation (Line(points={{-99,70},{-2,70}},
                                    color={0,0,127}));
  connect(qIntGai.y, zonEas.qGai_flow) annotation (Line(points={{-99,70},{-80,70},
          {-80,28},{30,28},{30,8},{38,8}},
                             color={0,0,127}));
  connect(qIntGai.y, zonWes.qGai_flow) annotation (Line(points={{-99,70},{-80,70},
          {-80,8},{-30,8}},     color={0,0,127}));
  ...
  connect(greEquT.y, and2.u1) annotation (Line(points={{102,-70},{106,-70},{106,
          -90},{108,-90}},
                     color={255,0,255}));
  connect(greEquH.y, and2.u2) annotation (Line(points={{102,-100},{106,-100},{106,
          -98},{108,-98}},
                     color={255,0,255}));
  ...
  connect(weaBus,out. weaBus) annotation (Line(
      points={{-180,40},{-180,-33.8},{-168,-33.8}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(building.weaBus, weaBus) annotation (Line(
      points={{-170,70},{-164,70},{-164,40},{-180,40}},
      color={255,204,51},
      thickness=0.5));
  connect(res1[1].port_b, zonNor.ports[1]) annotation (Line(points={{-114,0},{-38,
          0},{-38,36},{18,36},{18,40.9}},
                                      color={0,127,255}));
  connect(res1[2].port_b, zonWes.ports[1]) annotation (Line(points={{-114,0},{-38,
          0},{-38,-26},{-10,-26},{-10,-21.1}},
                                         color={0,127,255}));
  connect(res1[3].port_b, zonEas.ports[1]) annotation (Line(points={{-114,0},{-38,
          0},{-38,-26},{58,-26},{58,-21.1}},
                                         color={0,127,255}));
  connect(out.ports[1], res.port_b)
    annotation (Line(points={{-148,-34},{-134,-34}}, color={0,127,255}));
  connect(res.port_a, zonNor.ports[2]) annotation (Line(points={{-114,-34},{-46,
          -34},{-46,34},{22,34},{22,40.9}}, color={0,127,255}));
  connect(res.port_a, zonWes.ports[2]) annotation (Line(points={{-114,-34},{-6,-34},
          {-6,-21.1}}, color={0,127,255}));
  connect(res.port_a, zonEas.ports[2]) annotation (Line(points={{-114,-34},{62,-34},
          {62,-21.1}}, color={0,127,255}));
protected
  model Cooling
    extends Modelica.Blocks.Icons.Block;
    HeatTransfer.Sources.PrescribedHeatFlow preHeaFlo "Prescribed heat flow rate"
  ...
  equation
    ...
    connect(temSen.port, preHeaFlo.port) annotation (Line(points={{-60,-50},{-70,
            -50},{-70,-72},{84,-72},{84,0},{70,0},{70,0}},
                                                       color={191,0,0}));
