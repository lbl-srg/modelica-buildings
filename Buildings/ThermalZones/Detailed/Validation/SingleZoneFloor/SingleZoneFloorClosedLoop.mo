...
model SingleZoneFloorClosedLoop
  ...
  Controls.OBC.CDL.Continuous.AddParameter addPar[6](each p=273.15 + 22, each k
      =4)
  ...
  Controls.OBC.CDL.Continuous.LimPID conPIDEas(
    ...
    yMin=0,
    u_s(unit="K", displayUnit="degC"),
    u_m(unit="K", displayUnit="degC")) "Controller for heater in the east zone"
  ...
  Controls.OBC.CDL.Continuous.LimPID conPIDWes(
    ...
    yMin=0,
    u_s(unit="K", displayUnit="degC"),
    u_m(unit="K", displayUnit="degC")) "Controller for heater in the west zone"
  ...
  Controls.OBC.CDL.Continuous.LimPID conPIDCor(
    ...
    yMin=0,
    u_s(unit="K", displayUnit="degC"),
    u_m(unit="K", displayUnit="degC")) "Controller for heater in the core zone"
  ...
  Controls.OBC.CDL.Continuous.Sources.Pulse TSetRoo(
    amplitude=6,
    period=86400,
    offset=273.15 + 16,
    startTime=6*3600,
  ...
  Modelica.Blocks.Continuous.Integrator EHeaSou "South zone heating energy"
    annotation (Placement(transformation(extent={{90,26},{110,46}})));
  Modelica.Blocks.Continuous.Integrator EHeaEas "East zone heating energy"
    annotation (Placement(transformation(extent={{90,-12},{110,8}})));
  Modelica.Blocks.Continuous.Integrator EHeaNor "North zone heating energy"
    annotation (Placement(transformation(extent={{90,-52},{110,-32}})));
  Modelica.Blocks.Continuous.Integrator EHeaWes "West zone heating energy"
    annotation (Placement(transformation(extent={{90,-92},{110,-72}})));
  Modelica.Blocks.Continuous.Integrator EHeaCor "Core zone heating energy"
...
equation
  ...
  connect(addPar[1].y,hea.TSet) annotation (Line(points={{-38,130},{-30,130},{
          -30,96},{-22,96}},
                     color={0,0,127}));
  connect(fan.port_b,hea. port_a)   annotation (Line(points={{-38,88},{-20,88}},  color={0,127,255}));
  ...
  connect(TSetRoo.y, conPIDCor.u_s) annotation (Line(points={{-118,90},{-110,90},
          {-110,-130},{-102,-130}},
                                 color={0,0,127}));
  connect(hea.Q_flow,EHeaSinZon. u)
    annotation (Line(points={{1,96},{86,96}},               color={0,0,127}));
  connect(weaDat.weaBus, sinZonFlo.weaBus) annotation (Line(
      points={{0,140},{22.8,140},{22.8,133}},
      color={255,204,51},
      thickness=0.5));
  connect(heaSou.Q_flow, EHeaSou.u)
    annotation (Line(points={{1,36},{88,36}}, color={0,0,127}));
  connect(heaEas.Q_flow, EHeaEas.u)
    annotation (Line(points={{1,-2},{88,-2}}, color={0,0,127}));
  connect(heaNor.Q_flow, EHeaNor.u)
    annotation (Line(points={{1,-42},{88,-42}}, color={0,0,127}));
  connect(heaWes.Q_flow, EHeaWes.u)
    annotation (Line(points={{1,-82},{88,-82}}, color={0,0,127}));
  connect(heaCor.Q_flow, EHeaCor.u)
    annotation (Line(points={{1,-122},{88,-122}}, color={0,0,127}));
  ...
  experiment(
      StopTime=604800,
      Tolerance=1e-06),
  __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/Detailed/Validation/SingleZoneFloor/SingleZoneFloorClosedLoop.mos"
        "Simulate and plot"),
  Documentation(info="
  <html>
  <p>
