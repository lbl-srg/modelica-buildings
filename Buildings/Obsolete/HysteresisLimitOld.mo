...
block HysteresisLimitOld
  ...
  Buildings.Controls.OBC.CDL.Logical.Pre pre(pre_u_start=false)
    "Breaks Boolean loop"
    annotation (Placement(transformation(extent={{-260,80},{-240,100}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput heatingSignal annotation (
      Placement(transformation(extent={{-380,40},{-340,80}}),
        iconTransformation(extent={{-380,40},{-340,80}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput coolingSignal_Hysteresis
    "True if cooling allowed; false if cooling locked out"
    annotation (Placement(transformation(extent={{100,-112},{140,-72}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput coolingSignal annotation (
      Placement(transformation(extent={{-380,-140},{-340,-100}}),
        iconTransformation(extent={{-380,-140},{-340,-100}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput heatingSignal_Hysteresis
    "True if heating allowed; false if heating locked out"
    annotation (Placement(transformation(extent={{100,-26},{140,14}})));
  Buildings.Controls.OBC.CDL.Logical.Pre pre1(pre_u_start=false)
    "Breaks Boolean loop"
    annotation (Placement(transformation(extent={{-260,-100},{-240,-80}})));
  Buildings.Controls.Continuous.OffTimer offTimCol
    "Records time since cooling turned off"
    annotation (Placement(transformation(extent={{-220,-100},{-200,-80}})));
  Buildings.Controls.OBC.CDL.Continuous.Greater greCol
...
equation
  connect(gre3Hot.y,logSwiHot. u1) annotation (Line(points={{-158,90},{-110,90},
          {-110,40},{-102,40}},                 color={255,0,255}));
  ...
  connect(offTimHot.y,gre3Hot. u1) annotation (Line(points={{-199,90},{-182,90}},
                        color={0,0,127}));
  connect(pre.y,offTimHot. u) annotation (Line(points={{-238,90},{-222,90}},
                                                color={255,0,255}));
  connect(heatingSignal, pre.u)
    annotation (Line(points={{-360,60},{-280,60},{-280,90},{-262,90}},
                                                   color={255,0,255}));
  ...
  connect(greCol.y, logSwiCold.u1) annotation (Line(points={{-158,-90},{-120,-90},
          {-120,-102},{-102,-102}},           color={255,0,255}));
  ...
  connect(coolingSignal, pre1.u)
    annotation (Line(points={{-360,-120},{-290,-120},{-290,-90},{-262,-90}},
                                                     color={255,0,255}));
  connect(modTim.y, gre1.u1) annotation (Line(points={{-299,-32},{-280,-32},{-280,
          -52},{-262,-52}},       color={0,0,127}));
  connect(gre1.y, logSwiCold.u2) annotation (Line(points={{-238,-52},{-140,-52},
          {-140,-110},{-102,-110}},      color={255,0,255}));
  connect(TiCo.y, gre3Hot.u2) annotation (Line(points={{-299,12},{-268,12},{-268,
          60},{-188,60},{-188,82},{-182,82}},
                          color={0,0,127}));
  connect(TiCo.y, gre4.u2) annotation (Line(points={{-299,12},{-268,12},{-268,22},
  ...
  connect(TiH1.y, greCol.u2) annotation (Line(points={{-299,-70},{-280,-70},{-280,
          -112},{-188,-112},{-188,-98},{-182,-98}},
                            color={0,0,127}));
  connect(logSwiHot.y, and1.u2) annotation (Line(points={{-78,32},{-62,32},{-62,
          22},{-42,22}}, color={255,0,255}));
  connect(heatingSignal, not1.u) annotation (Line(points={{-360,60},{-280,60},{
          -280,122},{-84,122},{-84,90}},
                                    color={255,0,255}));
  connect(not1.y, and1.u1) annotation (Line(points={{-61,90},{-52,90},{-52,30},{
          -42,30}}, color={255,0,255}));
  connect(and1.y, coolingSignal_Hysteresis) annotation (Line(points={{-19,30},{20,
          30},{20,-92},{120,-92}}, color={255,0,255}));
  connect(and2.y, heatingSignal_Hysteresis) annotation (Line(points={{-19,-130},
          {40,-130},{40,-6},{120,-6}}, color={255,0,255}));
  connect(coolingSignal, not2.u) annotation (Line(points={{-360,-120},{-290,
          -120},{-290,-150},{-82,-150}},
                                   color={255,0,255}));
  ...
  annotation (defaultComponentName = "HysteresisLimit",Documentation(info="<html>
<p>
Cooling is locked out for a specified amount of time after heating turns off (typically 1hr). Heating is locked out for a specified amount of time after cooling turns off (typically 1hr).
        ...
        Text(
          ...
          textString="%name"),
        Rectangle(extent={{-340,-138},{100,100}},
        lineColor={0,0,127},
