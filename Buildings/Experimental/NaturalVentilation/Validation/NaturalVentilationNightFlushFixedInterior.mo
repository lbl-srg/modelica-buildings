...
model NaturalVentilationNightFlushFixedInterior "Validation model for night flush with fixed duration for an interior zone"
  ...
  Controls.OBC.CDL.Continuous.Sources.Constant TIntSet(k=293)
    ...
    annotation (Placement(transformation(extent={{-100,-4},{-56,40}})));
  ...
  NaturalVentilationNightFlushFixed natVenNitFluFix(
    TDryBulCut=273.15,
    TWetBulDif=0.02,
    winSpeLim=1000,
    TNitFluCut=273.15)
  ...
  Controls.OBC.CDL.Logical.Sources.Constant con(k=false)
    "False signal for manual override and rain"
...
equation
  ...
  connect(boundary.ports[1], doo.port_a1) annotation (Line(points={{68,174},{
          158,174},{158,116},{198,116}},
                                     color={0,127,255}));
  connect(doo.port_b2, boundary.ports[2]) annotation (Line(points={{198,104},{
          142,104},{142,170},{68,170}},      color={0,127,255}));
  ...
  connect(natVenNitFluFix.yWinOpe, doo.y) annotation (Line(points={{126.6,86},{151.5,
          86},{151.5,110},{197,110}}, color={0,0,127}));
  connect(booPul.y, natVenNitFluFix.uOcc) annotation (Line(points={{-58,70},{
          -52,70},{-52,96},{-46,96},{-46,97},{23.4,97}},
                                   color={255,0,255}));
  connect(TIntSet.y, natVenNitFluFix.uRooSet) annotation (Line(points={{-51.6,
          18},{-46,18},{-46,73},{23.4,73}},
                                        color={0,0,127}));
  connect(TAirDum.y, natVenNitFluFix.uDryBul) annotation (Line(points={{-49.6,-64},
          {-49.6,-52},{16,-52},{16,48},{23.4,48}}, color={0,0,127}));
  connect(temRoo.T, natVenNitFluFix.uRooMeaTem) annotation (Line(points={{361,-80},
          {356,-80},{356,-92},{8,-92},{8,27},{23.4,27}}, color={0,0,127}));
  connect(ram.y, natVenNitFluFix.uWinSpe) annotation (Line(points={{-18,10},{0,
          10},{0,37},{23.4,37}},
                             color={0,0,127}));
  connect(weaBus2.TWetBul, natVenNitFluFix.uWetBul) annotation (Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(con.y, natVenNitFluFix.uManOveRid) annotation (Line(points={{-58,110},
          {-20,110},{-20,116},{23.4,116}}, color={255,0,255}));
  connect(con.y, natVenNitFluFix.uRai) annotation (Line(points={{-58,110},{-20,
          110},{-20,106},{23.4,106}}, color={255,0,255}));
  connect(ram2.y, natVenNitFluFix.uForHi) annotation (Line(points={{-18,-70},{
          -10,-70},{-10,48},{10,48},{10,58},{23.4,58}}, color={0,0,127}));
...
</html>"),experiment(Tolerance=1e-6, StartTime=0, StopTime=864000), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/NaturalVentilation/Validation/NaturalVentilationNightFlushFixedInterior.mos"
