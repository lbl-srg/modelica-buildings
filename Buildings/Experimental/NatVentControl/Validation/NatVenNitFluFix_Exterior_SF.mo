...
model NatVenNitFluFix_Exterior_SF
  ...
  NatVentNightFlushFix natVentControlPlusNightFlush(TDryBulCut=280)
...
equation
  ...
  connect(conBel3.port_b, testCell_Radiant.surf_conBou[1]) annotation (Line(
        points={{474,-36},{480,-36},{480,20},{444,20},{444,46}}, color={191,0,0}));
  connect(airIn1.ports[1], testCell_Radiant.ports[1]) annotation (Line(points={{334,-26},
          {377,-26},{377,49},{423,49}},          color={0,127,255}));
  connect(testCell_Radiant.ports[2], doo.port_a2) annotation (Line(points={{423,51},
          {298,51},{298,104},{218,104}},     color={0,127,255}));
  connect(doo.port_b1, testCell_Radiant.ports[3]) annotation (Line(points={{218,116},
          {320,116},{320,53},{423,53}},      color={0,127,255}));
  connect(testCell_Radiant.ports[4], temRoom1.port_a) annotation (Line(points={{423,55},
          {402,55},{402,-96},{372,-96},{372,-90}},         color={0,127,255}));
  connect(intGai1.y, testCell_Radiant.qGai_flow) annotation (Line(points={{381,112},
          {400,112},{400,70},{416.4,70}}, color={0,0,127}));
  connect(weaDat2.weaBus, testCell_Radiant.weaBus) annotation (Line(
  ...
  connect(conBel1.port_b, testCell_Radiant.surf_surBou[1])
  ...
  connect(fixedTemperature.port, conBel1.port_a) annotation (Line(points={{574,
          18},{588,18},{588,-98},{406,-98},{406,-74},{416,-74}}, color={191,0,0}));
  connect(fixedTemperature.port, conBel3.port_a) annotation (Line(points={{574,
          18},{588,18},{588,-50},{446,-50},{446,-36},{454,-36}}, color={191,0,0}));
  connect(booleanConstant2.y, natVentControlPlusNightFlush.uManOveRid)
    annotation (Line(points={{-39,110},{-27.5,110},{-27.5,87.76},{23,87.76}},
        color={255,0,255}));
  connect(booleanConstant2.y, natVentControlPlusNightFlush.uRai) annotation (
      Line(points={{-39,110},{-27.5,110},{-27.5,77.36},{23,77.36}}, color={255,
          0,255}));
  connect(booPul.y, natVentControlPlusNightFlush.uOcc) annotation (Line(points=
          {{-38,70},{-16,70},{-16,68},{23,68}}, color={255,0,255}));
  connect(weaBus.TWetBul, natVentControlPlusNightFlush.uWetBul) annotation (
      Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(weaBus.TDryBul, natVentControlPlusNightFlush.uDryBul) annotation (
      Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(weaBus.winSpe, natVentControlPlusNightFlush.uWinSpe) annotation (Line(
      ...
      extent={{-3,-6},{-3,-6}},
      horizontalAlignment=TextAlignment.Right));
  connect(natVentControlPlusNightFlush.yWinOpe, doo.y) annotation (Line(points=
          {{155,56.56},{180,56.56},{180,110},{197,110}}, color={0,0,127}));
  connect(temRoom1.T, natVentControlPlusNightFlush.uRooMeaTem) annotation (Line(
        points={{361,-80},{352,-80},{352,-96},{0,-96},{0,-4.8},{23,-4.8}},
        color={0,0,127}));
  connect(InteriorSetpoint1.y, natVentControlPlusNightFlush.uRooSet)
    annotation (Line(points={{-51.6,-78},{-40,-78},{-40,43.04},{23,43.04}},
        color={0,0,127}));
  connect(forHiSF.TForecastHigh, natVentControlPlusNightFlush.uForHi)
    annotation (Line(points={{-74,-12.4},{-23.2,-12.4},{-23.2,27.44},{23,27.44}},
        color={0,0,127}));
  ...
  <p> fixme : Validation model in progress
        ...
        coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{
            640,200}})));
end NatVenNitFluFix_Exterior_SF;
