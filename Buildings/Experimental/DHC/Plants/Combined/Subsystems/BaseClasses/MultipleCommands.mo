...
block MultipleCommands
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput y1One
    ...
    annotation (Placement(transformation(extent={{100,80},{140,120}}), iconTransformation(
          extent={{100,70},{140,110}})));

  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput y1NotModOne
    if have_mode
    "On/Off signal: true if at least one unit is commanded On AND y1Mod is false"
    annotation (Placement(transformation(extent={{100,-40},{140,0}}),
        iconTransformation(extent={{100,-20},{140,20}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput nUniOnNotModBou
    if have_mode
    "Number of units that are commanded On AND y1Mod is false, with lower bound of 1"
    annotation (Placement(transformation(extent={{100,-80},{140,-40}}),
        iconTransformation(extent={{100,-50},{140,-10}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput nUniOnNotMod
    if have_mode
  ...
  Buildings.Controls.OBC.CDL.Continuous.MultiSum mulSum1(nin=nUni)
    if have_mode
    "Total"
...
equation
  ...
  connect(mulSum.y, greThr.u)
    annotation (Line(points={{42,40},{60,40},{60,100},{68,100}},
                                              color={0,0,127}));
  connect(max1.y, nUniOnBou)
    annotation (Line(points={{92,60},{120,60}}, color={0,0,127}));
  connect(mulSum.y, nUniOn) annotation (Line(points={{42,40},{60,40},{60,20},{120,
          20}},  color={0,0,127}));
  ...
  connect(onAndNotMod.y, booToRea1.u)
    annotation (Line(points={{-18,-40},{-12,-40}},
                                                 color={255,0,255}));
  ...
  connect(mulSum1.y, max2.u2) annotation (Line(points={{42,-40},{60,-40},{60,-66},
          {68,-66}}, color={0,0,127}));
  connect(one.y, max2.u1) annotation (Line(points={{42,0},{50,0},{50,-54},{68,-54}},
        color={0,0,127}));
  connect(one.y, max1.u2)
    annotation (Line(points={{42,0},{50,0},{50,54},{68,54}}, color={0,0,127}));
  connect(mulSum.y, max1.u1) annotation (Line(points={{42,40},{60,40},{60,66},{68,
          66}}, color={0,0,127}));
  connect(greThr1.y, y1NotModOne)
    annotation (Line(points={{92,-20},{120,-20}}, color={255,0,255}));
  connect(mulSum1.y, greThr1.u) annotation (Line(points={{42,-40},{60,-40},{60,-20},
          {68,-20}}, color={0,0,127}));
