...
model CoolingDirectControlledReturn
  ...
  Buildings.Experimental.DHC.EnergyTransferStations.Cooling.CoolingDirectControlledReturn coo(
...
equation
  ...
  connect(pro.y,loa.u)
    annotation (Line(points={{21,84},{110,84},{110,16},{118,16}},color={0,0,127}));
  connect(QCoo.y[1],pro.u2)
    annotation (Line(points={{-19,50},{-10,50},{-10,78},{-2,78}},color={0,0,127}));
  connect(pro.y,gai.u)
    annotation (Line(points={{21,84},{30,84},{30,50},{38,50}},color={0,0,127}));
  connect(TDisSupNoi.y,souDis.T_in)
    annotation (Line(points={{-119,14},{-102,14}},color={0,0,127}));
  connect(coo.port_a1,souDis.ports[1])
    annotation (Line(points={{-10,-36},{-20,-36},{-20,10},{-80,10}},color={0,127,255}));
  connect(coo.port_b1,pum.port_a)
    annotation (Line(points={{10,-36},{20,-36},{20,10},{80,10}},color={0,127,255}));
  connect(sinDis.ports[1],coo.port_b2)
    annotation (Line(points={{-80,-90},{-20,-90},{-20,-48},{-10,-48}},color={0,127,255}));
  connect(coo.port_a2,loa.port_b)
    annotation (Line(points={{10,-48},{20,-48},{20,-90},{150,-90},{150,10},{140,10}},color={0,127,255}));
  annotation (
    Icon(
      coordinateSystem(
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DHC/EnergyTransferStations/Cooling/Examples/CoolingDirectControlledReturn.mos" "Simulate and plot"),
    experiment(
      StartTime=0,
      StopTime=86400,
      Tolerance=1e-06),
