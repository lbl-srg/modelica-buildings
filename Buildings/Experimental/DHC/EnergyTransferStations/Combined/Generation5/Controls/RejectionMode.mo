...
block RejectionMode
  "Selection of heat or cold rejection mode"
  ...
  Buildings.Controls.OBC.CDL.Logical.Not noColRej
    "Cold rejection disabled"
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},rotation=90,origin={140,50})));
  Buildings.Controls.OBC.CDL.Logical.Or colRej
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uCoo
    ...
    annotation (Placement(transformation(extent={{-240,40},{-200,80}}),iconTransformation(extent={{-140,10},{-100,50}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uHea
    "Heating demand signal"
    annotation (Placement(transformation(extent={{-240,80},{-200,120}}),iconTransformation(extent={{-140,48},{-100,88}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput dTHeaWat
    "Difference between set point and minimum HW tank temperature"
    annotation (Placement(transformation(extent={{-240,-60},{-200,-20}}),iconTransformation(extent={{-140,-50},{-100,-10}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput dTChiWat
    "Difference between maximum CHW tank temperature and set point"
    annotation (Placement(transformation(extent={{-240,-100},{-200,-60}}),iconTransformation(extent={{-140,-90},{-100,-50}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput yHeaRej
    "Heat rejection enabled"
    annotation (Placement(transformation(extent={{200,20},{240,60}}),iconTransformation(extent={{100,30},{140,70}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput yColRej
    "Cold rejection enabled"
    annotation (Placement(transformation(extent={{200,-60},{240,-20}}),iconTransformation(extent={{100,-70},{140,-30}})));
...
equation
  ...
  connect(noCooDem.y,heaOnl.u2)
    annotation (Line(points={{-138,40},{-130,40},{-130,32},{-102,32}},color={255,0,255}));
  connect(heaCooHotDom.y,heaRejMay.u1)
    annotation (Line(points={{32,100},{58,100}},color={255,0,255}));
  connect(cooOnl.y,heaRejMay.u2)
    annotation (Line(points={{-78,80},{50,80},{50,92},{58,92}},color={255,0,255}));
  connect(heaCooColDom.y,colRej.u1)
    annotation (Line(points={{32,0},{58,0}},color={255,0,255}));
  connect(heaOnl.y,colRej.u2)
    annotation (Line(points={{-78,40},{40,40},{40,-8},{58,-8}},color={255,0,255}));
  connect(heaCoo.y,heaCooColDom.u1)
    annotation (Line(points={{-28,100},{-20,100},{-20,0},{8,0}},color={255,0,255}));
  connect(uHea,noHeaDem.u)
    annotation (Line(points={{-220,100},{-180,100},{-180,80},{-162,80}},color={255,0,255}));
  connect(uCoo,cooOnl.u2)
    annotation (Line(points={{-220,60},{-110,60},{-110,72},{-102,72}},color={255,0,255}));
  connect(uHea,heaCoo.u1)
    annotation (Line(points={{-220,100},{-52,100}},color={255,0,255}));
  connect(uCoo,noCooDem.u)
    annotation (Line(points={{-220,60},{-181,60},{-181,40},{-162,40}},color={255,0,255}));
  connect(uCoo,heaCoo.u2)
    annotation (Line(points={{-220,60},{-60,60},{-60,92},{-52,92}},color={255,0,255}));
  ...
  connect(HeaRej.y,yHeaRej)
    annotation (Line(points={{172,100},{180,100},{180,40},{220,40}},color={255,0,255}));
  connect(uHea,heaOnl.u1)
    annotation (Line(points={{-220,100},{-120,100},{-120,40},{-102,40}},color={255,0,255}));
  connect(add2.y,hys.u)
    annotation (Line(points={{-148,-60},{-142,-60}},color={0,0,127}));
  connect(dTHeaWat,add2.u1)
    annotation (Line(points={{-220,-40},{-180,-40},{-180,-54},{-172,-54}},color={0,0,127}));
  connect(dTChiWat,add2.u2)
    annotation (Line(points={{-220,-80},{-180,-80},{-180,-66},{-172,-66}},color={0,0,127}));
  connect(hys.y,hotDom.u)
    annotation (Line(points={{-118,-60},{-100,-60},{-100,0},{-82,0}},color={255,0,255}));
  connect(hys.y,heaCooColDom.u2)
    annotation (Line(points={{-118,-60},{-20,-60},{-20,-8},{8,-8},{8,-8}},color={255,0,255}));
  connect(hotDom.y,heaCooHotDom.u2)
    annotation (Line(points={{-58,0},{-40,0},{-40,60},{0,60},{0,92},{8,92}},color={255,0,255}));
  ...
  connect(colRej.y,yColRej)
    annotation (Line(points={{82,0},{140,0},{140,-40},{220,-40}},color={255,0,255}));
  annotation (
    Icon(
      coordinateSystem(
