...
block SideHot "Control block for hot side"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yCol(final unit="1")
    ...
    annotation (Placement(transformation(extent={{180,-60},{220,-20}}),
                                     iconTransformation(extent={{100,-60},{140,
            -20}})));
  ...
  Buildings.Controls.OBC.CDL.Logical.TrueFalseHold uHeaHol(trueHoldDuration=120,
      falseHoldDuration=0) "Hold heating enabled signal"
    annotation (Placement(transformation(extent={{0,-30},{20,-10}})));
  Buildings.Controls.OBC.CDL.Logical.TrueFalseHold uHeaHol1(trueHoldDuration=120,
      falseHoldDuration=0) "Hold heating enabled signal"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uHeaCoo
    "Enable signal for heating or cooling" annotation (Placement(transformation(
          extent={{-220,80},{-180,120}}),  iconTransformation(extent={{-140,60},
            {-100,100}})));
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TTop(final unit="K",
      displayUnit="degC")
...
equation
  ...
  connect(TBot, conHeaRej.u_m) annotation (Line(points={{-200,-120},{-140,-120},
          {-140,-60},{-100,-60},{-100,-32}},  color={0,0,127}));
  connect(mapFun.y, yAmb)
    annotation (Line(points={{72,40},{200,40}},    color={0,0,127}));
  connect(TSet, conColRej.u_s) annotation (Line(points={{-200,20},{-150,20},{
          -150,-80},{-112,-80}},
                             color={0,0,127}));
  connect(TTop, conColRej.u_m) annotation (Line(points={{-200,-40},{-160,-40},{
          -160,-100},{-100,-100},{-100,-92}},
                                        color={0,0,127}));
  connect(conHeaRej.y, greThr.u) annotation (Line(points={{-88,-20},{-52,-20}},
                               color={0,0,127}));
  connect(x1.y,mapFun. x1) annotation (Line(points={{32,60},{40,60},{40,48},{48,
          48}},      color={0,0,127}));
  connect(conHeaRej.y, rep.u) annotation (Line(points={{-88,-20},{-80,-20},{-80,
          40},{-62,40}},   color={0,0,127}));
  connect(rep.y,mapFun. u) annotation (Line(points={{-38,40},{48,40}},
                     color={0,0,127}));
  connect(f1.y,mapFun. f1) annotation (Line(points={{-8,60},{0,60},{0,44},{48,
          44}},                       color={0,0,127}));
  connect(f2.y,mapFun. f2) annotation (Line(points={{32,20},{40,20},{40,32},{48,
          32}},  color={0,0,127}));
  connect(x2.y,mapFun. x2) annotation (Line(points={{-8,20},{0,20},{0,36},{48,
          36}},      color={0,0,127}));
  connect(booToRea.y, yIsoAmb) annotation (Line(points={{142,0},{200,0}},
                                 color={0,0,127}));
  connect(TSet, conHeaRej.u_s) annotation (Line(points={{-200,20},{-150,20},{
          -150,-20},{-112,-20}},  color={0,0,127}));
  connect(uHeaCoo, conColRej.uEna) annotation (Line(points={{-200,100},{-120,
          100},{-120,-96},{-104,-96},{-104,-92}},
                                              color={255,0,255}));
  connect(conColRej.y, greThr1.u) annotation (Line(points={{-88,-80},{-80,-80},
          {-80,-60},{-52,-60}},   color={0,0,127}));
  connect(greThr.y, uHeaHol.u)
    annotation (Line(points={{-28,-20},{-2,-20}},  color={255,0,255}));
  connect(uHeaHol.y, booToRea.u) annotation (Line(points={{22,-20},{100,-20},{
          100,0},{118,0}},       color={255,0,255}));
  ...
  connect(hotDom.y, conHeaRej.uEna) annotation (Line(points={{62,-60},{80,-60},
          {80,-40},{-104,-40},{-104,-32}},   color={255,0,255}));
  connect(swi.y, yCol)
    annotation (Line(points={{142,-40},{200,-40}},   color={0,0,127}));
  connect(uHeaHol.y, swi.u2) annotation (Line(points={{22,-20},{100,-20},{100,
          -40},{118,-40}},   color={255,0,255}));
  connect(conColRej.y, swi.u3) annotation (Line(points={{-88,-80},{100,-80},{
          100,-48},{118,-48}},
                             color={0,0,127}));
  connect(zer.y, swi.u1) annotation (Line(points={{-38,100},{86,100},{86,-32},{
          118,-32}},  color={0,0,127}));
   annotation (
