...
partial block PartialSideHotCold
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uHeaCoo
    ...
    annotation (Placement(transformation(extent={{-220,160},{-180,200}}),iconTransformation(extent={{-140,60},{-100,100}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TSet(
    final unit="K",
    displayUnit="degC")
    "Supply temperature set point (heating or chilled water)"
    annotation (Placement(transformation(extent={{-220,20},{-180,60}}),iconTransformation(extent={{-140,-20},{-100,20}})));
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yAmb[nSouAmb](
    ...
    annotation (Placement(transformation(extent={{180,-100},{220,-60}}),iconTransformation(extent={{100,-30},{140,10}})));
  inner Modelica.StateGraph.StateGraphRoot stateGraphRoot
    annotation (Placement(transformation(extent={{-100,140},{-80,160}})));
  Modelica.StateGraph.InitialStep noDemand(
    nIn=2)
  ...
  Buildings.Controls.OBC.CDL.Continuous.Feedback errEna
    "Error for enabling"
    annotation (Placement(transformation(extent={{-90,30},{-70,50}})));
  Buildings.Controls.OBC.CDL.Continuous.Feedback errDis
    "Disabling error"
  ...
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToRea1(
    final realTrue=-1,
    final realFalse=1)
    "Output -1 if reverse acting, else 1"
...
equation
  ...
  connect(enaHeaCoo.y,t1.condition)
    annotation (Line(points={{52,40},{60,40},{60,80},{20,80},{20,128}},color={255,0,255}));
  connect(disHeaCoo.y,t2.condition)
    annotation (Line(points={{52,0},{80,0},{80,128}},color={255,0,255}));
  connect(TSet,errEna.u1)
    annotation (Line(points={{-200,40},{-92,40}},color={0,0,127}));
  connect(TSet,errDis.u1)
    annotation (Line(points={{-200,40},{-160,40},{-160,0},{-72,0}},color={0,0,127}));
  connect(zer.y,disHeaCoo.u1)
    annotation (Line(points={{2,-40},{20,-40},{20,0},{28,0}},color={0,0,127}));
  connect(proEna.y,enaHeaCoo.u1)
    annotation (Line(points={{2,40},{28,40}},color={0,0,127}));
  connect(proDis.y,disHeaCoo.u2)
    annotation (Line(points={{2,0},{12,0},{12,-8},{28,-8}},color={0,0,127}));
  connect(revAct.y,booToRea1.u)
    annotation (Line(points={{-78,80},{-72,80}},color={255,0,255}));
  connect(errEna.y,proEna.u2)
    annotation (Line(points={{-68,40},{-30,40},{-30,34},{-22,34}},color={0,0,127}));
  connect(errDis.y,proDis.u2)
    annotation (Line(points={{-48,0},{-30,0},{-30,-6},{-22,-6}},color={0,0,127}));
  connect(booToRea1.y,proDis.u1)
    annotation (Line(points={{-48,80},{-40,80},{-40,6},{-22,6}},color={0,0,127}));
  connect(booToRea1.y,proEna.u1)
    annotation (Line(points={{-48,80},{-40,80},{-40,46},{-22,46}},color={0,0,127}));
  connect(noDemand.outPort[1],t1.inPort)
    annotation (Line(points={{-19.5,140},{16,140}},color={0,0,0}));
  connect(t2.outPort,noDemand.inPort[1])
    annotation (Line(points={{81.5,140},{92,140},{92,160},{-50,160},{-50,140.5},{-41,140.5}},color={0,0,0}));
  connect(and2.y,yDem)
  ...
  connect(uHeaCoo,and2.u1)
    annotation (Line(points={{-200,180},{100,180},{100,100},{108,100}},color={255,0,255}));
  connect(proDis.y,e)
    annotation (Line(points={{2,0},{12,0},{12,-20},{170,-20},{170,0},{200,0}},color={0,0,127}));
  connect(deaBan.y,enaHeaCoo.u2)
    annotation (Line(points={{2,80},{10,80},{10,32},{28,32}},color={0,0,127}));
  annotation (
    Diagram(
      coordinateSystem(
