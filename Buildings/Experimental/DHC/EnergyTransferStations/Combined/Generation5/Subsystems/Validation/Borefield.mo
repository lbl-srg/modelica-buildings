...
model Borefield
  ...
  final parameter Modelica.SIunits.Distance cooBor[nBorHol,2]=
      .Buildings.Applications.DHC.EnergyTransferStations.BaseClasses.computeCoordinates(
  ...
  Fluid.Sources.Boundary_pT conWat(
    ...
    nPorts=2) "Condenser water boundary conditions" annotation (Placement(
        ...
        rotation=0,
        origin={-40,-2})));
  Fluid.Sensors.TemperatureTwoPort senTInl(redeclare final package Medium =
        Medium, m_flow_nominal=borFie.pum.m_flow_nominal)
    "Water inlet temperature" annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={-10,20})));
  Fluid.Sensors.TemperatureTwoPort senTOut(redeclare final package Medium =
        Medium, m_flow_nominal=borFie.pum.m_flow_nominal)
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp u(duration=1000, startTime=
       2500) "Control signal"
    annotation (Placement(transformation(extent={{-110,70},{-90,90}})));
equation
  connect(senTInl.port_b, conWat.ports[1])
    annotation (Line(points={{-20,20},{-30,20},{-30,0}},   color={0,127,255}));
  connect(conWat.ports[2], senTOut.port_a) annotation (Line(points={{-30,-4},{
          -30,-20},{-20,-20}},
                           color={0,127,255}));
  connect(senTInl.port_a, borFie.port_a) annotation (Line(points={{0,20},{20,20},
          {20,0},{40,0}},       color={0,127,255}));
  connect(borFie.port_b, senTOut.port_b) annotation (Line(points={{60,0},{80,0},
          {80,-20},{0,-20}},   color={0,127,255}));
  connect(TInlVal.y, conWat.T_in) annotation (Line(points={{-89,0},{-70,0},{-70,
          2},{-52,2}},  color={0,0,127}));
  connect(zer.y, borFie.yValIso[2]) annotation (Line(points={{-88,40},{30,40},{
          30,5},{38,5}},    color={0,0,127}));
  connect(u.y, borFie.u) annotation (Line(points={{-88,80},{32,80},{32,8},{38,8}},
               color={0,0,127}));
  connect(u.y, borFie.yValIso[1]) annotation (Line(points={{-88,80},{32,80},{32,
          3},{38,3}},  color={0,0,127}));
  annotation (
  Diagram(
  coordinateSystem(preserveAspectRatio=false, extent={{-120,-100},{100,100}})),
  __Dymola_Commands(file=
"modelica://Buildings/Resources/Scripts/Dymola/Applications/DHC/EnergyTransferStations/Combined/Generation5/Subsystems/Validation/Borefield.mos"
"Simulate and plot"),
    experiment(
      StopTime=20000,
      Tolerance=1e-06),
