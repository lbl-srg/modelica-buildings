...
model ChilledWaterBypass "Controller for chilled water bypass valve"
  ...
  Modelica.Blocks.Interfaces.RealInput masFloByp(final unit="kg/s")
    "Chilled water bypass loop mass flow rate"
    annotation (Placement(transformation(extent={{-140,-60},{-100,-20}})));
  Modelica.Blocks.Interfaces.RealOutput y "Bypass valve opening ratio"
  ...
  Modelica.Blocks.Math.Division mFloSca "Scaled mass flow rate measurement"
    annotation (Placement(transformation(extent={{20,-20},{40,0}})));
  Modelica.Blocks.Logical.Switch mFloSet "Bypass loop mass flow rate setpoint"
    annotation (Placement(transformation(extent={{-20,-60},{0,-40}})));
  Modelica.Blocks.Sources.Constant mSetOne(final k=mMin_flow)
    "Setpoint if one chiller is on"
    annotation (Placement(transformation(extent={{-70,-80},{-50,-60}})));
  Modelica.Blocks.Sources.Constant mSetTwo(final k=numChi*mMin_flow)
    "Setpoint if two chillers are on"
    annotation (Placement(transformation(extent={{-70,-40},{-50,-20}})));
  Modelica.Blocks.Math.BooleanToReal valSetSca "Scaled valve opening setpoint"
    annotation (Placement(transformation(extent={{-60,20},{-40,40}})));
equation
  connect(bypValCon.y, y) annotation (Line(points={{61,30},{70,30},{70,0},{110,0}},
        color={0,0,127}));
  connect(masFloByp, mFloSca.u1) annotation (Line(points={{-120,-40},{-80,-40},{
          -80,-4},{18,-4}}, color={0,0,127}));
  connect(mFloSca.y, bypValCon.u_m)
    annotation (Line(points={{41,-10},{50,-10},{50,18}}, color={0,0,127}));
  connect(mSetTwo.y, mFloSet.u1) annotation (Line(points={{-49,-30},{-46,-30},{-46,
          -42},{-22,-42},{-22,-42}}, color={0,0,127}));
  connect(mSetOne.y, mFloSet.u3) annotation (Line(points={{-49,-70},{-40,-70},{-40,
          -58},{-22,-58}}, color={0,0,127}));
  connect(mFloSet.y, mFloSca.u2) annotation (Line(points={{1,-50},{8,-50},{8,-16},
          {18,-16}}, color={0,0,127}));
