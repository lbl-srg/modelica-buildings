...
equation
  ...
  connect(addInt.u2,booToInt1.y)
    annotation (Line(points={{118,-56},{110,-56},{110,-70},{102,-70}},
                                                                  color={255,127,0}));
  ...
  connect(booToInt.y,addInt.u1)
    annotation (Line(points={{102,-30},{110,-30},{110,-44},{118,-44}},
                                                                  color={255,127,0}));
  connect(addInt.y,chiTwo.u)
    annotation (Line(points={{142,-50},{150,-50},{150,-30},{114,-30},{114,-10},{
          118,-10}},                                                                color={255,127,0}));
  connect(addInt.y,chiOne.u)
    annotation (Line(points={{142,-50},{150,-50},{150,-30},{114,-30},{114,30},{118,
          30}},                                                                   color={255,127,0}));
  connect(chiOne.y,y[1])
    annotation (Line(points={{142,30},{150,30},{150,-5},{170,-5}},
                                                               color={255,0,255}));
  connect(chiTwo.y,y[2])
    annotation (Line(points={{142,-10},{150,-10},{150,5},{170,5}},
                                                               color={255,0,255}));
  connect(on,offToOne.condition)
    annotation (Line(points={{-180,60},{-50,60},{-50,40},{-2,40}},
                                                 color={255,0,255}));
  ...
  connect(cp.u,pro.y)
    annotation (Line(points={{-82,0},{-89,0}},
                                             color={0,0,127}));
  connect(cp.y, thrOneToTwo.u) annotation (Line(points={{-59,0},{-50,0},{-50,-40},
          {-32,-40}}, color={0,0,127}));
  connect(cp.y, thrTwoToOne.u) annotation (Line(points={{-59,0},{-50,0},{-50,-70},
          {-32,-70}}, color={0,0,127}));
  connect(dT.u1, TChiWatRet) annotation (Line(points={{-152,12},{-156,12},{-156,
          20},{-180,20}}, color={0,0,127}));
  connect(TChiWatSup, dT.u2) annotation (Line(points={{-180,-20},{-156,-20},{-156,
          0},{-152,0}}, color={0,0,127}));
  connect(pro.u2, mFloChiWat) annotation (Line(points={{-112,-6},{-120,-6},{-120,
...
<ul>
<li>
When the plant enabling signal <code>on</code> changes from <code>false</code> to <code>true</code>, one chiller is enabled.
</li>
<li>
When the total cooling load <code>QLoa</code> exceeds 80 percent (adjustable) of one chiller&apos;s nominal capacity 
<code>QChi_nominal</code>, a second chiller is enabled.
</li>
<li>
When the total cooling load <code>QLoa</code> drops below 60 percent (adjustable) of one chiller&apos;s nominal capacity 
<code>QChi_nominal </code>(i.e., 30 percent each chiller), the second chiller is disabled. 
</li>
<li>
When the plant enabling signal <code>on</code> changes from <code>true</code> to <code>false</code>, 
the operating chiller is disabled.</li>
<li>
Parameter <code>tWai</code> assures a transitional time is kept between each operation.
</li>
</ul>
<p><br>
It is assumed that both chillers have the same capacity of <code>QChi_nominal</code>.
</p>
<p>
Note: This model can be used for plants with two chillers with or without waterside econimizer (WSE). 
For plants with WSE, extra control logic on top of this model needs to be added.
</p>
<p>
<img src=\"modelica://Buildings/Resources/Images/Experimental/DHC/CentralPlants/Cooling/Controls/ChillerStage.png\" alt=\"State graph\"/>. 
</p>
</html>"));
end ChillerStage;
