...
equation
  ...
  connect(bypValCon.y,valByp.y)
    annotation (Line(points={{-59,-160},{-30,-160},{-30,-102}},color={0,0,127}));
  connect(mValByp_flow.y,bypValCon.u_m)
    annotation (Line(points={{-99,-200},{-70,-200},{-70,-172}},color={0,0,127}));
  connect(senTCHWSup.T,dT.u2)
    annotation (Line(points={{210,-101},{210,-120},{-220,-120},{-220,44},{-212,44}},color={0,0,127}));
  connect(senTCHWRet.T,dT.u1)
    annotation (Line(points={{-240,-79},{-240,56},{-212,56}},color={0,0,127}));
  connect(dT.y,pro.u1)
    annotation (Line(points={{-189,50},{-180,50},{-180,56},{-172,56}},color={0,0,127}));
  connect(cp.u,pro.y)
    annotation (Line(points={{-142,50},{-149,50}},color={0,0,127}));
  ...
  connect(chiStaCon.y,mulChiSys.on)
    annotation (Line(points={{-219,196},{-160,196},{-160,160},{100,160},{100,4},{12,4}},color={255,0,255}));
  connect(chiStaCon.y[1],bypValCon.trigger)
    annotation (Line(points={{-219,195.5},{-160,195.5},{-160,160},{-260,160},{-260,-180},{-78,-180},{-78,-172}},color={255,0,255}));
  connect(chiStaCon.y,chiOn.u)
    annotation (Line(points={{-219,196},{-160,196},{-160,194},{-42,194}},color={255,0,255}));
  connect(chiOn.y,pumCW.u)
    annotation (Line(points={{-18,194},{8,194},{8,94},{18,94}},color={0,0,127}));
  connect(chiStaCon.y,cooTowWitByp.on)
    annotation (Line(points={{-219,196},{-160,196},{-160,94},{-42,94}},color={255,0,255}));
  ...
  connect(cp.y,chiStaCon.QLoa)
    annotation (Line(points={{-119,50},{-112,50},{-112,80},{-252,80},{-252,192},{-242,192}},color={0,0,127}));
...
The chilled water bypass loop is controlled to ensure a minimum flow of chilled water is running through the chillers all the time. 
The condenser water is cooled by two parallel cooling towers with a bypass loop. See 
<a href=\"modelica://Buildings.Experimental.DHC.CentralPlants.Cooling.Subsystems.CoolingTowerWithBypass\">
...
<li>
The chilled water loop is equiped with two parallel variable speed pumps, 
which are controlled to maitain a use-determined pressure difference setpoint at the demand side. 
The condenser water pumps are constant speed with prescribed mass flow rates.
</li>
<li>
...
See <a href=\"modelica://Buildings.Experimental.DHC.CentralPlants.Cooling.Controls.ChillerStage\">
Buildings.Experimental.DHC.CentralPlants.Cooling.Controls.ChillerStage</a> 
for the detailed control logic.
</li>
</ul>
<p>
<img src=\"modelica://Buildings/Resources/Images/Experimental/DHC/CentralPlants/Cooling/Plant.png\" alt=\"System schematics\"/>. 
</p>
</html>",
      revisions="<html>
<ul>
