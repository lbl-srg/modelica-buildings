...
partial model PartialBuildingWithPartialETS
  ...
  final parameter Modelica.SIunits.HeatFlowRate QHeaWat_flow_nominal(
    min=0)=ets.QHeaWat_flow_nominal
  ...
  final parameter Modelica.SIunits.HeatFlowRate QHotWat_flow_nominal(
    min=0)=ets.QHotWat_flow_nominal
  ...
  final parameter Modelica.SIunits.HeatFlowRate QChiWat_flow_nominal(
    max=0)=ets.QChiWat_flow_nominal
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_aSerAmb(
    ...
    m_flow(
      min=
        if allowFlowReversalSer then
          -Modelica.Constants.inf
        else
          0),
    h_outflow(
      start=MediumSer.h_default,
      nominal=MediumSer.h_default)) if typ == TypDisSys.CombinedGeneration5
    "Fluid connector for ambient water service supply line"
    annotation (Placement(transformation(extent={{-310,-210},{-290,-190}}),iconTransformation(extent={{-110,-10},{-90,10}})));
  Modelica.Fluid.Interfaces.FluidPort_b port_bSerAmb(
    ...
    annotation (Placement(transformation(extent={{290,-210},{310,-190}}),iconTransformation(extent={{90,-10},{110,10}})));
    ...
    m_flow(
      min=
        if allowFlowReversalSer then
          ...
          0),
    h_outflow(
      start=MediumSerHea_a.h_default,
      nominal=MediumSerHea_a.h_default)) if typ <> TypDisSys.Cooling and typ <> TypDisSys.CombinedGeneration5
    "Fluid connector for heating service supply line"
    annotation (Placement(transformation(extent={{-310,-250},{-290,-230}}),iconTransformation(extent={{-110,-50},{-90,-30}})));
  Modelica.Fluid.Interfaces.FluidPort_b port_bSerHea(
    ...
    h_outflow(
      start=MediumSer.h_default,
      nominal=MediumSer.h_default)) if typ <> TypDisSys.Cooling and typ <> TypDisSys.CombinedGeneration5
    "Fluid connector for heating service return line"
    annotation (Placement(transformation(extent={{290,-250},{310,-230}}),iconTransformation(extent={{90,-50},{110,-30}})));
  Modelica.Fluid.Interfaces.FluidPort_a port_aSerCoo(
    ...
    h_outflow(
      start=MediumSer.h_default,
      nominal=MediumSer.h_default)) if typ == TypDisSys.CombinedGeneration1 or typ == TypDisSys.CombinedGeneration2to4 or typ == TypDisSys.Cooling
    "Fluid connector for cooling service supply line"
    annotation (Placement(transformation(extent={{-310,-290},{-290,-270}}),iconTransformation(extent={{-110,-90},{-90,-70}})));
  Modelica.Fluid.Interfaces.FluidPort_b port_bSerCoo(
    ...
    annotation (Placement(transformation(extent={{290,-290},{310,-270}}),iconTransformation(extent={{90,-90},{110,-70}})));
  BoundaryConditions.WeatherData.Bus weaBus if have_weaBus
    "Weather data bus"
    annotation (Placement(transformation(extent={{-20,260},{20,300}}),iconTransformation(extent={{-10,90},{10,110}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput QHea_flow(
    ...
    annotation (Placement(transformation(extent={{300,260},{340,300}}),iconTransformation(extent={{-20,-20},{20,20}},rotation=-90,origin={50,-120})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput QCoo_flow(
    ...
    annotation (Placement(transformation(extent={{300,220},{340,260}}),iconTransformation(extent={{-20,-20},{20,20}},rotation=-90,origin={70,-120})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput PHea(
    final unit="W") if have_eleHea
    "Power drawn by heating system"
    annotation (Placement(transformation(extent={{300,182},{340,222}}),iconTransformation(extent={{100,70},{140,110}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput PCoo(
    final unit="W") if have_eleCoo
    "Power drawn by cooling system"
    annotation (Placement(transformation(extent={{300,140},{340,180}}),iconTransformation(extent={{100,50},{140,90}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput PFan(
    final unit="W") if have_fan
    "Power drawn by fan motors"
    annotation (Placement(transformation(extent={{300,100},{340,140}}),iconTransformation(extent={{100,30},{140,70}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput PPum(
    final unit="W") if have_pum
    "Power drawn by pump motors"
    annotation (Placement(transformation(extent={{300,60},{340,100}}),iconTransformation(extent={{100,10},{140,50}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput QFue_flow[nFue](
    each final unit="W") if nFue > 0
    "Fuel energy input rate"
    annotation (Placement(transformation(extent={{300,20},{340,60}}),iconTransformation(extent={{-20,-20},{20,20}},rotation=90,origin={90,120})));
  // COMPONENTS
  ...
  Buildings.Controls.OBC.CDL.Continuous.Gain mulQHea_flow(
    u(
      final unit="W"),
    final k=facMul) if bui.have_heaLoa
    "Scaling"
  ...
  Fluid.BaseClasses.MassFlowRateMultiplier mulSerHeaInl(
    ...
    final k=1/facMul,
    final allowFlowReversal=allowFlowReversalSer) if typ <> TypDisSys.Cooling and typ <> TypDisSys.CombinedGeneration5
    "Mass flow rate multiplier"
  ...
  Fluid.BaseClasses.MassFlowRateMultiplier mulSerCooInl(
    redeclare final package Medium=MediumSer,
    final k=1/facMul,
    final allowFlowReversal=allowFlowReversalSer) if typ == TypDisSys.CombinedGeneration1 or typ == TypDisSys.CombinedGeneration2to4 or typ == TypDisSys.Cooling
    "Mass flow rate multiplier"
...
initial equation
  assert(
    ets.have_heaWat == bui.have_heaWat,
    "In "+getInstanceName()+": The ETS component is configured with have_heaWat="+String(
      ets.have_heaWat)+" whereas the building is configured with have_heaWat="+String(
      bui.have_heaWat));
  assert(
    ets.have_chiWat == bui.have_chiWat,
    "In "+getInstanceName()+": The ETS component is configured with have_chiWat="+String(
      ets.have_chiWat)+" whereas the building is configured with have_chiWat="+String(
      bui.have_chiWat));
  if have_heaWat then
    assert(
      nPorts_heaWat > 0,
      "In "+getInstanceName()+": The ETS component is configured with have_heaWat="+String(
        ets.have_heaWat)+" but the number of fluid ports for chilled water
  ...
  if have_chiWat then
    assert(
      nPorts_chiWat > 0,
      "In "+getInstanceName()+": The ETS component is configured with have_chiWat="+String(
        ets.have_chiWat)+" but the number of fluid ports for chilled water
...
equation
  ...
  connect(weaBus,bui.weaBus)
    annotation (Line(points={{0,280},{0,59.4},{0.1,59.4}},color={255,204,51},thickness=0.5),Text(string="%first",index=-1,extent={{-3,6},{-3,6}},horizontalAlignment=TextAlignment.Right));
  connect(weaBus,ets.weaBus)
    annotation (Line(points={{0,280},{0,72},{4,72},{4,-26},{0,-26},{0,-25.9}},color={255,204,51},thickness=0.5),Text(string="%first",index=-1,extent={{-3,6},{-3,6}},horizontalAlignment=TextAlignment.Right));
  connect(bui.PHea,totPHea.u[1])
    annotation (Line(points={{32,56},{208,56},{208,202},{240,202}},color={0,0,127}));
  connect(ets.PHea,totPHea.u[idxPHeaETS])
    annotation (Line(points={{34,-48},{210,-48},{210,200},{240,200},{240,202}},color={0,0,127}));
  connect(bui.PCoo,totPCoo.u[1])
    annotation (Line(points={{32,52},{218,52},{218,160},{238,160}},color={0,0,127}));
  connect(ets.PCoo,totPCoo.u[idxPCooETS])
    annotation (Line(points={{34,-52},{220,-52},{220,158},{238,158},{238,160}},color={0,0,127}));
  connect(bui.PFan,totPFan.u[1])
    annotation (Line(points={{32,48},{224,48},{224,120},{238,120}},color={0,0,127}));
  connect(ets.PFan,totPFan.u[idxPFanETS])
    annotation (Line(points={{34,-56},{226,-56},{226,118},{238,118},{238,120}},color={0,0,127}));
  connect(bui.PPum,totPPum.u[1])
    annotation (Line(points={{32,44},{232,44},{232,80},{238,80}},color={0,0,127}));
  connect(ets.PPum,totPPum.u[idxPPumETS])
    annotation (Line(points={{34,-60},{230,-60},{230,82},{238,82},{238,80}},color={0,0,127}));
  connect(bui.QHea_flow,mulQHea_flow.u)
    annotation (Line(points={{32,64},{200,64},{200,280},{268,280}},color={0,0,127}));
  connect(mulQHea_flow.y,QHea_flow)
    annotation (Line(points={{292,280},{320,280}},color={0,0,127}));
  connect(bui.QCoo_flow,mulQCoo_flow.u)
    annotation (Line(points={{32,60},{204,60},{204,240},{268,240}},color={0,0,127}));
  ...
  connect(mulPFan.y,PFan)
    annotation (Line(points={{292,120},{300,120},{300,122},{306,122},{306,120},{320,120}},color={0,0,127}));
  ...
  connect(mulSerCooInl.port_b,ets.port_aSerCoo)
    annotation (Line(points={{-260,-280},{-160,-280},{-160,-84},{-30,-84}},color={0,127,255}));
  ...
  connect(ets.port_bSerCoo,mulSerCooOut.port_a)
    annotation (Line(points={{30,-84},{160,-84},{160,-280},{260,-280}},color={0,127,255}));
  ...
  connect(mulSerHeaInl.port_b,ets.port_aSerHea)
    annotation (Line(points={{-260,-240},{-164,-240},{-164,-80},{-30,-80}},color={0,127,255}));
  ...
  connect(mulSerAmbInl.port_b,ets.port_aSerAmb)
    annotation (Line(points={{-260,-200},{-168,-200},{-168,-76},{-30,-76}},color={0,127,255}));
  connect(ets.port_bSerHea,mulSerHeaOut.port_a)
    annotation (Line(points={{30,-80},{164,-80},{164,-240},{260,-240}},color={0,127,255}));
  ...
  connect(ets.port_bSerAmb,mulSerAmbOut.port_a)
    annotation (Line(points={{30,-76},{168,-76},{168,-200},{260,-200}},color={0,127,255}));
  ...
  connect(ets.QFue_flow,mulQFue_flow.u)
    annotation (Line(points={{34,-64},{260,-64},{260,40},{268,40}},color={0,0,127}));
  annotation (
    Icon(
      coordinateSystem(
