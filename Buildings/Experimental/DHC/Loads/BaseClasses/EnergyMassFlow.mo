...
protected
  ...
  parameter Real k = 2.5
    "Shape factor of typical characteristic";
  ...
  algorithm
    ...
    annotation (
      inverse(
        rat_m_flow=inverseCharacteristic(rat_Q_flow, have_masFlo, k)));
  end characteristic;
  ...
  algorithm
    ...
    annotation (
      inverse(
        rat_Q_flow=characteristic(rat_m_flow, have_masFlo, k)));
...
equation
  ...
  rat_m_flow_cor =
    if have_masFlo then
      min(1,
        m_flow_internal / m_flow_nominal *
        inverseCharacteristic(rat_Q_flow_tem_bou, have_masFlo, k) *
        Utilities.Math.Functions.inverseXRegularized(
  ...
  filter.u = if have_masFlo then rat_m_flow_cor else
    Utilities.Math.Functions.smoothMax(
      x1=rat_m_flow_cor,
      x2=if uEna then fra_m_flow_min else 0,
      deltaX=kReg);
...
is <i>Q&#775;_residual</i>.
Those two output variables are related to the prescribed load (input variable)
with <i>Q&#775;Pre = Q&#775;_actual + Q&#775;_residual</i>.
</li>
</li>
<li>
...
the whole distribution system and the terminal units it serves are abstracted
...
<p>
First, we assess the heat flow rate <i>Q&#775;Set</i> that would be transferred 
if the prescribed load <i>Q&#775;Pre</i> was met and if the suppply temperature 
was equal to the set point.
<p>
<i>Q&#775;Set = Q&#775;Pre * (TSupSet - TLoa) / (TSup_actual - TLoa)</i>
</p>
<p>
If that value exceeds the nominal heat flow rate, this indicates that
the load exceeds the system capacity under the actual operating
conditions. The exceeding fraction
<i>max(0, (Q&#775;Set - Q&#775;_nominal) / Q&#775;_nominal)</i>
is a correction factor applied to the prescribed load to compute
<i>Q&#775;_actual</i>.
</p>
<p>
...
bounded by the recirculation flow rate at minimum pump speed:
<i>m&#775; = max(m&#775;Min,
f<sup>-1</sup>(min(Q&#775;_nominal, Q&#775;Set) / Q&#775;_nominal))</i>,
</li>
<li>
if the mass flow rate is provided as an input, it still needs
to be corrected in case of a supply temperature mismatch:
<i>m&#775; = m&#775;Pre *
f<sup>-1</sup>(min(Q&#775;_nominal, Q&#775;Set) / Q&#775;_nominal) /
...
<p>
The corresponding correction factor is computed as the ratio between
the heat flow rate transferred with the actual mass flow rate and
the heat flow rate transferred with the prescribed mass flow rate—the
...
to impose the prescribed mass flow rate (whatever the flow resistance) so 
the correction factor is <code>1</code>.
</p>
