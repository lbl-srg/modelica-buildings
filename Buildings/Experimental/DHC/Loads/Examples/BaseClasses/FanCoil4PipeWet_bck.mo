...
model FanCoil4PipeWet_bck
  "Model of a four-pipe fan coil unit computing a required water mass flow rate"
  ...
  final parameter Modelica.SIunits.ThermalConductance CMin_nominal=
      min(mChiWat_flow_nominal * cpChiWat_nominal,
        mLoaCoo_flow_nominal * cpLoaCoo_nominal)
    "Minimum capacity flow rate at nominal conditions";
  final parameter Modelica.SIunits.ThermalConductance CMax_nominal=
      max(mChiWat_flow_nominal * cpChiWat_nominal,
        mLoaCoo_flow_nominal * cpLoaCoo_nominal)
    "Maximum capacity flow rate at nominal conditions";
  final parameter Real Z = CMin_nominal / CMax_nominal
    "Ratio of capacity flow rates (CMin/CMax) at nominal conditions";
  // What follows needs to be updated to use sensible heat flow rate, not total.
  final parameter Modelica.SIunits.ThermalConductance UA_nominal=
    Buildings.Fluid.HeatExchangers.BaseClasses.ntu_epsilonZ(
    eps=abs(QCoo_flow_nominal / (CMin_nominal * (T_aChiWat_nominal - T_aLoaCoo_nominal))),
    Z=Z,
    flowRegime=Integer(hexConCoo)) * CMin_nominal
  ...
  Fluid.HeatExchangers.WetEffectivenessNTU_Fuzzy_V2_2_4
                                                      hexCoo(
    redeclare final package Medium1 = Medium1,
    redeclare final package Medium2 = Medium2,
    ...
    final allowFlowReversal1=allowFlowReversal,
    final allowFlowReversal2=allowFlowReversalLoa) "Cooling coil"
...
equation
  ...
  connect(scaChiWatFloInl.port_b, hexCoo.port_a1) annotation (Line(points={{-160,
          -180},{-20,-180},{-20,-12},{0,-12}}, color={0,127,255}));
  connect(hexCoo.port_b1, scaChiWatFloOut.port_a) annotation (Line(points={{20,-12},
          {40,-12},{40,-180},{160,-180}}, color={0,127,255}));
  ...
  connect(hexHea.port_b2, scaLoaMasFloOut.port_a) annotation (Line(points={{-80,
          0},{-120,0},{-120,0},{-160,0}}, color={0,127,255}));
