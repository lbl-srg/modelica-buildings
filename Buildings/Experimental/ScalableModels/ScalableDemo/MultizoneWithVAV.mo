...
model MultizoneWithVAV
  ...
  Fluid.FixedResistances.PressureDrop dpZonBra[nZon,nFlo](
    m_flow_nominal=m_flow_nominal,
    redeclare package Medium = MediumA,
    dp_nominal=20) "Pressure drop for each zone branch" annotation (Placement(
        transformation(
        extent={{8,-7},{-8,7}},
        rotation=-90,
        origin={67,-6})));
  ...
  Fluid.Sensors.TemperatureTwoPort           TCoiHeaOut(redeclare package
  ...
  Fluid.Sensors.TemperatureTwoPort           TSup(redeclare package Medium =
        MediumA, m_flow_nominal=m_flow_nominal)
    annotation (Placement(transformation(extent={{0,-38},{16,-22}})));
  ...
  Examples.VAVReheat.Controls.CoolingCoilTemperatureSetpoint
                                          TSetCoo "Setpoint for cooling coil"
  ...
  Buildings.Experimental.ScalableModels.Controls.Economizer
                      conEco(
    dT=1,
  ...
  Buildings.BoundaryConditions.WeatherData.Bus
    ...
    annotation (Placement(transformation(extent={{-336,164},{-316,184}})));
  ...
  Buildings.Utilities.Math.Average ave(nin=5)
    ...
    annotation (Placement(transformation(extent={{110,74},{122,86}})));
...
equation
  ...
  connect(TMix.port_b, fil.port_a) annotation (Line(points={{-192,-30},{-184,-30},
      ...
      thickness=0.5));
  connect(cooCoi.port_b1, fan.port_a)
    annotation (Line(points={{-56,-30},{-40,-30}},color={0,127,255},
      thickness=0.5));
  connect(hex.port_b2, sinHea.ports[1]) annotation (Line(points={{-144,-42},{-148,
          -42},{-148,-66}}, color={0,127,255}));
  connect(cooCoi.port_b2, sinCoo.ports[1]) annotation (Line(points={{-76,-42},{-80,
  ...
  for iZon in 1:nZon loop
    for iFlo in 1:nFlo loop
     ...
     Line(points={{68.2,89.8},{68.2,126},{28,126}}, color={0,127,255},
        ...
        connect(multiZoneFluctuatingIHG.TcoolSetpoint[iZon, iFlo], controlBus.TRooSetCoo) annotation (Line(
      points={{88.8,62.8},{92,62.8},{92,54},{-66,54}},
      color={0,0,127},
      pattern=LinePattern.Dash));
        connect(multiZoneFluctuatingIHG.TheatSetpoint[iZon, iFlo], controlBus.TRooSetHea) annotation (Line(
      points={{88.8,68.4},{94,68.4},{94,54},{-66,54}},
      color={0,0,127},
      pattern=LinePattern.Dash));
    end for;
  end for;
  ...
  connect(cooCoi.port_a2, valCoo.port_b) annotation (Line(points={{-56,-42},{-54,
          -42},{-51,-42},{-51,-50}}, color={0,127,255}));
  ...
  connect(hex.port_b1, TCoiHeaOut.port_a) annotation (Line(points={{-124,-30},{-102,
          -30}},            color={0,127,255},
      thickness=0.5));
  connect(TCoiHeaOut.port_b, cooCoi.port_a1)
  ...
  connect(controlBus, vAVBranch.controlBus) annotation (
      Line(
      points={{-66,54},{-66,54},{-66,24},{-20,24},{-20,23.2},{52,23.2}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(modeSelector.cb, controlBus) annotation (Line(
      points={{-175.455,53.4545},{-103.728,53.4545},{-103.728,54},{-66,54}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%second",
      index=1,
  ...
  connect(modeSelector.cb, TSetCoo.controlBus) annotation (Line(
      points={{-175.455,53.4545},{-186,53.4545},{-186,-46},{-202,-46},{-202,
          -92.8},{-233.08,-92.8}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(controlBus, conEco.controlBus) annotation (Line(
      points={{-66,54},{-66,54},{-66,108},{-66,110},{-285.6,110},{-285.6,94.4}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(weaDat.weaBus, weaBus) annotation (Line(
      points={{-340,174},{-338,174},{-326,174}},
      color={255,204,51},
      thickness=0.5));
  connect(weaBus.TDryBul, TOut.u) annotation (Line(
      points={{-326,174},{-326,174},{-326,168},{-334,168},{-334,148},{-329.2,148}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(weaBus, amb.weaBus) annotation (Line(
      points={{-326,174},{-308,174},{-308,114},{-346,114},{-346,43.14},{-318,43.14}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(weaBus, multiZoneFluctuatingIHG.weaBus) annotation (Line(
      points={{-326,174},{-198,174},{-58,174},{-58,76},{51.6,76}},
      color={255,204,51},
      thickness=0.5));
  connect(TOut.y, controlBus.TOut) annotation (Line(
      points={{-315.4,148},{-315.4,148},{-66,148},{-66,54}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(multiZoneFluctuatingIHG.TRooAir, ave.u) annotation (Line(
        points={{89.2,83.2},{102,83.2},{102,80},{108.8,80}},         color={0,0,127},

      pattern=LinePattern.Dash));
  connect(multiZoneFluctuatingIHG.TRooAir, min.u) annotation (Line(
        points={{89.2,83.2},{102,83.2},{102,102},{108.8,102}},       color={0,0,127},

      pattern=LinePattern.Dash));
  connect(ave.y, controlBus.TRooAve) annotation (Line(
      points={{122.6,80},{130,80},{130,148},{-66,148},{-66,54}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(min.y, controlBus.TRooMin) annotation (Line(
      points={{122.6,102},{130,102},{130,148},{-66,148},{-66,54}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-360,-120},
            {140,200}})),                                        Diagram(
