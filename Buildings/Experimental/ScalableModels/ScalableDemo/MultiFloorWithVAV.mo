...
model MultiFloorWithVAV
  ...
  Fluid.HeatExchangers.WetCoilCounterFlow cooCoi[nFlo](
    ...
    each dp2_nominal=0,
    each dp1_nominal=0,
    each energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial) "Cooling coil"
  ...
  Fluid.Movers.SpeedControlled_y fanRet[nFlo](
    ...
    each energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial) "Return air fan"
  ...
  Buildings.Controls.Continuous.LimPID cooCoiCon[nFlo](
    ...
    each Ti=600,
    each k=0.01) "Controller for cooling coil"
  ...
  Buildings.Experimental.ScalableModels.Controls.ControlBus_withSub controlBus(nSubBus=nFlo)
...
equation
  for iFlo in 1:nFlo loop
    ...
    connect(conFanRet[iFlo].y, fanRet[iFlo].y)
      annotation (Line(
        points={{26.7,165},{36,165},{36,180},{-19.8,180},{-19.8,138}},
        color={0,0,127},
        pattern=LinePattern.Dash));
