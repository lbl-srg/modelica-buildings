within Buildings.Experimental.OpenBuildingControl.CDL.Logical.Composite;
package Validation "Collection of models that validate the composite logical blocks of the CDL"
  extends Modelica.Icons.ExamplesPackage;

annotation (preferredView="info", Documentation(info="<html>
<p>
This package contains models that validate the blocks in
<a href=\"modelica://Buildings.Experimental.OpenBuildingControl.CDL.Logical\">
Buildings.Experimental.OpenBuildingControl.CDL.Logical</a>.
</p>
<p>
The examples plot various outputs, which have been verified against
analytical solutions. These model outputs are stored as reference data to
allow continuous validation whenever models in the library change.
</p>
</html>"));
end Validation;
