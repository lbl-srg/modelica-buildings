...
model DeadbandControl "Validation model for deadband control"
  ...
  Buildings.Experimental.RadiantControl.SlabTempSignal.DeadbandControl deaCon(
    TDeaRel=TemDeaRel,
    TDeaNor=TemDeaNor,
    k=LastOcc,
    off_within_deadband=OffTru)
    annotation (Placement(transformation(extent={{-20,40},{0,60}})));
  Buildings.Experimental.RadiantControl.SlabTempSignal.DeadbandControl deaCon1(
    TDeaRel=TemDeaRel,
    TDeaNor=TemDeaNor,
    k=LastOcc,
    off_within_deadband=OffFal)
    annotation (Placement(transformation(extent={{-20,0},{0,20}})));
  Buildings.Experimental.RadiantControl.SlabTempSignal.DeadbandControl deaCon2(
    TDeaRel=TemDeaRel,
    TDeaNor=TemDeaNor,
    k=LastOcc,
    off_within_deadband=OffTru)
    annotation (Placement(transformation(extent={{-20,-40},{0,-20}})));
  Buildings.Experimental.RadiantControl.SlabTempSignal.DeadbandControl deaCon3(
    TDeaRel=TemDeaRel,
    TDeaNor=TemDeaNor,
    k=LastOcc,
    off_within_deadband=OffFal)
    annotation (Placement(transformation(extent={{-20,-80},{0,-60}})));
  Buildings.Experimental.RadiantControl.SlabTempSignal.DeadbandControl
    deaConTes(
    TDeaRel=TemDeaRel,
    TDeaNor=TemDeaNor,
    k=LastOcc,
...
</html>"),experiment(StartTime=0,StopTime=172800.0, Tolerance=1e-06),__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/RadiantControl/SlabTempSignal/Validation/DeadbandControl.mos"
