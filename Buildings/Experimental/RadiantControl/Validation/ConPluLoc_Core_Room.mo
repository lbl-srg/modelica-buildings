...
Fluid.Movers.FlowControlled_m_flow           pumHot(
  ...
  Fluid.Sources.Boundary_pT
    airOut2(redeclare package Medium = MediumA, nPorts=1)
                                                     "Air outlet for X3A"
    annotation (Placement(transformation(extent={{-274,122},{-254,142}})));
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature TOut1
    "Outside temperature"
    annotation (Placement(transformation(extent={{-58,246},{-38,266}})));
  Controls.OBC.CDL.Continuous.Sources.Constant           InteriorSetpoint2(k=294)
    annotation (Placement(transformation(extent={{-116,262},{-72,306}})));
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor conBel3(G=0.2)
    "Combined convection and radiation resistance below the slab"
    annotation (Placement(transformation(extent={{-204,284},{-184,304}})));
...
equation
  ...
  connect(InteriorSetpoint2.y,TOut1. T) annotation (Line(points={{-67.6,284},{-66,
          284},{-66,256},{-60,256}}, color={0,0,127}));
  connect(temRoom1.port_b, airOut2.ports[1]) annotation (Line(points={{-152,214},
          {-234,214},{-234,132},{-254,132}}, color={0,127,255}));
  connect(TOut1.port,conBel3. port_a)
    annotation (Line(points={{-38,256},{-204,256},{-204,294}},
                                                             color={191,0,0}));
  connect(conBel3.port_b,testCell_Radiant_Interior. surf_conBou[1]) annotation (
     Line(points={{-184,294},{-190,294},{-190,335.2},{-198,335.2}},
                                                            color={191,0,0}));
  connect(sla1.surf_a,testCell_Radiant_Interior. surf_surBou[1]) annotation (
      Line(points={{10,22},{-260,22},{-260,338},{-207.8,338}},
                                                          color={191,0,0}));
  connect(airIn1.ports[1], testCell_Radiant_Interior.ports[1]) annotation (Line(
        points={{-302,304},{-260,304},{-260,340},{-219,340}}, color={0,127,255}));
  connect(testCell_Radiant_Interior.ports[2], temRoom1.port_a) annotation (Line(
        points={{-219,344},{-219,210},{-152,210},{-152,194}}, color={0,127,255}));
