...
model ControlPlusLockoutPerimRoom
...
Fluid.Movers.FlowControlled_m_flow           pumHot(
    ...
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
    m_flow_nominal=mRad_flow_nominal,
    addPowerToMedium=false) "Hot water pump"
  ...
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToReaHtgPum(realTrue=
        mRad_flow_nominal) "Heating pump controller"
  ...
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToReaClgPum(realTrue=
        mRad_flow_nominal) "Cooling pump controller"
  ...
  Fluid.Sensors.TemperatureTwoPort temCoo(
    ...
    T_start=TRadCold_nominal,
    transferHeat=true,
    TAmb=TRadCold_nominal) "CoolingTemperature" annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-90,-40})));
  ...
  SlabTempSignal.SlabSetpointPerimeter slaSetPer
    annotation (Placement(transformation(extent={{-520,-60},{-500,-40}})));
  SlabTempSignal.Validation.BaseClasses.ForecastHighChicago forHiChi
...
Chilled water and hot water are provided to the slab by constant temperature flow sources, at 10C (cooling) and 40 C (heating). 
    ...
    __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/RadiantControl/Validation/ControlPlusLockoutPerimRoom.mos" "Simulate and plot"),
...
end ControlPlusLockoutPerimRoom;
