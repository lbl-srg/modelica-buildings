...
block OpenLoop "Open-loop controller"
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1ValHeaWatHpInlIso[nHp](
    ...
    each timeScale=1000,
    each period=5000) if cfg.have_heaWat and nHp>1
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1ValChiWatHpInlIso[nHp](
    each table=[0,0; 3,1; 5,1],
    each timeScale=1000,
    each period=5000) if cfg.have_chiWat and nHp>1
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1ValChiWatHpOutIso[nHp](
    each table=[0,0; 3,1; 5,1],
    each timeScale=1000,
    each period=5000) if cfg.have_chiWat
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1HeaHp[nHp](
    each table=[0,1; 3,0; 5,0],
    each timeScale=1000,
    each period=5000) if cfg.is_rev "Heat pump heating mode command"
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1PumHeaWatSec[cfg.nPumHeaWatSec](
    each table=[0,0; 1,1; 3,0; 5,0],
    each timeScale=1000,
    each period=5000) if cfg.typPumHeaWatSec<>Buildings.Templates.Plants.HeatPumps.Types.PumpsSecondary.None
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1PumChiWatSec[cfg.nPumChiWatSec](
    each table=[0,0; 3,1; 5,1],
    each timeScale=1000,
    each period=5000) if cfg.typPumChiWatSec<>Buildings.Templates.Plants.HeatPumps.Types.PumpsSecondary.None
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant yPumHeaWatSec(
    k=1) if cfg.typPumHeaWatSec<>Buildings.Templates.Plants.HeatPumps.Types.PumpsSecondary.None
    "Secondary HW pump speed signal"
    annotation (Placement(transformation(extent={{60,50},{80,70}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant yPumChiWatSec(
    k=1) if cfg.typPumChiWatSec<>Buildings.Templates.Plants.HeatPumps.Types.PumpsSecondary.None
...
equation
  ...
  connect(yPumHeaWatPri.y[1], busPumHeaWatPri.y);
  connect(y1Hp.y[1], busHp.y1);
  connect(y1HeaHp.y[1], busHp.y1Hea);
  connect(TSet.y, busHp.TSet);
  /* Control point connection - stop */
  connect(TChiWatSupSet.y, TSet.u3) annotation (Line(points={{-102,160},{-120,160},
          {-120,172},{-138,172}}, color={0,0,127}));
  connect(THeaWatSupSet.y, TSet.u1) annotation (Line(points={{-102,200},{-120,200},
          {-120,188},{-138,188}}, color={0,0,127}));
  connect(y1HeaHp.y[1], TSet.u2) annotation (Line(points={{-202,160},{-210,160},
          {-210,140},{-130,140},{-130,180},{-138,180}}, color={255,0,255}));
  connect(tru.y, TSet.u2) annotation (Line(points={{-102,240},{-130,240},{-130,180},
