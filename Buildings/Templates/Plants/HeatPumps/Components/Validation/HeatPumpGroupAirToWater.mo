...
model HeatPumpGroupAirToWater "Validation model for heat pump group"
  ...
  parameter Data.Controller datCtlHpAwNrv(
    cfg(
    have_chiWat=false,
  ...
  parameter Data.Controller datCtlHpAw(
    cfg(
    have_chiWat=true,
    typPumHeaWatSec=Buildings.Templates.Plants.HeatPumps.Types.PumpsSecondary.None,
    typDis=Buildings.Templates.Plants.HeatPumps.Types.Distribution.Constant1Only,
  ...
  parameter Buildings.Templates.Plants.HeatPumps.Components.Data.HeatPumpGroup
    ...
    dpHeaWatHp_nominal=Buildings.Templates.Data.Defaults.dpConWatChi,
    ...
    annotation (Placement(transformation(extent={{-130,42},{-110,62}})));
  parameter Buildings.Templates.Plants.HeatPumps.Components.Data.HeatPumpGroup
    datHpAw(
  ...
  BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    ...
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},rotation=0,
      origin={180,160})));
  Fluid.Sensors.TemperatureTwoPort TRet1[hpAwNrv.nHp](redeclare each final
      package Medium = Medium, each final m_flow_nominal=datHpAwNrv.mHeaWatHp_flow_nominal)
    "Return temperature"
    annotation (Placement(transformation(extent={{-80,130},{-60,150}})));
  Fluid.Sensors.TemperatureTwoPort TSup1[hpAwNrv.nHp](redeclare each final
      package Medium = Medium, each final m_flow_nominal=datHpAwNrv.mHeaWatHp_flow_nominal)
    "Supply temperature"
    annotation (Placement(transformation(extent={{60,130},{80,150}})));
  Controls.OpenLoop ctlHpAwNrv(
    final cfg=datCtlHpAwNrv.cfg,
    final dat=datCtlHpAwNrv) "Heat pump controller"
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant pHeaWatInl(k=sup.p +
        datHpAwNrv.dpHeaWatHp_nominal) "HW inlet pressure"
  ...
  Controls.OpenLoop ctlHpAw(
    final cfg=datCtlHpAw.cfg,
    final dat=datCtlHpAw) "Heat pump controller"
  ...
  Fluid.Sensors.TemperatureTwoPort TRet[hpAw.nHp](redeclare each final
      package Medium = Medium, each final m_flow_nominal=datHpAw.mHeaWatHp_flow_nominal)
    "Return temperature"
  ...
  Fluid.Sources.Boundary_pT sup1(
    ...
    "Boundary condition at distribution system supply"
    annotation (Placement(transformation(extent={{120,-50},{100,-30}})));
  Fluid.Sensors.TemperatureTwoPort TSup[hpAw.nHp](redeclare each final
      package Medium = Medium, each final m_flow_nominal=datHpAw.mHeaWatHp_flow_nominal)
    "Supply temperature"
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant pHeaWatInl1(k=sup.p + hpAw.dpHeaWatHp_nominal)
    "HW inlet pressure"
    annotation (Placement(transformation(extent={{-210,10},{-190,30}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant pChiWatInl(k=sup.p + hpAw.dpChiWatHp_nominal)
    "CHW inlet pressure"
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},rotation=0,
      origin={-200,-20})));
  ...
  Buildings.Templates.Components.Interfaces.Bus busHp[hpAw.nHp]
    "HP control bus" annotation (Placement(transformation(extent={{-120,-20},{
            -80,20}}), iconTransformation(extent={{-536,100},{-496,140}})));
equation
  connect(ctlHpAwNrv.bus, hpAwNrv.bus) annotation (Line(
      points={{-20,140},{0,140},{0,120}},
      color={255,204,51},
      thickness=0.5));
  connect(inlHp1.ports, TRet1.port_a)
    annotation (Line(points={{-100,140},{-80,140}}, color={0,127,255}));
  connect(TRet1.port_b, hpAwNrv.ports_aChiHeaWat) annotation (Line(points={{-60,140},
          {-50,140},{-50,120}},     color={0,127,255}));
  connect(hpAwNrv.ports_bChiHeaWat, TSup1.port_a)
    annotation (Line(points={{50,120},{50,140},{60,140}}, color={0,127,255}));
  connect(TSup1.port_b, sup.ports) annotation (Line(points={{80,140},{100,140}},
                              color={0,127,255}));
  connect(weaDat.weaBus, hpAwNrv.busWea) annotation (Line(
      points={{170,160},{24,160},{24,120}},
      color={255,204,51},
      thickness=0.5));
  connect(THeaWatRet.y, inlHp1.T_in) annotation (Line(points={{-148,120},{-140,
          120},{-140,144},{-122,144}},
                                  color={0,0,127}));
  connect(pHeaWatInl.y, inlHp1.p_in) annotation (Line(points={{-148,160},{-140,
          160},{-140,148},{-122,148}}, color={0,0,127}));
  connect(weaDat.weaBus, hpAw.busWea) annotation (Line(
      points={{170,160},{160,160},{160,0},{24,0},{24,-60}},
      color={255,204,51},
      thickness=0.5));
  connect(ctlHpAw.bus, hpAw.bus) annotation (Line(
      points={{-20,-40},{0,-40},{0,-60}},
      color={255,204,51},
      thickness=0.5));
  connect(inlHp.ports, TRet.port_a)
    annotation (Line(points={{-100,-40},{-80,-40}}, color={0,127,255}));
  connect(TRet.port_b, hpAw.ports_aChiHeaWat) annotation (Line(points={{-60,-40},
          {-50,-40},{-50,-60}}, color={0,127,255}));
  ...
  connect(THeaWatRet1.y, TRetAct.u1) annotation (Line(points={{-188,-60},{-180,
          -60},{-180,-72},{-162,-72}},
                                  color={0,0,127}));
  connect(TChiWatRet.y,TRetAct. u3)
    annotation (Line(points={{-188,-100},{-170,-100},{-170,-88},{-162,-88}},
                                                                  color={0,0,127}));
  connect(pHeaWatInl1.y, pInl_rel.u1) annotation (Line(points={{-188,20},{-180,
          20},{-180,8},{-162,8}},
                              color={0,0,127}));
  connect(pChiWatInl.y, pInl_rel.u3) annotation (Line(points={{-188,-20},{-180,-20},
          {-180,-8},{-162,-8}}, color={0,0,127}));
  connect(pInl_rel.y, inlHp.p_in) annotation (Line(points={{-138,0},{-132,0},{-132,
          -32},{-122,-32}}, color={0,0,127}));
  connect(TRetAct.y, inlHp.T_in) annotation (Line(points={{-138,-80},{-132,-80},
          {-132,-36},{-122,-36}}, color={0,0,127}));
  connect(busPla, ctlHpAw.bus) annotation (Line(
      points={{0,0},{0,-40},{-20,-40}},
      color={255,204,51},
      thickness=0.5));
  connect(busPla.hp, busHp) annotation (Line(
      points={{0,0},{-100,0}},
      color={255,204,51},
      thickness=0.5));
  connect(busHp[1].y1Hea, pInl_rel.u2) annotation (Line(
      points={{-100,0},{-100,20},{-170,20},{-170,0},{-162,0}},
      color={255,204,51},
      thickness=0.5));
  connect(pInl_rel.u2, TRetAct.u2) annotation (Line(points={{-162,0},{-170,0},{
