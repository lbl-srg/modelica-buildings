...
model PartialHeatPumpGroup
  ...
  */
  ...
  parameter Buildings.Templates.Plants.HeatPumps.Components.Data.HeatPumpGroup dat(
    ...
    annotation (
      Placement(transformation(extent={{170,170},{190,190}})), __ctrlFlow(
        enable=false));
  ...
  Modelica.Fluid.Interfaces.FluidPorts_b ports_bChiHeaWat[nHp](
    ...
    "CHW/HW supply" annotation (Placement(transformation(
        extent={{-10,-40},{10,40}},
        rotation=90,
  ...
  Modelica.Fluid.Interfaces.FluidPorts_a ports_aChiHeaWat[nHp](
    ...
    each h_outflow(start=MediumHeaWat.h_default, nominal=MediumHeaWat.h_default))
    "CHW/HW return" annotation (Placement(transformation(
        extent={{-10,-40},{10,40}},
        rotation=90,
        origin={120,200}), iconTransformation(
        extent={{-10,-40},{10,40}},
  ...
  MediumHeaWat.ThermodynamicState sta_aChiHeaWat[nHp]=
      MediumHeaWat.setState_phX(ports_aChiHeaWat.p,
                          noEvent(actualStream(ports_aChiHeaWat.h_outflow)),
                          noEvent(actualStream(ports_aChiHeaWat.Xi_outflow)))
      if show_T "CHW/HW medium properties in port_aChiHeaWat";
  MediumHeaWat.ThermodynamicState sta_bChiHeaWat[nHp]=
      MediumHeaWat.setState_phX(ports_bChiHeaWat.p,
                          noEvent(actualStream(ports_bChiHeaWat.h_outflow)),
                          noEvent(actualStream(ports_bChiHeaWat.Xi_outflow)))
      if show_T "CHW/HW medium properties in port_bChiHeaWat";
...
protected
  Buildings.Templates.Components.Interfaces.Bus busHp[nHp]
    "Heat pump control bus" annotation (Placement(transformation(extent={{-20,140},
            {20,180}}), iconTransformation(extent={{-522,206},{-482,246}})));
equation
  connect(bus.hp, busHp) annotation (Line(
      points={{0,200},{0,200},{0,160}},
      color={255,204,51},
      thickness=0.5));
