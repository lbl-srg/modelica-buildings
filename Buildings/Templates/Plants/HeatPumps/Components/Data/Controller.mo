...
record Controller
  ...
  parameter Modelica.Units.SI.PressureDifference dpHeaWatRemSet_max[:](
    ...
    annotation(Dialog(
      ...
      enable=cfg.have_heaWat
        and cfg.typCtl ==
          Buildings.Templates.Plants.HeatPumps.Types.Controller.AirToWater));
  parameter Real dpHeaWatLocSet_min(start=0, final unit="Pa", final min=0) =
    5 * 6895
  ...
  parameter Real dpHeaWatLocSet_max(start=1E5, final unit="Pa", final min=0)
    ...
    annotation(Dialog(
      ...
      enable=cfg.have_heaWat
        ...
        and (cfg.typPumHeaWatPriHp ==
          ...
          or cfg.typPumHeaWatPriShc ==
            Buildings.Templates.Plants.HeatPumps.Types.PumpsPrimary.Variable)));
  parameter Real yPumHeaWatSec_min(final unit="1", final min=0, final max=1) =
    0.1
    ...
    annotation(Dialog(
      ...
      enable=cfg.have_chiWat
        ...
        and (cfg.typDis ==
          ...
          or cfg.typPumChiWatSec <>
            Buildings.Templates.Plants.HeatPumps.Types.PumpsSecondary.None)));
  parameter Real dpChiWatLocSet_min(start=0, final unit="Pa", final min=0) =
    5 * 6895
  ...
  parameter Real dpChiWatLocSet_max(start=1E5, final unit="Pa", final min=0)
    ...
    annotation(Dialog(
      ...
      enable=cfg.have_chiWat
        ...
        and (cfg.typPumChiWatPriHp ==
          ...
          or cfg.typPumChiWatPriShc ==
            Buildings.Templates.Plants.HeatPumps.Types.PumpsPrimary.Variable)));
  parameter Real yPumChiWatSec_min(final unit="1", final min=0, final max=1) =
    0.1
  ...
  parameter Real plrSta(final max=1, final min=0, start=0.9, final unit="1") =
    0.9
  ...
  parameter Real THeaWatSupHrc_max(
    ...
    annotation(Dialog(group="Information provided by designer",
      enable=cfg.have_hrc));
  parameter Real COPHeaHrc_nominal(final min=1.1, final unit="1", start=2.8)
    "Sidestream HRC – Heating COP at design heating conditions"
    annotation(Dialog(group="Information provided by designer",
      enable=cfg.have_hrc));
  parameter Real capCooHrc_min(start=0, final min=0, final unit="W")
    "Sidestream HRC – Minimum cooling capacity below which cycling occurs"
    annotation(Dialog(group="Information provided by designer",
      enable=cfg.have_hrc));
  parameter Real capHeaHrc_min(start=0, final min=0, final unit="W")
