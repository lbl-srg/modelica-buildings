...
record HeatPumpGroup
  ...
  */
  ...
  final parameter Modelica.Units.SI.Temperature THeaWatRetHp_nominal=
    THeaWatSupHp_nominal-abs(capHeaHp_nominal)/cpHeaWat_default/mHeaWatHp_flow_nominal
  ...
  final parameter Modelica.Units.SI.Temperature TChiWatRetHp_nominal=
    if is_rev then
    TChiWatSupHp_nominal+abs(capCooHp_nominal)/cpChiWat_default/mChiWatHp_flow_nominal
  ...
  parameter Modelica.Units.SI.Temperature TSouHeaHp_nominal(
    ...
    annotation(Dialog(group="Nominal condition"));
  parameter Modelica.Units.SI.MassFlowRate mSouWatHeaHp_flow_nominal(
  ...
  parameter Modelica.Units.SI.PressureDifference dpSouWatHeaHp_nominal(
    min=0,
    start=Buildings.Templates.Data.Defaults.dpChiWatChi)
  ...
  final parameter Modelica.Units.SI.MassFlowRate mSouHeaHp_flow_nominal=
    if typ==Buildings.Templates.Components.Types.HeatPump.WaterToWater then
      mSouWatHeaHp_flow_nominal else
      Buildings.Templates.Data.Defaults.mAirFloByCapChi * abs(capHeaHp_nominal)
    "Source fluid mass flow rate in heating mode - Each heat pump";
  final parameter Modelica.Units.SI.PressureDifference dpSouHeaHp_nominal=
    if typ==Buildings.Templates.Components.Types.HeatPump.WaterToWater then
      dpSouWatHeaHp_nominal else Buildings.Templates.Data.Defaults.dpAirChi
    "Source fluid pressure drop in heating mode - Each heat pump";
  parameter Modelica.Units.SI.Temperature TSouCooHp_nominal(
    ...
    annotation(Dialog(group="Nominal condition",
    enable=is_rev));
  parameter Modelica.Units.SI.MassFlowRate mSouWatCooHp_flow_nominal(
  ...
  final parameter Modelica.Units.SI.MassFlowRate mSouCooHp_flow_nominal=
    if typ==Buildings.Templates.Components.Types.HeatPump.WaterToWater then
      mSouWatCooHp_flow_nominal else
      Buildings.Templates.Data.Defaults.mAirFloByCapChi * abs(capCooHp_nominal)
    "Source fluid mass flow rate in cooling mode - Each heat pump";
  ...
  replaceable parameter
    Buildings.Fluid.HeatPumps.ModularReversible.Data.TableData2D.GenericHeatPump modHeaHp(
      ...
      tabPEle=[0,TSouHeaHp_nominal;THeaWatSupHp_nominal,0],
      tabQCon_flow=[0,TSouHeaHp_nominal;THeaWatSupHp_nominal,0],
      tabUppBou=[TSouHeaHp_nominal,THeaWatSupHp_nominal],
      devIde="",
  ...
  replaceable parameter Buildings.Fluid.Chillers.ModularReversible.Data.TableData2D.Generic modCooHp(
      ...
      tabPEle=[0,TSouCooHp_nominal;TSouCooHp_nominal,0],
      tabQEva_flow=[0,TSouCooHp_nominal;TSouCooHp_nominal,0],
      tabLowBou=[TSouCooHp_nominal,TChiWatSupHp_nominal],
      devIde="",
  ...
  replaceable parameter
    Buildings.Fluid.HeatPumps.Data.EquationFitReversible.Generic perFitHp(
    ...
    coo(
      ...
      TRefSou=TSouCooHp_nominal)) constrainedby
