...
block TimerWithReset
  ...
  Buildings.Controls.OBC.CDL.Logical.Timer tim(final t=t)
    "Measuring time and check if bolean input has been true more than threshold time"
    annotation(Placement(transformation(extent={{40,-10},{60,10}})));
equation
  connect(reset, edg.u)
    annotation(Line(points={{-120,-80},{-82,-80}},
      color={255,0,255}));
  connect(edg.y, xor.u2)
    annotation(Line(points={{-58,-80},{-20,-80},{-20,-8},{-2,-8}},
      color={255,0,255}));
  connect(u, xor.u1)
    annotation(Line(points={{-120,0},{-2,0}},
      color={255,0,255}));
  connect(xor.y, tim.u)
    annotation(Line(points={{22,0},{38,0}},
      color={255,0,255}));
  connect(tim.y, y)
    annotation(Line(points={{62,0},{120,0}},
      color={0,0,127}));
  connect(tim.passed, passed)
    annotation(Line(points={{62,-8},{80,-8},{80,-80},{120,-80}},
      color={255,0,255}));
annotation(defaultComponentName="tim",
  Icon(coordinateSystem(preserveAspectRatio=true,
    ...
    Text(extent={{-64,62},{62,92}},
      textColor={0,0,0},
      textString="t=%t"),
    Ellipse(extent={{-83,7},{-69,-7}},
      lineColor=DynamicSelect({235,235,235},
        if u then {0,255,0} else {235,235,235}),
      fillColor=DynamicSelect({235,235,235},
        if u then {0,255,0} else {235,235,235}),
      fillPattern=FillPattern.Solid),
    Ellipse(extent={{71,-73},{85,-87}},
      lineColor=DynamicSelect({235,235,235},
        if passed then {0,255,0} else {235,235,235}),
      fillColor=DynamicSelect({235,235,235},
        if passed then {0,255,0} else {235,235,235}),
      fillPattern=FillPattern.Solid),
    Text(extent={{226,60},{106,10}},
      ...
      textString=DynamicSelect("",
        String(y, leftJustified=false, significantDigits=3)))}),
...
<ul>
  <li>
    ...
    Refactored with CDL blocks.
  </li>
  <li>
    October 21, 2024, by Antoine Gautier:<br />
    Refactored to ensure <code>passed=u</code> if <code>t=0</code>. This is
    for
