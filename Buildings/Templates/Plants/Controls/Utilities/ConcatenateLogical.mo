...
block ConcatenateLogical
  ...
  final parameter Integer idxExt1[nout] = if is_app
    then cat(1, {i for i in 1:nin1}, fill(1, nin2))
    else cat(1, fill(1, nin2), {i for i in 1:nin1})
    "Indices of first array to be extracted";
  final parameter Integer idxExt2[nout] = if is_app
    then cat(1, fill(1, nin1), {i for i in 1:nin2})
    else cat(1, {i for i in 1:nin2}, fill(1, nin1))
    "Indices of second array to be extracted";
  final parameter Boolean selInp[nout] = if is_app
    then cat(1, fill(true, nin1), fill(false, nin2))
    else cat(1, fill(false, nin2), fill(true, nin1))
...
equation
  ...
  connect(expInp.y, logSwi.u1)
    annotation(Line(points={{12,0},{20,0},{20,8},{58,8}},
      color={255,0,255}));
  connect(exp2.y, logSwi.u3)
    annotation(Line(points={{12,-80},{40,-80},{40,-8},{58,-8}},
      color={255,0,255}));
  connect(u2, exp2.u)
...
annotation(defaultComponentName="cat1",
  ...
  Documentation(
    info="<html>
<p>This block concatenates two Boolean arrays.</p>
<p>
  When <code>is_app = true</code> (default), the values of <code>u2</code> are
  appended after the values of <code>u1</code>.
</p>
<p>
  When <code>is_app = false</code>, the values of <code>u2</code> are
  prepended before the values of <code>u1</code>.
</p>
</html>",
    revisions="<html>
<ul>
