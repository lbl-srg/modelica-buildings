...
block PlantReset
  ...
  parameter Real dtRes(
    ...
    annotation(Dialog(tab="Advanced", group="Trim and respond"));
  parameter Integer nReqResIgn(final min=0) = 2 "Number of ignored requests"
    annotation(Dialog(tab="Advanced", group="Trim and respond"));
  parameter Real tri(
    final max=0,
    final unit="1")=-0.02
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput dpSet[nSenDpRem](
    each final min=0,
    each final unit="Pa") "Differential pressure setpoint"
  ...
  Buildings.Controls.OBC.ASHRAE.G36.Generic.TrimAndRespond triRes(
    ...
    final triAmo=tri)    "Compute plant reset with trim and respond logic "
  ...
  Buildings.Controls.OBC.CDL.Reals.Line resTSup "Supply temperature reset"
    annotation (Placement(transformation(extent={{120,-110},{140,-90}})));
  Buildings.Controls.OBC.CDL.Reals.Line resDp[nSenDpRem] "Differential pressure reset"
    annotation (Placement(transformation(extent={{120,-30},{140,-10}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant zer(final k=0) "Constant"
    annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant one(final k=1) "Constant"
    annotation (Placement(transformation(extent={{-10,-130},{10,-110}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant resDpMax(
    final k=resDp_max) "Constant"
    annotation (Placement(transformation(extent={{-10,-50},{10,-30}})));
  Buildings.Controls.OBC.CDL.Routing.RealScalarReplicator rep(final nout=
        nSenDpRem) "Replicate signal"
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant dpSetMax[nSenDpRem](final k
      =dpSet_max) "Constant"
    annotation (Placement(transformation(extent={{-90,-70},{-70,-50}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant resTSupMin(final k=
        resTSup_min) "Constant"
    annotation (Placement(transformation(extent={{-10,-90},{10,-70}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant TSupSetNom(final k=
        TSup_nominal)    "Constant"
    annotation (Placement(transformation(extent={{-90,-150},{-70,-130}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant dpSetMin(final k=dpSet_min)
    "Constant"
...
equation
  ...
  connect(resTSup.y, TSupSet)
    annotation (Line(points={{142,-100},{180,-100}},
                                               color={0,0,127}));
  connect(resDp.y, dpSet)
    annotation (Line(points={{142,-20},{180,-20}}, color={0,0,127}));
  connect(one.y, resTSup.x2) annotation (Line(points={{12,-120},{100,-120},{100,
          -104},{118,-104}}, color={0,0,127}));
  connect(res.y, resTSup.u) annotation (Line(points={{52,60},{104,60},{104,-100},
          {118,-100}}, color={0,0,127}));
  connect(zer.y, rep.u)
    annotation (Line(points={{12,0},{28,0}}, color={0,0,127}));
  connect(rep.y, resDp.x1) annotation (Line(points={{52,0},{80,0},{80,-12},{118,
          -12}}, color={0,0,127}));
  connect(res.y, rep1.u) annotation (Line(points={{52,60},{60,60},{60,40},{68,
          40}}, color={0,0,127}));
  connect(rep1.y, resDp.u) annotation (Line(points={{92,40},{100,40},{100,-20},
          {118,-20}}, color={0,0,127}));
  connect(rep2.y, resDp.x2) annotation (Line(points={{52,-40},{80,-40},{80,-24},
          {118,-24}}, color={0,0,127}));
  connect(dpSetMax.y, resDp.f2) annotation (Line(points={{-68,-60},{100,-60},{
          100,-28},{118,-28}}, color={0,0,127}));
  connect(TSupSetNom.y, resTSup.f2) annotation (Line(points={{-68,-140},{104,
          -140},{104,-108},{118,-108}}, color={0,0,127}));
  connect(dpSetMin.y, rep3.u)
    annotation (Line(points={{-68,20},{-52,20}}, color={0,0,127}));
  connect(rep3.y, resDp.f1) annotation (Line(points={{-28,20},{76,20},{76,-16},
          {118,-16}}, color={0,0,127}));
  connect(TSupSetMinMax.y, resTSup.f1) annotation (Line(points={{-68,-100},{80,
          -100},{80,-96},{118,-96}}, color={0,0,127}));
  connect(resTSupMin.y, resTSup.x1) annotation (Line(points={{12,-80},{80,-80},{
          80,-92},{118,-92}}, color={0,0,127}));
