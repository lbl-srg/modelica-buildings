...
model AirToWater
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Sin sin[1](
    ...
    freqHz={4/8000},
    each phase=3.1415926535898) if ctl.have_heaWat
  ...
  Buildings.Controls.OBC.CDL.Reals.Add dpChiWatRem[1] if ctl.have_chiWat
    "Differential pressure at remote location"
    annotation (Placement(transformation(extent={{-80,-110},{-60,-90}})));
  Pumps.Generic.ResetLocalDifferentialPressure resDpHeaWatLoc[1](each
      dpLocSet_max=ctl.dpHeaWatLocSet_max) if ctl.have_heaWat
    "Local HW DP reset"
    annotation (Placement(transformation(extent={{-40,-130},{-20,-110}})));
  Pumps.Generic.ResetLocalDifferentialPressure resDpChiWatLoc[1](each
      dpLocSet_max=ctl.dpChiWatLocSet_max) if ctl.have_chiWat
    "Local CHW DP reset"
  ...
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter dpHeaWatLoc(final k=5)
    if ctl.have_heaWat "Differential pressure local to the plant"
    annotation (Placement(transformation(extent={{-80,-150},{-60,-130}})));
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter dpChiWatLoc(final k=4)
    if ctl.have_chiWat "Differential pressure local to the plant"
    annotation (Placement(transformation(extent={{-80,-190},{-60,-170}})));
equation
  connect(ratV_flow.y[1], VHeaWat_flow.u)
    annotation (Line(points={{-138,-20},{-120,-20},{-120,0},{-112,0}},
      color={0,0,127}));
  connect(ratV_flow.y[2], VChiWat_flow.u)
    annotation (Line(points={{-138,-20},{-82,-20}},  color={0,0,127}));
  connect(ctl.y1Hp, y1Hp_actual.y1)
    annotation (Line(points={{42,56},{44,56},{44,80},{68,80}},
                                              color={255,0,255}));
  ...
  connect(ctl.y1PumHeaWatPri, y1PumHeaWatPri_actual1.y1)
    annotation (Line(points={{42,40},{60,40},{60,60},{98,60}},
                                                          color={255,0,255}));
  ...
  connect(ctl.y1PumChiWatSec, y1PumChiWatSec_actual.y1)
    annotation (Line(points={{42,32},{60,32},{60,0},{68,0}},
                                                        color={255,0,255}));
  connect(y1PumHeaWatPri_actual1.y1_actual, ctl.u1PumHeaWatPri_actual)
    annotation (Line(points={{122,60},{134,60},{134,102},{-6,102},{-6,50},{-2,
          50}},
      color={255,0,255}));
  connect(y1PumHeaWatSec_actual.y1_actual, ctl.u1PumHeaWatSec_actual)
    annotation (Line(points={{122,20},{138,20},{138,106},{-10,106},{-10,46},{-2,
          46}},
      color={255,0,255}));
  ...
  connect(u1Ava.y,ctl.u1AvaHp)
    annotation (Line(points={{-138,180},{-14,180},{-14,54},{-2,54}},
                                                                   color={255,0,255}));
  connect(TOut.y, ctl.TOut)
    annotation (Line(points={{-138,100},{-32,100},{-32,30.2},{-2,30.2}},
                                                                     color={0,0,127}));
  ...
  connect(ratV_flow.y[2], isDemCoo.u)
    annotation (Line(points={{-138,-20},{-120,-20},{-120,120},{-112,120}},
                                                                        color={0,0,127}));
  connect(isDemCoo.y, reqPlaChiWat.u)
    annotation (Line(points={{-88,120},{-82,120}},
                                              color={255,0,255}));
  connect(isDemHea.y, reqPlaHeaWat.u)
    annotation (Line(points={{-88,160},{-82,160}},
                                                color={255,0,255}));
  connect(reqPlaHeaWat.y, ctl.nReqHeaWat)
    annotation (Line(points={{-58,160},{-40,160},{-40,40},{-2,40}},
                                                                color={255,127,0}));
  connect(reqPlaChiWat.y, ctl.nReqChiWat)
    annotation (Line(points={{-58,120},{-40,120},{-40,38},{-2,38}},
                                                              color={255,127,0}));
  ...
  connect(THeaWatRet.y, ctl.THeaWatPriRet)
    annotation (Line(points={{-138,60},{-30,60},{-30,28.2},{-2,28.2}},
                                                                     color={0,0,127}));
  connect(TChiWatRet.y, ctl.TChiWatSecRet)
    annotation (Line(points={{-138,20},{-28,20},{-28,16},{-2,16}},
      color={0,0,127}));
  ...
  connect(VHeaWat_flow.y, ctl.VHeaWatSec_flow)
    annotation (Line(points={{-88,0},{-26,0},{-26,18},{-2,18}},
      color={0,0,127}));
  ...
  connect(VChiWat_flow.y, ctl.VChiWatSec_flow)
    annotation (Line(points={{-58,-20},{-22,-20},{-22,14},{-2,14}},
      color={0,0,127}));
  connect(ratV_flow.y, gai.u) annotation (Line(points={{-138,-20},{-120,-20},{
          -120,80},{-110,80}},    color={0,0,127}));
  connect(gai[1].y, reqResHeaWat.u)
    annotation (Line(points={{-86,80},{-82,80}},   color={0,0,127}));
  connect(gai[2].y, reqResChiWat.u) annotation (Line(points={{-86,80},{-84,80},
          {-84,40},{-82,40}},   color={0,0,127}));
  connect(reqResHeaWat.y, ctl.uReqResHeaWat) annotation (Line(points={{-58,80},
          {-38,80},{-38,36},{-2,36}}, color={255,127,0}));
  connect(reqResChiWat.y, ctl.uReqResChiWat) annotation (Line(points={{-58,40},
          {-36,40},{-36,34},{-2,34}},   color={255,127,0}));
  connect(sin.y, dpHeaWatRem.u2)
    annotation (Line(points={{-138,-66},{-82,-66}}, color={0,0,127}));
  connect(dpChiWatRem.y, ctl.dpChiWatRem) annotation (Line(points={{-58,-100},{
          -18,-100},{-18,6},{-2,6}}, color={0,0,127}));
  connect(dpHeaWatRem.y, ctl.dpHeaWatRem) annotation (Line(points={{-58,-60},{
          -20,-60},{-20,12},{-2,12}}, color={0,0,127}));
  connect(ctl.dpHeaWatRemSet, dpHeaWatRem.u1) annotation (Line(points={{42,14},
          {50,14},{50,-40},{-100,-40},{-100,-54},{-82,-54}}, color={0,0,127}));
  connect(ctl.dpChiWatRemSet, dpChiWatRem.u1) annotation (Line(points={{42,12},
          {48,12},{48,-80},{-90,-80},{-90,-94},{-82,-94}}, color={0,0,127}));
  connect(sin1.y, dpChiWatRem.u2)
    annotation (Line(points={{-138,-106},{-82,-106}}, color={0,0,127}));
  connect(dpHeaWatRem[1].y, dpHeaWatLoc.u) annotation (Line(points={{-58,-60},{
          -52,-60},{-52,-120},{-86,-120},{-86,-140},{-82,-140}}, color={0,0,127}));
  connect(ctl.dpHeaWatRemSet, resDpHeaWatLoc.dpRemSet) annotation (Line(points=
          {{42,14},{50,14},{50,-40},{-100,-40},{-100,-114},{-42,-114}}, color={
          0,0,127}));
  connect(dpChiWatRem[1].y, dpChiWatLoc.u) annotation (Line(points={{-58,-100},
          {-54,-100},{-54,-160},{-90,-160},{-90,-180},{-82,-180}}, color={0,0,
          127}));
  connect(ctl.dpChiWatRemSet, resDpChiWatLoc.dpRemSet) annotation (Line(points=
          {{42,12},{47.9167,12},{47.9167,-80},{-90,-80},{-90,-154},{-42,-154}},
        color={0,0,127}));
  connect(dpHeaWatRem.y, resDpHeaWatLoc.dpRem) annotation (Line(points={{-58,
          -60},{-52,-60},{-52,-126},{-42,-126}}, color={0,0,127}));
  connect(dpChiWatRem.y, resDpChiWatLoc.dpRem) annotation (Line(points={{-58,
          -100},{-54,-100},{-54,-166},{-42,-166}}, color={0,0,127}));
  connect(resDpChiWatLoc.dpLocSet, ctl.dpChiWatLocSet) annotation (Line(points=
          {{-18.2,-160},{-14,-160},{-14,4},{-2,4}}, color={0,0,127}));
  connect(dpChiWatLoc.y, ctl.dpChiWatLoc) annotation (Line(points={{-58,-180},{
          -12,-180},{-12,2},{-2,2}}, color={0,0,127}));
  connect(dpHeaWatLoc.y, ctl.dpHeaWatLoc) annotation (Line(points={{-58,-140},{
          -16,-140},{-16,8},{-2,8}}, color={0,0,127}));
  connect(resDpHeaWatLoc.dpLocSet, ctl.dpHeaWatLocSet) annotation (Line(points=
          {{-18.2,-120},{-10,-120},{-10,10},{-2,10}}, color={0,0,127}));
  annotation (
    __Dymola_Commands(
      file=
