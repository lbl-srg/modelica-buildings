...
block AirToWater
  ...
  final parameter Boolean have_pumChiWatPriDedHp = if have_hp
    and have_chiWat
    and not have_pumPriHdr then have_pumChiWatPriDedHp_select else false
    "Set to true for HP with separate dedicated primary CHW pumps"
    annotation(Evaluate=true);
  final parameter Boolean have_pumChiWatPri = have_chiWat
    and (have_pumPriHdr or have_pumChiWatPriDedHp or have_php)
    "Set to true for HP with separate dedicated primary pumps for CHW and HW circuits"
    annotation(Evaluate=true);
  ...
  final parameter Boolean have_pumHeaWatPriVar = have_pumHeaWatPri
    and (is_priOnl or have_pumHeaWatPriVar_select)
    "Set to true for variable speed primary HW pumps, false for constant speed pumps"
    annotation(Evaluate=true);
  final parameter Boolean have_pumChiWatPriVar = have_pumChiWatPri
    and (is_priOnl or have_pumChiWatPriVar_select)
    "Set to true for variable speed primary CHW pumps, false for constant speed pumps"
    annotation(Evaluate=true);
  ...
  final parameter Integer nPumHeaWatPriDedHp = if have_hp and not have_pumPriHdr
    then nHp else 0
    "Number of dedicated primary HW pumps serving HP"
    annotation(Evaluate=true);
  final parameter Integer nPumHeaWatPriDedPhp = if have_php
    and not have_pumPriHdr then nPhp else 0
    "Number of dedicated primary HW pumps serving polyvalent HP"
    annotation(Evaluate=true);
  final parameter Integer nPumHeaWatPri = if have_pumHeaWatPri
    and have_pumPriHdr then nPumHeaWatPri_select
    elseif have_pumHeaWatPri and not have_pumPriHdr then nHp + nPhp else 0
    "Number of primary HW pumps"
    annotation(Evaluate=true);
  ...
  final parameter Integer nPumChiWatPriDedHp = if have_pumChiWatPriDedHp
    then nHp else 0
    "Number of dedicated primary CHW pumps serving HP"
    annotation(Evaluate=true);
  final parameter Integer nPumChiWatPriDedPhp = if have_php
    and not have_pumPriHdr then nPhp else 0
    "Number of dedicated primary CHW pumps serving polyvalent HP"
    annotation(Evaluate=true);
  final parameter Integer nPumChiWatPri = if have_pumChiWatPri
    and have_pumPriHdr then nPumChiWatPri_select
  ...
  final parameter Integer nPumHeaWatSec = if have_pumHeaWatSec
    then nPumHeaWatSec_select else 0
    "Number of secondary HW pumps"
    annotation(Evaluate=true);
  ...
  final parameter Integer nPumChiWatSec = if have_pumChiWatSec
    then nPumChiWatSec_select else 0
    "Number of secondary CHW pumps"
    annotation(Evaluate=true);
  ...
  parameter Real VHeaWatPhp_flow_min[nPhp](
    ...
    annotation(Evaluate=true,
      Dialog(group="Information provided by designer",
        enable=have_php and is_priOnl));
  parameter Real VHeaWatPri_flow_nominal(min=0, start=0, unit="m3/s") = sum(
    cat(1, VHeaWatHp_flow_nominal, VHeaWatPhp_flow_nominal))
  ...
  parameter Real dpHeaWatRemSet_max[nSenDpHeaWatRem](
    ...
    annotation(Dialog(
      ...
      enable=have_heaWat and have_pumHeaWatSec));
  parameter Real dpHeaWatRemSet_min(min=0, start=5 * 6894, unit="Pa") = 5 * 6894
    "Minimum value to which the HW differential pressure can be reset - Remote sensor"
    annotation(Dialog(group="Information provided by designer",
      enable=have_heaWat and have_pumHeaWatSec));
  parameter Real yPumHeaWatPriHdrSet(max=2, min=0, start=1, unit="1")
    ...
    annotation(Dialog(
      ...
      enable=have_heaWat
        ...
        and have_pumPriHdr));
  parameter Real yPumHeaWatPriDedHpSet(max=2, min=0, start=1, unit="1")
  ...
  parameter Real yPumHeaWatPriDedPhpSet(max=2, min=0, start=1, unit="1")
  ...
  parameter Real VChiWatPhp_flow_min[nPhp](
    ...
    annotation(Evaluate=true,
      Dialog(group="Information provided by designer",
        enable=have_php and is_priOnl));
  parameter Real VChiWatPri_flow_nominal(min=0, start=0, unit="m3/s") = sum(
    cat(1, VChiWatHp_flow_nominal, VChiWatPhp_flow_nominal))
    "Primary CHW volume flow rate"
    annotation(Evaluate=true,
      Dialog(group="Information provided by designer",
        enable=have_chiWat and is_priOnl and have_pumPriHdr));
  parameter Real VChiWatSec_flow_nominal(min=0, start=1E-6, unit="m3/s")
  ...
  parameter Real dpChiWatRemSet_max[nSenDpChiWatRem](
    ...
    annotation(Dialog(
      group="Information provided by testing, adjusting, and balancing contractor",
      enable=have_pumChiWatSec));
  parameter Real dpChiWatRemSet_min(min=0, start=5 * 6894, unit="Pa") = 5 * 6894
    "Minimum value to which the CHW differential pressure can be reset - Remote sensor"
    annotation(Dialog(group="Information provided by designer",
      enable=have_pumChiWatSec));
  parameter Real yPumChiWatPriHdrSet(max=2, min=0, start=1, unit="1")
  ...
  parameter Real yPumChiWatPriDedHpSet(max=2, min=0, start=1, unit="1")
    ...
    annotation(Dialog(
      ...
      enable=have_chiWat
        ...
        and have_hp
        and not have_pumPriHdr));
  parameter Real yPumChiWatPriDedPhpSet(max=2, min=0, start=1, unit="1")
    ...
    annotation(Dialog(
      ...
      enable=have_chiWat
        ...
        and have_php
        and not have_pumPriHdr));
  parameter Real cp_default(min=0, unit="J/(kg.K)") = 4184
    "Default specific heat capacity used to compute required capacity"
    annotation(Evaluate=true,
      Dialog(group="Information provided by designer"));
  parameter Real rho_default(min=0, unit="kg/m3") = 996
  ...
  parameter Integer nReqIgnChiWat(min=0) = 0
    ...
    annotation(Dialog(tab="Advanced",
      group="Plant enable"));
  parameter Real dTOutLck(min=0, unit="K") = 0.5
    "Hysteresis for outdoor air lockout temperature"
    annotation(Dialog(tab="Advanced",
      group="Plant enable"));
  parameter Real dtRunEna(min=0, unit="s") = 15 * 60
    "Minimum runtime of enable and disable states"
    annotation(Dialog(tab="Advanced",
      group="Plant enable"));
  parameter Real dtReqDis(min=0, unit="s") = 3 * 60
  ...
  // incompatible sizes for variable and its start value.
  parameter Integer idxAltHp_select[:](each final min=1) = 1:nHp
    "Indices of lead/lag alternate heat pumps"
    annotation(Evaluate=true,
      Dialog(group="Equipment staging and rotation",
        enable=have_hp and not have_php));
  final parameter Integer idxAltHp[:] = if have_hp and not have_php
    then idxAltHp_select else 1:nHp
    "Indices of lead/lag alternate heat pumps"
    annotation(Evaluate=true,
      Dialog(group="Equipment staging and rotation"));
  ...
  parameter Real dtRunSta(min=0, unit="s", displayUnit="min") = 900
  ...
  parameter Real dtOff(min=0, unit="s") = 900
    ...
    annotation(Dialog(tab="Advanced",
      group="Equipment staging and rotation"));
  parameter Real dtOffHp(min=0, unit="s") = 180
  ...
  parameter Real dtRunPumSta(min=0, start=600, unit="s") = 600
  ...
  parameter Real dtRunFaiSafPumSta(min=0, start=300, unit="s") = 300
    "Runtime before triggering stage change command based on failsafe condition"
    annotation(Dialog(tab="Advanced",
      group="Pump staging",
      enable=have_pumPriHdr and have_pumPriCtlDp
        or have_pumSecHdr and have_pumSecCtlDp));
  parameter Real dtRunFaiSafLowYPumSta(min=0, start=600, unit="s") = dtRunPumSta
    "Runtime before triggering stage change command based on low pump speed failsafe condition"
    annotation(Dialog(tab="Advanced",
      group="Pump staging",
      enable=have_pumPriHdr and have_pumPriCtlDp
        or have_pumSecHdr and have_pumSecCtlDp));
  parameter Real dVOffUpPumSta(max=1, min=0, start=0.03, unit="1") = 0.03
    "Stage up flow point offset"
    annotation(Dialog(tab="Advanced",
      group="Pump staging",
      enable=have_pumPriHdr and have_pumPriCtlDp
        or have_pumSecHdr and have_pumSecCtlDp));
  parameter Real dVOffDowPumSta(max=1, min=0, start=0.03, unit="1") =
    dVOffUpPumSta
    ...
    annotation(Dialog(tab="Advanced",
      ...
      enable=have_pumPriHdr and have_pumPriCtlDp
        or have_pumSecHdr and have_pumSecCtlDp));
  parameter Real dpOffPumSta(min=0, start=1E4, unit="Pa") = 1E4
    "Stage change ∆p point offset (>0)"
    annotation(Dialog(tab="Advanced",
      group="Pump staging",
      enable=have_pumPriHdr and have_pumPriCtlDp
        or have_pumSecHdr and have_pumSecCtlDp));
  parameter Real yUpPumSta(min=0, start=0.99, unit="1") = 0.99
    "Stage up pump speed point"
    annotation(Dialog(tab="Advanced",
      group="Pump staging",
      enable=have_pumPriHdr and have_pumPriCtlDp
        or have_pumSecHdr and have_pumSecCtlDp));
  parameter Real yDowPumSta(min=0, start=0.4, unit="1") = 0.4
    "Stage down pump speed point"
    annotation(Dialog(tab="Advanced",
      group="Pump staging",
      enable=have_pumPriHdr and have_pumPriCtlDp
        or have_pumSecHdr and have_pumSecCtlDp));
  parameter Real dtHol(min=0, unit="s") = 900
    "Minimum hold time during stage change"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec or have_pumChiWatSec));
  parameter Real resDpHeaWat_max(max=1, min=0, unit="1") = 0.5
    "Upper limit of plant reset interval for HW differential pressure reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec));
  parameter Real resTHeaWatSup_min(max=1, min=0, unit="1") = resDpHeaWat_max
    "Lower limit of plant reset interval for HW supply temperature reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec));
  parameter Real resDpChiWat_max(max=1, min=0, unit="1") = 0.5
    "Upper limit of plant reset interval for CHW differential pressure reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_pumChiWatSec));
  parameter Real resTChiWatSup_min(max=1, min=0, unit="1") = resDpChiWat_max
    "Lower limit of plant reset interval for CHW supply temperature reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_pumChiWatSec));
  parameter Real res_init(max=1, min=0, unit="1") = 1
    "Initial reset value"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec or have_pumChiWatSec));
  parameter Real res_min(max=1, min=0, unit="1") = 0
    "Minimum reset value"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec or have_pumChiWatSec));
  parameter Real res_max(max=1, min=0, unit="1") = 1
    "Maximum reset value"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec or have_pumChiWatSec));
  parameter Real dtDel(min=100 * 1E-15, unit="s") = 900
    "Delay time before the reset begins"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec or have_pumChiWatSec));
  parameter Real dtResHeaWat(min=1E-3, unit="s") = 300
  ...
  parameter Integer nReqResIgnHeaWat(min=0) = 2
    ...
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec));
  parameter Real triHeaWat(max=0, unit="1") = -0.02
    "Trim amount for HW plant reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec));
  parameter Real rspHeaWat(min=0, unit="1") = 0.03
    "Respond amount for HW plant reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec));
  parameter Real rspHeaWat_max(min=0, unit="1") = 0.07
    "Maximum response per time interval for HW plant reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_heaWat and have_pumHeaWatSec));
  parameter Real dtResChiWat(min=1E-3, unit="s") = 300
  ...
  parameter Integer nReqResIgnChiWat(min=0) = 2
    ...
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_pumChiWatSec));
  parameter Real triChiWat(max=0, unit="1") = -0.02
    "Trim amount for CHW plant reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_pumChiWatSec));
  parameter Real rspChiWat(min=0, unit="1") = 0.03
    "Respond amount for CHW plant reset"
    annotation(Dialog(tab="Advanced",
      group="Plant reset",
      enable=have_pumChiWatSec));
  parameter Real rspChiWat_max(min=0, unit="1") = 0.07
  ...
  parameter Real TiCtlDpChiWat(
    ...
    annotation(Dialog(tab="Advanced",
      group="Loop differential pressure",
      enable=have_chiWat));
  parameter Real yPumHeaWatSec_min(max=1, min=0, start=0.1, unit="1") = 0.1
    "Minimum secondary HW pump speed"
    annotation(Dialog(tab="Advanced",
      group="Pump control",
      enable=have_pumHeaWatSec));
  parameter Real yPumChiWatSec_min(max=1, min=0, start=0.1, unit="1") = 0.1
    "Minimum secondary CHW pump speed"
    annotation(Dialog(tab="Advanced",
      group="Pump control",
      enable=have_pumChiWatSec));
  parameter Real kValMinByp(min=0, start=1) = 1
    "Gain of controller"
    annotation(Dialog(tab="Advanced",
  ...
  parameter Real THeaWatSupHrc_max(
    ...
    annotation(Dialog(group="Information provided by designer",
      enable=have_hrc));
  parameter Real COPHeaHrc_nominal(min=1.1, start=2.8, unit="1")
    "Sidestream HRC – Heating COP at design heating conditions"
    annotation(Dialog(group="Information provided by designer",
      enable=have_hrc));
  parameter Real capCooHrc_min(min=0, start=0, unit="W")
    "Sidestream HRC – Minimum cooling capacity below which cycling occurs"
    annotation(Dialog(group="Information provided by designer",
      enable=have_hrc));
  parameter Real capHeaHrc_min(min=0, start=0, unit="W")
    "Sidestream HRC – Minimum heating capacity below which cycling occurs"
    annotation(Dialog(group="Information provided by designer",
      enable=have_hrc));
  parameter Real dtLoaHrc(min=0, start=600, unit="s") = 600
    "Runtime with sufficient load before enabling"
    annotation(Dialog(tab="Advanced",
      group="Sidestream HRC",
      enable=have_hrc));
  parameter Real dtTem1Hrc(min=0, start=180, unit="s") = 180
    "Runtime with first temperature threshold exceeded before disabling"
    annotation(Dialog(tab="Advanced",
      group="Sidestream HRC",
      enable=have_hrc));
  parameter Real dtTem2Hrc(min=0, start=60, unit="s") = 60
  ...
  Subsequences.SelectModeState selMod(
    ...
    annotation(Placement(transformation(extent={{50,346},{70,370}})));
  ...
  Subsequences.EquipmentAvailability avaHeaCooHp[nHp](
  ...
  StagingRotation.SortRuntime sorRunTimHp(final idxEquAlt=idxAltHp, nin=nHp)
    if have_hp
    "Sort lead/lag alternate HP by increasing runtime"
  ...
  Pumps.Generic.StagingHeadered staPumHeaWatPri(
    ...
    final have_valInlIso=if have_hp then have_valHpInlIso
      else have_valPhpInlIso,
    final have_valOutIso=if have_hp then have_valHpOutIso
      else have_valPhpOutIso,
    final nEqu=nHp + nPhp,
  ...
  Utilities.PlaceholderReal THeaWatRet(
    final have_inp=have_senTHeaWatPriRet and have_senVHeaWatPri,
  ...
  Utilities.PlaceholderReal TChiWatRet(
    final have_inp=have_senTChiWatPriRet and have_senVChiWatPri,
    final have_inpPh=true)
    if have_chiWat
    "For staging logic select primary sensor if both temperature and flow sensors available"
  ...
  Buildings.Controls.OBC.CDL.Routing.RealScalarReplicator repTChiWatSupHpSet(
    final nout=nHp)
    if have_chiWat and have_hp
    "Replicate CHWST setpoint"
    annotation(Placement(transformation(extent={{210,-150},{230,-130}})));
  Buildings.Controls.OBC.CDL.Routing.RealScalarReplicator repTHeaWatSupHpSet(
    final nout=nHp)
    if have_heaWat and have_hp
  ...
  MinimumFlow.ControllerHeatPumps ctlFloMin(
    ...
    final VChiWat_flow_min={if i <= nHp then VChiWatHp_flow_min[i]
      else VChiWatPhp_flow_min[i - nHp] for i in 1:nHp + nPhp},
    final VChiWat_flow_nominal={if i <= nHp then VChiWatHp_flow_nominal[i]
      else VChiWatPhp_flow_nominal[i - nHp] for i in 1:nHp + nPhp},
    final VHeaWat_flow_min={if i <= nHp then VHeaWatHp_flow_min[i]
      else VHeaWatPhp_flow_min[i - nHp] for i in 1:nHp + nPhp},
    final VHeaWat_flow_nominal={if i <= nHp then VHeaWatHp_flow_nominal[i]
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant u1AvaPhp[nPhp](
    ...
    annotation(Placement(transformation(extent={{-230,410},{-210,430}}),
      iconTransformation(extent={{-240,220},{-200,260}})));
  PolyvalentHeatPumps.StagingParameters staPhp(final nHp=nHp, final nPhp=nPhp)
    ...
    annotation(Placement(transformation(extent={{-230,470},{-210,490}})));
  ...
  Buildings.Controls.OBC.CDL.Routing.RealScalarReplicator repTChiWatSupPhpSet(
    final nout=nPhp)
    if have_chiWat and have_php
    "Replicate CHWST setpoint"
    annotation(Placement(transformation(extent={{210,-190},{230,-170}})));
  PolyvalentHeatPumps.AssertMinimumRuntime assMinRunTimPhp(final nUni=nPhp)
    if have_php
    "Assert polyvalent HP minimum runtime"
    annotation(Placement(transformation(extent={{160,450},{140,470}})));
  PolyvalentHeatPumps.AssertMinimumRuntime assMinOffTimPhp(
    use_runTim=false,
    final nUni=nPhp,
    dt_min=15 * 60)
    if have_php
    "Assert polyvalent HP minimum off-time"
    annotation(Placement(transformation(extent={{130,450},{110,470}})));
  StagingRotation.SortRuntime sorRunTimPhp(nin=nPhp)
    if have_php
    "Sort polyvalent HP by increasing runtime"
  ...
  Buildings.Controls.OBC.CDL.Logical.And y1ShcPhp[nPhp]
    ...
    annotation(Placement(transformation(extent={{160,390},{140,410}}),
      iconTransformation(extent={{-240,220},{-200,260}})));
  Subsequences.AssertMinimumRuntime assMinRunTimHp(
    final have_chiWat=have_chiWat,
    final nUni=nHp)
    if have_hp
...
equation
  ...
  connect(avaStaHea.y1, idxStaHea.u1AvaSta)
    annotation(Line(points={{-88,330},{-56,330},{-56,354},{-12,354}},
      color={255,0,255}));
  connect(idxStaHea.y, selMod.uStaHea)
    annotation(Line(points={{12,360},{48,360}},
      color={255,127,0}));
  ...
  connect(seqEve.y1ValHeaWatInlIso[1:nHp], y1ValHeaWatHpInlIso)
    annotation(Line(points={{162,300.333},{242,300.333},{242,400},{280,400}},
      color={255,0,255}));
  connect(seqEve.y1ValChiWatInlIso[1:nHp], y1ValChiWatHpInlIso)
    annotation(Line(points={{162,295.667},{238,295.667},{238,360},{280,360}},
      color={255,0,255}));
  connect(seqEve.y1ValChiWatOutIso[1:nHp], y1ValChiWatHpOutIso)
    annotation(Line(points={{162,293.333},{236,293.333},{236,340},{280,340}},
      color={255,0,255}));
  ...
  connect(chaStaHea.y1Up, idxStaHea.u1Up)
    annotation(Line(points={{-18,322},{-16,322},{-16,362},{-12,362}},
      color={255,0,255}));
  connect(chaStaHea.y1Lck, chaStaCoo.u1Lck)
    annotation(Line(
      points={{-18,314},{-14,314},{-14,302},{-50,302},{-50,62},{-42,62}},
      color={255,0,255}));
  connect(chaStaHea.uStaOppNexHig, chaStaCoo.yStaNexHig)
    annotation(Line(
      points={{-42,332},{-46,332},{-46,82},{-12,82},{-12,65.8},{-18,65.8}},
      color={255,0,255}));
  ...
  connect(chaStaCoo.y1Dow, idxStaCoo.u1Dow)
    annotation(Line(points={{-18,58},{-14,58},{-14,98},{-12,98}},
      color={255,0,255}));
  connect(idxStaCoo.y, selMod.uStaCoo)
    annotation(Line(points={{12,100},{30,100},{30,358},{48,358}},
      color={255,127,0}));
  ...
  connect(idxStaCoo.y, chaStaCoo.uSta)
    annotation(Line(
      ...
      color={255,127,0}));
  connect(staPumHeaWatPri.y1Ded_actual, seqEve.u1PumHeaWatPriDed_actual)
    annotation(Line(
      points={{162,208},{198,208},{198,268},{130,268},{130,298},{138,298}},
      color={255,0,255}));
  ...
  connect(staPumHeaWatPri.y1[1:nPumHeaWatPriDedHp], y1PumHeaWatPriDedHp)
    annotation(Line(points={{162,200},{246,200},{246,220},{280,220}},
      color={255,0,255}));
  connect(staPumChiWatPri.y1Ded_actual, seqEve.u1PumChiWatPriDed_actual)
    annotation(Line(points={{212,188},{212,272},{134,272},{134,291},{138,291}},
      color={255,0,255}));
  ...
  connect(seqEve.y1PumChiWatPri, staPumChiWatPri.u1PumPri)
    annotation(Line(points={{162,286.333},{178,286.333},{178,182},{188,182}},
      color={255,0,255}));
  ...
  connect(VChiWatPri_flow, VChiWatSta_flow.u)
    annotation(Line(points={{-280,-40},{-192,-40}},
      color={0,0,127}));
  ...
  connect(seqEve.y1ValChiWatInlIso, staPumChiWatPri.u1ValInlIso)
    annotation(Line(points={{162,295.667},{182,295.667},{182,186},{188,186}},
      color={255,0,255}));
  connect(seqEve.y1ValChiWatOutIso, staPumChiWatPri.u1ValOutIso)
    annotation(Line(points={{162,293.333},{180,293.333},{180,184},{188,184}},
      color={255,0,255}));
  ...
  connect(u1AvaHp.y, sorRunTimHp.u1Ava)
    annotation(Line(points={{-208,280},{-200,280},{-200,264},{-152,264}},
      color={255,0,255}));
  ...
  connect(comStaCoo.y1, chaStaCoo.u1StaPro)
    annotation(Line(
      points={{-18,24},{-10,24},{-10,42},{-46,42},{-46,64},{-42,64}},
      color={255,0,255}));
  ...
  connect(comStaHea.y1, chaStaHea.u1StaPro)
    annotation(Line(
      points={{-18,284},{-10,284},{-10,276},{-62,276},{-62,324},{-42,324}},
      color={255,0,255}));
  ...
  connect(dpChiWatLoc, ctlPumChiWatSec.dpLoc)
    annotation(Line(points={{-280,-320},{184,-320},{184,-16},{188,-16}},
      color={0,0,127}));
  connect(resChiWat.TSupSet, repTChiWatSupHpSet.u)
    annotation(Line(points={{72,-46},{116,-46},{116,-140},{208,-140}},
      color={0,0,127}));
  connect(resHeaWat.TSupSet, repTHeaWatSupHpSet.u)
    annotation(Line(points={{72,234},{118,234},{118,-120},{150,-120}},
      color={0,0,127}));
  ...
  connect(u1PumHeaWatPriHdr_actual, ctlPumPri.u1PumHeaWatPriHdr_actual)
    annotation(Line(
      points={{-280,240},{-258,240},{-258,86},{110,86},{110,90},{188,90}},
      color={255,0,255}));
  ...
  connect(u1PumHeaWatPriDedHp_actual, ctlPumPri.u1PumHeaWatPriDedHp_actual)
    annotation(Line(
      points={{-280,220},{-258,220},{-258,86},{-36,86},{-36,88},{188,88}},
      color={255,0,255}));
  connect(u1PumChiWatPriDedHp_actual, ctlPumPri.u1PumChiWatPriDedHp_actual)
    annotation(Line(
      points={{-280,160},{-256,160},{-256,84},{176,84},{176,66},{188,66}},
      color={255,0,255}));
  ...
  connect(u1PumHeaWatPriDedHp_actual, ctlFloMin.u1PumHeaWatPriDedHp_actual)
    annotation(Line(
      points={{-280,220},{-258,220},{-258,86},{112,86},{112,-233.8},{200,-233.8}},
      color={255,0,255}));
  ...
  connect(repTChiWatSupHpSet.y, TChiWatSupHpSet)
    annotation(Line(points={{232,-140},{280,-140}},
      color={0,0,127}));
  ...
  connect(idxStaCoo.y, avaStaHea.uStaOpp)
    annotation(Line(
      points={{12,100},{16,100},{16,124},{-122,124},{-122,336},{-112,336}},
      color={255,127,0}));
  connect(idxStaHea.y, avaStaCoo.uStaOpp)
    annotation(Line(
      points={{12,360},{20,360},{20,118},{-116,118},{-116,76},{-112,76}},
      color={255,127,0}));
  connect(idxStaCoo.y, chaStaHea.uStaOpp)
    annotation(Line(
      points={{12,100},{16,100},{16,122},{-116,122},{-116,346},{-48,346},{-48,330},{-42,330}},
      color={255,127,0}));
  connect(idxStaHea.y, chaStaCoo.uStaOpp)
    annotation(Line(
      points={{12,360},{20,360},{20,120},{-44,120},{-44,70},{-42,70}},
      color={255,127,0}));
  connect(chaStaHea.staTra, selMod.staTraHea)
    annotation(Line(points={{-18,330},{22,330},{22,368},{48,368}},
      color={0,0,127}));
  connect(chaStaCoo.staTra, selMod.staTraCoo)
    annotation(Line(points={{-18,70},{22,70},{22,366},{48,366}},
      color={0,0,127}));
  connect(sorRunTimHp.yIdx, selMod.uIdxSorHp)
    annotation(Line(points={{-128,264},{24,264},{24,364},{48,364}},
      color={255,127,0}));
  connect(sorRunTimPhp.yIdx, selMod.uIdxSorPhp)
    annotation(Line(points={{-128,454},{32,454},{32,362},{48,362}},
      color={255,127,0}));
  connect(u1AvaHp.y, avaHeaCooHp.u1Ava)
    annotation(Line(points={{-208,280},{-200,280},{-200,232},{-152,232}},
      color={255,0,255}));
  connect(avaHeaCooHp.y1Hea, avaStaHea.u1Ava[1:nHp])
    annotation(Line(
      points={{-128,248},{-128,248},{-120,248},{-120,330},{-112,330}},
      color={255,0,255}));
  connect(avaHeaCooHp.y1Coo, avaStaCoo.u1Ava[1:nHp])
    annotation(Line(points={{-128,240},{-120,240},{-120,70},{-112,70}},
      color={255,0,255}));
  ...
  connect(avaHeaCooPhp.y1Shc, avaStaCoo.u1Ava[nHp + nPhp + 1:nHp + 2 * nPhp])
    annotation(Line(points={{-128,392},{-120,392},{-120,70},{-112,70}},
      color={255,0,255}));
  connect(avaHeaCooPhp.y1Hea, avaStaHea.u1Ava[nHp + 1:nHp + nPhp])
    annotation(Line(points={{-128,408},{-124,408},{-124,330},{-112,330}},
      color={255,0,255}));
  connect(avaHeaCooPhp.y1Shc, avaStaHea.u1Ava[nHp + nPhp + 1:nHp + 2 * nPhp])
    annotation(Line(points={{-128,392},{-120,392},{-120,330},{-112,330}},
      color={255,0,255}));
  connect(u1AvaPhp.y, avaHeaCooPhp.u1Ava)
    annotation(Line(points={{-208,420},{-180,420},{-180,392},{-152,392}},
      color={255,0,255}));
  connect(selMod.y1HeaPhp, avaHeaCooPhp.u1EnaHea)
    annotation(Line(
      points={{72,358},{88,358},{88,382},{-158,382},{-158,408},{-152,408}},
      color={255,0,255}));
  connect(selMod.y1ShcPhp, avaHeaCooPhp.u1EnaShc)
    annotation(Line(
      points={{72,354},{92,354},{92,386},{-154,386},{-154,400},{-152,400}},
      color={255,0,255}));
  connect(selMod.y1HeaHp, avaHeaCooHp.u1EnaHea)
    annotation(Line(points={{72,366},{72,378},{-158,378},{-158,248},{-152,248}},
      color={255,0,255}));
  connect(selMod.y1CooPhp, avaHeaCooPhp.u1EnaCoo)
    annotation(Line(
      points={{72,356},{90,356},{90,384},{-156,384},{-156,404},{-152,404}},
      color={255,0,255}));
  connect(selMod.y1CooHp, avaHeaCooHp.u1EnaCoo)
    annotation(Line(
      points={{72,361.8},{74,361.8},{74,380},{-160,380},{-160,244},{-152,244}},
      color={255,0,255}));
  connect(avaHeaCooHp.y1Coo, selMod.u1AvaCooHp)
    annotation(Line(
      points={{-128,240},{-114,240},{-114,244},{40,244},{40,354},{48,354}},
      color={255,0,255}));
  connect(avaHeaCooHp.y1Hea, selMod.u1AvaHeaHp)
    annotation(Line(points={{-128,248},{28,248},{28,356},{48,356}},
      color={255,0,255}));
  connect(avaHeaCooPhp.y1Hea, selMod.u1AvaHeaPhp)
  ...
  connect(seqEve.y1ValChiWatInlIso[nHp + 1:nHp + nPhp], y1ValChiWatPhpInlIso)
    annotation(Line(points={{162,295.667},{238,295.667},{238,280},{280,280}},
      color={255,0,255}));
  ...
  connect(seqEve.y1ValChiWatOutIso[nHp + 1:nHp + nPhp], y1ValChiWatPhpOutIso)
    annotation(Line(points={{162,293.333},{238,293.333},{238,260},{280,260}},
      color={255,0,255}));
  connect(idxStaCoo.y, idxStaHea.uStaOpp)
    annotation(Line(
      points={{12,100},{18,100},{18,124},{-118.182,124},{-118.182,346},{-48,346},{-48,351},{-12,351}},
      color={255,127,0}));
  connect(idxStaHea.y, idxStaCoo.uStaOpp)
    annotation(Line(
      points={{12,360},{20,360},{20,120},{-40,120},{-40,91},{-12,91}},
      color={255,127,0}));
  ...
  connect(resChiWat.TSupSet, repTChiWatSupPhpSet.u)
    annotation(Line(points={{72,-46},{116,-46},{116,-180},{208,-180}},
      color={0,0,127}));
  connect(resHeaWat.TSupSet, repTHeaWatSupPhpSet.u)
    annotation(Line(points={{72,234},{118,234},{118,-160},{148,-160}},
      color={0,0,127}));
  connect(repTHeaWatSupPhpSet.y, THeaWatSupPhpSet)
    annotation(Line(points={{172,-160},{280,-160}},
      color={0,0,127}));
  connect(repTChiWatSupPhpSet.y, TChiWatSupPhpSet)
    annotation(Line(points={{232,-180},{280,-180}},
      color={0,0,127}));
  connect(seqEve.y1Hp, y1Hp)
    annotation(Line(points={{162,312},{228,312},{228,480},{280,480}},
      color={255,0,255}));
  connect(seqEve.y1HeaHp, y1HeaHp)
    annotation(Line(points={{162,309.667},{230,309.667},{230,460},{280,460}},
      color={255,0,255}));
  connect(seqEve.y1HeaPhp, y1HeaPhp)
    annotation(Line(
      points={{162,307.333},{196,307.333},{232,307.333},{232,440},{280,440}},
      color={255,0,255}));
  connect(seqEve.y1CooPhp, y1CooPhp)
    annotation(Line(points={{162,305},{234,305},{234,420},{280,420}},
      color={255,0,255}));
  connect(selMod.y1HeaHpPhp, seqEve.u1HeaHpPhp)
    annotation(Line(points={{72,364},{82,364},{82,312},{138,312}},
      color={255,0,255}));
  connect(selMod.y1ShcPhp, seqEve.u1ShcPhp)
    annotation(Line(points={{72,354},{78,354},{78,306},{138,306},{138,307.333}},
      color={255,0,255}));
  connect(selMod.y1CooHpPhp, seqEve.u1CooHpPhp)
    annotation(Line(points={{72,360},{80,360},{80,309.667},{138,309.667}},
      color={255,0,255}));
  connect(y1HeaPhp, assMinRunTimPhp.u1Hea)
    annotation(Line(points={{280,440},{248,440},{248,468},{162,468}},
      color={255,0,255}));
  connect(y1CooPhp, assMinRunTimPhp.u1Coo)
    annotation(Line(points={{280,420},{244,420},{244,452},{162,452}},
      color={255,0,255}));
  connect(y1HeaPhp, assMinOffTimPhp.u1Hea)
    annotation(Line(
      points={{280,440},{248,440},{248,468},{180,468},{180,476},{134,476},{134,468},{132,468}},
      color={255,0,255}));
  connect(y1CooPhp, assMinOffTimPhp.u1Coo)
    annotation(Line(
      points={{280,420},{244,420},{244,452},{180,452},{180,444},{134,444},{134,452},{132,452}},
      color={255,0,255}));
  connect(u1Php_actual, sorRunTimPhp.u1Run)
    annotation(Line(points={{-280,440},{-186,440},{-186,466},{-152,466}},
      color={255,0,255}));
  connect(u1AvaPhp.y, sorRunTimPhp.u1Ava)
    annotation(Line(points={{-208,420},{-180,420},{-180,454},{-152,454}},
      color={255,0,255}));
  connect(idxStaCoo.y, selMod.uStaCoo)
    annotation(Line(points={{12,100},{18,100},{18,358},{48,358}},
      color={255,127,0}));
  connect(chaStaCoo.staTra, selMod.staTraCoo)
    annotation(Line(points={{-18,70},{22,70},{22,366},{48,366}},
      color={0,0,127}));
  connect(y1HeaPhp, y1ShcPhp.u1)
    annotation(Line(points={{280,440},{199,440},{199,400},{162,400}},
      color={255,0,255}));
  connect(y1CooPhp, y1ShcPhp.u2)
    annotation(Line(points={{280,420},{222,420},{222,392},{162,392}},
      color={255,0,255}));
  connect(y1ShcPhp.y, comStaCoo.u1[nHp + nPhp + 1:nHp + 2 * nPhp])
    annotation(Line(points={{138,400},{-54,400},{-54,30},{-42,30}},
      color={255,0,255}));
  connect(y1ShcPhp.y, comStaHea.u1[nHp + nPhp + 1:nHp + 2 * nPhp])
    annotation(Line(points={{138,400},{-54,400},{-54,290},{-42,290}},
      color={255,0,255}));
  connect(y1Hp, assMinRunTimHp.u1)
    annotation(Line(points={{280,480},{106,480},{106,468},{102,468}},
      color={255,0,255}));
  connect(y1HeaHp, assMinRunTimHp.u1Hea)
    annotation(Line(
      points={{280,460},{174,460},{174,442},{106,442},{106,452},{102,452}},
      color={255,0,255}));
  connect(y1Hp, assMinOffTimHp.u1)
    annotation(Line(points={{280,480},{76,480},{76,468},{72,468}},
      color={255,0,255}));
  connect(y1HeaHp, assMinOffTimHp.u1Hea)
    annotation(Line(
...
annotation(defaultComponentName="ctl",
  Icon(coordinateSystem(preserveAspectRatio=true,
    ...
    Text(extent={{-150,510},{150,470}},
      textString="%name",
      textColor={0,0,255})}),
  Diagram(coordinateSystem(extent={{-260,-500},{260,500}})),
  Documentation(
    info="<html>
<p>
