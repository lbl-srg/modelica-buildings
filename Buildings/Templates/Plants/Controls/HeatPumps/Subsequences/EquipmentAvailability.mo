...
block EquipmentAvailability
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1EnaShc
    if is_php
    "Simultaneous heating and cooling enable command"
  ...
  Buildings.Controls.OBC.CDL.Logical.Timer tim(final t=dtOff)
...
equation
  ...
  connect(runHea.y, preRunHea.u)
    annotation(Line(
      points={{42,40},{50,40},{50,60},{-120,60},{-120,80},{-112,80}},
      color={255,0,255}));
  ...
  connect(u1Ava, avaAllHea.u2)
    annotation(Line(points={{-240,-140},{180,-140},{180,32},{188,32}},
      color={255,0,255}));
  connect(u1Ava, avaAllCoo.u2)
    annotation(Line(points={{-240,-140},{180,-140},{180,-88},{188,-88}},
      color={255,0,255}));
  ...
  connect(avaAllCoo.y, y1Coo)
    annotation(Line(points={{212,-80},{240,-80}},
      color={255,0,255}));
  connect(u1Ava, y1Shc)
    annotation(Line(points={{-240,-140},{240,-140}},
      color={255,0,255}));
  connect(avaHea.y, runHea.u1)
    annotation(Line(points={{-48,80},{0,80},{0,40},{18,40}},
      color={255,0,255}));
  connect(reqHea.y, runHea.u2)
    annotation(Line(points={{-88,40},{-10,40},{-10,32},{18,32}},
      color={255,0,255}));
  connect(entHea.y, avaHea.u1)
    annotation(Line(points={{-88,140},{-80,140},{-80,80},{-72,80}},
      color={255,0,255}));
  connect(preRunHea.y, avaHea.u2)
    annotation(Line(points={{-88,80},{-84,80},{-84,72},{-72,72}},
      color={255,0,255}));
  connect(avaCoo.y, runCoo.u1)
    annotation(Line(points={{-6,-20},{0,-20},{0,-60},{18,-60}},
      color={255,0,255}));
  connect(avaCoo.y, avaAllCoo.u1)
    annotation(Line(points={{-6,-20},{0,-20},{0,-80},{188,-80}},
      color={255,0,255}));
  connect(reqCoo.y, runCoo.u2)
    annotation(Line(points={{-88,-60},{-10,-60},{-10,-68},{18,-68}},
      color={255,0,255}));
  connect(preRunCoo.y, avaCoo.u2)
    annotation(Line(points={{-88,-20},{-46,-20},{-46,-28},{-30,-28}},
      color={255,0,255}));
  connect(entCoo.y, avaCoo.u1)
    annotation(Line(points={{-48,20},{-40,20},{-40,-20},{-30,-20}},
      color={255,0,255}));
annotation(defaultComponentName="avaHeaCoo",
  ...
  Documentation(
    info="<html>
<p>
  If a reversible heat pump is commanded enabled in either heating or cooling
  mode, it is removed from the staging order of the opposite mode until it has
  been off for <code>dtOff</code>.
</p>
<p>
  If a polyvalent heat pump is commanded enabled in single mode, it is removed
  from the staging order of the opposite single mode until it has been off or
  operating in simultaneous heating and cooling mode for <code>dtOff</code>.
</p>
...
<p>
  Since the enable commands are computed from the availability signals within
  the plant controller, the block is structured to avoid creating an algebraic
  loop at simulation time and at initialization: the heating availability has
  no direct dependency on any enable command, and the cooling availability
  directly depends on the heating enable command only. If both single mode
  commands are received at the same time event, the heating command prevails
  and the equipment is reported unavailable for cooling.
</p>
<p>
  ...
  available or already operating in that mode. Commands received before the
  off time has elapsed are disregarded, so the availability outputs remain
  false even if an enable command is issued.
</p>
<p>At initial time the equipment is available for all modes.</p>
</html>",
    revisions="<html>
<ul>
  <li>
    July 2, 2026, by Antoine Gautier:<br />
    Refactored using CDL blocks in place of <code>Modelica.StateGraph</code>
    components.
  </li>
  <li>
