...
block Staging
  ...
  Buildings.Controls.OBC.CDL.Routing.BooleanExtractSignal sigPumPriDed(nin=nEqu,
      nout=nPum) if is_pri and not is_hdr
  ...
  Primary.DisableLeadDedicated disDed[nPum](have_req=false)
    if is_pri and not is_hdr "Disable dedicated primary pump"
  ...
  Buildings.Controls.OBC.CDL.Logical.Latch enaPriDed[nPum]
    if is_pri and not is_hdr "Enable/disable dedicated primary pump"
  ...
  Primary.EnableLeadHeadered enaLeaHdrPri(
    ...
    final nValIso=2*nEqu) if is_pri and is_hdr
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1Pla
    if not is_pri and is_hdr "Plant enable signal" annotation (Placement(
  ...
  Buildings.Controls.OBC.CDL.Conversions.BooleanToInteger booToInt1(final
      integerTrue=1, final integerFalse=0) if is_pri and is_hdr and not
    is_ctlDp "Convert lead pump enable signal to integer"
...
equation
  ...
  connect(u1Ava.y, sorRunTimHdr.u1Ava) annotation (Line(points={{-28,0},{-24,0},
          {-24,14},{-12,14}},             color={255,0,255}));
  ...
  connect(enaLeaHdrPri.y1, nPumHdrDp.u1Lea) annotation (Line(points={{-28,-60},{
          -20,-60},{-20,-14},{-12,-14}}, color={255,0,255}));
  connect(plaValInlIso.y, enaLeaHdrPri.u1ValIso[1:nEqu])
    annotation (Line(points={{-118,-40},{-60,-40},{-60,-60},{-52,-60}},
                                                    color={255,0,255}));
  connect(plaValOutIso.y, enaLeaHdrPri.u1ValIso[nEqu + 1:2*nEqu]) annotation (
      Line(points={{-88,-60},{-52,-60}},                     color={255,0,255}));
  ...
  connect(enaLeaHdrPri.y1, booToInt1.u) annotation (Line(points={{-28,-60},{-20,
          -60},{-20,100},{-12,100}}, color={255,0,255}));
  connect(nPumHdrPriNotDp.y, voiPumLeaDis.u1) annotation (Line(points={{-78,120},
