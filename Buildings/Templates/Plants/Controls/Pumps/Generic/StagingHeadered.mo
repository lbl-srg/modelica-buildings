...
block StagingHeadered
  ...
  final parameter Real staPum[nPum, nPum](
    ...
    "Pump staging matrix"
    annotation(Evaluate=true);
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1PumPri[nEqu]
    if is_pri and (not is_hdr or is_hdr and not is_ctlDp)
    "Primary pump request from event sequencing logic"
  ...
  StagingRotation.SortRuntime sorRunTimHdr(nin=nPum)
    if is_hdr
...
equation
  ...
  connect(staHdrDp.y1Up, nPumHdrDp.u1Up)
    annotation(Line(points={{-108,6},{-100,6},{-100,2},{-12,2}},
      color={255,0,255}));
  connect(staHdrDp.y1Dow, nPumHdrDp.u1Dow)
    annotation(Line(points={{-108,-6},{-100,-6},{-100,-2},{-12,-2}},
      color={255,0,255}));
  connect(u1PumPri, booToInt.u)
    annotation(Line(points={{-180,120},{-142,120}},
      color={255,0,255}));
  ...
  connect(u1Ava.y, sorRunTimHdr.u1Ava)
    annotation(Line(
      points={{12,-100},{20,-100},{20,-20},{-18,-20},{-18,34},{-12,34}},
      color={255,0,255}));
  connect(V_flow, staHdrDp.V_flow)
    annotation(Line(points={{-180,0},{-142,0},{-142,4},{-132,4}},
      color={0,0,127}));
  connect(u1PumPri, sigPumPriDed.u)
    annotation(Line(points={{-180,120},{-156,120},{-156,-160},{68,-160}},
      color={255,0,255}));
  ...
  connect(enaLeaHdrPri.y1, nPumHdrDp.u1Lea)
    annotation(Line(points={{-48,-100},{-20,-100},{-20,6},{-12,6}},
      color={255,0,255}));
  connect(phValInlIso.y, enaLeaHdrPri.u1ValIso[1:nEqu])
    annotation(Line(points={{-108,-80},{-80,-80},{-80,-100},{-72,-100}},
      color={255,0,255}));
  ...
  connect(u1ValOutIso, phValInlIso.uPh)
    annotation(Line(points={{-180,-120},{-148,-120},{-148,-86},{-132,-86}},
      color={255,0,255}));
  connect(u1ValInlIso, phValOutIso.uPh)
    annotation(Line(points={{-180,-100},{-152,-100},{-152,-126},{-132,-126}},
      color={255,0,255}));
  connect(u1Pla, nPumHdrDp.u1Lea)
    annotation(Line(points={{-180,160},{-20,160},{-20,6},{-12,6}},
      color={255,0,255}));
  ...
  connect(nPumHdrPriNotDp0.y, nPumHdrPriNotDp.u1)
    annotation(Line(points={{-78,120},{-60,120},{-60,126},{-12,126}},
      color={255,127,0}));
  connect(booToInt1.y, nPumHdrPriNotDp.u2)
    annotation(Line(points={{-40,-38},{-40,114},{-12,114}},
      color={255,127,0}));
  connect(sorRunTimHdr.yIdx[1], y1LeaHdr_actual.index)
    annotation(Line(points={{12,34},{40,34},{40,48}},
      color={255,127,0}));
  ...
  connect(dpSet, staHdrDp.dpSet)
    annotation(Line(points={{-180,-20},{-140,-20},{-140,0},{-132,0}},
      color={0,0,127}));
  connect(dp, staHdrDp.dp)
    annotation(Line(points={{-180,-40},{-138,-40},{-138,-4},{-132,-4}},
      color={0,0,127}));
...
annotation(defaultComponentName="staPum",
  ...
  Documentation(
    info="<html>
<h5>
  Plants with headered primary pumps that are not controlled to maintain
...
<h5>
  Plants with headered primary pumps that are controlled to maintain
  differential pressure or flow setpoint
</h5>
<p>
  Primary pumps are lead/lag alternated as described in
...
<p>
  The lead secondary pump is enabled when the plant is enabled. Otherwise, the
  lead secondary pump is disabled.
</p>
...
<p>
  At its current stage of development, this block contains no logic for
  handling faulted equipment. It is therefore assumed that all pumps are
...
<ul>
  <li>
    ...
    Added failsafe conditions for headered variable speed pumps using ∆p pump
    speed control.
  </li>
  <li>
