...
block SelectSortedAvailable
  ...
  Buildings.Controls.OBC.CDL.Interfaces.IntegerInput n
    "Number of required units" annotation (Placement(transformation(extent={{-140,40},
            {-100,80}}),      iconTransformation(extent={{-140,-20},{-100,20}})));
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput y1[nEqu] "Enable signal"
  ...
  Buildings.Controls.OBC.CDL.Routing.IntegerVectorReplicator avaIntAltRep(nin=nEqu,
      nout=nEquAlt)         "Replicate to support reordering at next step"
    annotation (Placement(transformation(extent={{-60,-10},{-40,10}})));
  Buildings.Controls.OBC.CDL.Routing.IntegerExtractor avaIntIdxEquAlt[nEquAlt](each final
            nin=nEqu)
...
equation
  connect(u1Ava,avaInt. u) annotation (Line(points={{-120,0},{-92,0}},
                            color={255,0,255}));
  connect(avaIntAltRep.y,avaIntIdxEquAlt. u)
    annotation (Line(points={{-38,0},{-32,0}},       color={255,127,0}));
  connect(avaIntIdxEquAlt.y,voiUna. u1) annotation (Line(points={{-8,0},{0,0},{
          0,6},{8,6}},             color={255,127,0}));
  connect(avaInt.y,avaIntAltRep. u)
    annotation (Line(points={{-68,0},{-62,0}},       color={255,127,0}));
  connect(truArrCon.y1, y1)
    annotation (Line(points={{82,0},{120,0}}, color={255,0,255}));
  connect(n, truArrCon.u) annotation (Line(points={{-120,60},{50,60},{50,0},{58,
          0}}, color={255,127,0}));
  connect(uIdxSor, avaIntIdxEquAlt.index) annotation (Line(points={{-120,-60},{
          -20,-60},{-20,-12}}, color={255,127,0}));
  connect(uIdxSor, voiUna.u2) annotation (Line(points={{-120,-60},{0,-60},{0,-6},
          {8,-6}}, color={255,127,0}));
  connect(voiUna.y, truArrCon.uIdx) annotation (Line(points={{32,0},{40,0},{40,
          -6},{58,-6}}, color={255,127,0}));
