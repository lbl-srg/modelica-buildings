...
model StageChangeCommand_hybridPlant
  ...
  Buildings.Templates.Plants.Controls.StagingRotation.StageChangeCommand_hybridPlant
                                                                         chaSta(
  ...
  EquipmentEnable_hybridPlant                                         enaEqu(
    nEquAlt=2,
    nSta=5,
    nEqu=3)
    "Enable equipment"
...
equation
  ...
  connect(con.y, enaEqu.staEqu);
  connect(idxEquLeaLag.y, enaEqu.uIdxAltSor) annotation (Line(points={{-78,100},
          {42,100},{42,6},{58,6}}, color={255,127,0}));
  annotation (
    __Dymola_Commands(
      file=
        "modelica://Buildings/Resources/Scripts/Dymola/Templates/Plants/Controls/StagingRotation/Validation/StageChangeCommand_hybridPlant.mos"
        "Simulate and plot"),
    experiment(
      StopTime=20000.0,
      Tolerance=1e-06),
...
end StageChangeCommand_hybridPlant;
