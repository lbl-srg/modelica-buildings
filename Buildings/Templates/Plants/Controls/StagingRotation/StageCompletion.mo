...
block StageCompletion
  ...
  Buildings.Controls.OBC.CDL.Logical.And onInMod[nin]
...
equation
  ...
  connect(lckAnyCha.y, chaAndMat.u1)
    annotation (Line(points={{32,60},{48,60}},color={255,0,255}));
  ...
  connect(disAndOff.y, onOrOff.u2)
    annotation (Line(points={{-48,-40},{-40,-40},{-40,-8},{-32,-8}},color={255,0,255}));
  connect(onOrOff.y, allTru.u)
    annotation (Line(points={{-8,0},{6,0}}, color={255,0,255}));
  connect(allTru.y, chaAndMat.u2)
    annotation (Line(points={{30,0},{40,0},{40,52},{48,52}},
      color={255,0,255}));
  connect(chaSta.y, lckChaSta.u)
    annotation (Line(points={{-108,100},{88,100}},color={255,0,255}));
  connect(chaSta.y, lckAnyCha.clr)
    annotation (Line(points={{-108,100},{0,100},{0,54},{8,54}},      color={255,0,255}));
  ...
  connect(u1, u1Pre.u)
    annotation (Line(points={{-180,0},{-152,0}}, color={255,0,255}));
  connect(u1Pre.y, enaAndOn.u1)
    annotation (Line(points={{-128,0},{-72,0}}, color={255,0,255}));
  connect(u1Pre.y, cha.u) annotation (Line(points={{-128,0},{-120,0},{-120,60},
          {-72,60}}, color={255,0,255}));
  connect(u1Pre.y, disAndOff.u1) annotation (Line(points={{-128,0},{-120,0},{
          -120,-40},{-72,-40}}, color={255,0,255}));
  connect(onInMod.y, enaAndOn.u2) annotation (Line(points={{-88,-60},{-80,-60},
          {-80,-8},{-72,-8}}, color={255,0,255}));
  connect(onInMod.y, disAndOff.u2) annotation (Line(points={{-88,-60},{-80,-60},
          {-80,-48},{-72,-48}}, color={255,0,255}));
  connect(u1Pre.y, onInMod.u1) annotation (Line(points={{-128,0},{-120,0},{-120,
          -60},{-112,-60}}, color={255,0,255}));
  connect(u1_actual, onInMod.u2) annotation (Line(points={{-180,-60},{-124,-60},
          {-124,-68},{-112,-68}}, color={255,0,255}));
  annotation (
...
<ul>
<li>
Following a stage transition event, there is a change in the 
enable command of at least one equipment.
</li>
<li>
After that change, every unit that is commanded on is proven on and
every unit that is commanded off is proven off, as defined in the
section \"Operating mode handling\" below.
</li>
</ul>
<p>
...
<h4>Operating mode handling</h4>
<p>
A separate instance of this block must be used for heating and for
cooling.
The enable command is specific to a given operating mode, whereas the
returned status is not, and the active operating mode is not available
as an input point.
Therefore, a unit is considered proven on in a given mode if both the
enable command for that mode and the returned status are true.
Conversely, a unit is considered proven off in a given mode if either
the enable command for that mode or the returned status is false.
</p>
</html>", revisions="<html>
<ul>
<li>
July 10, 2026, by Antoine Gautier:<br/>
Use both enable command and returned status to handle
