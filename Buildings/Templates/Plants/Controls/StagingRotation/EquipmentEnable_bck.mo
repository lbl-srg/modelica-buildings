...
block EquipmentEnable_bck
  "Return array of equipment to enable at given stage – No rotation logic"
  ...
  Buildings.Controls.OBC.CDL.Integers.Subtract nAlt
    "Number of lead/lag alternate equipment to run to meet stage requirement"
    annotation (Placement(transformation(extent={{40,70},{60,90}})));
  Buildings.Controls.OBC.CDL.Routing.IntegerExtractSignal extIntSig[nEqu](
    each final nin=nEqu,
    final nout={i for i in 1:nEqu})
    "For each row i, extract only the first i elements"
    annotation (Placement(transformation(extent={{70,30},{90,50}})));
  Buildings.Controls.OBC.CDL.Routing.IntegerVectorReplicator intVecRep(
    nin=nEqu,
    nout=nEqu) "Replicate array"
    annotation (Placement(transformation(extent={{40,30},{60,50}})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToInteger toInt1[nEqu]
    "Cas to integer"
    annotation (Placement(transformation(extent={{0,30},{20,50}})));
  Buildings.Controls.OBC.CDL.Integers.MultiSum nEquIdx[nEqu](final nin={i for i in
            1:nEqu})
    "Sum for each row to compute vector of cumulated number of lead/lag equipment up to index"
    annotation (Placement(transformation(extent={{100,30},{120,50}})));
  Buildings.Controls.OBC.CDL.Integers.LessEqual intLesEqu[nEqu]
    "Compare to required number of equipment to disable unneeded equipment"
    annotation (Placement(transformation(extent={{130,30},{150,50}})));
  Buildings.Controls.OBC.CDL.Routing.IntegerScalarReplicator repNAlt(final nout
      =nEqu) "Replicate signal"
...
equation
  ...
  for i in 1:nEqu loop
    for j in 1:i loop
      connect(extIntSig[i].y[j], nEquIdx[i].u[j])
  ...
  connect(nEquSta.y, nAlt.u1) annotation (Line(points={{-18,80},{30,80},{30,86},
          {38,86}}, color={255,127,0}));
  connect(intVecRep.y,extIntSig. u)
    annotation (Line(points={{62,40},{68,40}},  color={255,0,255}));
  connect(isReqAltAva.y, toInt1.u)
    annotation (Line(points={{-18,40},{-2,40}},
                                              color={255,0,255}));
  connect(toInt1.y, intVecRep.u)
    annotation (Line(points={{22,40},{38,40}}, color={255,127,0}));
  connect(nEquIdx.y, intLesEqu.u1)
    annotation (Line(points={{122,40},{128,40}}, color={255,127,0}));
  connect(nAlt.y, repNAlt.u)
    annotation (Line(points={{62,80},{78,80}}, color={255,127,0}));
  connect(repNAlt.y, intLesEqu.u2) annotation (Line(points={{102,80},{126,80},{
          126,32},{128,32}}, color={255,127,0}));
  connect(isReqAltAva.y, isReqAltAvaNee.u2) annotation (Line(points={{-18,40},{-8,
          40},{-8,-8},{48,-8}},                color={255,0,255}));
  connect(isReqAltAvaNee.y, ena.u1)
    annotation (Line(points={{72,0},{78,0}},   color={255,0,255}));
  connect(intLesEqu.y, isReqAltAvaNee.u1) annotation (Line(points={{152,40},{154,
          40},{154,20},{44,20},{44,0},{48,0}},     color={255,0,255}));
  ...
  connect(nReq.y, nAlt.u2) annotation (Line(points={{-18,0},{30,0},{30,74},{38,74}},
        color={255,127,0}));
