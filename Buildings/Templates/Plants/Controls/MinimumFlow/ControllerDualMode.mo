...
block ControllerDualMode "CHW and/or HW minimum flow bypass valve controller"
  ...
  parameter Boolean have_chiWat
    ...
    annotation (Evaluate=true);
  parameter Boolean have_pumChiWatPri(start=not have_heaWat)
  ...
  parameter Integer nEnaChiWat=nEnaChiWat
    ...
    annotation (Evaluate=true,
    Dialog(connectorSizing=true),HideResult=true);
  parameter Real VHeaWat_flow_nominal[nEqu](start=fill(0, nEqu), unit="m3/s")
    "Design HW flow rate – Each unit"
    annotation (Dialog(enable=have_heaWat));
  parameter Real VHeaWat_flow_min[nEqu](start=fill(0, nEqu), unit="m3/s")
    "Minimum HW flow rate – Each unit"
    annotation (Dialog(enable=have_heaWat));
  parameter Real VChiWat_flow_nominal[nEqu](start=fill(0, nEqu), unit="m3/s")
    "Design CHW flow rate – Each unit"
    annotation (Dialog(enable=have_chiWat));
  parameter Real VChiWat_flow_min[nEqu](start=fill(0, nEqu), unit="m3/s")
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput VChiWatPri_flow(final unit="m3/s")
    if have_chiWat
    "Primary CHW volume flow rate" annotation (Placement(transformation(extent={{-140,
            -140},{-100,-100}}),      iconTransformation(extent={{-140,-100},{-100,
            -60}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput VHeaWatPri_flow(final unit="m3/s")
    if have_heaWat
    "Primary HW volume flow rate" annotation (Placement(transformation(extent={{-140,
            -120},{-100,-80}}),      iconTransformation(extent={{-140,-80},{-100,
            -40}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1PumHeaWatPri_actual[nEnaHeaWat]
    ...
    "Primary HW pump status" annotation (Placement(transformation(extent={{-140,
            -60},{-100,-20}}), iconTransformation(extent={{-140,-40},{-100,0}})));
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1Equ[nEqu]
    "Equipment enable command" annotation (Placement(transformation(extent={{-140,
            100},{-100,140}}),iconTransformation(extent={{-140,80},{-100,120}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput VHeaWatPriSet_flow(final
      ...
      Placement(transformation(extent={{100,80},{140,120}}), iconTransformation(
  ...
  Utilities.PlaceholderLogical phPumChiWatPri[nEnaChiWat](each final have_inp=
        have_heaWat and have_chiWat and not have_pumChiWatPri,
  ...
  Buildings.Controls.OBC.CDL.Logical.And pumOnAndCoo[nEqu] if have_heaWat and
    have_chiWat and not have_pumChiWatPri and not (have_valInlIso or
    have_valOutIso)
    "True if pump proven on and equipment in cooling mode – Case with common CHW and HW dedicated pumps"
    annotation (Placement(transformation(extent={{-50,-70},{-30,-50}})));
  Buildings.Controls.OBC.CDL.Logical.And pumOnAndHea[nEqu] if have_heaWat and
    have_chiWat and not have_pumChiWatPri and not (have_valInlIso or
    have_valOutIso)
    "True if pump proven on and equipment in heating mode – Case with common CHW and HW dedicated pumps"
    annotation (Placement(transformation(extent={{-50,30},{-30,50}})));
  Utilities.PlaceholderLogical phPumHeaWatPri[nEnaHeaWat](each final have_inp=
        have_heaWat and have_chiWat and not have_pumChiWatPri,           each final
            have_inpPh=true)
    if have_heaWat and not (have_valInlIso or have_valOutIso)
    "Use HW pump signal in case the plant does not have common CHW and HW dedicated pumps"
    annotation (Placement(transformation(extent={{10,30},{30,50}})));
equation
  connect(ctlFloMinHeaWat.y, yValHeaWatMinByp) annotation (Line(points={{72,80},
          {80,80},{80,60},{120,60}}, color={0,0,127}));
  connect(ctlFloMinHeaWat.VPriSet_flow, VHeaWatPriSet_flow) annotation (Line(
        points={{72,86},{80,86},{80,100},{120,100}},
                                                   color={0,0,127}));
  connect(ctlFloMinChiWat.y, yValChiWatMinByp)
    annotation (Line(points={{72,-40},{80,-40},{80,-60},{120,-60}},
                                                  color={0,0,127}));
  connect(ctlFloMinChiWat.VPriSet_flow, VChiWatPriSet_flow) annotation (Line(
        points={{72,-34},{80,-34},{80,-20},{120,-20}}, color={0,0,127}));
  ...
  connect(phHeaCoo.y, u1Coo.u)
    annotation (Line(points={{-68,100},{-52,100}},
                                                 color={255,0,255}));
  connect(u1Equ, onAndHea.u1) annotation (Line(points={{-120,120},{0,120},{0,100},
          {8,100}},               color={255,0,255}));
  connect(onAndHea.y, ctlFloMinHeaWat.u1Equ) annotation (Line(points={{32,100},{
          40,100},{40,88},{48,88}},
                                color={255,0,255}));
  ...
  connect(u1ValHeaWatInlIso, ctlFloMinHeaWat.u1ValInlIso) annotation (Line(
        points={{-120,80},{40,80},{40,84},{48,84}}, color={255,0,255}));
  connect(u1ValHeaWatOutIso, ctlFloMinHeaWat.u1ValOutIso) annotation (Line(
        points={{-120,60},{42,60},{42,80},{48,80}}, color={255,0,255}));
  connect(VChiWatPri_flow, ctlFloMinChiWat.VPri_flow) annotation (Line(points={{-120,
          -120},{40,-120},{40,-48},{48,-48}},      color={0,0,127}));
  connect(u1ValChiWatInlIso, ctlFloMinChiWat.u1ValInlIso) annotation (Line(
        points={{-120,20},{-84,20},{-84,-36},{48,-36}}, color={255,0,255}));
  connect(u1ValChiWatOutIso, ctlFloMinChiWat.u1ValOutIso) annotation (
      Line(points={{-120,0},{-72,0},{-72,-40},{48,-40}}, color={255,0,255}));
  connect(VHeaWatPri_flow, ctlFloMinHeaWat.VPri_flow) annotation (Line(points={{-120,
          -100},{-76,-100},{-76,72},{48,72}},    color={0,0,127}));
  connect(u1Equ, onAndCoo.u1) annotation (Line(points={{-120,120},{0,120},{0,-20},
          {8,-20}},              color={255,0,255}));
  connect(u1Coo.y, onAndCoo.u2) annotation (Line(points={{-28,100},{-20,100},{-20,
  ...
  connect(u1Coo.y, pumOnAndCoo.u1) annotation (Line(points={{-28,100},{-20,100},
          {-20,-28},{-60,-28},{-60,-60},{-52,-60}},
                                               color={255,0,255}));
  connect(u1PumHeaWatPri_actual, pumOnAndCoo.u2) annotation (Line(points={{-120,
          -40},{-80,-40},{-80,-68},{-52,-68}}, color={255,0,255}));
  connect(phHeaCoo.y, onAndHea.u2) annotation (Line(points={{-68,100},{-60,100},
          {-60,86},{4,86},{4,92},{8,92}},color={255,0,255}));
  connect(u1PumHeaWatPri_actual, pumOnAndHea.u2) annotation (Line(points={{-120,
          -40},{-80,-40},{-80,32},{-52,32}}, color={255,0,255}));
  connect(phHeaCoo.y, pumOnAndHea.u1) annotation (Line(points={{-68,100},{-60,100},
          {-60,40},{-52,40}}, color={255,0,255}));
  connect(phPumHeaWatPri.y, ctlFloMinHeaWat.u1PumPri_actual) annotation (Line(
        points={{32,40},{44,40},{44,76},{48,76}}, color={255,0,255}));
  connect(pumOnAndHea.y, phPumHeaWatPri.u)
    annotation (Line(points={{-28,40},{8,40}}, color={255,0,255}));
  connect(u1PumHeaWatPri_actual, phPumHeaWatPri.uPh) annotation (Line(points={{-120,
          -40},{-80,-40},{-80,20},{4,20},{4,34},{8,34}}, color={255,0,255}));
  connect(pumOnAndCoo.y, phPumChiWatPri.u)
    annotation (Line(points={{-28,-60},{8,-60}}, color={255,0,255}));
  connect(u1PumChiWatPri_actual, phPumChiWatPri.uPh) annotation (Line(points={{
          -120,-60},{-90,-60},{-90,-80},{0,-80},{0,-66},{8,-66}}, color={255,0,
          255}));
  annotation (
