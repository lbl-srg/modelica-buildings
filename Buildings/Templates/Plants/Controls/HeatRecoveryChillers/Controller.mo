...
block Controller
  ...
  parameter Real THeaWatSup_max(
    ...
    annotation (Dialog(group="Information provided by designer"));
  parameter Real COPHea_nominal(min=1.1, unit="1")
    "Heating COP at design heating conditions"
    annotation (Dialog(group="Information provided by designer"));
  parameter Real capCoo_min(min=0, unit="W")
    "Minimum cooling capacity below which cycling occurs"
    annotation (Dialog(group="Information provided by designer"));
  parameter Real capHea_min(min=0, unit="W")
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TChiWatRetUpsHrc(final unit="K",
      displayUnit="degC") "CHW return temperature upstream of HRC" annotation (
      Placement(transformation(extent={{-160,-20},{-120,20}}),
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput THeaWatRetUpsHrc(final unit="K",
      displayUnit="degC") "HW return temperature upstream of HRC" annotation (
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1ReqFloChiWat
    if have_reqFlo "CHW flow request from HRC" annotation (Placement(
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1ReqFloConWat
    if have_reqFlo "CW flow request from HRC" annotation (Placement(
        transformation(extent={{-160,100},{-120,140}}), iconTransformation(
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput y1PumChiWat
    "HRC CHW pump enable command" annotation (Placement(transformation(extent={{120,100},
            {160,140}}),          iconTransformation(extent={{100,-60},{140,-20}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput y1PumHeaWat
    "HRC HW pump enable command" annotation (Placement(transformation(extent={{120,60},
            {160,100}}),     iconTransformation(extent={{100,-100},{140,-60}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput TSupSet(
    final unit="K",
    displayUnit="degC") "Active supply temperature setpoint"
  ...
  Pumps.Primary.EnableDedicated pumChiWat(final have_reqFlo=have_reqFlo)
    "CHW pump control"
    annotation (Placement(transformation(extent={{30,110},{50,130}})));
  Pumps.Primary.EnableDedicated pumHeaWat(final have_reqFlo=have_reqFlo)
    "HW pump control"
...
equation
  ...
  connect(loaChiWat.QReq_flow, ena.QChiWatReq_flow) annotation (Line(points={{-68,0},
          {-60,0},{-60,-2},{-32,-2}},      color={0,0,127}));
  connect(loaHeaWat.QReq_flow, ena.QHeaWatReq_flow) annotation (Line(points={{-68,
          -160},{-64,-160},{-64,-4},{-32,-4}},   color={0,0,127}));
  ...
  connect(loaChiWat.QReq_flow, setMod.QChiWatReq_flow) annotation (Line(points={{-68,0},
          {-60,0},{-60,-56},{28,-56}},             color={0,0,127}));
  connect(loaHeaWat.QReq_flow, setMod.QHeaWatReq_flow) annotation (Line(points={{-68,
          -160},{-64,-160},{-64,-60},{28,-60}},          color={0,0,127}));
  ...
  connect(setMod.TSupSet, TSupSet)
    annotation (Line(points={{52,-66},{60,-66},{60,-80},{140,-80}},
                                                                 color={0,0,127}));
  connect(ena.y1, y1)
  ...
  connect(ena.y1, pumChiWat.u1) annotation (Line(points={{-8,0},{0,0},{0,128},{
          28,128}},   color={255,0,255}));
  connect(ena.y1, pumChiWat.u1Equ) annotation (Line(points={{-8,0},{0,0},{0,124},
          {28,124}},       color={255,0,255}));
  connect(u1ReqFloChiWat, pumChiWat.u1ReqFlo) annotation (Line(points={{-140,
          140},{20,140},{20,116},{28,116}},
                                          color={255,0,255}));
  connect(u1Hrc_actual, pumChiWat.u1Equ_actual) annotation (Line(points={{-140,
          160},{22,160},{22,120},{28,120}},
                                          color={255,0,255}));
  connect(ena.y1, pumHeaWat.u1) annotation (Line(points={{-8,0},{0,0},{0,88},{
          28,88}},   color={255,0,255}));
  connect(ena.y1, pumHeaWat.u1Equ) annotation (Line(points={{-8,0},{0,0},{0,84},
          {28,84}},      color={255,0,255}));
  connect(u1ReqFloConWat, pumHeaWat.u1ReqFlo) annotation (Line(points={{-140,
          120},{18,120},{18,76},{28,76}},
                                        color={255,0,255}));
  connect(u1Hrc_actual, pumHeaWat.u1Equ_actual) annotation (Line(points={{-140,
          160},{22,160},{22,80},{28,80}},
                                        color={255,0,255}));
  connect(pumHeaWat.y1, y1PumHeaWat) annotation (Line(points={{52,80},{140,80}},
                         color={255,0,255}));
  connect(pumChiWat.y1, y1PumChiWat)
    annotation (Line(points={{52,120},{140,120}}, color={255,0,255}));
  connect(setMod.y1Coo, preMod.u) annotation (Line(points={{52,-60},{60,-60},{60,
          -20},{52,-20}},    color={255,0,255}));
  connect(preMod.y, ena.u1CooHrc) annotation (Line(points={{28,-20},{-40,-20},{-40,
          2},{-32,2}},      color={255,0,255}));
  annotation (
    Diagram(
      coordinateSystem(
