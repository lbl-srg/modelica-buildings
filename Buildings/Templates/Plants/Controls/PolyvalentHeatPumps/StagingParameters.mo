...
block StagingParameters
  ...
  final parameter Integer staPhp[nSta + 1, nSta + 1] = {min(
    iHea, iCoo) for iCoo in 0:nSta, iHea in 0:nSta}
    "Minimum between cooling and heating stage";
  final parameter Integer staHeaRes[nSta + 1, nSta + 1] = {iHea -
    nPhpShc[iHea + 1, iCoo + 1] for iCoo in 0:nSta, iHea in 0:nSta}
    "Residual heating stage count after SHC assignment";
  final parameter Integer staCooRes[nSta + 1, nSta + 1] = {iCoo -
    nPhpShc[iHea + 1, iCoo + 1] for iCoo in 0:nSta, iHea in 0:nSta}
    "Residual cooling stage count after SHC assignment";
  final parameter Integer nHpAva[nSta + 1, nSta + 1] = {nHp -
    nHpHea[iHea + 1, iCoo + 1] for iCoo in 0:nSta, iHea in 0:nSta}
    "Number of HP still available after heating assignment";
  final parameter Integer nPhpAva[nSta + 1, nSta + 1] = {nPhp -
    nPhpShc[iHea + 1, iCoo + 1] for iCoo in 0:nSta, iHea in 0:nSta}
    "Number of polyvalent HP still available after SHC assignment";
  final parameter Integer nHpHea[nSta + 1, nSta + 1] = {min(
    staHeaRes[iHea + 1, iCoo + 1], nHp) for iCoo in 0:nSta, iHea in 0:nSta}
    "Number of HP in heating mode";
  final parameter Integer nHpCoo[nSta + 1, nSta + 1] = {min(
    staCooRes[iHea + 1, iCoo + 1],
    nHpAva[iHea + 1, iCoo + 1]) for iCoo in 0:nSta, iHea in 0:nSta}
    "Number of HP in cooling mode";
  final parameter Integer nPhpHea[nSta + 1, nSta + 1] = {min(
  ...
  final parameter Integer nPhpCoo[nSta + 1, nSta + 1] = {min(
  ...
  final parameter Integer nPhpShc[nSta + 1, nSta + 1] = {min(
    staPhp[iHea + 1, iCoo + 1], nPhp) for iCoo in 0:nSta, iHea in 0:nSta}
    "Number of polyvalent HP in SHC mode";
  final parameter Boolean is_feasible[nSta + 1, nSta + 1] = {(staHeaRes[iHea +
    1, iCoo + 1] - nHpHea[iHea + 1, iCoo + 1] - nPhpHea[iHea + 1, iCoo + 1] ==
    0)
  ...
  final parameter Real staCoo[(nSta + 1) * nSta, nHp + 2 * nPhp] =
    {(if not is_feasible[div(r - 1, nSta) + 1, mod(r - 1, nSta) + 2] then 0
      ...
      then nPhpCoo[div(r - 1, nSta) + 1, mod(r - 1, nSta) + 2] / max(nPhp, 1)
        ...
        nPhp, 1)) for iEqu in 1:nHp + 2 * nPhp, r in 1:(nSta + 1) * nSta}
