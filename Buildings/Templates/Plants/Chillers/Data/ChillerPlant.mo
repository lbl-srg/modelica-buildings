...
record ChillerPlant
  ...
  parameter Buildings.Templates.Plants.Chillers.Components.Data.ChillerGroup chi(
    final nChi=cfg.nChi,
  ...
  parameter Buildings.Templates.Components.Data.Valve valChiWatMinByp(
    ...
    dpValve_nominal=max(chi.dpChiWatChi_nominal) *
      max(ctl.VChiWatChi_flow_min ./ ctl.VChiWatChi_flow_nominal) ^ 2)
  ...
  parameter Buildings.Templates.Components.Data.PumpMultiple pumChiWatSec(
    ...
    m_flow_nominal=if cfg.have_pumChiWatSec
      then fill(
        ctl.VChiWatSec_flow_nominal[1] * cfg.rhoChiWat_default /
          cfg.nPumChiWatSec,
        cfg.nPumChiWatSec)
  ...
  parameter Buildings.Templates.Plants.Chillers.Components.Data.CoolerGroup coo(
    ...
    mConWatCoo_flow_nominal=fill(
      sum(ctl.VConWatChi_flow_nominal) * cfg.rhoCon_default / cfg.nCoo,
      cfg.nCoo),
  ...
  parameter Buildings.Templates.Components.Data.PumpMultiple pumConWat(
    ...
    final typ=if cfg.typChi ==
      Buildings.Templates.Components.Types.Chiller.WaterCooled
      then Buildings.Templates.Components.Types.Pump.Multiple
      else Buildings.Templates.Components.Types.Pump.None,
    m_flow_nominal=fill(
      sum(coo.mConWatCoo_flow_nominal) / cfg.nPumConWat, cfg.nPumConWat))
    "CW pumps"
    annotation(Dialog(group="CW loop",
      enable=cfg.typChi ==
        Buildings.Templates.Components.Types.Chiller.WaterCooled));
...
annotation(Documentation(
  info="<html>
<p>
  This record provides the set of sizing and operating parameters for CHW
  plant models that can be found within
