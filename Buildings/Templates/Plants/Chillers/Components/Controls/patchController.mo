...
block patchController
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant FIXME_TConWatSup(final k=
        303.75) if closeCoupledPlant and not have_airCoo
    "Invalid condition in subcomponent"
    annotation (Placement(transformation(extent={{-980,-690},{-960,-670}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput FIXME_dpChiWatPumSet[
    nSenChiWatPum](final quantity=fill("PressureDifference", nSenChiWatPum),
      final unit=fill("Pa", nSenChiWatPum)) if have_locSenChiWatPum
    "Invalid condition" annotation (Placement(transformation(extent={{1068,550},
            {1108,590}}), iconTransformation(extent={{140,210},{180,250}})));
equation
  ...
  connect(FIXME_TConWatSup.y, towCon.TConWatSup) annotation (Line(points={{-958,
          -680},{-380,-680},{-380,-660},{-268,-660}}, color={0,0,127}));
  connect(chiWatSupSet.dpChiWatPumSet, FIXME_dpChiWatPumSet) annotation (Line(
        points={{-476,452},{305,452},{305,570},{1088,570}}, color={0,0,127}));
end patchController;
