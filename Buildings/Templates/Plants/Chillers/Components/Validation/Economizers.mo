...
model Economizers
  "Validation model for waterside economizers"
  ...
  parameter Modelica.Units.SI.PressureDifference dpConWatEco_nominal =
    ...
    annotation(Dialog(group="Nominal condition"));
  parameter Modelica.Units.SI.HeatFlowRate capEco_nominal(final min=0) = 1e6
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.TimeTable yValChiWatEcoByp(
    table=[0, 1; 1, 1; 1.5, 0; 2, 0],
    timeScale=1000)
    "WSE CHW bypass valve opening signal"
    annotation(Placement(transformation(extent={{-130,-10},{-110,10}})));
  Fluid.Sources.Boundary_pT bouChiWatSup(
    redeclare final package Medium=MediumChiWat,
    final nPorts=2)
    "CHW supply boundary condition"
    annotation(Placement(transformation(extent={{10,-10},{-10,10}},
      rotation=-90,
      origin={80,-90})));
  Buildings.Templates.Plants.Chillers.Components.Economizers.HeatExchangerWithValve ecoVal(
  ...
  Buildings.Templates.Plants.Chillers.Components.Economizers.HeatExchangerWithPump ecoPum(
    redeclare final package MediumChiWat=MediumChiWat,
    redeclare final package MediumConWat=MediumConWat,
    final dat=datEcoPum,
    hex(show_T=true),
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1PumChiWatEco(
    ...
    timeScale=1000,
    period=2000)
    "WSE CHW pump Start/Stop signal"
    annotation(Placement(transformation(extent={{-130,30},{-110,50}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.TimeTable yPumChiWatEco(
    table=[0, 0; 1, 0; 1.5, 1; 2, 1],
    timeScale=1000)
    "WSE CHW pump speed signal"
  ...
  Buildings.Templates.Components.Interfaces.Bus busValConWatEcoIso
    "WSE CW isolation valve control bus"
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1ValConWatEcoIso(
    ...
    timeScale=1000,
    period=2000)
    "WSE CW isolation valve opening signal"
...
equation
  ...
  connect(bouConWatSup.ports[1], ecoVal.port_aConWat)
    annotation(Line(points={{79,40},{-40,40},{-40,28}},
      color={0,127,255}));
  connect(ecoVal.port_bConWat, bouConWatRet.ports[1])
    annotation(Line(points={{-40,12},{-61,12},{-61,50}},
      color={0,127,255}));
  connect(busPla, ecoVal.bus)
    annotation(Line(points={{0,60},{0,30}},
      color={255,204,51},
      thickness=0.5));
  connect(yValChiWatEcoByp.y[1], busValChiWatEcoByp.y)
    annotation(Line(points={{-108,0},{-100,0},{-100,76},{0,76},{0,100}},
      color={0,0,127}));
  connect(bouConWatSup.ports[2], ecoPum.port_aConWat)
    annotation(Line(points={{81,40},{60,40},{60,-50},{-40,-50},{-40,-72}},
      color={0,127,255}));
  connect(ecoPum.port_bConWat, bouConWatRet.ports[2])
    annotation(Line(points={{-40,-88},{-60,-88},{-60,50},{-59,50}},
      color={0,127,255}));
  connect(y1PumChiWatEco.y[1], busPumChiWatEco.y1)
    annotation(Line(points={{-108,40},{-50,40},{-50,-20},{0,-20}},
      color={255,0,255}));
  connect(yPumChiWatEco.y[1], busPumChiWatEco.y)
    annotation(Line(points={{-108,-40},{0,-40},{0,-20}},
      color={0,0,127}));
  connect(busPumChiWatEco, busPla1.pumChiWatEco)
    annotation(Line(points={{0,-20},{0,-60}},
      color={255,204,51},
      thickness=0.5));
  connect(busPla1, ecoPum.bus)
    annotation(Line(points={{0,-60},{0,-70}},
      color={255,204,51},
      thickness=0.5));
  connect(busValConWatEcoIso, busPla.valConWatEcoIso)
    annotation(Line(points={{40,80},{40,60},{0,60}},
      color={255,204,51},
      thickness=0.5));
  connect(busValConWatEcoIso1, busPla1.valConWatEcoIso)
    annotation(Line(points={{40,-40},{40,-60},{0,-60}},
      color={255,204,51},
      thickness=0.5));
  connect(busValChiWatEcoByp, busPla.valChiWatEcoByp)
    annotation(Line(points={{0,100},{0,60}},
      color={255,204,51},
      thickness=0.5));
  connect(y1ValConWatEcoIso.y[1], busValConWatEcoIso.y1)
    annotation(Line(points={{-108,120},{100,120},{100,80},{40,80}},
      color={255,0,255}));
  connect(y1ValConWatEcoIso.y[1], busValConWatEcoIso1.y1)
    annotation(Line(points={{-108,120},{100,120},{100,-40},{40,-40}},
      color={255,0,255}));
  connect(ecoPum.port_b, bouChiWatSup.ports[2])
    annotation(Line(points={{10,-80},{81,-80}},
      color={0,127,255}));
  connect(bouChiWatRet1.ports[1], ecoPum.port_a)
    annotation(Line(points={{-70,-80},{-10,-80}},
      color={0,127,255}));
