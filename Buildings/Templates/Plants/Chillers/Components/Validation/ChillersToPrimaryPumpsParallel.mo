...
model ChillersToPrimaryPumpsParallel
  ...
  Plants.Chillers.Components.Routing.ChillersToPrimaryPumps rou(
  ...
  Fluid.FixedResistances.PressureDrop resEva[nChi](
    ...
    "Flow resistance for chiller evaporator"
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1PumChiWatPri[nChi](
    ...
    each timeScale=1000,
    each period=2000)
    "Primary CHW pumps Start/Stop signal"
    annotation(Placement(transformation(extent={{-220,270},{-200,290}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant yPumChiWatPri(k=1)
  ...
  Buildings.Templates.Components.Actuators.Valve valChiWatEcoByp(
    redeclare final package Medium=MediumChiWat,
    final typ=Buildings.Templates.Components.Types.Valve.TwoWayModulating,
  ...
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToRea1[nChi]
    ...
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      rotation=-90,
      origin={240,-170})));
  Buildings.Controls.OBC.CDL.Reals.MultiSum comSigLoa1(
    k=fill(1 / nChi, nChi),
    nin=nChi)
    "Compute load modulating signal"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
  ...
  Buildings.Templates.Components.Routing.PassThroughFluid pas(
    redeclare final package Medium=MediumChiWat)
    "Direct fluid pass-through for no WSE"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      rotation=90,
      origin={-100,30})));
equation
  ...
  connect(mChiWatChi_flow.port_a, rou.ports_bRet[1:nChi])
    annotation(Line(
      points={{-130,-20},{-106,-20},{-106,-26.1194},{-80,-26.1194}},
      color={0,127,255}));
  connect(mChiWatChi_flow.port_b, TChiWatChiEnt.port_a)
    annotation(Line(points={{-150,-20},{-170,-20}},
      color={0,127,255}));
  connect(TChiWatChiEnt.port_b, resEva.port_a)
    annotation(Line(points={{-190,-20},{-200,-20},{-200,0}},
      color={0,127,255}));
  connect(y1PumChiWatPri.y[1], busPumChiWatPri.y1)
    annotation(Line(points={{-198,280},{160,280},{160,200},{200,200}},
      color={255,0,255}));
  connect(yPumChiWatPri.y, busPumChiWatPri.y)
    annotation(Line(points={{-228,300},{180,300},{180,220},{200,220},{200,200}},
      color={0,0,127}));
  connect(busPumChiWatPri, pumChiWatPri.bus)
    annotation(Line(points={{200,200},{200,190},{0,190},{0,230}},
      color={255,204,51},
      thickness=0.5));
  connect(outPumChiWatPri.port_b, TChiWatPriSup.port_a)
    annotation(Line(points={{40,220},{50,220}},
      color={0,127,255}));
  connect(TChiWatPriSup.port_b, mChiWatPri_flow.port_a)
    annotation(Line(points={{70,220},{90,220}},
      color={0,127,255}));
  connect(loa.port_a, mChiWatPri_flow.port_b)
    annotation(Line(points={{30,-20},{120,-20},{120,220},{110,220}},
      color={0,127,255}));
  connect(loa.port_b, rou.port_aRet)
    annotation(Line(points={{10,-20},{-16,-20},{-16,-26.1194},{-40,-26.1194}},
      color={0,127,255}));
  connect(bouChiWat.ports[1], loa.port_b)
    annotation(Line(points={{0,-30},{0,-20},{10,-20}},
      color={0,127,255}));
  connect(coo.port_b, rou.ports_aSup[1:nChi])
    annotation(Line(points={{-160,220},{-120,220},{-120,226.119},{-80,226.119}},
      color={0,127,255}));
  connect(resEva.port_b, coo.port_a)
    annotation(Line(points={{-200,20},{-200,220},{-180,220}},
      color={0,127,255}));
  connect(TChiWat.y, coo.T_in)
    annotation(Line(
      points={{-228,80},{-220,80},{-220,240},{-174,240},{-174,232}},
      color={0,0,127}));
  connect(rou.ports_bSup, pumChiWatPri.ports_a)
    annotation(Line(points={{-40,226.119},{-26,226.119},{-26,220},{-10,220}},
      color={0,127,255}));
  ...
  connect(mChiWatChi_flow1.port_a, rou1.ports_bRet[1:nChi])
    annotation(Line(
      points={{-130,-140},{-100,-140},{-100,-326.119},{-80,-326.119}},
      color={0,127,255}));
  connect(mChiWatChi_flow1.port_b, TChiWatChiEnt1.port_a)
    annotation(Line(points={{-150,-140},{-170,-140}},
      color={0,127,255}));
  connect(TChiWatChiEnt1.port_b, resEva1.port_a)
    annotation(Line(points={{-190,-140},{-200,-140},{-200,-120}},
      color={0,127,255}));
  connect(y1PumChiWatPri.y[1], busPumChiWatPri1.y1)
    annotation(Line(points={{-198,280},{160,280},{160,-100},{200,-100}},
      color={255,0,255}));
  connect(busPumChiWatPri1, pumChiWatPri1.bus)
    annotation(Line(points={{200,-100},{200,-104},{0,-104},{0,-70}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(TChiWatPriSup1.port_b, mChiWatPri_flow1.port_a)
    annotation(Line(points={{70,-80},{90,-80}},
      color={0,127,255}));
  connect(loa1.port_a, mChiWatPri_flow1.port_b)
    annotation(Line(points={{30,-320},{120,-320},{120,-80},{110,-80}},
      color={0,127,255}));
  connect(loa1.port_b, rou1.port_aRet)
    annotation(Line(points={{10,-320},{-16,-320},{-16,-326.119},{-40,-326.119}},
      color={0,127,255}));
  connect(bouChiWat1.ports[1], loa1.port_b)
    annotation(Line(points={{0,-330},{0,-320},{10,-320}},
      color={0,127,255}));
  connect(coo1.port_b, rou1.ports_aSup[1:nChi])
    annotation(Line(
      points={{-160,-80},{-120,-80},{-120,-73.8806},{-80,-73.8806}},
      color={0,127,255}));
  connect(resEva1.port_b, coo1.port_a)
    annotation(Line(points={{-200,-100},{-200,-80},{-180,-80}},
      color={0,127,255}));
  connect(TChiWat.y, coo1.T_in)
    annotation(Line(
      points={{-228,80},{-220,80},{-220,-60},{-174,-60},{-174,-68}},
      color={0,0,127}));
  connect(rou1.ports_bSup, pumChiWatPri1.ports_a)
    annotation(Line(points={{-40,-73.8806},{-26,-73.8806},{-26,-80},{-10,-80}},
      color={0,127,255}));
  connect(mChiWatEco_flow.port_b, TChiWatEcoEnt.port_a)
    annotation(Line(points={{-150,-320},{-170,-320}},
      color={0,127,255}));
  connect(TChiWatEcoEnt.port_b, resEco.port_a)
    annotation(Line(points={{-190,-320},{-200,-320},{-200,-300}},
      color={0,127,255}));
  connect(resEco.port_b, cooEco.port_a)
    annotation(Line(points={{-200,-280},{-200,-260},{-180,-260}},
      color={0,127,255}));
  connect(TChiWat[1].y, cooEco.T_in)
    annotation(Line(
      points={{-228,80},{-220,80},{-220,-170},{-174,-170},{-174,-248}},
      color={0,0,127}));
  connect(cooEco.port_b, rou1.ports_aSup[nChi + 1])
    annotation(Line(
      points={{-160,-260},{-120,-260},{-120,-73.8806},{-80,-73.8806}},
      color={0,127,255}));
  connect(busValChiWatEcoByp, valChiWatEcoByp.bus)
    annotation(Line(points={{200,-280},{200,-290},{-110,-290}},
      color={255,204,51},
      thickness=0.5));
  connect(yValChiWatEcoByp.y[1], busValChiWatEcoByp.y)
    annotation(Line(points={{-138,240},{150,240},{150,-280},{200,-280}},
      color={0,0,127}));
  connect(busPla, rou1.bus)
    annotation(Line(points={{200,-60},{-60,-60},{-60,-70}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(rou1.ports_bRet[nChi + 1], mChiWatEco_flow.port_a)
    annotation(Line(
      points={{-80,-326.119},{-106,-326.119},{-106,-320},{-130,-320}},
      color={0,127,255}));
  connect(mChiWatEco_flow.port_a, valChiWatEcoByp.port_a)
    annotation(Line(points={{-130,-320},{-120,-320},{-120,-300}},
      color={0,127,255}));
  ...
  connect(booToRea.y, comSigLoa.u)
    annotation(Line(points={{240,58},{240,52}},
      color={0,0,127}));
  connect(busPumChiWatPri.y1_actual, booToRea.u)
    annotation(Line(points={{200,200},{240,200},{240,82}},
      color={255,204,51},
      thickness=0.5));
  connect(comSigLoa.y, loa.u)
    annotation(Line(points={{240,28},{240,-14},{32,-14}},
      color={0,0,127}));
  ...
  connect(comSigLoa1.y, loa1.u)
    annotation(Line(points={{240,-212},{240,-314},{32,-314}},
      color={0,0,127}));
  connect(busPumChiWatPri1.y1_actual, booToRea1.u)
    annotation(Line(points={{200,-100},{240,-100},{240,-158}},
      color={255,204,51},
      thickness=0.5));
  connect(rou.ports_bRet[nChi + 1], pas.port_a)
    annotation(Line(points={{-80,-26.1194},{-100,-26.1194},{-100,20}},
      color={0,127,255}));
  connect(pas.port_b, rou.ports_aSup[nChi + 1])
    annotation(Line(points={{-100,40},{-100,226.119},{-80,226.119}},
      color={0,127,255}));
annotation(Diagram(coordinateSystem(extent={{-260,-400},{260,320}})),
  experiment(StopTime=2000,
    Tolerance=1e-06),
  __Dymola_Commands(
    file="modelica://Buildings/Resources/Scripts/Dymola/Templates/Plants/Chillers/Components/Validation/ChillersToPrimaryPumpsParallel.mos"
      "Simulate and plot"),
  Documentation(
    info="<html>
<p>
  ...
  for various plant configurations where the chillers are connected in
  parallel. The validation uses open-loop controls.
</p>
<ul>
  <li>The first configuration has no waterside economizer.</li>
  <li>
