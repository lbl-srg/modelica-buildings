...
model ChillerGroupWaterCooled
  ...
  parameter Modelica.Units.SI.HeatFlowRate capChi_nominal[nChi](each min=0) =
    ...
    annotation(Dialog(group="Nominal condition"));
  parameter Real COPChi_nominal[nChi](each min=0) =
    fill(Buildings.Templates.Data.Defaults.COPChiWatCoo, nChi)
...
equation
  ...
  connect(TConWat.y, tow.T_in)
    annotation(Line(points={{-218,140},{-200,140},{-200,-68}},
      color={0,0,127}));
  connect(tow.port_b, inlPumConWat.port_a)
    annotation(Line(points={{-186,-80},{-170,-80}},
      color={0,127,255}));
  ...
  connect(chi.ports_bCon, outConWatChi.ports_b)
    annotation(Line(points={{-100,100},{-130,100}},
      color={0,127,255}));
  connect(outConWatChi.port_a, tow.port_a)
    annotation(Line(points={{-150,100},{-220,100},{-220,-80},{-206,-80}},
      color={0,127,255}));
  ...
  connect(pumConWat.ports_b, outPumConWat.ports_a)
    annotation(Line(points={{-130,-80},{-130,-80}},
      color={0,127,255}));
  ...
  connect(tow.port_b, bouCon.ports[1])
    annotation(Line(points={{-186,-80},{-180,-80},{-180,-100}},
      color={0,127,255}));
  connect(chi.bus, busPla)
    annotation(Line(points={{-80,104.2},{-80,140}},
      color={255,204,51},
      thickness=0.5));
  connect(ctl.bus, busPla)
    annotation(Line(points={{-160,180},{-80,180},{-80,140}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(busPla.pumConWat, pumConWat.bus)
    annotation(Line(points={{-80,140},{-140,140},{-140,-70}},
      color={255,204,51},
      thickness=0.5),
      Text(string="%first",
        index=-1,
  ...
  connect(booToRea.y, comSigLoa.u)
    annotation(Line(points={{200,68},{200,62}},
      color={0,0,127}));
  connect(comSigLoa.y, loa.u)
    annotation(Line(points={{200,38},{200,-74},{90,-74}},
      color={0,0,127}));
  connect(busChi.y1_actual, booToRea.u)
    annotation(Line(points={{160,140},{200,140},{200,92}},
      color={255,204,51},
      thickness=0.5));
  connect(busPla.chi, busChi)
    annotation(Line(points={{-80,140},{160,140}},
      color={255,204,51},
      thickness=0.5));
...
<p>
  ...
  configuration. The controller is automatically configured (by means of
