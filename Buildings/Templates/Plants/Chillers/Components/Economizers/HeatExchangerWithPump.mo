within Buildings.Templates.Plants.Chillers.Components.Economizers;
...
equation
  ...
  connect(port_a, TChiWatEcoEnt.port_a)
    annotation(Line(points={{-100,0},{-20,0},{-20,10}},
      color={0,127,255}));
annotation(defaultComponentName="eco",
  Documentation(
    info="<html>
<p>
  This is a model of a waterside economizer where a variable speed pump is
  used to control the CHW flow rate through the heat exchanger. The CW flow
  rate is modulated by means of a two-way valve. As per standard practice, the
  model includes a temperature sensor on the CHW side, upstream of the heat
  exchanger. This sensor is typically used in conjunction with a temperature
  sensor in the CHW return pipe, upstream of the WSE, to reset the pump speed
...
<ul>
  ...
  Icon(graphics={Bitmap(extent={{300,-320},{380,-240}},
