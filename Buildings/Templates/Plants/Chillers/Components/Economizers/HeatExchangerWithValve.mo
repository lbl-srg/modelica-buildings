...
equation
  ...
  connect(port_a, hex.port_a2)
    annotation(Line(points={{-100,0},{-20,0},{-20,68},{-10,68}},
      color={0,127,255}));
  connect(port_a, valChiWatByp.port_a)
    annotation(Line(points={{-100,0},{-10,0}},
      color={0,127,255}));
  connect(valChiWatByp.port_b, port_b)
    annotation(Line(points={{10,0},{100,0}},
      color={0,127,255}));
  connect(hex.port_a2, dpChiWatEco.port_a)
    annotation(Line(points={{-10,68},{-20,68},{-20,40},{-10,40}},
      color={0,127,255}));
...
annotation(defaultComponentName="eco",
  Documentation(
    info="<html>
<p>
  This is a model of a waterside economizer where a modulating heat exchanger
  bypass valve is used to control the CHW flow rate through the heat
  exchanger. The CW flow rate is modulated by means of a two-way valve. As per
  standard practice, the model includes a differential pressure sensor on the
  CHW side that measures the pressure drop across the heat exchanger.
</p>
</html>",
    revisions="<html>
<ul>
