...
model WaterCooled
  ...
  Buildings.Templates.Components.Sensors.Temperature TConWatRet(
    ...
    "CW return temperature (from chillers to coolers)"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      rotation=0,
      origin={-80,40})));
...
equation
  ...
  connect(inlPumConWat.ports_b, pumConWat.ports_a)
    annotation(Line(points={{-90,-192},{-90,-192}},
      color={0,0,0},
      thickness=0.5));
  connect(busWea, coo.busWea)
    annotation(Line(points={{0,280},{-170,280},{-170,94}},
      color={255,204,51},
      thickness=0.5));
  connect(inlPumConWat.port_a, bouConWat.ports[1])
    annotation(Line(points={{-110,-192},{-110,-212},{-100,-212},{-100,-240}},
      color={0,127,255},
      visible=viewDiagramAll));
  connect(pumConWat.ports_b, inlConChi.ports_a)
    annotation(Line(points={{-70,-192},{-70,-192}},
      color={0,0,0},
      thickness=0.5));
  connect(outConChi.port_b, TConWatRet.port_b)
    annotation(Line(points={{-60,0},{-60,40},{-70,40}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash));
  connect(TConWatRet.port_a, coo.port_a)
    annotation(Line(points={{-90,40},{-140,40},{-140,64},{-190,64}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash));
  connect(coo.port_b, TConWatSup.port_b)
    annotation(Line(points={{-210,64},{-210,-192},{-170,-192}},
      color={0,0,0},
      thickness=0.5));
  connect(TConWatSup.port_a, inlPumConWat.port_a)
    annotation(Line(points={{-150,-192},{-110,-192}},
      color={0,0,0},
      thickness=0.5));
annotation(Documentation(
  info="<html>
<p>
  This template represents a chilled water plant with water-cooled compression
  chillers.
</p>
<p>
  The table below lists all available equipment configurations. The first
...
<table summary=\"summary\" border=\"1\">
  ...
  <tr>
    <td>Chiller head pressure control</td>
    <td>
  ...
  <tr>
    <td>Chiller CHW isolation valve</td>
    <td>
    ...
    <td>
      If the primary CHW pumps are dedicated, the option with no isolation
      valve is automatically selected.<br />
      If the primary CHW pumps are headered, the choice between two-way
      modulating valves and two-way two-position valves is possible. A
      modulating valve is recommended on primary-only variable flow systems to
      allow for slow changes in flow during chiller staging. Sometimes
      electric valve timing may be sufficiently slow that two-position valves
      can provide stable performance. Two-position valves are acceptable on
      primary-secondary systems.
    </td>
  </tr>
  <tr>
    <td>Chiller CW isolation valve</td>
    <td>
      No valve<br />
  ...
  <tr>
    <td>Primary CHW pump arrangement</td>
    <td>
      <b>Headered</b><br />
      <font color=\"gray\"><em>Dedicated</em></font>
    </td>
    <td>
      Headered pumps are required (and automatically selected) for
      configurations with
      <ul>
  ...
  <tr>
    <td>Type of primary CHW pumps for constant flow configurations</td>
    <td>
    ...
    <td>
      ...
      to constant flow primary-only plants, for example, a plant serving only
      one or two very large air handlers.
    </td>
  </tr>
  <tr>
    <td>Coolers</td>
  ...
  <tr>
    <td>Cooler inlet and outlet isolation valves</td>
    <td>
  ...
  <tr>
    ...
    <td>
      Headered pumps are required (and automatically selected) for
      configurations with waterside economizer.
    </td>
  </tr>
  <tr>
    ...
    <td>
      Variable speed pumps are required (and automatically selected) for
      configurations with waterside economizer.
    </td>
  </tr>
  <tr>
    <td>Waterside economizer</td>
    <td>
