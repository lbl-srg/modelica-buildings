...
model BoilerGroup
  ...
  parameter Buildings.Templates.Plants.Boilers.HotWater.Components.Data.BoilerGroup dat(
    final nBoi=nBoi,
    final typMod=typMod)
  ...
  final parameter Modelica.Units.SI.PressureDifference dpHeaWat_nominal[nBoi] =
    dpFixedHeaWat_nominal .+ (if typValBoiIso <>
  ...
  Buildings.Templates.Components.Interfaces.Bus busBoiNon[nBoi]
    if not is_con
    "Boiler control bus - Non-condensing boilers"
    annotation(Placement(transformation(extent={{-100,140},{-60,180}}),
      iconTransformation(extent={{-350,6},{-310,46}})));
  ...
  Buildings.Templates.Components.Interfaces.Bus busValBoiConIso[nBoi]
    ...
    annotation(Placement(transformation(extent={{140,140},{180,180}}),
      iconTransformation(extent={{-350,6},{-310,46}})));
equation
  connect(ports_aHeaWat, boiPol.port_a)
    annotation(Line(points={{200,-100},{-20,-100},{-20,0},{-10,0}},
      color={0,127,255}));
  connect(boiPol.port_b, valBoiIso.port_a)
    annotation(Line(points={{10,0},{20,0},{20,120},{150,120}},
      color={0,127,255}));
  ...
  connect(bus.boiCon, busBoiCon)
    annotation(Line(points={{0,200},{0,180},{-40,180},{-40,160}},
      color={255,204,51},
      thickness=0.5));
  connect(bus.boiNon, busBoiNon)
    annotation(Line(points={{0,200},{0,180},{-80,180},{-80,160}},
      color={255,204,51},
      thickness=0.5));
  connect(busBoiNon, boiPol.bus)
    annotation(Line(points={{-80,160},{-80,10},{0,10}},
      color={255,204,51},
      thickness=0.5));
  connect(valBoiIso.bus, busValBoiConIso)
    annotation(Line(points={{160,130},{160,160}},
      color={255,204,51},
      thickness=0.5));
  connect(busValBoiNonIso, valBoiIso.bus)
    annotation(Line(points={{100,160},{100,130},{160,130}},
      color={255,204,51},
      thickness=0.5));
  connect(bus.valBoiNonIso, busValBoiNonIso)
    annotation(Line(points={{0,200},{0,180},{100,180},{100,160}},
      color={255,204,51},
      thickness=0.5));
  connect(bus.valBoiConIso, busValBoiConIso)
    annotation(Line(points={{0,200},{4,200},{4,184},{160,184},{160,160}},
      color={255,204,51},
      thickness=0.5));
  connect(ports_aHeaWat, boiTab.port_a)
    annotation(Line(points={{200,-100},{-20,-100},{-20,-50},{-10,-50}},
      color={0,127,255}));
  connect(boiTab.port_b, valBoiIso.port_a)
  ...
  connect(busBoiNon, boiTab.bus)
    annotation(Line(points={{-80,160},{-80,-40},{0,-40}},
      color={255,204,51},
      thickness=0.5));
annotation(Icon(coordinateSystem(preserveAspectRatio=false,
  extent={{-400,-700},{400,700}}),
  graphics={Line(points={{200,500},{400,500}},
    color={0,0,0},
  ...
  Line(visible=nBoi >= 4,
    ...
    pattern=LinePattern.Solid,
    thickness=5),
  Text(extent={{-151,-712},{149,-752}},
    textColor={0,0,255},
    textString="%name"),
  Bitmap(visible=nBoi >= 1,
  ...
  Text(visible=is_con,
    ...
    textColor={0,0,0},
    textString="CON"),
  Text(extent={{-160,420},{200,380}},
    textColor={0,0,0},
    textString="BOI-1"),
  Line(points={{200,400},{400,400}},
    color={0,0,0},
    pattern=LinePattern.Dash,
    thickness=5),
  Rectangle(extent={{200,240},{-160,60}},
    lineColor={0,0,0},
    lineThickness=1,
    visible=nBoi >= 2),
  Text(visible=nBoi >= 2 and is_con,
  ...
  Line(visible=nBoi >= 2,
    ...
    pattern=LinePattern.Dash,
    thickness=5),
  Rectangle(extent={{200,-60},{-160,-240}},
    lineColor={0,0,0},
    lineThickness=1,
    visible=nBoi >= 3),
  Text(visible=nBoi >= 3 and is_con,
  ...
  Text(visible=nBoi >= 3,
    ...
    textColor={0,0,0},
    textString="BOI-3"),
  Line(points={{200,-200},{400,-200}},
    color={0,0,0},
    pattern=LinePattern.Dash,
    thickness=5,
    visible=nBoi >= 3),
  Rectangle(visible=nBoi >= 4,
    extent={{200,-360},{-160,-540}},
    lineColor={0,0,0},
    lineThickness=1),
  Text(visible=nBoi >= 4 and is_con,
  ...
  Documentation(
    info="<html>
<p>This model represents a group of hot water boilers.</p>
<p>Modeling features and limitations:</p>
<ul>
  <li>
    All units are either condensing boilers or non-condensing boilers,
    depending on the value of the Boolean parameter <code>is_con</code>. In
    order to represent a hybrid plant with both condensing and non-condensing
    boilers, two instances of this model must be used.
  </li>
  <li>
    All units are modeled based on the same boiler model, specified with the
    parameter <code>typMod</code> which is based on the enumeration
    ...
    However, the boiler characteristics such as the design capacity and HW
    flow rate may be different from one unit to another.
  </li>
  <li>
    Two-way two-position boiler isolation valves are automatically included in
    case of headered primary HW pumps, as specified with the parameter
...
<ul>
  <li>
    Sub-bus <code>boiCon[:]</code> (resp. <code>boiNon[:]</code>) storing all
    signals dedicated to each condensing boiler (resp. non-condensing boiler),
    with a dimensionality of one
    <ul>
      <li>
        Boiler Enable signal <code>boi(Con|Non)[:].y1</code> : DO signal
        dedicated to each unit, with a dimensionality of one
      </li>
      <li>
        Boiler HW supply temperature setpoint
        <code>boi(Con|Non)[:].THeaWatSupSet</code>: AO signal dedicated to
        each unit, with a dimensionality of one
      </li>
      <li>
        Boiler firing rate <code>boi(Con|Non)[:].y_actual</code>: AI signal
        dedicated to each unit, with a dimensionality of one
      </li>
      <li>
  ...
  <li>
    ...
    storing all signals dedicated to each condensing boiler (resp.
    non-condensing boiler) isolation valve (if any), with a dimensionality of
    one
    <ul>
      <li>
        Valve opening command <code>valBoi(Con|Non)Iso[:].y1</code> : DO
        signal dedicated to each unit, with a dimensionality of one
      </li>
      <li>
        Valve open end switch status
        <code>valBoi(Con|Non)Iso[:].y1_actual</code> : DI signal dedicated to
        each unit, with a dimensionality of one
      </li>
      <li>
        Valve closed end switch status
        <code>valBoi(Con|Non)Iso[:].y0_actual</code> : DI signal dedicated to
        each unit, with a dimensionality of one
      </li>
    </ul>
  </li>
