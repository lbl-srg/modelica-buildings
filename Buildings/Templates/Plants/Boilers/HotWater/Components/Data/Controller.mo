...
record Controller
  ...
  parameter Modelica.Units.SI.Temperature THeaWatConSup_nominal(
    ...
    annotation(Dialog(group="Temperature setpoints",
      enable=cfg.have_boiCon and cfg.have_boiNon));
  ...
  parameter Modelica.Units.SI.VolumeFlowRate VHeaWatPriCon_flow_nominal(
    ...
    annotation(Dialog(group="Capacity",
      enable=cfg.typCtl ==
        ...
        and cfg.have_pumHeaWatPriVarCon
  ...
  parameter Modelica.Units.SI.VolumeFlowRate VHeaWatPriNon_flow_nominal(
    ...
    annotation(Dialog(group="Capacity",
      enable=cfg.typCtl ==
        ...
        and cfg.typPumHeaWatSec ==
          Buildings.Templates.Plants.Boilers.HotWater.Types.PumpsSecondary.None
        and cfg.have_pumHeaWatPriVarNon
  ...
  parameter Modelica.Units.SI.PressureDifference dpHeaWatRemSet_max[:](
    ...
    annotation(Dialog(
      ...
      enable=cfg.typCtl ==
        Buildings.Templates.Plants.Boilers.HotWater.Types.Controller.Guideline36
        and ((cfg.have_pumHeaWatPriVarCon or cfg.have_pumHeaWatPriVarNon)
          and cfg.typPumHeaWatSec ==
            Buildings.Templates.Plants.Boilers.HotWater.Types.PumpsSecondary.None
          or cfg.typPumHeaWatSec <>
            Buildings.Templates.Plants.Boilers.HotWater.Types.PumpsSecondary.None)));
  ...
  parameter Real yPumHeaWatPri_min(final unit="1", final min=0, final max=1) =
    0.1
  ...
  parameter Real yPumHeaWatPriSta_min[:](
    ...
    annotation(Dialog(
      ...
      enable=cfg.typCtl ==
        ...
        and cfg.typPumHeaWatSec <>
          Buildings.Templates.Plants.Boilers.HotWater.Types.PumpsSecondary.None));
  parameter Integer sta[:, :](each final min=0, each final max=1)
...
annotation(defaultComponentName="datCtl",
  Documentation(
    info="<html>
<p>
  This record provides the set of sizing and operating parameters for HW plant
  controllers that can be found within
...
<p>
  For hybrid plants, boilers shall be indexed so that condensing boilers have
