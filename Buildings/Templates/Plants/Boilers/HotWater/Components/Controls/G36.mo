...
block G36
  ...
  extends Buildings.Templates.Plants.Boilers.HotWater.Components.Interfaces.PartialController(
  ...
  final parameter Integer nPumPri = cfg.nPumHeaWatPriCon + cfg.nPumHeaWatPriNon
  ...
  parameter Integer nPumPri_nominal(final max=nPumPri, final min=1) = nPumPri
  ...
  final parameter Buildings.Controls.OBC.ASHRAE.G36.Plants.Boilers.Types.SecondaryPumpSpeedControl speConTypSec =
    if cfg.have_senDpHeaWatRemWir
    then Buildings.Controls.OBC.ASHRAE.G36.Plants.Boilers.Types.SecondaryPumpSpeedControl.RemoteDP
...
equation
  ...
  connect(busValBoiNonIso.y1_actual, ctlLooPri.uHotWatIsoValOpe[(cfg.nBoiCon +
    1):nBoi]);
  connect(busValBoiConIso.y0_actual, ctlLooPri.uHotWatIsoValClo[1:cfg.nBoiCon]);
  connect(busValBoiNonIso.y0_actual, ctlLooPri.uHotWatIsoValClo[(cfg.nBoiCon +
    1):nBoi]);
  ...
  connect(ctlLooPri.TBoiHotWatSupSet[(cfg.nBoiCon +
    1):nBoi], busBoiNon.THeaWatSupSet);
  ...
  connect(reqResHeaWatAirHan.y, reqResHeaWat.u1)
    annotation(Line(points={{208,120},{198,120},{198,126},{194,126}},
      color={255,127,0}));
  connect(reqResHeaWatEquZon.y, reqResHeaWat.u2)
    annotation(Line(points={{208,-160},{198,-160},{198,114},{194,114}},
      color={255,127,0}));
  connect(reqPlaHeaWatAirHan.y, reqPlaHeaWat.u1)
    annotation(Line(points={{208,162},{200,162},{200,166},{192,166}},
      color={255,127,0}));
  connect(reqPlaHeaWatEquZon.y, reqPlaHeaWat.u2)
    annotation(Line(points={{208,-120},{200,-120},{200,154},{192,154}},
      color={255,127,0}));
  connect(idxPumHeaWatSec.y, ctlPumHeaWatSec.uPumLeaLag)
    annotation(Line(points={{12,100},{44,100},{44,98.2},{48,98.2}},
      color={255,127,0}));
  connect(ctlLooPri.yPla, ctlPumHeaWatSec.uPlaEna)
    annotation(Line(points={{12,9.9},{30,9.9},{30,90},{48,90}},
      color={255,0,255}));
  ...
  connect(busAirHan.reqPlaHeaWat, reqPlaHeaWatAirHan.u)
    annotation(Line(points={{260,140},{240,140},{240,162},{232,162}},
      color={255,204,51},
      thickness=0.5));
  connect(busAirHan.reqResHeaWat, reqResHeaWatAirHan.u)
    annotation(Line(points={{260,140},{240,140},{240,120},{232,120}},
      color={255,204,51},
      thickness=0.5));
  connect(ctlLooPri.yPriPum, ctlPumHeaWatSec.uPriPum)
    annotation(Line(points={{12,-10.5},{32,-10.5},{32,78},{48,78}},
      color={255,0,255}));
  connect(ctlLooPri.yMaxSecPumSpe, ctlPumHeaWatSec.uMaxSecPumSpeCon)
    annotation(Line(points={{12,-7.1},{34,-7.1},{34,62},{48,62}},
      color={0,0,127}));
  connect(reqPlaHeaWat.y, ctlPumHeaWatSec.plaReq)
    annotation(Line(points={{168,160},{40,160},{40,86},{48,86}},
      color={255,127,0}));
  connect(reqPlaHeaWat.y, ctlLooPri.plaReq)
    annotation(Line(points={{168,160},{-20,160},{-20,23.5},{-12,23.5}},
      color={255,127,0}));
...
<ul>
  ...
  <li>
    Inside the sub-bus <code>busAirHan[:]</code> (resp.
    <code>busEquZon[:]</code>), with a dimensionality of one
    <ul>
      <li>
        HW plant requests yielded by the air handler or zone equipment
        controller <code>bus(AirHan|EquZon)[:].reqHeaWatPla</code>: AI signal
        (Integer), with a dimensionality of one
      </li>
      <li>
        HW reset requests yielded by the air handler or zone equipment
        controller <code>bus(AirHan|EquZon)[:].reqHeaWatRes</code>: AI signal
        (Integer), with a dimensionality of one
      </li>
    </ul>
  </li>
