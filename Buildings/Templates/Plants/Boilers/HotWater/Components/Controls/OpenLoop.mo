...
block OpenLoop
  extends Buildings.Templates.Plants.Boilers.HotWater.Components.Interfaces.PartialController(
    final typ=Buildings.Templates.Plants.Boilers.HotWater.Types.Controller.OpenLoop);
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1Con_default[cfg.nBoiCon](
    each table=[0, 0; 1, 1; 2, 0],
    each timeScale=1000,
    each period=2000)
    "Default Enable signal for all condensing units"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1Non_default[cfg.nBoiNon](
    each table=[0, 0; 1, 1; 2, 0],
    each timeScale=1000,
    each period=2000)
    "Default Enable signal for all non-condensing units"
    annotation(Placement(transformation(extent={{-10,-50},{10,-30}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1BoiCon[cfg.nBoiCon](
    table=y1Con_default.table,
    timeScale=y1Con_default.timeScale,
    period=y1Con_default.period)
    if cfg.have_boiCon
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant THeaWatConSupSet(
    ...
    final k=dat.THeaWatConSup_nominal)
    if cfg.have_boiCon
  ...
  Buildings.Controls.OBC.CDL.Routing.RealScalarReplicator rep(
    ...
    final nout=cfg.nBoiCon)
    if cfg.have_boiCon
    "Replicate signal"
    annotation(Placement(transformation(extent={{-150,230},{-170,250}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1ValBoiConIso[cfg.nBoiCon](
    table=y1Con_default.table,
    timeScale=y1Con_default.timeScale,
    period=y1Con_default.period)
    if cfg.have_boiCon
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1BoiNon[cfg.nBoiNon](
    table=y1Non_default.table,
    timeScale=y1Non_default.timeScale,
    period=y1Non_default.period)
    if cfg.have_boiNon
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant THeaWatSupSet(
    y(final unit="K", displayUnit="degC"),
    final k=dat.THeaWatSup_nominal)
    if cfg.have_boiNon
  ...
  Buildings.Controls.OBC.CDL.Routing.RealScalarReplicator rep1(
    y(each final unit="K", each displayUnit="degC"),
    final nout=cfg.nBoiNon)
    if cfg.have_boiNon
    "Replicate signal"
    annotation(Placement(transformation(extent={{-90,210},{-110,230}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1ValBoiNonIso[cfg.nBoiNon](
    table=y1Non_default.table,
    timeScale=y1Non_default.timeScale,
    period=y1Non_default.period)
    if cfg.have_boiNon
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1PumHeaWatPriCon[cfg.nPumHeaWatPriCon](
    table=y1Con_default.table,
    timeScale=y1Con_default.timeScale,
    period=y1Con_default.period)
    if cfg.have_boiCon
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant yPumHeaWatPriCon(
    y(final unit="1"),
    k=1)
    if cfg.have_boiCon and cfg.have_pumHeaWatPriVarCon
    "Primary HW pump speed signal - Condensing Boilers"
    annotation(Placement(transformation(extent={{-120,50},{-140,70}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1PumHeaWatPriNon[cfg.nPumHeaWatPriNon](
    table=y1Non_default.table,
    timeScale=y1Non_default.timeScale,
    period=y1Non_default.period)
    if cfg.have_boiNon
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant yPumHeaWatPriNon(
    y(final unit="1"),
    k=1)
    if cfg.have_boiNon and cfg.have_pumHeaWatPriVarNon
...
equation
  ...
  connect(yPumHeaWatPriCon.y, busPumHeaWatPriCon.y)
    annotation(Line(points={{-142,60},{-240,60},{-240,80}},
      color={0,0,127}));
  connect(y1PumHeaWatSec.y[1], busPumHeaWatSec.y1)
    annotation(Line(points={{-142,-40},{-200,-40}},
      color={255,0,255}));
  connect(yPumHeaWatSec.y, busPumHeaWatSec.y)
    annotation(Line(points={{-142,-80},{-200,-80},{-200,-40}},
      color={0,0,127}));
  connect(y1PumHeaWatPriNon.y[1], busPumHeaWatPriNon.y1)
    annotation(Line(points={{-82,80},{-160,80}},
      color={255,0,255}));
  ...
  connect(yPumHeaWatPriNon.y, busPumHeaWatPriNon.y)
    annotation(Line(points={{-82,40},{-160,40},{-160,80}},
      color={0,0,127}));
  connect(y1BoiCon.y[1], busBoiCon.y1)
    annotation(Line(points={{-142,200},{-240,200},{-240,160}},
      color={255,0,255}));
  connect(THeaWatConSupSet.y, rep.u)
    annotation(Line(points={{-142,240},{-148,240}},
      color={0,0,127}));
  connect(rep.y, busBoiCon.THeaWatSupSet)
    annotation(Line(points={{-172,240},{-240,240},{-240,160}},
      color={0,0,127}));
  connect(THeaWatSupSet.y, rep1.u)
    annotation(Line(points={{-82,220},{-88,220}},
      color={0,0,127}));
  connect(rep1.y, busBoiNon.THeaWatSupSet)
    annotation(Line(points={{-112,220},{-160,220},{-160,160}},
      color={0,0,127}));
  connect(y1BoiNon.y[1], busBoiNon.y1)
    annotation(Line(points={{-82,180},{-160,180},{-160,160}},
      color={255,0,255}));
  connect(y1ValBoiConIso.y[1], busValBoiConIso.y1)
    annotation(Line(points={{-142,140},{-240,140},{-240,120}},
      color={255,0,255}));
  connect(yValHeaWatMinByp.y, busValHeaWatMinByp.y)
    annotation(Line(points={{-142,20},{-194,20},{-194,40},{-200,40}},
      color={0,0,127}));
...
<p>
  ...
  <a href=\"modelica://Buildings.Templates.Plants.Boilers.HotWater.BoilerPlant\">
    Buildings.Templates.Plants.Boilers.HotWater.BoilerPlant</a>. It is only
