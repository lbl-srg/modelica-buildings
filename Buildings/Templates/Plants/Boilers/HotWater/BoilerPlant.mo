...
model BoilerPlant
  "Boiler plant"
  extends Buildings.Templates.Plants.Boilers.HotWater.Interfaces.PartialBoilerPlant(
  ...
  Buildings.Templates.Plants.Boilers.HotWater.Components.BoilerGroup boiNon(
    redeclare final package Medium=Medium,
    final typMod=typMod,
    final nBoi=nBoiNon,
    final is_con=false,
  ...
  Buildings.Templates.Components.Routing.MultipleToMultiple inlPumHeaWatPriCon(
    ...
    final have_comLeg=typArrPumHeaWatPriCon ==
  ...
  Buildings.Templates.Components.Sensors.VolumeFlowRate VHeaWatPriSupCon_flow(
    ...
    final m_flow_nominal=mHeaWatPriCon_flow_nominal,
    final allowFlowReversal=allowFlowReversal,
    final have_sen=ctl.have_senVHeaWatPriCon
      and ctl.locSenVHeaWatPri ==
  ...
  Buildings.Templates.Components.Routing.Junction junInlBoiCon(
    ...
    icon_pipe2=if have_boiCon
      then Buildings.Templates.Components.Types.IntegrationPoint.Return
      else Buildings.Templates.Components.Types.IntegrationPoint.None,
    icon_pipe3=if have_bypHeaWatFixCon or have_valHeaWatMinBypCon
      then (if have_boiNon
        then Buildings.Templates.Components.Types.IntegrationPoint.Return
        else Buildings.Templates.Components.Types.IntegrationPoint.Supply)
      else Buildings.Templates.Components.Types.IntegrationPoint.None)
  ...
  Buildings.Templates.Components.Routing.Junction junOutBoiCon(
    ...
    icon_pipe1=if have_bypHeaWatFixCon or have_valHeaWatMinBypCon
      then (if have_boiNon
        then Buildings.Templates.Components.Types.IntegrationPoint.Return
        else Buildings.Templates.Components.Types.IntegrationPoint.Supply)
      else Buildings.Templates.Components.Types.IntegrationPoint.None,
    icon_pipe2=if have_boiNon
      then Buildings.Templates.Components.Types.IntegrationPoint.Return
      else Buildings.Templates.Components.Types.IntegrationPoint.Supply,
    icon_pipe3=if have_boiCon
  ...
  Buildings.Templates.Components.Routing.MultipleToMultiple inlPumHeaWatPriNon(
    ...
    final have_comLeg=typArrPumHeaWatPriNon ==
      Buildings.Templates.Components.Types.PumpArrangement.Headered,
  ...
  Buildings.Templates.Components.Sensors.VolumeFlowRate VHeaWatPriSupNon_flow(
    ...
    final m_flow_nominal=mHeaWatPriNon_flow_nominal,
    final allowFlowReversal=allowFlowReversal,
    final have_sen=ctl.have_senVHeaWatPriNon
      and ctl.locSenVHeaWatPri ==
        Buildings.Templates.Plants.Boilers.HotWater.Types.SensorLocation.Supply,
    final text_flip=false,
    final typ=Buildings.Templates.Components.Types.SensorVolumeFlowRate.FlowMeter,
  ...
  Buildings.Templates.Components.Sensors.Temperature THeaWatPriSupNon(
    ...
    icon_pipe=Buildings.Templates.Components.Types.IntegrationPoint.Supply)
    if have_boiNon
    "Primary HW supply temperature - Non-condensing boilers"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      rotation=0,
      origin={10,0})));
  ...
  Buildings.Templates.Components.Routing.Junction junInlBoiNon(
    ...
    icon_pipe1=if have_boiNon
      then Buildings.Templates.Components.Types.IntegrationPoint.Return
      else Buildings.Templates.Components.Types.IntegrationPoint.Supply,
    icon_pipe2=if have_bypHeaWatFixNon or have_valHeaWatMinBypNon
      then Buildings.Templates.Components.Types.IntegrationPoint.Supply
      else Buildings.Templates.Components.Types.IntegrationPoint.None,
    icon_pipe3=if have_boiNon
      then Buildings.Templates.Components.Types.IntegrationPoint.Return
      else Buildings.Templates.Components.Types.IntegrationPoint.None)
    "Fluid junction"
    annotation(Placement(transformation(extent={{-10,10},{10,-10}},
  ...
  Buildings.Templates.Components.Routing.Junction junOutBoiNon(
    ...
    icon_pipe2=Buildings.Templates.Components.Types.IntegrationPoint.Supply,
    icon_pipe3=if have_bypHeaWatFixNon or have_valHeaWatMinBypNon
      then Buildings.Templates.Components.Types.IntegrationPoint.Supply
      else Buildings.Templates.Components.Types.IntegrationPoint.None)
    "Fluid junction"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
  ...
  replaceable Buildings.Templates.Plants.Boilers.HotWater.Components.Controls.G36 ctl
    constrainedby Buildings.Templates.Plants.Boilers.HotWater.Components.Interfaces.PartialController(
      final cfg=cfg,
      final dat=dat.ctl)
    "Plant controller"
  ...
  Fluid.Sources.Boundary_pT bouHeaWat(
    ...
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      ...
      origin={70,-270})));
  protected
  Buildings.Templates.Components.Interfaces.Bus busLooCon
    if have_boiCon
    "Condensing boiler loop control bus"
    annotation(Placement(transformation(extent={{-300,140},{-260,180}}),
      iconTransformation(extent={{-466,50},{-426,90}})));
  Buildings.Templates.Components.Interfaces.Bus busLooNon
    if have_boiNon
    "Non-condensing boiler loop control bus"
...
equation
  ...
  connect(inlBoiCon.ports_b, boiCon.ports_aHeaWat)
    annotation(Line(points={{-140,-240},{-140,-240}},
      color={0,127,255}));
  ...
  connect(outPumHeaWatSec.port_b, VHeaWatSecSup_flow.port_a)
    annotation(Line(points={{160,0},{180,0}},
      color={0,0,0},
      thickness=0.5,
  ...
  connect(busEquZon, ctl.busEquZon)
    annotation(Line(points={{300,100},{280,100},{280,134},{10,134}},
      color={255,204,51},
      thickness=0.5),
      Text(string="%first",
        index=-1,
  ...
  connect(VHeaWatPriRetCon_flow.port_b, inlBoiCon.port_a)
    annotation(Line(points={{-40,-240},{-120,-240}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash,
      visible=have_boiCon));
  ...
  connect(junOutBoiNon.port_2, inlPumHeaWatSec.port_a)
    annotation(Line(points={{70,0},{100,0}},
      ...
      thickness=0.5,
      visible=have_pumHeaWatSec));
  connect(junOutBoiNon.port_2, supHeaWat.port_a)
    annotation(Line(points={{70,0},{120,0}},
      color={0,0,0},
      thickness=0.5,
      visible=not have_pumHeaWatSec));
  ...
  connect(outPumHeaWatPriCon.port_b, VHeaWatPriSupCon_flow.port_a)
    annotation(Line(points={{-50,-140},{-40,-140}},
      color={0,0,0},
      thickness=0.5,
      visible=have_boiCon));
  ...
  connect(VHeaWatPriSupNon_flow.port_b, THeaWatPriSupNon.port_a)
    annotation(Line(points={{-20,0},{0,0}},
      color={0,0,0},
      thickness=0.5,
      visible=have_boiNon));
  connect(junInlBoiNon.port_3, THeaWatPlaRetNon.port_a)
    annotation(Line(points={{50,-100},{20,-100}},
      color={0,0,0},
      thickness=0.5,
      visible=have_boiNon,
      pattern=LinePattern.Dash));
  ...
  connect(VHeaWatPriRetNon_flow.port_b, inlBoiNon.port_a)
    annotation(Line(points={{-40,-100},{-120,-100}},
      color={0,0,0},
      thickness=0.5,
      visible=have_boiNon,
      pattern=LinePattern.Dash));
  ...
  connect(boiNon.ports_bHeaWat, inlPumHeaWatPriNon.ports_a)
    annotation(Line(points={{-140,0},{-110,0}},
      color={0,0,0},
      thickness=0.5,
      visible=have_boiNon));
  connect(busLooCon, bus.looCon)
    annotation(Line(points={{-280,160},{-260,160},{-260,140},{-300,140}},
      color={255,204,51},
      thickness=0.5));
  connect(busLooNon, bus.looNon)
    annotation(Line(points={{-280,120},{-260,120},{-260,140},{-300,140}},
      color={255,204,51},
      thickness=0.5));
...
<p>
  The table below lists all available equipment configurations. If any default
  is provided, it is displayed in bold characters. Options shown in
...
<table summary=\"summary\" border=\"1\">
  ...
  <tr>
    <td>Type of secondary HW pumps</td>
    <td>
      None (primary-only)<br />
      Variable speed, centralized
    </td>
    <td>
      Constant speed secondary pumps are not supported as they are generally
      not advisable on any boiler system (see section 5.21.7.11 in ASHRAE,
      2021 for further explanations).<br />
...
<ul>
  ...
  Diagram(graphics={Rectangle(extent={{260,-240},{261,0}},
    lineColor={0,0,0},
    visible=ctl.have_senDpHeaWatLoc)}));
end BoilerPlant;
