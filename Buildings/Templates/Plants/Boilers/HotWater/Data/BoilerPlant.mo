...
record BoilerPlant
  ...
  parameter Buildings.Templates.Components.Data.Valve valHeaWatMinBypCon(
    ...
    m_flow_nominal=if cfg.have_valHeaWatMinBypCon
      then max(ctl.VHeaWatBoiCon_flow_min) * cfg.rhoHeaWat_default else 0,
    dpValve_nominal=Buildings.Templates.Data.Defaults.dpValBypMin)
    "HW minimum flow bypass valve - Condensing boilers"
    annotation(Dialog(group="Primary HW loop",
      enable=cfg.have_valHeaWatMinBypCon));
  parameter Buildings.Templates.Components.Data.Valve valHeaWatMinBypNon(
    final typ=Buildings.Templates.Components.Types.Valve.TwoWayModulating,
    m_flow_nominal=if cfg.have_valHeaWatMinBypNon
      then max(ctl.VHeaWatBoiNon_flow_min) * cfg.rhoHeaWat_default else 0,
    dpValve_nominal=Buildings.Templates.Data.Defaults.dpValBypMin)
    "HW minimum flow bypass valve - Non-condensing boilers"
    annotation(Dialog(group="Primary HW loop",
      enable=cfg.have_valHeaWatMinBypNon));
  parameter Buildings.Templates.Components.Data.PumpMultiple pumHeaWatSec(
    ...
    annotation(Dialog(group="Secondary HW loop",
      enable=cfg.typPumHeaWatSec ==
        Buildings.Templates.Plants.Boilers.HotWater.Types.PumpsSecondary.Centralized));
annotation(defaultComponentName="dat",
  Documentation(
    info="<html>
<p>
  This record provides the set of sizing and operating parameters for the
  boiler plant model
