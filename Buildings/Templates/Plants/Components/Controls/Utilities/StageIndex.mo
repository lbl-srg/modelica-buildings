...
block StageIndex "Evaluation of stage index from staging signals"
  parameter Integer nSta(start=1, final min=1)
    "Number of stages"
    annotation(Evaluate=true);
  parameter Modelica.Units.SI.Time dtRun(final min=0)=0
  ...
  Buildings.Controls.OBC.CDL.Logical.Timer tim[nSta](
    passed(each start=false),
    each final t=dtRun) if dtRun>0
  ...
  Buildings.Controls.OBC.CDL.Routing.BooleanVectorFilter booVecFil(
    final nin=nSta,
    final nout=nSta) if dtRun<=0
    "Direct pass-through when no minimum runtime"
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=-90,
        origin={20,80})));
  FirstTrueIndex idxFirAva(nin=nSta) "Return index of first available stage"
    annotation (Placement(transformation(extent={{-200,-130},{-180,-110}})));
  LastTrueIndex idxLasAva(nin=nSta) "Return index of last available stage"
...
equation
  for i in 1:(nSta - 1) loop
    connect(enaLag[i].outPort, sta[i+1].inPort[1])
      annotation (Line(points={{101.5,140},{110,140},{110,180},{-20,180},{-20,140},
            {-11,140}},    color={0,0,0}));
  end for;
  if nSta > 1 then
    for i in 2:nSta loop
      connect(disLag[i].outPort, sta[i-1].inPort[2])
        annotation (Line(points={{121.5,120},{140,120},{140,180},{-20,180},{-20,
              140},{-11,140}}, color={0,0,0}));
    end for;
  end if;
  // Stage down command from stage 1 brings back to stage 1.
  connect(disLag[1].outPort, sta[1].inPort[3])
    annotation (Line(points={{121.5,120},{140,120},{140,200},{-140,200},{-140,140},
          {-11,140}}, color={0,0,0}));
  ...
  connect(orUna1.y, disLag.condition) annotation (Line(points={{102,-120},{120,-120},
          {120,108}}, color={255,0,255}));
  connect(maxInt.y, staUna.index) annotation (Line(points={{-158,-200},{-140,-200},
          {-140,-172}}, color={255,127,0}));
  connect(una.y, staUna.u)
    annotation (Line(points={{-158,-160},{-152,-160}}, color={255,0,255}));
  connect(rep4.y, orUna.u2) annotation (Line(points={{52,-140},{60,-140},{60,-20},
          {68,-20}}, color={255,0,255}));
  connect(staUpAndUna.y, rep4.u)
    annotation (Line(points={{22,-140},{28,-140}},color={255,0,255}));
  connect(staUna.y, staUpAndUna.u2) annotation (Line(points={{-128,-160},{-10,-160},
          {-10,-148},{-2,-148}},  color={255,0,255}));
  connect(staDowAndUna.y, rep3.u)
    annotation (Line(points={{22,-180},{28,-180}},color={255,0,255}));
  connect(rep3.y, orUna1.u2) annotation (Line(points={{52,-180},{70,-180},{70,-128},
          {78,-128}},       color={255,0,255}));
  connect(idxFirAct.y, staUp.u1) annotation (Line(points={{172,100},{200,100},{200,
          -200},{-90,-200},{-90,-140},{-52,-140}},   color={255,127,0}));
  connect(idxFirAct.y, staDow.u1) annotation (Line(points={{172,100},{200,100},{
          200,-200},{-90,-200},{-90,-180},{-52,-180}},   color={255,127,0}));
  connect(staUp.y, staUpAndUna.u1)
    annotation (Line(points={{-28,-140},{-2,-140}},  color={255,0,255}));
  connect(pre.y, staDow.u2) annotation (Line(points={{172,60},{180,60},{180,-220},
          {-80,-220},{-80,-188},{-52,-188}}, color={255,127,0}));
  connect(pre.y, staUp.u2) annotation (Line(points={{172,60},{180,60},{180,-220},
          {-80,-220},{-80,-148},{-52,-148}}, color={255,127,0}));
  connect(staDow.y, staDowAndUna.u2)
    annotation (Line(points={{-28,-180},{-20,-180},{-20,-188},{-2,-188}},
                                                     color={255,0,255}));
  connect(staUna.y, staDowAndUna.u1) annotation (Line(points={{-128,-160},{-10,-160},
          {-10,-180},{-2,-180}},  color={255,0,255}));
  connect(idxFirAct.y, pre.u) annotation (Line(points={{172,100},{200,100},{200,
  ...
  connect(sta.active, booVecFil.u)
    annotation (Line(points={{0,129},{0,100},{20,100},{20,92}},
                                                      color={255,0,255}));
  connect(rep2.y, runAndEna.u2) annotation (Line(points={{-58,40},{0,40},{0,32},
          {28,32}}, color={255,0,255}));
  connect(booVecFil.y, runAndEna.u1)
    annotation (Line(points={{20,68},{20,40},{28,40}}, color={255,0,255}));
  connect(tim.passed, runAndEna.u1) annotation (Line(points={{-8,52},{8,52},{8,40},
          {28,40}}, color={255,0,255}));
  connect(idxFirAct.y, y)
    annotation (Line(points={{172,100},{240,100}}, color={255,127,0}));
  connect(u1Ava, idxFirAva.u1) annotation (Line(points={{-240,-160},{-210,-160},
          {-210,-120},{-202,-120}},
                                  color={255,0,255}));
  connect(u1Ava, idxLasAva.u1) annotation (Line(points={{-240,-160},{-210,-160},
          {-210,-80},{-202,-80}}, color={255,0,255}));
  ...
  connect(idxFirAct.y, higAva.u2) annotation (Line(points={{172,100},{200,100},{
          200,-200},{-120,-200},{-120,-88},{-112,-88}}, color={255,127,0}));
  connect(idxFirAct.y, lowAva.u2) annotation (Line(points={{172,100},{200,100},{
          200,-200},{-120,-200},{-120,-128},{-112,-128}}, color={255,127,0}));
  ...
  connect(rep6.y, runAndDow.u[3]) annotation (Line(points={{-58,-120},{20,-120},
          {20,-37.6667},{28,-37.6667}},
                          color={255,0,255}));
  connect(rep5.y, runAndUp.u[3]) annotation (Line(points={{-58,-80},{0,-80},{0,2.33333},
          {28,2.33333}},
                   color={255,0,255}));
  connect(booVecFil.y, runAndUp.u[1])
    annotation (Line(points={{20,68},{20,-2.33333},{28,-2.33333}},color={255,0,255}));
  connect(tim.passed, runAndUp.u[1]) annotation (Line(points={{-8,52},{8,52},
          {8,-2.33333},{28,-2.33333}}, color={255,0,255}));
  connect(tim.passed, runAndDow.u[1]) annotation (Line(points={{-8,52},{8,52},{
          8,-42.3333},{28,-42.3333}},
                                    color={255,0,255}));
  connect(rep.y, runAndUp.u[2])
    annotation (Line(points={{-58,0},{28,0},{28,0}}, color={255,0,255}));
  connect(rep1.y, runAndDow.u[2]) annotation (Line(points={{-58,-40},{28,-40},{28,
          -40}},      color={255,0,255}));
  connect(booVecFil.y, runAndDow.u[1])
    annotation (Line(points={{20,68},{20,-42.3333},{28,-42.3333}},
                              color={255,0,255}));
  connect(one.y, maxInt.u1) annotation (Line(points={{-188,-180},{-184,-180},{-184,
          -194},{-182,-194}}, color={255,127,0}));
  connect(idxFirAct.y, maxInt.u2)
    annotation (Line(points={{172,100},{200,100},{
          200,-200},{-120,-200},{-120,-220},{-184,-220},{-184,-206},{-182,-206}},
        color={255,127,0}));
  annotation (
...
FIXME: Prevent down/up transition if no lower/upper stage available.
<p>
