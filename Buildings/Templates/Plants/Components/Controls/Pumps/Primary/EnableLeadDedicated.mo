...
block EnableLeadDedicated "Plants with dedicated primary pumps"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1Req if have_req
    "HW or CHW request from lead equipment"
    annotation (Placement(transformation(
    extent={{-140,-80},{-100,-40}}), iconTransformation(extent={{-300,-56},
      {-260,-16}})));
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1EquLea
    ...
    annotation (Placement(transformation(extent={
            {-140,0},{-100,40}}), iconTransformation(extent={{-300,-56},{-260,-16}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput u1EquLea_actual
    ...
    annotation (Placement(transformation(extent={{-140,-40},
            {-100,0}}), iconTransformation(extent={{-300,-56},{-260,-16}})));
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant tru(final k=true)
    if not have_req "Placeholder constant"
    annotation (Placement(transformation(extent={{-80,-90},{-60,-70}})));
  Buildings.Controls.OBC.CDL.Logical.Or offOrNotReq if not have_req
    "Return true if lead equipment proven off OR not requesting flow"
    annotation (Placement(transformation(extent={{-10,-70},{10,-50}})));
  Buildings.Controls.OBC.CDL.Logical.Timer timOff(final t=dtOff)
    "Return true if lead equipment is proven off for specified duration"
    annotation (Placement(transformation(extent={{-50,-30},{-30,-10}})));
  Buildings.Controls.OBC.CDL.Logical.And disAndOffOrNotReq if not have_req
...
equation
  ...
  connect(u1, lat.u) annotation (Line(points={{-120,60},{60,60},{60,0},{70,0}},
        color={255,0,255}));
  connect(u1EquLea, dis.u)
    annotation (Line(points={{-120,20},{-82,20}}, color={255,0,255}));
  connect(u1EquLea_actual, off.u)
    annotation (Line(points={{-120,-20},{-82,-20}}, color={255,0,255}));
  connect(tru.y, offOrNotReq.u2) annotation (Line(points={{-58,-80},{-40,-80},{-40,
          -68},{-12,-68}}, color={255,0,255}));
  connect(u1Req, offOrNotReq.u2) annotation (Line(points={{-120,-60},{-40,-60},{
          -40,-68},{-12,-68}}, color={255,0,255}));
  connect(off.y, timOff.u)
    annotation (Line(points={{-58,-20},{-52,-20}}, color={255,0,255}));
  connect(timOff.passed, offOrNotReq.u1) annotation (Line(points={{-28,-28},{-20,
          -28},{-20,-60},{-12,-60}}, color={255,0,255}));
  connect(offOrNotReq.y, disAndOffOrNotReq.u2) annotation (Line(points={{12,-60},
          {20,-60},{20,-28},{28,-28}}, color={255,0,255}));
  connect(dis.y, disAndOffOrNotReq.u1) annotation (Line(points={{-58,20},{0,20},
          {0,-20},{28,-20}}, color={255,0,255}));
  connect(disAndOffOrNotReq.y, lat.clr) annotation (Line(points={{52,-20},{60,-20},
          {60,-6},{70,-6}}, color={255,0,255}));
  annotation (
