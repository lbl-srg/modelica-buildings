...
block EnableLeadHeadered
  ...
  parameter Buildings.Templates.Plants.Components.Controls.Types.EquipmentConnection typCon
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput uValIso[nValIso]
    ...
    annotation (Placement(transformation(extent={{-140,-60},{-100,-20}}),
      iconTransformation(extent={{-244,-92},{-204,-52}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput y1
    "Lead pump enable signal"
    annotation (Placement(transformation(extent={{100,-20},{140,20}}),
      iconTransformation(extent={{-300,-56},{-260,-16}})));
  Buildings.Controls.OBC.CDL.Logical.MultiOr anyOpe(
    nin=nValIso)
  ...
  Buildings.Controls.OBC.CDL.Logical.MultiAnd allClo(
    nin=nValIso)
    if typCon == Buildings.Templates.Plants.Components.Controls.Types.EquipmentConnection.Parallel
  ...
  Buildings.Controls.OBC.CDL.Reals.GreaterThreshold ope[nValIso](
    each final t=0)
    if typValIso == Buildings.Templates.Plants.Components.Controls.Types.Actuator.Modulating
  ...
  Buildings.Controls.OBC.CDL.Logical.MultiOr anyClo(
    nin=nValIso)
  ...
  Buildings.Controls.OBC.CDL.Logical.MultiAnd allOpe(
    nin=nValIso)
    if typCon == Buildings.Templates.Plants.Components.Controls.Types.EquipmentConnection.Series
...
equation
  ...
  connect(u1ValIso, clo.u)
    annotation (Line(points={{-120,0},{-40,0},{-40,-40},{-32,-40}},color={255,0,255}));
  ...
  connect(allClo.y, lat.clr)
    annotation (Line(points={{32,-40},{50,-40},{50,-6},{68,-6}},color={255,0,255}));
  connect(clo.y, anyClo.u)
    annotation (Line(points={{-8,-40},{0,-40},{0,80},{8,80}},color={255,0,255}));
  connect(u1ValIso, allOpe.u)
    annotation (Line(points={{-120,0},{-40,0},{-40,40},{8,40}},color={255,0,255}));
  connect(allOpe.y, lat.clr)
    annotation (Line(points={{32,40},{50,40},{50,-6},{68,-6}},color={255,0,255}));
  connect(anyClo.y, lat.u)
