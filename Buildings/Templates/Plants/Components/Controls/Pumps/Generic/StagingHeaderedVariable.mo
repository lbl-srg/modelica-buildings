...
block StagingHeaderedVariable "Staging logic for headered variable speed pumps"

  parameter Integer nPum(final min=1)
    "Number of primary pumps that operate at design conditions"
    annotation(Evaluate=true);
  parameter Real VPri_flow_nominal(final min=1E-6, final unit="m3/s")
  ...
  parameter Real dVOffUp(final min=0, final max=1, final unit="1")=0.03
    "Stage up flow point offset"
    annotation(Evaluate=true);
  parameter Real dVOffDow(final min=0, final max=1, final unit="1")=dVOffUp
  ...
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter norV(final k=1/
        VPri_flow_nominal) "Normalize to design value"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput y1Dow
    "Stage down command" annotation (Placement(transformation(extent={{120,-60},
            {160,-20}}), iconTransformation(extent={{100,-60},{140,-20}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput y1_actual[nPum]
    "Pump status" annotation (Placement(transformation(extent={{-160,80},{-120,
            120}}), iconTransformation(extent={{-140,20},{-100,60}})));
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter norN(final k=1/nPum)
                           "Normalize to design value"
    annotation (Placement(transformation(extent={{-60,30},{-40,50}})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToRea[nPum]
    "Convert to real value"
    annotation (Placement(transformation(extent={{-90,90},{-70,110}})));
  Buildings.Controls.OBC.CDL.Reals.Greater gre "Compare to stage up flow point"
    annotation (Placement(transformation(extent={{30,-10},{50,10}})));
  Buildings.Controls.OBC.CDL.Reals.Less les "Compare to stage down flow point"
    annotation (Placement(transformation(extent={{30,-50},{50,-30}})));
  Buildings.Controls.OBC.CDL.Reals.AddParameter poiDow(p=-1/nPum - dVOffDow)
    "Calculate stage down flow point "
    annotation (Placement(transformation(extent={{-20,10},{0,30}})));
  Buildings.Controls.OBC.CDL.Reals.AddParameter poiUp(p=-dVOffUp)
  ...
  Buildings.Controls.OBC.CDL.Reals.MultiSum nOpe(nin=nPum)
    "Return number of operating pumps"
    annotation (Placement(transformation(extent={{-60,90},{-40,110}})));
  Buildings.Controls.OBC.CDL.Logical.MultiOr anyCha(nin=nPum)
    "Return true when any pump status changes"
    annotation (Placement(transformation(extent={{-50,-70},{-30,-50}})));
equation
  connect(norN.y, poiDow.u) annotation (Line(points={{-38,40},{-30,40},{-30,20},
          {-22,20}},color={0,0,127}));
  connect(norN.y, poiUp.u) annotation (Line(points={{-38,40},{-30,40},{-30,60},{
          -22,60}},color={0,0,127}));
  connect(norV.y, gre.u1)
    annotation (Line(points={{-38,0},{28,0}}, color={0,0,127}));
  connect(poiUp.y, gre.u2) annotation (Line(points={{2,60},{20,60},{20,-8},{28,
          -8}},
        color={0,0,127}));
  connect(poiDow.y, les.u2) annotation (Line(points={{2,20},{10,20},{10,-48},{
          28,-48}},
                 color={0,0,127}));
  connect(norV.y, les.u1) annotation (Line(points={{-38,0},{0,0},{0,-40},{28,
          -40}},
        color={0,0,127}));
  connect(les.y, runDow.u) annotation (Line(points={{52,-40},{68,-40}},
                    color={255,0,255}));
  ...
  connect(runUp.passed, y1Up) annotation (Line(points={{92,-8},{100,-8},{100,40},
          {140,40}}, color={255,0,255}));
  connect(runDow.passed, y1Dow) annotation (Line(points={{92,-48},{100,-48},{
          100,-40},{140,-40}},
                           color={255,0,255}));
  ...
  connect(y1_actual, cha.u) annotation (Line(points={{-140,100},{-100,100},{
          -100,-60},{-92,-60}}, color={255,0,255}));
  connect(nOpe.y, norN.u) annotation (Line(points={{-38,100},{-30,100},{-30,70},
          {-70,70},{-70,40},{-62,40}}, color={0,0,127}));
  connect(cha.y, anyCha.u)
    annotation (Line(points={{-68,-60},{-52,-60}}, color={255,0,255}));
  connect(anyCha.y, runUp.reset) annotation (Line(points={{-28,-60},{60,-60},{
          60,-8},{68,-8}}, color={255,0,255}));
  connect(anyCha.y, runDow.reset) annotation (Line(points={{-28,-60},{60,-60},{
