...
model StagingHeaderedVariable "Validation model"
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.TimeTable ratFlo(
    table=[0,0; 1,1; 1.5,1; 2,0], timeScale=3600)
    "Primary flow ratio to design value"
    annotation (Placement(transformation(extent={{-80,-10},{-60,10}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable enaLea(table=[0,1; 8000,
        0], period=8400)
    "Lead pump enable signal"
    annotation (Placement(transformation(extent={{-80,30},{-60,50}})));
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter VPri_flow(final k=
        VPri_flow_nominal) "Primary flow rate"
    annotation (Placement(transformation(extent={{-40,-10},{-20,10}})));
  Buildings.Templates.Plants.Components.Controls.Pumps.Generic.StagingHeaderedVariable
    sta(final nPum=nPum, final VPri_flow_nominal=VPri_flow_nominal)
  ...
  Buildings.Controls.OBC.CDL.Integers.GreaterEqualThreshold staPum[nPum](final
      t={i for i in 1:nPum}) "Evaluate pump status"
    annotation (Placement(transformation(extent={{20,50},{0,70}})));
  Buildings.Controls.OBC.CDL.Routing.IntegerScalarReplicator rep(nout=nPum)
    "Replicate signal"
...
equation
  ...
  connect(sta.y1Dow, idxSta.u1Dow) annotation (Line(points={{22,-4},{40,-4},{40,
          -2},{48,-2}}, color={255,0,255}));
  connect(ava.y, idxSta.u1Ava) annotation (Line(points={{-58,-40},{40,-40},{40,-6},
          {48,-6}}, color={255,0,255}));
  ...
  connect(staPum.y, sta.y1_actual) annotation (Line(points={{-2,60},{-10,60},{
          -10,4},{-2,4}},
                      color={255,0,255}));
  connect(enaLea.y[1], idxSta.u1Lea) annotation (Line(points={{-58,40},{40,40},
          {40,6},{48,6}}, color={255,0,255}));
  annotation (
  __Dymola_Commands(
  file=
  ...
  "Simulate and plot"),
  experiment(StopTime=8400.0,Tolerance=1e-06),
    Documentation(
      info="<html>
<p>
...
First implementation.
    ...
    Diagram(graphics={Polygon(points={{214,66},{214,66}}, lineColor={28,108,200})}));
