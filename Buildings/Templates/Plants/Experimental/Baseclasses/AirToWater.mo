...
block AirToWater "Controller for AWHP plant"
  ...
  Controls.Enabling.Enable enaHea(
    ...
    final sch=schHea) if have_heaWat "Heating mode enable"
  ...
  Controls.StagingRotation.StageAvailability avaStaHea(final nSta=nSta, final
  ...
  Controls.StagingRotation.EventSequencing seqEve[nHpTot](
    ...
    each final dtOff=dtOffHp) "Event sequencing"
    annotation (Placement(transformation(extent={{140,284},{160,312}})));
  Controls.StagingRotation.StageAvailability avaStaCoo(final nSta=nSta, final
      nEqu=nHpTot) if have_chiWat "Evaluate cooling stage availability"
  ...
  Controls.StagingRotation.SortRuntime sorRunTimHea(final idxEquAlt=idxEquAlt,
  ...
  Controls.Enabling.Enable enaCoo(
    ...
    final sch=schCoo) if have_chiWat "Cooling mode enable"
  ...
  Controls.StagingRotation.EquipmentAvailability avaEquHeaCoo[nHpTot](
    each final have_heaWat=have_heaWat,
    each final have_chiWat=have_chiWat,
  ...
  Controls.Pumps.Generic.StagingHeadered staPumHeaWatPri(
    ...
    final yUp=yUpPumSta,
    final yDow=yDowPumSta) if have_pumHeaWatPri "Primary HW pump staging"
  ...
  Controls.Pumps.Generic.StagingHeadered staPumChiWatSec(
    ...
    final dpOff=dpOffPumSta,
    final yUp=yUpPumSta,
    final yDow=yDowPumSta) if have_pumChiWatSec "Secondary CHW pump staging"
  ...
  Controls.Pumps.Generic.StagingHeadered staPumHeaWatSec(
    ...
    final dpOff=dpOffPumSta,
    final yUp=yUpPumSta,
    final yDow=yDowPumSta) if have_pumHeaWatSec "Secondary HW pump staging"
    annotation (Placement(transformation(extent={{140,150},{160,170}})));
  Controls.Utilities.PlaceholderReal THeaWatRet(final have_inp=
        have_senTHeaWatPriRet, final have_inpPh=true) if have_heaWat
    "Select HW return temperature sensor"
    annotation (Placement(transformation(extent={{-230,30},{-210,50}})));
  Controls.Utilities.PlaceholderReal VHeaWatSta_flow(final have_inp=
        have_senVHeaWatPri, final have_inpPh=true) if have_heaWat
  ...
  Controls.Utilities.PlaceholderReal TChiWatRet(final have_inp=
        have_senTChiWatPriRet, final have_inpPh=true) if have_chiWat
    "Select CHW return temperature sensor"
    annotation (Placement(transformation(extent={{-230,-30},{-210,-10}})));
  Controls.Utilities.PlaceholderReal VChiWatSta_flow(final have_inp=
        have_senVChiWatPri, final have_inpPh=true) if have_chiWat
    "For staging logic select primary flow sensor if both primary and secondary sensors available"
  ...
  Controls.StagingRotation.StageCompletion comStaCoo(final nin=nHpTot)
    if have_chiWat "Check successful completion of cooling stage change"
    annotation (Placement(transformation(extent={{-40,-10},{-20,10}})));
  Controls.StagingRotation.StageCompletion comStaHea(final nin=nHpTot)
    if have_heaWat "Check successful completion of heating stage change"
  ...
  Controls.Pumps.Primary.VariableSpeed ctlPumPri(
  ...
  Controls.HeatRecoveryChillers.Controller hrc(
  ...
  Controls.MinimumFlow.ControllerDualMode ctlFloMin(
    final have_chiWat=have_chiWat,
    final have_heaWat=have_heaWat,
  ...
  Controls.Utilities.PlaceholderReal VHeaWatLoa_flow(final have_inp=is_priOnl,
  ...
  Controls.Utilities.PlaceholderReal VChiWatLoa_flow(final have_inp=is_priOnl,
  ...
  Controls.StagingRotation.HybridOperation ctlPlaHyb(
    final have_heaWat=have_heaWat,
    final have_chiWat=have_chiWat,
    final nHp=nHp,
...
equation
  ...
  connect(sorRunTimCoo.yIdx, enaEquCoo.uIdxAltSor)
    annotation (Line(points={{-18,24},{24,24},{24,106},{38,106}},color={255,127,0}));
  ...
  connect(comStaHea.y1, chaStaHea.u1StaPro)
    annotation (Line(points={{-18,250},{-10,250},{-10,306},{-46,306},{-46,322},{
          -42,322}},
      color={255,0,255}));
...
if have_HpShc then
  ...
  connect(ctlPlaHyb.y1HeaCoo, chaStaCoo.u1HeaCoo) annotation (Line(points={{82,-100},
          {104,-100},{104,-12},{72,-12},{72,84},{-52,84},{-52,66},{-42,66}},
        color={255,0,255}));
