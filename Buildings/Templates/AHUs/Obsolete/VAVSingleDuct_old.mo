within Buildings.Templates.AHUs.Obsolete;
model VAVSingleDuct_old "VAV single duct with relief"
  ...
  ***/
  ...
  inner replaceable Buildings.Templates.BaseClasses.Dampers.None damOut
    constrainedby Buildings.Templates.Interfaces.Damper(redeclare final package
              Medium =
               MediumAir) "Outdoor air damper" annotation (
    choices(
  ...
  replaceable Buildings.Templates.BaseClasses.Sensors.None VSup_flow
    constrainedby Buildings.Templates.Interfaces.Sensor(redeclare final package
              Medium =
               MediumAir) "Supply air volume flow rate sensor" annotation (
  ...
  replaceable Buildings.Templates.BaseClasses.Sensors.None pSup_rel
    constrainedby Buildings.Templates.Interfaces.Sensor(redeclare final package
              Medium =
               MediumAir) "Duct static pressure sensor" annotation (
  ...
  replaceable Buildings.Templates.BaseClasses.Sensors.None pRet_rel
    constrainedby Buildings.Templates.Interfaces.Sensor(redeclare final package
              Medium =
               MediumAir) "Return static pressure sensor" annotation (
...
equation
  ...
  connect(port_Out, dpOutMin.port_a) annotation (Line(points={{-300,-200},{-280,
          -200},{-280,-140},{-270,-140}}, color={0,127,255}));
  connect(dpOutMin.port_b, damOutMin.port_a)
    annotation (Line(points={{-250,-140},{-242,-140}}, color={0,127,255}));
  connect(VOutMin_flow.port_b, TMix.port_a) annotation (Line(points={{-162,-140},
          {-140,-140},{-140,-200},{-100,-200}}, color={0,127,255}));
  connect(dpOutMin.port_bRef, TMix.port_a) annotation (Line(points={{-260,-150},
          {-260,-160},{-140,-160},{-140,-200},{-100,-200}}, color={0,127,255}));
  ...
  connect(busAHU, damRel.busCon) annotation (Line(
      ...
      color={255,204,51},
      thickness=0.5));
  connect(busAHU, dpOutMin.busCon) annotation (Line(
  ...
  connect(damRet.busCon, busAHU) annotation (Line(
      ...
      color={255,204,51},
      thickness=0.5));
  connect(VOutMin_flow.busCon, busAHU) annotation (Line(
      points={{-172,-130},{-172,0},{-300,0}},
      color={255,204,51},
      thickness=0.5));
  connect(TOutMin.busCon, busAHU) annotation (Line(
