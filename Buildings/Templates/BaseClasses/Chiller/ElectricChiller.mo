...
model ElectricChiller
  extends Buildings.Templates.Interfaces.Chiller(
    ...
    each final C2_nominal=C2_nominal) "Chiller"
    annotation (Placement(transformation(extent={{-10,-8},{10,12}})));
  inner replaceable Valve.None val_1 constrainedby Valve.None annotation (
      Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-20,30})));
  inner replaceable Pump.None pum_1 constrainedby Pump.None annotation (
      Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=270,
        origin={20,30})));
  inner replaceable Pump.None pum_2 constrainedby Pump.None annotation (
      Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-20,-30})));
  inner replaceable Valve.None val_2 constrainedby Valve.None annotation (
      Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=270,
        origin={20,-30})));
equation
  ...
  connect(busCon.val_1, val_1.busCon) annotation (Line(
      points={{0,100},{0,80},{-60,80},{-60,30},{-30,30}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(busCon.out.on, chi.on) annotation (Line(
      points={{0.1,100.1},{0.1,16},{0,16},{0,80},{-60,80},{-60,5},{-12,5}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(busCon.inp.P, chi.P) annotation (Line(
      points={{0.1,100.1},{0.1,80},{60,80},{60,11},{11,11}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
