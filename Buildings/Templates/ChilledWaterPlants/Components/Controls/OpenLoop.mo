...
block OpenLoop "Open loop controller (output signals only)"
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.TimeTable y1ValChiWatMinByp(
    table=[0,0; 1,0; 1,1; 2,1],
    timeScale=1000,
    period=2000) "CHW minimum flow bypass valve opening signal"
...
protected
  ...
  Buildings.Templates.Components.Interfaces.Bus busPumConWat
    "CW pumps control bus" annotation (Placement(transformation(extent={{120,
            -200},{160,-160}}),
                         iconTransformation(extent={{-316,184},{-276,224}})));
  ...
  Buildings.Templates.Components.Interfaces.Bus busPumChiWatSec
    ...
    annotation (Placement(transformation(
          extent={{120,-140},{160,-100}}), iconTransformation(extent={{-316,184},{-276,
            224}})));
...
equation
  connect(yPumChiWatPri.y, busPumChiWatPri.y)
    annotation (Line(points={{-108,-80},{140,-80}},
                                                  color={0,0,127}));
  connect(busPumChiWatPri, bus.pumChiWatPri) annotation (Line(
      points={{140,-80},{160,-80},{160,0},{220,0}},
      color={255,204,51},
      thickness=0.5));
  connect(yPumChiWatSec.y, busPumChiWatSec.y)
    annotation (Line(points={{-108,-140},{140,-140},{140,-120}},
                                                          color={0,0,127}));
  connect(busPumChiWatSec, bus.pumChiWatSec) annotation (Line(
      points={{140,-120},{180,-120},{180,0},{220,0}},
      color={255,204,51},
      thickness=0.5));
  connect(yPumConWat.y, busPumConWat.y)
    annotation (Line(points={{-108,-200},{140,-200},{140,-180}},
                                                             color={0,0,127}));
  ...
  connect(yPumConWatDed.y, busPumConWat.y)
    annotation (Line(points={{-78,-220},{140,-220},{140,-180}},
                                                            color={0,0,127}));
  ...
  connect(y1PumChiWatSec.y[1], busPumChiWatSec.y1)
    annotation (Line(points={{-138,-120},{140,-120}},
                                                    color={255,0,255}));
  ...
  connect(y1PumChiWatPri.y[1], busPumChiWatPri.y1) annotation (Line(points={{-138,
          -60},{140,-60},{140,-80}}, color={255,0,255}));
  connect(y1PumConWat.y[1], busPumConWat.y1) annotation (Line(points={{-138,
          -180},{140,-180}},             color={255,0,255}));
  connect(y1ValChiWatMinByp.y[1], busValChiWatMinByp.y1) annotation (Line(
        points={{-138,-20},{140,-20}}, color={255,0,255}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(busValChiWatMinByp, bus.valChiWatMinByp) annotation (Line(
      points={{140,-20},{170,-20},{170,0},{220,0}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%second",
      index=-1,
