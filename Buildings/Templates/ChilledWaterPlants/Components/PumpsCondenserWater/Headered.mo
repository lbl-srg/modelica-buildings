within Buildings.Templates.ChilledWaterPlants.Components.PumpsCondenserWater;
model Headered "Headered condenser pumps"
  ...
  inner replaceable Buildings.Templates.Components.Valves.TwoWayTwoPosition valConWatChiIso[nChi]
    constrainedby
    Buildings.Templates.Components.Valves.Interfaces.PartialValve(
      redeclare each final package Medium = Medium,
      final dat=dat.valConWatChiIso)
...
equation
  ...
  connect(del.ports[1], pum.port_b)
    ...
    color={0,127,255}));
  connect(valConWatChiIso.port_a, del.ports[2:nChi+1])
  ...
  connect(valConWatChiIso.port_b, ports_b)
    annotation (Line(points={{80,0},{100,0}}, color={0,127,255}));
  connect(bus.valConWatChiIso, valConWatChiIso.bus) annotation (Line(
