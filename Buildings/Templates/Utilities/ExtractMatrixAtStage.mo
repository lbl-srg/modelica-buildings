...
block ExtractMatrixAtStage
  ...
  Controls.OBC.CDL.Integers.Multiply mulInt "(u-1)*nSta*nEqu"
  ...
  Controls.OBC.CDL.Integers.Sources.Constant offCst[nOut](
    final k=if is_transpose then {(mod(p-1, nSta))*nEqu + (div(p-1, nSta) + 1) for p in 1:nOut}
  ...
  Controls.OBC.CDL.Routing.RealExtractSignal extReaSig[if is_transpose then nEqu else nSta](
    each final nin=nOut,
    each final nout=if is_transpose then nSta else nEqu,
    final extract=if is_transpose then {(i - 1)*nSta + j for j in 1:nSta, i in 1:nEqu}
...
equation
  ...
  connect(extIndRea.y, reaVecRep.u) annotation (Line(points={{52,70},{68,70}}, color={0,0,127}));
  connect(reaVecRep.y, extReaSig.u) annotation (Line(points={{92,70},{118,70}}, color={0,0,127}));
  connect(extReaSig.y, y) annotation (Line(points={{142,70},{124,70},{124,0},{120,0}}, color={0,0,127}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)));
end ExtractMatrixAtStage;
