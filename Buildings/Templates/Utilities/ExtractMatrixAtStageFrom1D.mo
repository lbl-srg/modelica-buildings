...
block ExtractMatrixAtStageFrom1D
  parameter Real sta[:]
  ...
  Controls.OBC.CDL.Routing.RealExtractSignal extReaSig[nSta - 1](
    each final nin=nOut,
    each final nout=nEqu,
    final extract={{(j - 1)*nEqu + i for i in 1:nEqu} for j in 1:nSta - 1})
    annotation (Placement(transformation(extent={{120,60},{140,80}})));
  Controls.OBC.CDL.Interfaces.RealOutput y[nSta - 1,nEqu] annotation (Placement(
        transformation(extent={{100,-20},{140,20}}), iconTransformation(extent={
            {100,-20},{140,20}})));
  Controls.OBC.CDL.Routing.RealVectorReplicator reaVecRep(nin=nOut, nout=nSta - 1)
...
end ExtractMatrixAtStageFrom1D;
