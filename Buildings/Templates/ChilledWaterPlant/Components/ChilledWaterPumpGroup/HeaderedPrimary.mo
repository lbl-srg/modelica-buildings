within Buildings.Templates.ChilledWaterPlant.Components.ChilledWaterPumpGroup;
model HeaderedPrimary
  extends Buildings.Templates.ChilledWaterPlant.Components.ChilledWaterPumpGroup.Interfaces.ChilledWaterPumpGroup(
    final typ=Types.ChilledWaterPumpGroup.HeaderedPrimary,
    final has_secPum = false);
  ...
  Fluid.FixedResistances.Junction splByp(redeclare package Medium = Medium,
    ...
    "Common leg or bypass splitter" annotation (Placement(transformation(
        ...
        rotation=0,
        origin={40,0})));
  Fluid.Actuators.Valves.TwoWayLinear valByp if has_byp "Bypass valve"
                                                            annotation (
...
equation
  ...
  connect(splByp.port_3, valByp.port_a) annotation (Line(points={{40,-10},{40,-30},
          {1.77636e-15,-30},{1.77636e-15,-50}}, color={0,127,255}));
  ...
  connect(busCon.out.yValWSE, valWSE.y) annotation (Line(
      points={{0,100},{0,80},{-20,80},{-20,-40},{-70,-40},{-70,-48}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
