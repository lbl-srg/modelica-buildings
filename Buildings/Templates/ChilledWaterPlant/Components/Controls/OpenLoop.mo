...
equation
  ...
  connect(busCon.valPriMinFloByp.y, yValPriMinFloByp.y);
  connect(busCon.valChiWatChiByp.y1, yValChiWatChiByp.y);

  connect(busCon.chi.TChiWatChiSupSet, chiTSet.y);
  connect(busCon.chi.y1, chiOn.y);
  connect(busCon.valChiWatChiIso.y, yValChiWatChiIso.y);
  connect(busCon.valConWatChiIso.y1, yValConWatChiIso.y);
