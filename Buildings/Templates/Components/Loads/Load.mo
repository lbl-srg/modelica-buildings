...
model Load "Model of a load on a hydronic circuit"
  ...
  parameter Buildings.Fluid.HydronicConfigurations.Types.Control typ
    ...
    annotation (Evaluate=true, choices(
      choice=Buildings.Fluid.HydronicConfigurations.Types.Control.Heating "Heating",
      choice=Buildings.Fluid.HydronicConfigurations.Types.Control.Cooling "Cooling"));
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yVal(final unit="1")
    "Valve demand signal" annotation (Placement(transformation(extent={{100,60},
            {140,100}}),iconTransformation(extent={{100,60},{140,100}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput dTLiq(final unit="K",
      displayUnit="K") "Liquid deltaT" annotation (Placement(transformation(
          extent={{100,-60},{140,-20}}), iconTransformation(extent={{100,-90},{
            140,-50}})));
  ...
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter modMAir(k=
        mAir_flow_nominal) "Scale with design flow"
                                                  annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
...
initial equation
  assert(typ<>Buildings.Fluid.HydronicConfigurations.Types.Control.None
    and typ<>Buildings.Fluid.HydronicConfigurations.Types.Control.ChangeOver,
    "In " + getInstanceName() +
    ": The type of built-in controls cannot be None or Change-over: select any other valid option.");
equation
  connect(souAir.ports[1], coi.port_a2) annotation (Line(points={{20,20},{16,20},
          {16,12},{10,12}}, color={0,127,255}));
  connect(outAir.ports[1],TAirLvg. port_b) annotation (Line(points={{-70,20},{-40,
          20}},                   color={0,127,255}));
  connect(TAirLvg.port_a, coi.port_b2) annotation (Line(points={{-20,20},{-16,20},
          {-16,12},{-10,12}}, color={0,127,255}));
  ...
  connect(TLiqLvg.T, dT.u1)
    annotation (Line(points={{60,-11},{60,-34},{68,-34}}, color={0,0,127}));
  connect(TLiqEnt.T, dT.u2) annotation (Line(points={{-80,-11},{-80,-20},{58,-20},
          {58,-46},{68,-46}}, color={0,0,127}));
  connect(heaFlo.y, Q_flow)
    annotation (Line(points={{71,-80},{120,-80}}, color={0,0,127}));
  connect(yLoa_actual, loaFra.y)
    annotation (Line(points={{120,40},{71,40}}, color={0,0,127}));
  connect(u, conPID.u_s)
    annotation (Line(points={{-120,80},{-12,80}},color={0,0,127}));
  connect(loaFra.y, conPID.u_m) annotation (Line(points={{71,40},{80,40},{80,60},
          {0,60},{0,68}},   color={0,0,127}));
  connect(u, max1.u1) annotation (Line(points={{-120,80},{-60,80},{-60,-74},{-52,
          -74}}, color={0,0,127}));
  connect(uMin.y, max1.u2) annotation (Line(points={{-68,-80},{-60,-80},{-60,-86},
          {-52,-86}},  color={0,0,127}));
  connect(mul.y, modMAir.u)
    annotation (Line(points={{12,-60},{18,-60}},            color={0,0,127}));
  connect(max1.y, mul.u2) annotation (Line(points={{-28,-80},{-20,-80},{-20,-66},
          {-12,-66}}, color={0,0,127}));
  connect(enaRea.y, mul.u1) annotation (Line(points={{-28,-40},{-20,-40},{-20,-54},
          {-12,-54}},color={0,0,127}));
  connect(modMAir.y, souAir.m_flow_in)
    annotation (Line(points={{42,-60},{46,-60},{46,12},{42,12}},
                                                      color={0,0,127}));
  connect(u1, conPID.uEna)
    annotation (Line(points={{-120,40},{-4,40},{-4,68}}, color={255,0,255}));
  connect(u1, enaRea.u) annotation (Line(points={{-120,40},{-56,40},{-56,-40},{-52,
          -40}}, color={255,0,255}));
  annotation (
