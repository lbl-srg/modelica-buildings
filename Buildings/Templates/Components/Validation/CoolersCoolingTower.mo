...
model CoolersCoolingTower
  ...
  Fluid.Sensors.TemperatureTwoPort TConWatSup(
    ...
    final m_flow_nominal=dat.mConWat_flow_nominal)
...
equation
  ...
  connect(y1.y[1], y.u)
    annotation(Line(points={{-58,60},{-42,60}},
      color={255,0,255}));
  connect(y.y, bus.y)
    annotation(Line(points={{-18,60},{0,60},{0,20}},
      color={0,0,127}));
  connect(coo.port_b, TConWatSup.port_a)
    annotation(Line(points={{10,-20},{30,-20}},
      color={0,127,255}));
  connect(TConWatSup.port_b, supConWat.ports[1])
    annotation(Line(points={{50,-20},{60,-20}},
      color={0,127,255}));
  connect(TWetBul.y, busWea.TWetBul)
    annotation(Line(points={{-58,20},{-48,20},{-48,20.1},{-39.9,20.1}},
      color={0,0,127}));
  connect(busWea, coo.busWea)
    annotation(Line(points={{-40,20},{-40,0},{-6,0},{-6,-10}},
      color={255,204,51},
      thickness=0.5));
  connect(bus, coo.bus)
    annotation(Line(points={{0,20},{0,-10}},
      color={255,204,51},
      thickness=0.5));
...
annotation(Diagram(coordinateSystem(extent={{-100,-100},{100,100}},
  grid={2,2})),
  __Dymola_Commands(
    file="modelica://Buildings/Resources/Scripts/Dymola/Templates/Components/Validation/CoolersCoolingTower.mos"
      "Simulate and plot"),
...
<p>
  ...
  <a href=\"modelica://Buildings.Templates.Components.Coolers.CoolingTower\">
    Buildings.Templates.Components.Coolers.CoolingTower</a> at nominal
  conditions.
</p>
</html>",
    revisions="<html>
