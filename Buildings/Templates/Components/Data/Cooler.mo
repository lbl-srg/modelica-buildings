...
record Cooler
  "Record for condenser water cooling equipment"
  ...
  parameter Buildings.Templates.Components.Types.Cooler typ
    ...
    annotation(Evaluate=true,
      Dialog(group="Configuration",
        enable=false));
  parameter Modelica.Units.SI.MassFlowRate mConWat_flow_nominal(final min=0)
    "CW mass flow rate"
    annotation(Dialog(group="Nominal condition"));
  parameter Modelica.Units.SI.PressureDifference dpConWatFri_nominal(
    final min=0,
    start=if typ == Buildings.Templates.Components.Types.Cooler.CoolingTowerOpen
  ...
  parameter Modelica.Units.SI.Temperature TAirEnt_nominal(
    ...
    annotation(Dialog(group="Nominal condition",
      enable=typ == Buildings.Templates.Components.Types.Cooler.DryCooler));
  ...
  parameter Modelica.Units.SI.Power PFan_nominal(
    final min=0,
    start=Buildings.Templates.Data.Defaults.ratPFanByMFloConWatTow *
