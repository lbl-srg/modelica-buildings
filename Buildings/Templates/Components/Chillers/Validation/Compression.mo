...
model Compression "Validation model for compression chiller component"
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant TChiWatSupSet(k=
        datChiAirCoo.TChiWatSup_nominal, y(final unit="K", displayUnit="degC"))
    "CHWST setpoint" annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-110,80})));
  Buildings.Templates.Components.Chillers.Compression chiAirCoo(
    ...
    final energyDynamics=energyDynamics) "Air-cooled chiller"
  ...
  Fluid.Sensors.TemperatureTwoPort TSup(redeclare final package Medium =
        MediumLiq, final m_flow_nominal=datChiAirCoo.mChiWat_flow_nominal)
    "Supply temperature"
  ...
  Fluid.Sources.Boundary_pT retChiWat(
    ...
    use_T_in=true,
    nPorts=1) "Boundary conditions of CHW at chiller inlet"
    annotation (Placement(transformation(extent={{-70,10},{-50,30}})));
  Fluid.Sensors.TemperatureTwoPort TRet(redeclare final package Medium =
        MediumLiq, final m_flow_nominal=datChiAirCoo.mChiWat_flow_nominal)
    "Return temperature"
  ...
  Fluid.Sources.Boundary_pT supConWat(
    ...
    nPorts=1) "Boundary conditions of CW at chiller inlet"
    annotation (Placement(transformation(extent={{130,-70},{110,-50}})));
  Fluid.Sources.Boundary_pT retConWat(
    redeclare final package Medium = MediumLiq,
    p=Buildings.Templates.Data.Defaults.pChiWat_rel_nominal + 101325,
    nPorts=1) "Boundary condition of CW at chiller outlet"
  ...
  Fluid.Sources.Boundary_pT sinAir(redeclare final package Medium = MediumAir,
      nPorts=1) "Air sink"
    annotation (Placement(transformation(extent={{-70,50},{-50,70}})));
  Fluid.Sources.MassFlowSource_T           souAir(redeclare final package
      Medium = MediumAir,
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant TChiWatSupSet1(k=
        datChiWatCoo.TChiWatSup_nominal, y(final unit="K", displayUnit="degC"))
    "CHWST setpoint" annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-110,-40})));
protected
  Interfaces.Bus bus "Chiller control bus"
    annotation (Placement(transformation(extent={{-20,60},{20,100}}),
      iconTransformation(extent={{-276,6},{-236,46}})));
  Interfaces.Bus bus2 "Chiller control bus"
    annotation (Placement(transformation(extent={{-20,-60},{20,-20}}),
      iconTransformation(extent={{-276,6},{-236,46}})));
initial equation
  Modelica.Utilities.Streams.print("Coef at rating conditions = " +
  String(
     Buildings.Utilities.Math.Functions.biquadratic(a=datChiWatCoo.per.capFunT,
...
equation
  connect(TSup.port_b, supChiWat.ports[1])
    annotation (Line(points={{70,20},{110,20},{110,19}},
                                                       color={0,127,255}));
  connect(y1.y[1], bus.y1)
    annotation (Line(points={{-98,120},{0,120},{0,80}},
      color={255,0,255}));
  connect(bus, chiAirCoo.bus) annotation (Line(
      points={{0,80},{30,80},{30,36}},
      color={255,204,51},
      thickness=0.5));
  connect(retChiWat.ports[1], TRet.port_a) annotation (Line(points={{-50,20},{
          -40,20},{-40,20},{-30,20}},
                                color={0,127,255}));
  connect(TSup2.port_b, supChiWat.ports[2]) annotation (Line(points={{70,-100},
          {100,-100},{100,20},{110,20},{110,21}},
                                              color={0,127,255}));
  connect(bus2, chiWatCoo.bus) annotation (Line(
      points={{0,-40},{30,-40},{30,-84}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(supConWat.ports[1], resConWat.port_a) annotation (Line(points={{110,-60},
          {90,-60}},                      color={0,127,255}));
  ...
  connect(resConWat.port_b, chiWatCoo.port_a1) annotation (Line(points={{70,-60},
          {40,-60},{40,-88}},          color={0,127,255}));
  connect(chiWatCoo.port_b1, retConWat.ports[1]) annotation (Line(points={{20,-88},
          {20,-60},{-50,-60}},  color={0,127,255}));
  connect(TChiWatRet.y, retChiWat.T_in) annotation (Line(points={{-98,24},{-72,
          24}},                  color={0,0,127}));
  connect(TChiWatSupSet.y, bus.TSupSet)
    annotation (Line(points={{-98,80},{0,80}},          color={0,0,127}));
  connect(sinAir.ports[1], chiAirCoo.port_b1) annotation (Line(points={{-50,60},
          {20,60},{20,32}},            color={0,127,255}));
  connect(souAir.ports[1], chiAirCoo.port_a1) annotation (Line(points={{110,60},
          {40,60},{40,32}},        color={0,127,255}));
