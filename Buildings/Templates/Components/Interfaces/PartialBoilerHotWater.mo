...
partial model PartialBoilerHotWater
  "Interface class for hot water boiler models"
  ...
  Buildings.Controls.OBC.Utilities.PIDWithEnable ctlPID
...
initial equation
  ...
  then
    assert(
      mHeaWat_flow_nominal <= dat.per.m_flow_nominal,
      "In " + getInstanceName() + ": " +
        "The design HW flow rate of the boiler model (" + String(
          mHeaWat_flow_nominal) +
        " kg/s) should be lower than the value from " +
        "the performance data record (" + String(dat.per.m_flow_nominal) +
        " kg/s)",
      level=AssertionLevel.warning);
    assert(
      dpHeaWat_nominal >= dat.per.dp_nominal,
      "In " + getInstanceName() + ": " +
        "The design HW pressure drop of the boiler model (" + String(
          dpHeaWat_nominal) + " Pa) should be higher than the value from " +
        "the performance data record (" + String(dat.per.dp_nominal) + " Pa)",
      level=AssertionLevel.warning);
    assert(
      cap_nominal <= dat.per.Q_flow_nominal,
      "In " + getInstanceName() + ": " +
        "The design capacity of the boiler model (" + String(cap_nominal) +
...
equation
  ...
  connect(boi.port_b, port_b)
    annotation(Line(points={{10,-60},{80,-60},{80,0},{100,0}},
      color={0,127,255}));
  connect(boi.T, bus.THeaWatSup)
    annotation(Line(points={{11,-52},{40,-52},{40,98},{0,98},{0,100}},
      color={0,0,127}));
  connect(ctlPID.y, boi.y)
    annotation(Line(points={{-28,0},{-20,0},{-20,-52},{-12,-52}},
      color={0,0,127}));
  connect(boi.T, ctlPID.u_m)
    annotation(Line(points={{11,-52},{20,-52},{20,-40},{-40,-40},{-40,-12}},
      color={0,0,127}));
  connect(bus.y1, ctlPID.uEna)
    annotation(Line(
      points={{0,100},{0,20},{-60,20},{-60,-20},{-44,-20},{-44,-12}},
      color={255,204,51},
      thickness=0.5));
  connect(bus.THeaWatSupSet, ctlPID.u_s)
    annotation(Line(points={{0,100},{0,20},{-60,20},{-60,0},{-52,0}},
      color={255,204,51},
      thickness=0.5));
  connect(ctlPID.y, bus.y_actual)
    annotation(Line(points={{-28,0},{20,0},{20,96},{0,96},{0,100}},
      color={0,0,127}));
annotation(Documentation(
  info="<html>
<p>
  This partial class provides a standard interface for hot water boiler
  models. It includes a replaceable instance of
  <a href=\"modelica://Buildings.Fluid.Boilers.BaseClasses.PartialBoiler\">
    Buildings.Fluid.Boilers.BaseClasses.PartialBoiler</a>. This model is used to
  construct the hot water boiler models within
...
<ul>
  <li>
    Boiler Enable signal <code>y1</code> : DO signal, with a dimensionality of
    zero
  </li>
  <li>
    Boiler HW supply temperature setpoint <code>THeaWatSupSet</code>: AO
    signal, with a dimensionality of zero
  </li>
  <li>
    Boiler firing rate <code>y_actual</code>: AI signal, with a dimensionality
    of zero
  </li>
  <li>
    Boiler HW supply temperature <code>THeaWatSup</code>: AI signal, with a
...
<ul>
  ...
  Icon(graphics={Rectangle(extent={{100,60},{-100,-60}},
    lineColor={0,0,0},
    lineThickness=1),
