...
partial model PartialHeatPumpTableData2DLoadDepSHC
  ...
  Buildings.Controls.OBC.CDL.Logical.Or or2
    "Combined enable signals for checking component proven on"
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={40,80})));
equation
  ...
  connect(busWea, hp.weaBus) annotation (Line(
      points={{-40,-140},{-22,-140},{-22,26},{-8,26},{-8,8},{0,8},{0,4}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(hp.nUniHea, bus.nUniHea) annotation (Line(points={{3,-18},{2,-18},{2,
          -32},{16,-32},{16,116},{0,116},{0,160}},
                                              color={255,127,0}));
  connect(bus.y1HeaOn, hp.onHea) annotation (Line(
      points={{0,160},{0,18},{-20,18},{-20,-8},{-12,-8}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(bus.y1CooOn, hp.onCoo) annotation (Line(
      points={{0,160},{0,16},{-20,16},{-20,-10},{-12,-10}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(or2.y, y1_actual.y1)
    annotation (Line(points={{40,92},{40,108}}, color={255,0,255}));
  connect(bus.y1HeaOn, or2.u1) annotation (Line(
      points={{0,160},{0,56},{40,56},{40,68}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(bus.y1CooOn, or2.u2) annotation (Line(
      points={{0,160},{0,56},{48,56},{48,68}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
