...
void ZoneExchange(
  ...
  if (FMU_EP_VERBOSITY >= TIMESTEP)
    writeFormatLog("Exchanging data with EnergyPlus: t = %.2f, initialCall = %d, mode = %s, zone = %s.\n",
  ...
  if (initialCall){
    ...
    if (FMU_EP_VERBOSITY >= MEDIUM)
      writeFormatLog("Initial call for zone %s with time = %.f\n", zone->modelicaNameThermalZone, time);
  }
  else
  {
    if (FMU_EP_VERBOSITY >= TIMESTEP)
      writeFormatLog("Did not enter initialization mode for zone %s., isInitialized = %d\n",
  ...
  *dQConSen_flow = (QConSenPer_flow-zone->outputs->valsSI[1])/dT;
  ...
  if (bui->mode == initializationMode){
    ...
    *tNext = bui->time; /* Return start time for next event time */
    ...
    if (FMU_EP_VERBOSITY >= TIMESTEP)
      writeFormatLog("Calling do_event_iteration after setting inputs for zone = %s\n", zone->modelicaNameThermalZone);
  ...
  *QPeo_flow    = zone->outputs->valsSI[3];
  ...
  if (FMU_EP_VERBOSITY >= TIMESTEP)
    writeFormatLog("Returning from ZoneExchange with nextEventTime = %.2f, TRad_degC = %.2f, zone = %s, mode = %s, ptr=%p, nZon=%d \n",
