...
def extract_data(outFile):
    ...
    for line in fin:
        ...
        if (count > 34 and count <= 44):
            temp = line.split()
            p_Int.append(float(temp[1].strip()))
            x_Int.append(float(temp[2].strip()))
            T_Int.append(float(temp[3].strip())+273.15)
    data = {
        'T_Bor': T_Bor,
        'p_Int': p_Int,
        'x_Int': x_Int,
...
if __name__=='__main__':
    ...
    for i in range (10,20):
        dblInp.append(273.15+10)
    dblInp.append(273.15+15)
    dblInp.append(1200)
    # # Initial borehole wall temperature at the start of modelica simulation
    # T_start = [dblInp[i] for i in range(10,20)]
    # # Current outdoor temperature
    ...
    state = {'tLast': 600, 
             ...
             'T_tough': [283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15, 283.15]}

    test = doStep(dblInp, state)
