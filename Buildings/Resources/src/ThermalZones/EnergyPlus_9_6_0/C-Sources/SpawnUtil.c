...
void delete_old_epfmi_binaries(FMUBuilding* bui){
  ...
  mallocString(lenPat, "Failed to allocate memory in delete_old_epfmi_binaries() for path.",
...
#ifdef _WIN32 /* Win32 or Win64 */
  /* Specify file mask */
  lenFilMas = lenPat + strlen("epfmi*");
  mallocString(lenFilMas, "Failed to allocate memory in delete_old_epfmi_binaries() for filMas.",
  ...
  {
     if (bui->logLevel >= MEDIUM)
      SpawnFormatMessage("%.3f %s: Did not find old epfmi binaries when searching for '%s'.\n", bui->time, bui->modelicaNameBuilding, filMas);
    free(path);
    free(filMas);
    return;
  }
  do
  {
              ...
              && strcmp(fdFile.cFileName, "..") != 0)
      {
          ...
          lenFil = lenPat + strlen(fdFile.cFileName);
          ...
          strcpy(libName, path);
          strcat(libName, fdFile.cFileName);
  ...
  while (nFil--){
    lenFil = lenPat + strlen(namelist[nFil]->d_name);
