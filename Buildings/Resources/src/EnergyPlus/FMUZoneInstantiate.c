...
int loadLib(const char* libPath, FMU *fmu) {
  ...
  if (!h) {
    ...
    //libPath);
    printf("Unable to load the EnergyPlus functions library with path %s.\n",
    libPath);
  }
#else
