...
DEBUG="$(find ./Buildings/Templates/. -type f -name *.mo -exec md5sum {} \; | sort -f -k 2)"
echo $DEBUG

CHECKSUM="$(find ./Buildings/Templates/. -type f -name *.mo -exec md5sum {} \; | sort -f -k 2 | awk '{ print $1; }' | md5sum | awk '{ print $1; }')"
