...
model C_fmu
  "EnergyPlus model of building C exported as FMU and imported as Modelica model"
// Model automatically generated by Dymola from FMI model description
  extends BaseClasses.Template_fmu(fmi_instanceName="C_fmu");
protected
...
package fmi_Functions
  ...
  extends ExternalObject;
    function constructor "Initialize FMI model"
      ...
      output fmiModel fmi;
      external"C" fmi = C_fmiInstantiateModel2(instanceName, loggingOn, fmuLocation)
      annotation(Header="
#ifndef CFMI_C
#define CFMI_C 1
...
#endif
#ifndef C_Instantiate_C
#define C_Instantiate_C 1
#include <stdlib.h>
#ifndef C_MYSTRCMP_C
#define C_MYSTRCMP_C 1
int Cmystrcmp(const void *_a, const void *_b) {
  char *a = _a;
  char *const *b = _b;
  return strcmp(a, *b);
}
#endif
void CLogger(fmiComponent c, fmiString instanceName, fmiStatus status,
...
void * C_fmiInstantiateModel2(const char*instanceName, fmiBoolean loggingOn, const char* fmuLocation) {
  static fmiCoSimCallbackFunctions funcs = {&CLogger, &calloc, &free, NULL};
  struct dy_Extended* res;

  res = calloc(1, sizeof(struct dy_Extended));
  if (res!=0) {
    if (!(res->hInst=LoadLibrary(\"C.so\"))) {
      ModelicaError(\"Loading of FMU dynamic link library (C.so) failed!\");
      return 0;
    }
    if(!(res->dyFmiInstantiateSlave=(fmiInstantiateSlaveFunc)GetProcAddress(res->hInst,\"C_fmiInstantiateSlave\"))){
      ModelicaError(\"GetProcAddress failed for fmiInstantiateSlave!\");
      return 0;
    }
    if (!(res->dyFmiFreeSlaveInstance=(fmiFreeSlaveInstanceFunc)GetProcAddress(res->hInst,\"C_fmiFreeSlaveInstance\"))) {
      ModelicaError(\"GetProcAddress failed for fmiFreeSlaveInstance!\");
      return 0;
    }
    if (!(res->dyFmiInitializeSlave=(fmiInitializeSlaveFunc)GetProcAddress(res->hInst,\"C_fmiInitializeSlave\"))) {
      ModelicaError(\"GetProcAddress failed for fmiInitializeSlave!\");
      return 0;
    }
    if (!(res->dyFmiDoStep=(fmiDoStepFunc)GetProcAddress(res->hInst,\"C_fmiDoStep\"))) {
      ModelicaError(\"GetProcAddress failed for fmiDoStep!\");
      return 0;
    }
    if (!(res->dyFmiTerminateSlave=(fmiTerminateSlaveFunc)GetProcAddress(res->hInst,\"C_fmiTerminateSlave\"))) {
      ModelicaError(\"GetProcAddress failed for fmiTerminateSlave!\");
      return 0;
    }
    if (!(res->dyFmiSetReal=(fmiSetRealFunc)GetProcAddress(res->hInst,\"C_fmiSetReal\"))) {
      ModelicaError(\"GetProcAddress failed for fmiSetReal!\");
      return 0;
    }
    if (!(res->dyFmiGetReal=(fmiGetRealFunc)GetProcAddress(res->hInst,\"C_fmiGetReal\"))) {
      ModelicaError(\"GetProcAddress failed for fmiGetReal!\");
      return 0;
    }
    if (!(res->dyFmiSetInteger=(fmiSetIntegerFunc)GetProcAddress(res->hInst,\"C_fmiSetInteger\"))) {
      ModelicaError(\"GetProcAddress failed for fmiSetInteger!\");
      return 0;
    }
    if (!(res->dyFmiGetInteger=(fmiGetIntegerFunc)GetProcAddress(res->hInst,\"C_fmiGetInteger\"))) {
      ModelicaError(\"GetProcAddress failed for fmiGetInteger!\");
      return 0;
    }
    if (!(res->dyFmiSetBoolean=(fmiSetBooleanFunc)GetProcAddress(res->hInst,\"C_fmiSetBoolean\"))) {
      ModelicaError(\"GetProcAddress failed for fmiSetBoolean!\");
      return 0;
    }
    if (!(res->dyFmiGetBoolean=(fmiGetBooleanFunc)GetProcAddress(res->hInst,\"C_fmiGetBoolean\"))) {
      ModelicaError(\"GetProcAddress failed for fmiGetBoolean!\");
      return 0;
    }
    res->m=res->dyFmiInstantiateSlave(instanceName, \"06615e6122bf0fa394df0a2afb28dd0c\", fmuLocation, \"application/x-dymolakernel\", 0, fmiFalse, fmiFalse, funcs, loggingOn);
    if (0==res->m) {free(res);res=0;ModelicaError(\"InstantiateSlave failed\");}
    else {res->dyTriggered=0;res->dyTime=res->dyLastTime=-1e37;}
  }
  return res;
}
#endif", Library="C", LibraryDirectory="modelica://Buildings/Resources/Library/FMU/C/binaries",
...
#define C_Free_C 1
#include <stdlib.h>
void C_fmiFreeModelInstance2(void*m) {
  ...
  if (a) {
    ...
    function fmiInitializeSlave
      ...
      external"C" C_fmiInitializeSlave2(fmi, tStart, forceShutDownAtTStop, tStop);
      annotation (Header="
#ifndef CFMI_C
#define CFMI_C 1
#include \"FMI/fmiFunctions_.h\"
#include \"FMI/fmiImport.h\"
#endif
#ifndef C_InitializeSlave_C
#define C_InitializeSlave_C 1
#include <stdlib.h>
double C_fmiInitializeSlave2(void*m, fmiReal tStart, fmiBoolean forceShutDownAtTStop, fmiReal tStop) {
  struct dy_Extended*a=m;
  ...
  return 1e37;
   ...
   function fmiDoStep
     ...
     output Real postAvailable=preAvailable;
     external"C" stepOK =
                         C_fmiDoStep2(fmi, currentTime, stepSize);
     annotation (Header="
#ifndef CFMI_C
#define CFMI_C 1
#include \"FMI/fmiFunctions_.h\"
...
#define C_DoStep_C 1
#include <stdlib.h>
double C_fmiDoStep2(void*m, double currentTime, double stepSize) {
  struct dy_Extended*a=m;
  fmiStatus status=fmiFatal;
  if (a) {
...
#endif
#ifndef C_SetReal_C
#define C_SetReal_C 1
#include <stdlib.h>
void C_fmiSetReal2(void*m, const int*refs, size_t nrefs, const double*vals) {
  struct dy_Extended*a=m;
  fmiStatus status=fmiFatal;
  if (a) {
...
#endif
#ifndef C_GetReal_C
#define C_GetReal_C 1
#include <stdlib.h>
void C_fmiGetReal2(void*m, const int*refs, size_t nrefs, double*vals) {
  struct dy_Extended*a=m;
  fmiStatus status=fmiFatal;
  if (a) {
...
#include \"FMI/fmiImport.h\"
#endif
#ifndef C_GetInteger_C
#define C_GetInteger_C 1
#include <stdlib.h>
void C_fmiGetInteger2(void*m, const int*refs, size_t nrefs, int*vals) {
  struct dy_Extended*a=m;
  fmiStatus status=fmiFatal;
  if (a) {
...
#include \"FMI/fmiImport.h\"
#endif
#ifndef C_SetInteger_C
#define C_SetInteger_C 1
#include <stdlib.h>
void C_fmiSetInteger2(void*m, const int*refs, size_t nrefs, int*vals) {
  struct dy_Extended*a=m;
  fmiStatus status=fmiFatal;
  if (a) {
...
#include \"FMI/fmiImport.h\"
#endif
#ifndef C_GetBoolean_C
#define C_GetBoolean_C 1
void C_fmiGetBoolean2(void*m, const int* refs, size_t nr, int* vals) {
  int i;
  struct dy_Extended*a=m;
  fmiStatus status=fmiFatal;
  if (a) {
...
#include \"FMI/fmiImport.h\"
#endif
#ifndef C_SetBoolean_C
#define C_SetBoolean_C 1
void C_fmiSetBoolean2(void*m, const int* refs, size_t nr, const int* vals,int*dummy) {
  int i;
  struct dy_Extended*a=m;
  fmiStatus status=fmiFatal;
