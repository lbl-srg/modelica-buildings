...
public
  ...
  Buildings.Utilities.Diagnostics.AssertEquality assEquTDewPoi(
    u2(unit="K"),
    threShold=10,
    startTime=startTime + 36000)
    "Assert that the data from the weather bus corresponds with the data in the time series"
    annotation (Placement(transformation(extent={{-40,24},{-20,44}})));
  Buildings.Utilities.Diagnostics.AssertEquality assEquHDirNor(
    u1(unit="W/m2"),
    startTime=startTime + 36000,
    threShold=300)
    "Assert that the data from the weather bus corresponds with the data in the time series"
    annotation (Placement(transformation(extent={{-40,-44},{-20,-24}})));
  Buildings.Utilities.Diagnostics.AssertEquality assEquHDif(
    u1(unit="W/m2"),
    startTime=startTime + 36000,
    threShold=300)
    "Assert that the data from the weather bus corresponds with the data in the time series"
