...
model LinearRegressionCampus
  ...
  Buildings.BuildingLoads.LinearRegression buiA(fileName="modelica://Buildings/Resources/Data/BuildingLoads/Examples/smallOffice_1.txt",
    V_nominal_AC=VDis,
  ...
  Buildings.Electrical.AC.ThreePhasesBalanced.Conversion.ACACConverter acac(eta=0.9,
  ...
  line dt(
    ...
    commercialCable_low=Buildings.Electrical.Transmission.LowVoltageCables.Cu35())
    "Distribution line"
  ...
  line a(
    ...
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=0,
        origin={250,-20})));
  Buildings.BuildingLoads.LinearRegression buiB(fileName="modelica://Buildings/Resources/Data/BuildingLoads/Examples/smallOffice_1.txt",
    V_nominal_AC=VDis,
    V_nominal_DC=VDC) "Building B"
    annotation (Placement(transformation(extent={{170,30},{190,50}})));
  Buildings.BuildingLoads.LinearRegression buiC(fileName="modelica://Buildings/Resources/Data/BuildingLoads/Examples/smallOffice_1.txt",
    V_nominal_AC=VDis,
    V_nominal_DC=VDC) "Building C"
    annotation (Placement(transformation(extent={{108,30},{128,50}})));
  Buildings.BuildingLoads.LinearRegression buiD(fileName="modelica://Buildings/Resources/Data/BuildingLoads/Examples/smallOffice_1.txt",
    V_nominal_AC=VDis,
    V_nominal_DC=VDC) "Building D"
    annotation (Placement(transformation(extent={{-10,-90},{10,-70}})));
  Buildings.BuildingLoads.LinearRegression buiE(fileName="modelica://Buildings/Resources/Data/BuildingLoads/Examples/smallOffice_1.txt",
    V_nominal_AC=VDis,
    V_nominal_DC=VDC) "Building E"
    annotation (Placement(transformation(extent={{50,-90},{70,-70}})));

  Buildings.Electrical.AC.ThreePhasesBalanced.Sensors.GeneralizedSensor senAC
    "Sensor in AC line after the transformer"
    annotation (Placement(transformation(extent={{-80,-30},{-60,-10}})));
  Buildings.Electrical.AC.ThreePhasesBalanced.Conversion.ACDCConverter acdc(conversionFactor=
        VDC/VDis, eta=0.9) "AC/DC converter"
    annotation (Placement(transformation(extent={{300,-30},{320,-10}})));
  Buildings.Electrical.AC.ThreePhasesBalanced.Sensors.GeneralizedSensor senA
    "Sensor in AC line at building A"
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=90,
        origin={280,10})));
  Buildings.Electrical.DC.Storage.Battery bat(EMax=P_dt*24*3600*0.1) "Battery"
    annotation (Placement(transformation(extent={{376,-10},{396,-30}})));

  Buildings.BuildingLoads.Examples.BaseClasses.BatteryControl_V
                   conBat(PMax=PDCGen/100, VDis=VDis) "Battery controller"
    annotation (Placement(transformation(extent={{358,-114},{378,-94}})));
  Buildings.Electrical.DC.Sources.PVSimple pv(A=100*150) "PV array"
  ...
  Buildings.Electrical.DC.Sources.WindTurbine tur(h=50, scale=500e3)
    "Wind turbine"
...
protected
  model resistiveLine
    extends Buildings.Electrical.AC.ThreePhasesBalanced.Lines.TwoPortResistance(R=0.3e-3*l);
...
model DummyLine
  ...
  parameter Buildings.Electrical.Transmission.Cables.Cable cable=
      Buildings.Electrical.Transmission.Functions.selectCable(
        P=P_nominal,
        V=V_nominal,
        mode=Buildings.Electrical.Types.CableMode.automatic) "Type of cable"
...
equation
  ...
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-240,
    ...
    experiment(
      ...
      Commands(file=
          "modelica://Buildings/Resources/Scripts/Dymola/BuildingLoads/Examples/LinearRegressionCampus.mos"
        "Simulate and plot"));
