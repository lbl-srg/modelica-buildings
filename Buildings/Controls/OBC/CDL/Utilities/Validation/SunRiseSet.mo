...
model SunRiseSet "Test model for the block SunRiseSet"
  ...
  Buildings.Utilities.Time.ModelTime modTim
    "Model time from the beginning of the year to the end"
  ...
  Buildings.Controls.OBC.CDL.Utilities.SunRiseSet sunRiseSet_sf(
    ...
    sunRise(fixed=true),
    sunSet(fixed=true))
    "Sunrise ans sunset hours in San Francisco as a normal test example"
...
annotation (
  ...
  __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Utilities/Validation/SunRiseSet.mos"
        ...
        <p>This example includes 3 tests for the SunRiseSet component: a normal case, 
        an arctic and antarctic case.</p>
        <p>The normal case is represented by San Francisco, 
        where there is sunrise and sunset every day.</p>
        <p>In the arctic circle (in the north hemisphere), 
        there is no sunset in summer and no sunrise in winter for consecutive days.</p>
        <p>In the antarctic circle (in the south hemisphere), there is no sunrise in summer 
        and no sunset in winter for consecutive days.</p>
</html>", revisions="<html>
<ul>
<li>November 27, 2018, by Kun Zhang:<br/>First implementation. </li>
</ul>
</html>"));
end SunRiseSet;
