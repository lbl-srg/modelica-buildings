...
block TimeTable "Table look-up with respect to time with constant segments"
  ...
  "Table matrix with time as a first column (in seconds, unless timeScale is not 1) and any further column as integer sheduled value output";
...
protected
  ...
  Buildings.Controls.OBC.CDL.Conversions.RealToInteger realToInteger
...
equation
  connect(tab.y, realToInteger.u) annotation (Line(points={{9,0},{18,0}},  color={0,0,127}));
  connect(realToInteger.y, y) annotation(Line(points={{42,0},{120,0}},         color={255,127,0}));
