...
model PIDWithResetScale
  ...
  Sources.Constant resVal(k=0) "Reset value"
    annotation (Placement(transformation(extent={{-80,-10},{-60,10}})));
  Logical.Sources.Constant resSig(k=false) "Reset signal"
    annotation (Placement(transformation(extent={{-80,-40},{-60,-20}})));
  Buildings.Controls.OBC.CDL.Continuous.PIDWithReset pidNoSca(
    controllerType=Buildings.Controls.OBC.CDL.Types.SimpleController.PID,
    k=2,
    Ti=1,
    Td=2,
  ...
  Buildings.Controls.OBC.CDL.Continuous.Gain gai1(k=1/1000)
...
equation
  connect(pulse.y,pidSca. u_s) annotation (Line(
      points={{-58,70},{-30,70},{-30,50},{-2,50}},
      color={0,0,127}));
  connect(pla1.u, pidSca.y)
    annotation (Line(points={{38,50},{22,50}}, color={0,0,127}));
  connect(pla1.y, pidSca.u_m) annotation (Line(points={{62,50},{82,50},{82,0},{10,
          0},{10,38}}, color={0,0,127}));
  connect(resVal.y, pla1.y_reset_in) annotation (Line(points={{-58,0},{30,0},{30,
  ...
  connect(gai.u, pulse.y) annotation (Line(points={{-42,-50},{-50,-50},{-50,70},
          {-58,70}}, color={0,0,127}));
  connect(pidNoSca.y, pla2.u)
    annotation (Line(points={{22,-50},{38,-50}}, color={0,0,127}));
  connect(pla2.y, gai1.u) annotation (Line(points={{62,-50},{70,-50},{70,-80},{54,
          -80}}, color={0,0,127}));
  connect(gai1.y, pidNoSca.u_m)
    annotation (Line(points={{30,-80},{10,-80},{10,-60}}, color={0,0,127}));
  connect(pla2.y_reset_in, resVal.y) annotation (Line(points={{38,-58},{30,-58},
          {30,0},{-58,0}},   color={0,0,127}));
  connect(resSig.y, pla2.trigger) annotation (Line(points={{-58,-30},{26,-30},{26,
          -64},{50,-64},{50,-62}}, color={255,0,255}));
  connect(booPul.y, pidSca.trigger)
    annotation (Line(points={{-58,30},{4,30},{4,38}}, color={255,0,255}));
  connect(booPul.y, pidNoSca.trigger) annotation (Line(points={{-58,30},{-10,30},
          {-10,-72},{4,-72},{4,-62}}, color={255,0,255}));
 annotation (
 experiment(StopTime=100.0, Tolerance=1e-06),
__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Continuous/Validation/PIDWithResetScale.mos"
        "Simulate and plot"),
