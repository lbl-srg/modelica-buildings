...
model IntegratorWithReset "Test model for integrator with reset"
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant cons(
    final k=10) "Constant as source term"
    annotation (Placement(transformation(extent={{-60,60},{-40,80}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.Pulse
    booleanPulse(
    final width=0.5,
    final period=0.2) "Boolean pulse"
    annotation (Placement(transformation(extent={{-60,-80},{-40,-60}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.SampleTrigger
    sampleTrigger(final period=0.2) "Sample trigger"
    annotation (Placement(transformation(extent={{-60,0},{-40,20}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp ramp(
    final height=-1,
    final duration=1,
    final offset=-2) "Ramp as a source term"
    annotation (Placement(transformation(extent={{-60,-40},{-40,-20}})));

equation
  connect(cons.y, intWitRes1.u) annotation (Line(points={{-38,70},{-20,70},{-20,
          30},{-2,30}},  color={0,0,127}));
  connect(cons.y, intWitRes2.u) annotation (Line(points={{-38,70},{-20,70},{-20,
          -10},{-2,-10}},           color={0,0,127}));
  connect(sampleTrigger.y, intWitRes1.trigger) annotation (Line(points={{-38,10},
          {10,10},{10,18}},           color={255,0,255}));
  ...
  connect(ramp.y, intWitRes1.y_reset_in) annotation (Line(points={{-38,-30},{-14,
          -30},{-14,22},{-2,22}}, color={0,0,127}));
  connect(ramp.y, intDef.y_reset_in) annotation (Line(points={{-38,-30},{-14,-30},
