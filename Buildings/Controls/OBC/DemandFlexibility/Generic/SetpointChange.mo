...
protected
  ...
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal sigChaSetChaDel(
    realTrue=setChaDel, realFalse=-1*setChaDel) if incSteSetCha
    "Sign change for the setpoint change delta"
    annotation (Placement(transformation(extent={{-100,60},{-80,80}})));
  Buildings.Controls.OBC.CDL.Reals.Add addCurSet if incSteSetCha
    "Adding setpoint change delta to the current setpoint"
    annotation (Placement(transformation(extent={{-60,60},{-40,80}})));
  Buildings.Controls.OBC.CDL.Reals.Switch swiMinMax if not incSteSetCha
