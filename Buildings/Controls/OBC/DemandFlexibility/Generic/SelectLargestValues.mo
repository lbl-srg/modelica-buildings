...
block SelectLargestValues "Select largest values"
  ...
  CDL.Reals.Sources.Constant con2[nVal](k=1:1:nVal)
    annotation (Placement(transformation(extent={{-260,-40},{-240,-20}})));
  CDL.Reals.MultiplyByParameter gai[nVal](k=smaNum)
    annotation (Placement(transformation(extent={{-220,-40},{-200,-20}})));
  CDL.Reals.Add add2[nVal]
    annotation (Placement(transformation(extent={{-180,0},{-160,20}})));
  CDL.Routing.RealExtractor                        extIndRea(nin=nVal)
    annotation (Placement(transformation(extent={{80,0},{100,20}})));
  CDL.Routing.RealScalarReplicator                        reaScaRep1(nout=nVal)
    annotation (Placement(transformation(extent={{120,0},{140,20}})));
  CDL.Reals.Less les[nVal]
    annotation (Placement(transformation(extent={{160,-40},{180,-20}})));
  CDL.Logical.Not not2[nVal]
...
equation
  ...
  connect(con2.y, gai.u)
    annotation (Line(points={{-238,-30},{-222,-30}}, color={0,0,127}));
  connect(gai.y, add2.u2) annotation (Line(points={{-198,-30},{-190,-30},{-190,
          4},{-182,4}}, color={0,0,127}));
  connect(u, add2.u1) annotation (Line(points={{-300,0},{-240,0},{-240,16},{
          -182,16}}, color={0,0,127}));
  connect(add2.y, swi.u3) annotation (Line(points={{-158,10},{-60,10},{-60,2},{
          -2,2}}, color={0,0,127}));
  connect(add2.y, mulMin.u) annotation (Line(points={{-158,10},{-150,10},{-150,
          30},{-142,30}}, color={0,0,127}));
  connect(nSel, extIndRea.index)
    annotation (Line(points={{-300,-60},{90,-60},{90,-2}}, color={255,127,0}));
  connect(sort.y, extIndRea.u)
    annotation (Line(points={{62,10},{78,10}}, color={0,0,127}));
  connect(extIndRea.y, reaScaRep1.u)
    annotation (Line(points={{102,10},{118,10}}, color={0,0,127}));
  connect(reaScaRep1.y, les.u2) annotation (Line(points={{142,10},{150,10},{150,
          -38},{158,-38}}, color={0,0,127}));
  connect(add2.y, les.u1) annotation (Line(points={{-158,10},{-60,10},{-60,-30},
          {158,-30}}, color={0,0,127}));
  connect(les.y, not2.u)
    annotation (Line(points={{182,-30},{198,-30}}, color={255,0,255}));
  connect(not2.y, and1.u2) annotation (Line(points={{222,-30},{230,-30},{230,-8},
