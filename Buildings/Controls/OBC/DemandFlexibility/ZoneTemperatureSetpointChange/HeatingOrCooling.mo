...
block HeatingOrCooling
  ...
  final parameter Boolean use_demCon=zonConVar == Buildings.Controls.OBC.DemandFlexibility.Types.ZoneControlVariant.Variant_3
  ...
  CDL.Interfaces.RealInput TCurZon[nZon] "Current zone temperature" annotation
    (Placement(transformation(extent={{-260,20},{-220,60}}), iconTransformation(
  ...
  CDL.Interfaces.RealInput PBuiThrVar if zonConVar == Buildings.Controls.OBC.DemandFlexibility.Types.ZoneControlVariant.Variant_4
    "Variable threshold for the electricity demand of the building" annotation
  ...
  Subsequences.ZoneQualification zonQua(
    ...
    final use_demCon=use_demCon,
    final nZon=nZon)
    annotation (Placement(transformation(extent={{-80,112},{-60,148}})));
  Subsequences.ZonePriorization zonPri(
    final nZon=nZon,
    final airConMod=airConMod)
    if nZon>1
  ...
  CDL.Routing.IntegerScalarReplicator intScaRep(final nout=nZon)
    annotation (Placement(transformation(extent={{-40,-62},{-20,-42}})));
  CDL.Integers.Sources.Constant conNSel(final k=nSel) if nZon>1
    "A constant for the number of zones to select for prioritization"
    annotation (Placement(transformation(extent={{0,40},{20,60}})));
  CDL.Reals.Sources.Constant conPBuiThr(
    final k=PBuiThrCon)
  ...
  CDL.Logical.Sources.Constant con[nZon](final k=fill(true, nZon)) if nZon==1
    annotation (Placement(transformation(extent={{60,40},{80,60}})));
equation
  connect(zonQua.disFla, zonPri.disFla) annotation (Line(points={{-58,130},{-40,
          130},{-40,96},{58,96}},  color={255,0,255}));
  connect(zonPri.yEna, zonCon.uEna) annotation (Line(points={{82,90},{100,90},{100,
          -100},{118,-100}},
                          color={255,0,255}));
  connect(sam.y, TComZonSet)
    annotation (Line(points={{202,0},{240,0}}, color={0,0,127}));
  connect(zonCon.TComZonSet, sam.u) annotation (Line(points={{142,-110},{160,-110},
          {160,0},{178,0}},
                          color={0,0,127}));
  connect(rouZonFla, zonQua.rouZonFla) annotation (Line(points={{-240,160},{-200,
          160},{-200,146},{-82,146}},
                           color={255,0,255}));
  connect(PBui, zonQua.PBui) annotation (Line(points={{-240,120},{-200,120},{-200,
          142},{-82,142}},                     color={0,0,127}));
  connect(PBuiThrVar, zonQua.PBuiThr) annotation (Line(points={{-240,80},{-150,80},
          {-150,138},{-82,138}}, color={0,0,127}));
  connect(TCurZon, zonQua.TZon) annotation (Line(points={{-240,40},{-140,40},{-140,
          134},{-82,134}},      color={0,0,127}));
  connect(TCurZonSet, zonQua.TZonSet) annotation (Line(points={{-240,0},{-130,0},
          {-130,130},{-82,130}},      color={0,0,127}));
  connect(demFleMod, zonQua.demFleMod) annotation (Line(points={{-240,-40},{-120,
          -40},{-120,126},{-82,126}},      color={255,127,0}));
  connect(TPreTarSet, zonQua.TPreTarSet) annotation (Line(points={{-240,-80},{-110,
          -80},{-110,122},{-82,122}},    color={0,0,127}));
  connect(TSheTarSet, zonQua.TSheTarSet) annotation (Line(points={{-240,-120},{-100,
          -120},{-100,118},{-82,118}},  color={0,0,127}));
  connect(TDefSet, zonQua.TDefSet) annotation (Line(points={{-240,-160},{-90,-160},
          {-90,114},{-82,114}},     color={0,0,127}));
  connect(TCurZon, zonPri.TZon) annotation (Line(points={{-240,40},{-140,40},{-140,
          92},{58,92}},       color={0,0,127}));
  connect(TCurZonSet, zonPri.TZonSet) annotation (Line(points={{-240,0},{-40,0},
          {-40,88},{58,88}},       color={0,0,127}));
  connect(demFleMod, intScaRep.u) annotation (Line(points={{-240,-40},{-120,-40},
          {-120,-52},{-42,-52}}, color={255,127,0}));
  connect(intScaRep.y, zonCon.demFleMod) annotation (Line(points={{-18,-52},{80,
          -52},{80,-104},{118,-104}},
                                  color={255,127,0}));
  connect(TCurZonSet, zonCon.TCurZonSet) annotation (Line(points={{-240,0},{60,0},
          {60,-108.2},{118,-108.2}},                            color={0,0,127}));
  connect(TPreTarSet, zonCon.TPreTarSet) annotation (Line(points={{-240,-80},{40,
          -80},{40,-112},{118,-112}},   color={0,0,127}));
  connect(TSheTarSet, zonCon.TSheTarSet) annotation (Line(points={{-240,-120},{80,
          -120},{80,-116},{118,-116}},   color={0,0,127}));
  connect(TDefSet, zonCon.TDefSet) annotation (Line(points={{-240,-160},{100,-160},
          {100,-120},{118,-120}},    color={0,0,127}));
  connect(conNSel.y, zonPri.nSel) annotation (Line(points={{22,50},{40,50},{40,84},
          {58,84}}, color={255,127,0}));
  connect(conPBuiThr.y, zonQua.PBuiThr) annotation (Line(points={{-158,110},{-150,
          110},{-150,138},{-82,138}}, color={0,0,127}));
  connect(con.y, zonCon.uEna) annotation (Line(points={{82,50},{100,50},{100,
