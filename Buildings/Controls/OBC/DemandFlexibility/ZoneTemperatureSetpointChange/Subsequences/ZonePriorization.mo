...
block ZonePriorization
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput yEna[nZon] "True: enable setpoint change"
  ...
  Buildings.Controls.OBC.DemandFlexibility.Generic.SelectSmallestValues selSmaDTZon(final nVal=nZon) if airConMod
    "Select the zones with the smallest zone temperature difference"
    annotation (Placement(transformation(extent={{20,40},{40,60}})));
  Buildings.Controls.OBC.DemandFlexibility.Generic.SelectLargestValues selLarDTZon(final nVal=nZon) if not airConMod
    "Select the zones with the largest zone temperature difference"
    annotation (Placement(transformation(extent={{20,-60},{40,-40}})));
equation
  connect(TZon, dTZon.u1) annotation (Line(points={{-140,20},{-100,20},{-100,6},
          {-82,6}}, color={0,0,127}));
  connect(TZonSet, dTZon.u2) annotation (Line(points={{-140,-20},{-100,-20},{
          -100,-6},{-82,-6}},
                         color={0,0,127}));
  connect(disFla, selSmaDTZon.disFla) annotation (Line(points={{-140,60},{0,60},
          {0,56},{18,56}}, color={255,0,255}));
  connect(disFla, selLarDTZon.disFla) annotation (Line(points={{-140,60},{0,60},
          {0,-44},{18,-44}}, color={255,0,255}));
  connect(dTZon.y, selSmaDTZon.u) annotation (Line(points={{-58,0},{-20,0},{-20,
          50},{18,50}}, color={0,0,127}));
  connect(dTZon.y, selLarDTZon.u) annotation (Line(points={{-58,0},{-20,0},{-20,
          -50},{18,-50}}, color={0,0,127}));
  connect(nSel, selLarDTZon.nSel) annotation (Line(points={{-140,-60},{-40,-60},
          {-40,-56},{18,-56}}, color={255,127,0}));
  connect(nSel, selSmaDTZon.nSel) annotation (Line(points={{-140,-60},{-40,-60},
          {-40,44},{18,44}}, color={255,127,0}));
  connect(selSmaDTZon.y, yEna) annotation (Line(points={{42,50},{80,50},{80,0},
          {140,0}}, color={255,0,255}));
  connect(selLarDTZon.y, yEna) annotation (Line(points={{42,-50},{80,-50},{80,0},
          {140,0}}, color={255,0,255}));
