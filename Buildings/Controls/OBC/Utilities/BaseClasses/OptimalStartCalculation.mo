...
protected
  ...
  Buildings.Controls.OBC.CDL.Discrete.TriggeredMovingMean samTemSloAve(final n=
        nDay) "Calculate the averaged temperature slope over the past n days"
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant timReaMin(final k=
        tReaMin)
  ...
  Buildings.Controls.OBC.CDL.Continuous.GreaterThreshold greThr(
    final t=thrOptOn) "The threshold for optOn signal becomes true"
  ...
  CDL.Continuous.Max timRea "Time required to reach the set point"
  ...
  CDL.Logical.Switch dTUse "dT used in the calculations (to avoid negative dT)"
...
equation
  connect(tim.y, triSam.u)  annotation (Line(points={{-188,10},{-130,10}},
                                              color={0,0,127}));
  connect(falEdg.y, triSam.trigger) annotation (Line(points={{-126,-20},{-118,-20},
          {-118,-1.8}},color={255,0,255}));
  connect(not1.y, lat.clr) annotation (Line(points={{-198,74},{-190,74}},
                      color={255,0,255}));
  connect(temSlo.y, samTemSloAve.u)
    annotation (Line(points={{22,0},{58,0}}, color={0,0,127}));
  connect(TDif, hys.u) annotation (Line(points={{-300,120},{-272,120},{-272,74},
          {-262,74}},
                color={0,0,127}));
  connect(staCal, dTCalOn.trigger) annotation (Line(points={{-300,40},{-30,40},{
          -30,68.2}}, color={255,0,255}));
  connect(TDif, dTHVACOn.u)
    annotation (Line(points={{-300,120},{-42,120}}, color={0,0,127}));
  connect(pre.y, dTHVACOn.trigger) annotation (Line(points={{412,-6},{420,-6},{420,
          104},{-30,104},{-30,108.2}}, color={255,0,255}));
  connect(pre.y, lat.u) annotation (Line(points={{412,-6},{420,-6},{420,150},{-190,
          150},{-190,80}},             color={255,0,255}));
  connect(edg.y, triSam1.trigger) annotation (Line(points={{-126,-80},{-118,-80},
          {-118,-61.8}},
                       color={255,0,255}));
  connect(triSam1.y, add1.u2) annotation (Line(points={{-106,-50},{-100,-50},{-100,
          -6},{-90,-6}}, color={0,0,127}));
  connect(triSam.y, add1.u1) annotation (Line(points={{-106,10},{-100,10},{-100,
          6},{-90,6}},
                    color={0,0,127}));
  connect(TDif, dTCalOn.u) annotation (Line(points={{-300,120},{-140,120},{-140,
          80},{-42,80}}, color={0,0,127}));
  connect(tNexOcc, add2.u2) annotation (Line(points={{-300,-140},{272,-140},{272,
          -86},{278,-86}}, color={0,0,127}));
  connect(add2.y, hysOpt.u) annotation (Line(points={{302,-80},{318,-80}},
                     color={0,0,127}));
  connect(min.y, add2.u1) annotation (Line(points={{262,-6},{270,-6},{270,-74},{
          278,-74}},                 color={0,0,127}));
  connect(staCal, truHol.u) annotation (Line(points={{-300,40},{-260,40},{-260,
          10},{-242,10}}, color={255,0,255}));
  connect(truHol.y, tim.u)    annotation (Line(points={{-218,10},{-212,10}}, color={255,0,255}));
  connect(tim.y, triSam1.u) annotation (Line(points={{-188,10},{-168,10},{-168,-50},
          {-130,-50}},color={0,0,127}));
  connect(min.y, tOpt) annotation (Line(points={{262,-6},{270,-6},{270,60},{460,
          60}},
        color={0,0,127}));
  connect(not1.u, hys.y)    annotation (Line(points={{-222,74},{-238,74}}, color={255,0,255}));
  connect(lat.y, edg.u) annotation (Line(points={{-166,80},{-162,80},{-162,-80},
          {-150,-80}},
                     color={255,0,255}));
  connect(lat.y, falEdg.u) annotation (Line(points={{-166,80},{-162,80},{-162,-20},
          {-150,-20}},color={255,0,255}));
  connect(tOptCal.y, samTimOpt.u)
    annotation (Line(points={{182,0},{188,0}}, color={0,0,127}));
  connect(staCal, samTimOpt.trigger) annotation (Line(points={{-300,40},{146,40},
          {146,-20},{200,-20},{200,-11.8}}, color={255,0,255}));
  connect(dTHVACOn.y, temSlo.u1) annotation (Line(points={{-18,120},{-10,120},{-10,
          6},{-2,6}}, color={0,0,127}));
  connect(min.y, greThr.u)  annotation (Line(points={{262,-6},{318,-6}},
                                                                       color={0,0,127}));
  connect(hysOpt.y, and2.u2) annotation (Line(points={{342,-80},{350,-80},{350,-14},
          {358,-14}},      color={255,0,255}));
  connect(greThr.y, and2.u1) annotation (Line(points={{342,-6},{358,-6}},
                      color={255,0,255}));
  connect(and2.y, pre.u) annotation (Line(points={{382,-6},{388,-6}},
                      color={255,0,255}));
  connect(pre.y, optOn) annotation (Line(points={{412,-6},{420,-6},{420,-60},{460,
          -60}},                color={255,0,255}));
  connect(timRea.y, temSlo.u2)
    annotation (Line(points={{-18,-6},{-2,-6}},color={0,0,127}));
  connect(timRea.u1, add1.y)
    annotation (Line(points={{-42,0},{-66,0}},
                                             color={0,0,127}));
  connect(timRea.u2, timReaMin.y) annotation (Line(points={{-42,-12},{-42,-32},{
          -58,-32}},      color={0,0,127}));
  ...
  connect(defTemSlo.y, temSloAve.u2) annotation (Line(points={{110,-30},{112,-30},
          {112,-12},{118,-12}}, color={0,0,127}));
  connect(min.u1, samTimOpt.y)
    annotation (Line(points={{238,0},{212,0}}, color={0,0,127}));
  connect(maxStaTim.y, min.u2) annotation (Line(points={{232,-30},{234,-30},{234,
          -12},{238,-12}}, color={0,0,127}));
  ...
  connect(dTCalOn.y, dTUse.u1) annotation (Line(points={{-18,80},{2,80},{2,100},
          {52,100},{52,88},{78,88}}, color={0,0,127}));
  connect(con.y, dTUse.u3) annotation (Line(points={{62,60},{72,60},{72,72},{78,
          72}}, color={0,0,127}));
  connect(dTUse.y, tOptCal.u1) annotation (Line(points={{102,80},{150,80},{150,6},
          {158,6}}, color={0,0,127}));
  connect(triAve.u2, samTri.y)
    annotation (Line(points={{38,-40},{22,-40}}, color={255,0,255}));
  connect(reqStaUp.y, triAve.u1) annotation (Line(points={{32,80},{36,80},{36,-32},
