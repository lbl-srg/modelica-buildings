...
model ControllerHeatingFan "Controller for heating and cooling"
  ...
  CDL.Interfaces.BooleanInput u annotation (Placement(transformation(extent={{
            -120,-90},{-100,-70}}), iconTransformation(extent={{-140,-110},{
            -100,-70}})));
equation
  connect(TSetRooHea, conHeaCoi.u_s)
    annotation (Line(points={{-110,80},{-60,80},{-60,-40},{-12,-40}},
                                                     color={0,0,127}));
  connect(conHeaCoi.y, yHea)
    annotation (Line(points={{11,-40},{60,-40},{110,-40}},
                                                 color={0,0,127}));
  connect(conFan.u_s, TSetRooCoo) annotation (Line(points={{18,40},{-46,40},{
          -46,20},{-110,20}},                    color={0,0,127}));
  connect(TRoo, conFan.u_m) annotation (Line(points={{-110,-20},{30,-20},{30,28}},
                                                color={0,0,127}));

  connect(conFan.y, yFan)
    annotation (Line(points={{41,40},{41,40},{110,40}},
                                                   color={0,0,127}));
  connect(TRoo, conHeaCoi.u_m) annotation (Line(points={{-110,-20},{-80,-20},{
          -80,-66},{0,-66},{0,-52}}, color={0,0,127}));
  connect(u, conHeaCoi.trigger) annotation (Line(points={{-110,-80},{-8,-80},{
          -8,-52}}, color={255,0,255}));
  annotation (
