...
block ShadingEnable "Generic shading device enable/disable seqence"
  ...
  parameter Boolean use_solIrr = false
    "Set to true if using solar irradiation instead of temeprature as a treshold variable"
    annotation(Dialog(group = "Conditional"));

  parameter Modelica.SIunits.Temperature TTre = 25 if not use_solIrr
    "Temperature threshold (either zone or outdoor air)"
    annotation(Evaluate=true);

  parameter Modelica.SIunits.Irradiance irrTre = 1500 if use_solIrr
    "Solar irradiation threshold"
    annotation(Evaluate=true);
  ...
  parameter Real irrHighTre = irrTre if use_solIrr
    "Upper limit for the irradiation hysteresis";
  parameter Real irrLowTre = (irrHighTre - irrDiff) if use_solIrr
    "Lower limit for the irradiation hysteresis";

  CDL.Interfaces.BooleanInput uEnable "Shading device enable schedule"
    annotation (Placement(transformation(extent={{-120,-60},{-80,-20}}),
    iconTransformation(extent={{-140,-80},{-100,-40}})));
  CDL.Interfaces.RealInput T "Zone or oudoor air temperature"
    annotation (Placement(transformation(extent={{-120,20},{-80,60}}),
    iconTransformation(extent={{-140,40},{-100,80}})));
  CDL.Interfaces.RealInput Irr "Total solar radiation on horizontal or on window surface"
  ...
  CDL.Continuous.Hysteresis hysIrr(
    ...
    "Solar irradiation signal hysteresis"
