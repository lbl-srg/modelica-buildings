...
block Configurator "Configures chiller staging"
  ...
  final parameter Real chiExtMat[nSta, nChi] = {j for i in 1:nChi, j in 1:nSta}
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yStaNomCap[nSta](
    ...
    extent={{260,100},{280,120}}),
                                 iconTransformation(extent={{100,60},{120,80}})));

  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yStaMinCap[nSta](
    final unit="W", final quantity="Power")
    "Stage minimal capacities" annotation (Placement(transformation(extent={{260,40},
            {280,60}}),      iconTransformation(extent={{100,20},{120,40}})));
...
//protected
  ...
  Buildings.Controls.OBC.CDL.Interfaces.IntegerOutput staTyp[nSta](
    final max=nSta) "Chiller stage type" annotation (Placement(transformation(
          extent={{260,-50},{280,-30}}),   iconTransformation(extent={{100,40},
            {120,60}})));
  ...
  Buildings.Controls.OBC.CDL.Utilities.Assert assMes[nSta](message=fill("Chillers are not staged according to G36 recommendation in stage:",
        nSta))
  ...
  CDL.Routing.IntegerReplicator intRep1[nSta](nout=fill(nChi, nSta))
    annotation (Placement(transformation(extent={{-160,-170},{-140,-150}})));
  CDL.Continuous.Sources.Constant chiExtMatr[nSta,nChi](final k=chiExtMat)
    "Transposed staging matrix"
    annotation (Placement(transformation(extent={{-200,-220},{-180,-200}})));
  CDL.Conversions.RealToInteger reaToInt2[nSta,nChi]
    annotation (Placement(transformation(extent={{-160,-220},{-140,-200}})));
  CDL.Integers.Equal intEqu1[nSta,nChi]
    annotation (Placement(transformation(extent={{-100,-170},{-80,-150}})));
  CDL.Conversions.BooleanToReal booToRea1[nSta,nChi]
    annotation (Placement(transformation(extent={{-60,-170},{-40,-150}})));
  CDL.Continuous.Sources.Constant chiStaMat1[nSta,nChi](final k=staMat)
    "Transposed staging matrix"
    annotation (Placement(transformation(extent={{-60,-220},{-40,-200}})));
  CDL.Continuous.Product                        pro1[nSta,nChi]
  ...
  CDL.Continuous.GreaterThreshold chiInSta[nChi](threshold=fill(0.5, nChi))
...
equation
  connect(chiNomCaps.y, staNomCaps.u) annotation (Line(points={{-179,310},{-140,
          310},{-140,270},{-122,270}},
                                color={0,0,127}));
  connect(staMinCaps.y, yStaMinCap) annotation (Line(points={{-99,190},{80,190},
          {80,50},{270,50}}, color={0,0,127}));
  ...
  connect(lesThr.y, staAva) annotation (Line(points={{-19,70},{60,70},{60,20},{270,
          20}},      color={255,0,255}));
  ...
  connect(matMax.y,reaToInt. u)
    annotation (Line(points={{-19,-50},{-2,-50}},
                                               color={0,0,127}));
  connect(reaToInt.y, staTyp) annotation (Line(points={{21,-50},{30,-50},{30,-40},
  ...
  connect(chiExtMatr.y, reaToInt2.u)
  ...
  connect(reaToInt2.y, intEqu1.u2) annotation (Line(points={{-139,-210},{-120,-210},
  ...
  connect(intEqu.y, assMes.u)
    annotation (Line(points={{181,-90},{198,-90}}, color={255,0,255}));
  connect(lesThr.y, booToRea2.u) annotation (Line(points={{-19,70},{-12,70},{-12,
          240},{-2,240}}, color={255,0,255}));
  connect(booToRea2.y, pro2.u2) annotation (Line(points={{21,240},{30,240},{30,264},
          {38,264}}, color={0,0,127}));
  connect(staNomCaps.y, pro2.u1) annotation (Line(points={{-99,270},{-30,270},{-30,
          276},{38,276}}, color={0,0,127}));
  connect(pro2.y, matGai.u)
    annotation (Line(points={{61,270},{78,270},{78,270}}, color={0,0,127}));
