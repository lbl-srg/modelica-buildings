...
model Status
  ...
  CDL.Continuous.Sources.Pulse TOutWetSig(
    amplitude=5,
    period=2*15*60,
    offset=TOutWetBul) "Measured outdoor air wet bulb temperature"
  ...
  Buildings.Controls.OBC.ASHRAE.PrimarySystem.ChillerPlant.Economizer.Status
    wseSta1
           "Water side economizer enable status sequence"
    annotation (Placement(transformation(extent={{40,0},{60,20}})));
  CDL.Continuous.Sources.Constant
                              chiWatFlow1(k=VChiWat_flow)
                            "Chilled water flow"
    annotation (Placement(transformation(extent={{-30,-40},{-10,-20}})));
  CDL.Continuous.Sources.Constant
                              TOutWetSig1(k=TOutWetBul)
                       "Measured outdoor air wet bulb temperature"
    annotation (Placement(transformation(extent={{-30,60},{-10,80}})));
  CDL.Continuous.Sources.Constant constTowFanSig1(k=1)
    "Cooling tower fan full load signal"
    annotation (Placement(transformation(extent={{-30,-80},{-10,-60}})));
  CDL.Continuous.Sources.Constant
                              TChiWatRetSig1(k=TChiWatRet)
    "Chilled water return temperature upstream of WSE"
  ...
  CDL.Continuous.Sources.Constant constTowFanSig2(k=1)
    "Cooling tower fan full load signal"
    annotation (Placement(transformation(extent={{90,-80},{110,-60}})));
  CDL.Continuous.Sources.Constant
                              TChiWatRetSig2(k=TChiWatRet)
    "Chilled water return temperature upstream of WSE"
...
equation
  ...
  connect(TChiWatRetDow.y, wseSta.TChiWatWseDow)
    annotation (Line(points={{-119,10},{-72,10}},
                                                color={0,0,127}));
