...
model PlantEnable "Validation model for PlantEnable sequence"
  Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.Generic.PlantEnable plaEna(
    final nIgnReq=2,
    final plaOffThrTim=10*60,
    final plaOnThrTim=15*60,
    final schTab=[0,0; 1,1; 18,1; 24,1])
    "Testing time-variance for all inputs"
    annotation (Placement(transformation(extent={{-20,50},{0,70}})));
  Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.Generic.PlantEnable plaEna1(final nIgnReq=2)
    "Testing time-varying number of requests"
    annotation (Placement(transformation(extent={{-20,-50},{0,-30}})));
  Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.Generic.PlantEnable plaEna2(final nIgnReq=2)
    "Testing time-varying outdoor air temperature"
    annotation (Placement(transformation(extent={{80,50},{100,70}})));
  Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.Generic.PlantEnable plaEna3(
    final nIgnReq=2,
    final schTab=[0,0; 1,1; 18,1; 24,1])
...
protected
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant con1(final k=273 + (75 - 32)/1.8)
    "Input for outdoor air temperature"
    annotation (Placement(transformation(extent={{-90,-70},{-70,-50}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant con2(final k=3)
    "Input for number of requests"
    annotation (Placement(transformation(extent={{10,-30},{30,-10}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant con3(final k=273 + (75 - 32)/1.8)
    "Input for outdoor air temperature"
...
equation
  ...
  connect(con.y, reaToInt2.u)
    annotation (Line(points={{32,80},{38,80}}, color={0,0,127}));
  connect(reaToInt3.u, con2.y)
    annotation (Line(points={{38,-20},{32,-20}}, color={0,0,127}));
  connect(plaEna1.supResReq, reaToInt1.y) annotation (Line(points={{-22,-35},{-30,
          -35},{-30,-20},{-38,-20}}, color={255,127,0}));
  connect(plaEna2.supResReq, reaToInt2.y) annotation (Line(points={{78,65},{70,65},
          {70,80},{62,80}}, color={255,127,0}));
  connect(plaEna2.TOut, sin5.y) annotation (Line(points={{78,55},{70,55},{70,40},
          {32,40}}, color={0,0,127}));
  connect(plaEna3.supResReq, reaToInt3.y) annotation (Line(points={{78,-35},{70,
          -35},{70,-20},{62,-20}}, color={255,127,0}));
  connect(plaEna3.TOut, con3.y) annotation (Line(points={{78,-45},{70,-45},{70,-60},
          {32,-60}}, color={0,0,127}));
  connect(reaToInt.y, plaEna.supResReq) annotation (Line(points={{-38,80},{-30,80},
          {-30,65},{-22,65}}, color={255,127,0}));
  connect(sin1.y, plaEna.TOut) annotation (Line(points={{-68,40},{-30,40},{-30,55},
          {-22,55}}, color={0,0,127}));
  connect(con1.y, plaEna1.TOut) annotation (Line(points={{-68,-60},{-30,-60},{-30,
          -45},{-22,-45}}, color={0,0,127}));
  annotation (
