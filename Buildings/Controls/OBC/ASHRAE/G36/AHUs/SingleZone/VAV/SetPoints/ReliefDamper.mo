...
block ReliefDamper
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput uOutDamPos(
    final min=0,
    final max=1,
    final unit="1") "Outdoor damper position"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yRelDam(
    final unit="1",
    final min=0,
    final max=1) "Relief damper position setpoint"
...
equation
  ...
  connect(uSupFan, and2.u2) annotation (Line(points={{-140,-80},{-40,-80},{-40,-48},
          {-22,-48}}, color={255,0,255}));
  connect(and2.y, swi.u2)
    annotation (Line(points={{2,-40},{78,-40}}, color={255,0,255}));
  connect(uOutDamPos, relDam.u)
    annotation (Line(points={{-140,50},{18,50}}, color={0,0,127}));
  connect(uOutDamPosMin, relDam.x1) annotation (Line(points={{-140,100},{-40,100},
  ...
  connect(relDam.y, swi.u1) annotation (Line(points={{42,50},{60,50},{60,-32},{78,
          -32}}, color={0,0,127}));
  connect(uOutDamPos, greThr.u) annotation (Line(points={{-140,50},{-110,50},{-110,
...
<ul>
<li>
<code>minRelPos</code>: the relief damper position that maintainx a building pressure
...
<li>
<code>maxRelPos</code>: the relief damper position that maintainx a building pressure
...
<p>
Relief damper position shall be reset lineary from <code>minRelPos</code> to
<code>maxRelPos</code> as the commanded economizer damper position goes from
<code>uOutDamPosMin</code> to 100% open.
</p>
