...
model DisableChillers
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant conWatPum[2](
    final k={true,false})
    "Condenser water pump commanded setpoint"
    annotation (Placement(transformation(extent={{-40,-90},{-20,-70}})));

equation
  connect(chiSta.y, disPlaFroChi.uChi) annotation (Line(points={{-18,80},{44,80},
          {44,20},{58,20}},   color={255,0,255}));
  connect(chiSta.y, disPlaFroChi.uChiWatReq) annotation (Line(points={{-18,80},
          {40,80},{40,18},{58,18}},color={255,0,255}));
  connect(chiSta.y, disPlaFroChi.uConWatReq) annotation (Line(points={{-18,80},
          {48,80},{48,14},{58,14}},  color={255,0,255}));
  connect(chiIsoVal.y, disPlaFroChi.uChiWatIsoVal) annotation (Line(points={{-58,50},
          {36,50},{36,16},{58,16}}, color={0,0,127}));
  connect(conIsoVal1.y, disPlaFroChi.uConWatIsoVal) annotation (Line(points={{-18,20},
          {20,20},{20,12},{58,12}},color={0,0,127}));
  connect(pumSpe.y, disPlaFroChi.uChiWatPumSpe) annotation (Line(points={{-18,-30},
          {28,-30},{28,8},{58,8}}, color={0,0,127}));
  connect(pumSpe.y, disPlaFroChi.uConWatPumSpe) annotation (Line(points={{-18,-30},
          {32,-30},{32,6},{58,6}}, color={0,0,127}));
