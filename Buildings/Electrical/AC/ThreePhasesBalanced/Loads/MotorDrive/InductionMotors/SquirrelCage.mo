...
<p>This is a simplified model of a squirrel cage type induction motor with electrical 
and mechanical interface, which is based on the per-phase equivalent circuit model for an induction motor.</p>
<p>The model inputs include load torque, as well as frequency and voltage from 
the power supply system at the electrical connector as the AC interface.</p>
<p>Besides the electromagnetic torque equation from 
...
main governing equations used to construct the model are as follows:</p>
<p>The motor-load torque balance equation:</p>
<p align=\"center\" style=\"font-style:italic;\">dW<sub>r</sub>&nbsp;/dt&nbsp;=&nbsp;
(tau<sub>e</sub>&nbsp;-&nbsp;tau<sub>m</sub>)&nbsp;/&nbsp;J</p>
<p>Where, the <i>W<sub>r</sub></i> is angular velocity of rotor [rad/s], 
<i>tau<sub>e</sub></i> is electromagnetizing torque generated by motor [N.m], 
...
[(R<sub>eq</sub>)<sup>2</sup>&nbsp;+&nbsp;(X<sub>eq</sub>)<sup>2</sup>]</p>
...
[(R<sub>eq</sub>)<sup>2</sup>&nbsp;+&nbsp;(X<sub>eq</sub>)<sup>2</sup>]</p>
<p><i>R<sub>eq</sub></i> and <i>X<sub>eq</sub></i> are the equivalent resistance and 
reactance of induction motor per-phase equivalent circuit, can be calculated as follows: </p>
...
*&nbsp;(X<sub>r</sub>&nbsp;+&nbsp;X<sub>m</sub>)<sup>2</sup>]</p>
...
+&nbsp;(s)<sup>2</sup>&nbsp;*&nbsp;(X<sub>r</sub>&nbsp;+&nbsp;X<sub>m</sub>)<sup>2</sup>]</p>
<p>Where, the subscripts <i>s</i>, <i>r</i> and <i>m</i> represent the stator, 
rotor and magnetizing part, respectively. The ciruit diagram is referred to 
<a href=\"modelica://Buildings.Electrical.AC.ThreePhasesBalanced.Loads.MotorDrive.InductionMotors.BaseClasses.MotorMachineInterface\">BaseClasses.MotorMachineInterface</a>.
</p>
</html>",
revisions="<html>
