...
<p>This block is to implement equations of the simplified model of squirrel cage type 
induction motor. The induction motor operation is based on the principle of electromagnetic induction; 
there are two magnetic fields occurring, one from the stator and one from the rotor, 
the rotor field is induced by the stator field. Therefore, an induction motor can 
be modeled as a rotating transformer. </p>
<p>The per-phase equivalent circuit model for an induction motor supplied by 
balanced three-phase supply based on the transformer model is represented below. </p>
<p align=\"center\"><img src=\"modelica://Buildings/Resources/Images/Electrical/AC/ThreePhasesBalanced/Loads/MotorDrive/InductionMotors/induction_motor_equivalent_circuit.png\"/> </p>
...
single-phase torque by the number of phases. </p>
<h4>Reference</h4>
