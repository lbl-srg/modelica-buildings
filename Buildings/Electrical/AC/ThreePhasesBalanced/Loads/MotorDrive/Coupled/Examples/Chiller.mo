...
model Chiller "This example shows how to use the motor coupled chiller model"
  ...
  MotorDrive.Coupled.Chiller chi(
    ...
    X_s=1.106,
    X_r=0.464,
    X_m=26.3) "Chiller" annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
...
equation
  connect(TCon_in.y,sou1. T_in) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(sou1.ports[1], chi.port_a1) annotation (Line(points={{-40,30},{-22,30},
  ...
  connect(senTem.port_b, chi.port_b2) annotation (Line(points={{-20,-30},{-16,
          -30},{-16,-6},{-10,-6}}, color={0,127,255}));
  connect(senTem.T, chi.meaPoi)
    annotation (Line(points={{-30,-19},{-30,3},{-11,3}}, color={0,0,127}));
