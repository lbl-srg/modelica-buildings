...
partial model ThreeWayValveParameters
  ...
  parameter Real fraK_BypVal(min=0, max=1) = 0.7 if use_Controller
  ...
  parameter Real l_BypVal[2](min=1e-10, max=1) = {0.0001,0.0001} if use_Controller
    "Bypass valve leakage, l=Kv(y=0)/Kv(y=1)"
    annotation(Dialog(group="Three-way Valve",enable=use_Controller));
  parameter Real R=50 if use_Controller
    "Rangeability, R=50...100 typically for bypass valve"
    annotation(Dialog(group="Three-way Valve",enable=use_Controller));
