...
model ParallelElectricChiler
  "Ensembled multiple electric chillers via vector"
  extends Buildings.ChillerWSE.BaseClasses.PartialParallelElectricEIR(
    redeclare final Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage val1[n](
      each final R=R,
    ...
    redeclare final Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage val2[n](
      each final R=R,
      each final delta0=delta0,
      ...
      each final init=initValve,
      each final y_start=yValve1_start),
    redeclare final Buildings.Fluid.Chillers.ElectricEIR chi[n](
