...
protected
  ...
  Real pol_y=sum(
    c .* {y_actual^i for i in 0:size(c,1)-1})
...
initial equation
  ...
  if size(
    ...
    for i in 0:nP/2 loop
      assert(
        sum(
          {c[j+1]*j*(i/nP)^(j-1) for j in 1:size(c,1)-1}) >= 0,
        "The provided valve polynomial coefficients 
      do not lead to a strictly increasing characteristic for y="+String(
          i/nP)+". This is not allowed.");
    end for;
  end if;
  assert(
    c[1] >= 0,
    "The provided valve polynomial coefficients do not lead to 
