...
protected
  ...
  protected
    Modelica.SIunits.PressureDifference dpPos=Buildings.Utilities.Math.Functions.smoothMax(
      dp,
      -dp,
      dp_small)
  ...
  function exponentialDamper_inv
    ...
    output Real y
      "Fractional opening";
  protected
    parameter Integer sizeSupSpl=size(
      kSupSpl,
      1)
...
initial equation
  (kSupSpl,idx_sorted)=Modelica.Math.Vectors.sort(
    kSupSpl_raw,
    ascending=true);
