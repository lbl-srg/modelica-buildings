...
equation
  ...
  connect(pSet.y, steSin.p_in) annotation (Line(points={{41,50},{86,50},{86,18},
          {80,18}}, color={0,0,127}));
  connect(watSou.ports[1], boi.port_a) annotation (Line(points={{-30,10},{0,10}},
                              color={0,127,255}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)),
experiment(Tolerance=1e-6, StopTime=100.0),
__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Boilers/BaseClasses/Examples/Boiling.mos"
        "Simulate and plot"));
end Boiling;
