...
model Active
  ...
  Buildings.Fluid.DataCenterEquipment.Racks.BaseClasses.ControlledFan conFan(
    ...
    "Rear door heat exchanger fan controlled for leaving air temperature"
...
equation
  connect(conFan.port_b, reaDooHex.port_a2) annotation (Line(points={{30,-60},{16,
          -60},{16,-6},{10,-6}}, color={0,127,255}));
  ...
  connect(senTAirOut.port_b, portAir_b)
    annotation (Line(points={{-60,-60},{-100,-60}},
                                                  color={0,127,255}));
  connect(senTAirOut.T, conFan.TMea) annotation (Line(points={{-50,-49},{-50,-40},
          {56,-40},{56,-56},{52,-56}}, color={0,0,127}));
  connect(TSet, conFan.TSet) annotation (Line(points={{-120,0},{-60,0},{-60,-20},
          {54,-20},{54,-52},{52,-52}},
                              color={0,0,127}));
  connect(conFan.P, PFan) annotation (Line(points={{29,-54},{26,-54},{26,0},{110,
          0}}, color={0,0,127}));

  connect(senTAirReaDooHexIn.port_b, conFan.port_a)
