...
partial model FlowMachineInterface
  ...
  final parameter Modelica.SIunits.VolumeFlowRate V_flow_nominal=
    data.pressure.V_flow[size(data.pressure.V_flow,1)]
  ...
  // Speed
  Modelica.Blocks.Interfaces.RealOutput N_actual(min=0, max=data.N_nominal,
                                                 final quantity="AngularVelocity",
                                                 final unit="1/min",
                                                 nominal=data.N_nominal)
    annotation (Placement(transformation(extent={{100,40},{120,60}}),
        iconTransformation(extent={{100,40},{120,60}})));

  // "Shaft rotational speed in rpm";
  Real r_N(min=0, start=N_start/data.N_nominal, unit="1")
    "Ratio N_actual/N_nominal";
  Real r_V(start=1, unit="1") "Ratio V_flow/V_flow_max";

protected
  Modelica.Blocks.Interfaces.RealOutput N_filtered(min=0, start=N_start, max=data.N_nominal) if
  ...
  Modelica.Blocks.Continuous.Filter filter(
     ...
     u_nominal=data.N_nominal,
     u(final quantity="AngularVelocity", final unit="1/min", nominal=data.N_nominal),
     y(final quantity="AngularVelocity", final unit="1/min", nominal=data.N_nominal),
  ...
  parameter Integer nOri = size(data.pressure.V_flow,1)
  ...
  parameter Real preDer3[nOri+2](each fixed=false)
    "Derivatives of flow rate vs. pressure at the support points";
  parameter Real powDer[size(data.power.V_flow,1)]=
   if data.use_powerCharacteristic then
     Buildings.Utilities.Math.Functions.splineDerivatives(
                   x=data.power.V_flow,
                   y=data.power.P,
                   ensureMonotonicity=Buildings.Utilities.Math.Functions.isMonotonic(x=data.power.P,
                                                                                     strict=false))
   else
     zeros(size(data.power.V_flow,1))
...
initial algorithm
  ...
  assert(size(data.pressure.V_flow, 1) > 1, "Must have at least two data points for pressure.V_flow.");
  assert(Buildings.Utilities.Math.Functions.isMonotonic(x=data.pressure.V_flow, strict=true) and
  data.pressure.V_flow[1] > -Modelica.Constants.eps,
...
" + getArrayAsString(data.pressure.V_flow, "pressure.V_flow"));

  // Check if V_flow_max or dpMax are provided by user
  haveVMax  :=(abs(data.pressure.dp[nOri])   < Modelica.Constants.eps);
  haveDPMax :=(abs(data.pressure.V_flow[1])  < Modelica.Constants.eps);
  // Assign V_flow_max and dpMax
  if haveVMax then
    V_flow_max :=data.pressure.V_flow[nOri];
  else
    assert((data.pressure.V_flow[nOri]-data.pressure.V_flow[nOri-1])
         /((data.pressure.dp[nOri]-data.pressure.dp[nOri-1]))<0,
...
Received 
" + getArrayAsString(data.pressure.dp, "dp"));
    V_flow_max :=data.pressure.V_flow[nOri] - (data.pressure.V_flow[nOri] - data.pressure.V_flow[
      nOri - 1])/((data.pressure.dp[nOri] - data.pressure.dp[nOri - 1]))*data.pressure.dp[nOri];
  end if;
  if haveDPMax then
    dpMax :=data.pressure.dp[1];
  else
    dpMax :=data.pressure.dp[1] - ((data.pressure.dp[2] - data.pressure.dp[1])/(data.pressure.V_flow[
      2] - data.pressure.V_flow[1]))*data.pressure.V_flow[1];
  end if;
  ...
  for i in 1:nOri-1 loop
    if ((data.pressure.dp[i+1]-data.pressure.dp[i])/(data.pressure.V_flow[i+1]-data.pressure.V_flow[i]) >= -kRes) then
...
of the fan or pump satisfies the minimum decrease condition
        ...
        (data.pressure.dp[i+1]-data.pressure.dp[i])
d[i] = --------------------------------------------------- < " + String(-kRes) + "
       (data.pressure.V_flow[i+1]-data.pressure.V_flow[i])
 
 is " + getArrayAsString({(data.pressure.dp[i+1]-data.pressure.dp[i])
        /(data.pressure.V_flow[i+1]-data.pressure.V_flow[i]) for i in 1:nOri-1}, "d") + "
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    for i in 1:nOri loop
      pCur1.dp[i]  :=data.pressure.dp[i] + data.pressure.V_flow[i] * kRes;
      pCur1.V_flow[i] := data.pressure.V_flow[i];
    end for;
  ...
  elseif haveVMax or haveDPMax then  // ----- Curve 2
    ...
    if haveVMax then
      ...
      pCur2.dp[1]     := dpMax;
      for i in 1:nOri loop
        pCur2.dp[i+1]  := data.pressure.dp[i] + data.pressure.V_flow[i] * kRes;
        pCur2.V_flow[i+1] := data.pressure.dp[i];
      end for;
    else
      for i in 1:nOri loop
        pCur2.dp[i]  := data.pressure.dp[i] + data.pressure.V_flow[i] * kRes;
        pCur2.V_flow[i] := data.pressure.V_flow[i];
      end for;
    ...
    pCur3.dp[1]     := dpMax;
    for i in 1:nOri loop
      pCur3.dp[i+1]  := data.pressure.dp[i] + data.pressure.V_flow[i] * kRes;
      pCur3.V_flow[i+1] := data.pressure.V_flow[i];
    end for;
...
equation
  ...
  if data.use_powerCharacteristic then
    ...
    if homotopyInitialization then
      P = homotopy(actual=cha.power(data=data.power, V_flow=VMachine_flow, r_N=r_N, d=powDer),
                      simplified=VMachine_flow/V_flow_nominal*
                            cha.power(data=data.power, V_flow=V_flow_nominal, r_N=1, d=powDer));
    else
      P = (rho/rho_default)*cha.power(data=data.power, V_flow=VMachine_flow, r_N=r_N, d=powDer);
    end if;
    ...
    if homotopyInitialization then
      etaHyd = homotopy(actual=cha.efficiency(data=data.hydraulicEfficiency,     r_V=r_V, d=hydDer),
                        simplified=cha.efficiency(data=data.hydraulicEfficiency, r_V=1,   d=hydDer));
      etaMot = homotopy(actual=cha.efficiency(data=data.motorEfficiency,     r_V=r_V, d=motDer),
                        simplified=cha.efficiency(data=data.motorEfficiency, r_V=1,   d=motDer));
    else
      etaHyd = cha.efficiency(data=data.hydraulicEfficiency, r_V=r_V, d=hydDer);
      etaMot = cha.efficiency(data=data.motorEfficiency,     r_V=r_V, d=motDer);
    end if;
...
<ul>
<li>
If <code>data.use_powerCharacteristic = false</code>, then the data points for
...
<li>
If <code>data.use_powerCharacteristic = true</code>, then the data points for
