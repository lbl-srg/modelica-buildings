...
<p>This example model provides a validation for the Nrpm model. A Wilo Stratos 80/1-12 pump is simulated on five different rpms for a changing load. The resulting for the head (pressure increase) and mass flow rate are plotted (colored lines) over the pump data sheet. The resulting figure is shown below.</p>
