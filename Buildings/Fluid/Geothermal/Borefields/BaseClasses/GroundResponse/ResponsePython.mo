...
model ResponsePython
  ...
  Modelica.Blocks.Interfaces.RealInput TBorWal_pre[nSeg](
    final unit=fill("K", nSeg),
    displayUnit=fill("degC", nSeg),
    quantity=fill("ThermodynamicTemperature", nSeg))
    "Borehole outer wall temperature" annotation (Placement(transformation(
          extent={{-140,-20},{-100,20}}),  iconTransformation(extent={{-120,-70},
            {-100,-50}})));
...
equation
  ...
  connect(TBorWal_pre, mul.u2)
    annotation (Line(points={{-120,0},{-22,0}}, color={0,0,127}));
  connect(clock.y, mul.u3[1]) annotation (Line(points={{-59,-50},{-40,-50},{-40,
          -7},{-22,-7}}, color={0,0,127}));
  connect(mul.y, pyt.uR)
    annotation (Line(points={{1,0},{18,0}}, color={0,0,127}));
  connect(QBor_flow, mul.u1) annotation (Line(points={{-120,60},{-40,60},{-40,7},
          {-22,7}}, color={0,0,127}));
