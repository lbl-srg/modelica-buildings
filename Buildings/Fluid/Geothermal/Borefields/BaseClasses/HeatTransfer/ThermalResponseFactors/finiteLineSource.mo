  ...
  if t > 0 and(includeRealSource or includeMirrorSource) then
    ...
    if includeRealSource then
      if(burDep2+len2) < burDep1 then
        disMin := sqrt(
          dis^2+(burDep1-burDep2-len2)^2);
      elseif burDep2 >(burDep1+len1) then
        disMin := sqrt(
          dis^2+(burDep1-burDep2+len1)^2);
      else
        disMin := dis;
      end if;
    else
      disMin := sqrt(
        dis^2+(burDep1+burDep2)^2);
    end if;
    ...
    if t >= timTre then
      ...
      h_21 := Modelica.Math.Nonlinear.quadratureLobatto(
        function Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.finiteLineSource_Integrand(
          dis=dis,
          len1=len1,
          burDep1=burDep1,
          len2=len2,
