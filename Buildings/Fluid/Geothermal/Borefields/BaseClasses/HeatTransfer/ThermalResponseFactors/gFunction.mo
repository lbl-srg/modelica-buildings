...
algorithm
  ...
  tSho := Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.timeGeometric(
    ...
    tSho_max,
    nTimSho);
  tLon := Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.timeGeometric(
    tLon_min,
    tLon_max,
    nTimLon);
  ...
  for k in 1:nTimSho loop
    // Finite line source solution
    FLS := Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.finiteLineSource(
      tSho[k],
      aSoi,
      rLin,
      hBor,
      ...
      dBor);
    // Infinite line source solution
    ILS := 0.5*Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.infiniteLineSource(
      tSho[k],
      aSoi,
      rLin);
    // Cylindrical heat source solution
    CHS := 2*Modelica.Constants.pi*Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.cylindricalHeatSource(
      tSho[k],
      aSoi,
      rBor,
      rBor);
  ...
  for k in 1:nTimLon-1 loop
    for i in 1:nBor loop
      for j in i:nBor loop
        ...
        if i == j then
          ...
          dis := sqrt(
            (cooBor[i,1]-cooBor[j,1])^2+(cooBor[i,2]-cooBor[j,2])^2);
        end if;
        ...
        if Done[i,j] < k then
          ...
          for m in 1:nSeg loop
            hSegRea[m] := Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.finiteLineSource(
              tLon[k+1],
              aSoi,
              dis,
              hBor/nSeg,
          ...
          for m in 1:(2*nSeg-1) loop
            hSegMir[m] := Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.finiteLineSource(
              tLon[k+1],
              aSoi,
              dis,
              hBor/nSeg,
          ...
          for m in 1:nBor loop
            for n in m:nBor loop
              if m == n then
                ...
                dis_mn := sqrt(
                  (cooBor[m,1]-cooBor[n,1])^2+(cooBor[m,2]-cooBor[n,2])^2);
              end if;
              if abs(
                dis_mn-dis) < relTol*dis then
                // Add thermal response factor to coefficient matrix A
                for u in 1:nSeg loop
                  for v in 1:nSeg loop
                    A[(m-1)*nSeg+u,(n-1)*nSeg+v] := hSegRea[abs(
                      u-v)+1]+hSegMir[u+v-1];
                    A[(n-1)*nSeg+v,(m-1)*nSeg+u] := hSegRea[abs(
                      u-v)+1]+hSegMir[u+v-1];
                  end for;
                end for;
    ...
    X := Modelica.Math.Matrices.solve(
      A,
      B);
