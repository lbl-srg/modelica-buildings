...
function cylindricalHeatSource_Integrand
  ...
  output Real y
    "Value of integrand";
algorithm
  y := 1.0/(u^2*Modelica.Constants.pi^2)*(exp(
    -u^2*Fo)-1.0)/(Buildings.Utilities.Math.Functions.besselJ1(
    u)^2+Buildings.Utilities.Math.Functions.besselY1(
    u)^2)*(Buildings.Utilities.Math.Functions.besselJ0(
    p*u)*Buildings.Utilities.Math.Functions.besselY1(
    u)-Buildings.Utilities.Math.Functions.besselJ1(
    u)*Buildings.Utilities.Math.Functions.besselY0(
    p*u));
  annotation (
    Inline=true,
    Documentation(
