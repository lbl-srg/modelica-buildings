...
initial equation
  assert(
    r_a < r_b,
    "Error: Model requires r_a < r_b.");
  assert(
    0 < r_a,
    "Error: Model requires 0 < r_a.");
  // ****************** Comments *********************:
  ...
  assert(
    abs(
      r[nSta+1]-r_b) < 1E-3,
    "Error: Wrong computation of radius. r[nSta+1]="+String(
      r[nSta+1]));
  ...
  G[1]=2*Modelica.Constants.pi*k*h/Modelica.Math.log(
    rC[1]/r_a);
  G[nSta+1]=2*Modelica.Constants.pi*k*h/Modelica.Math.log(
    r_b/rC[nSta]);
  for i in 2:nSta loop
    G[i]=2*Modelica.Constants.pi*k*h/Modelica.Math.log(
      rC[i]/rC[i-1]);
  end for;
  ...
  if not soiDat.steadyState then
    if steadyStateInitial then
      ...
      for i in 1:nSta loop
        T[i]=TInt_start+(TExt_start-TInt_start)/Modelica.Math.log(
          r_b/r_a)*Modelica.Math.log(
          rC[i]/r_a);
      end for;
    end if;
  end if;
