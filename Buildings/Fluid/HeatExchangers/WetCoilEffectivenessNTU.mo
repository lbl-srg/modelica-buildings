...
<p><br>This model describes a cooling coil that is capable of simulating fully-dry, partially-wet, and fully-wet regimes. The model is developed for counter flow heat exchangers but is also applicable for the cross-flow configuration (it is recommended to have over four tube rows for the cross-flow heat exchangers (Elmahdy and Mitalas, 1977 and Braun, 1988). The model can also be used for a heat exchanger, which acts as both heating coil (for some period of time) and cooling coil (for the others). However, it is not recommended to use this model for heating coil only or for cooling coil with no water condensation for computational efficiency. </p>
<h4>Main equations</h4>
<p>The coil model consists of two-equation sets, one for the fully-dry mode and the other for the fully-wet mode. For the fully-dry mode, the &epsilon;-NTU approach (Elmahdy and Mitalas, 1977) is used. For the fully-wet mode, equations from Braun (1988) and Mitchell and Braun (2012a and b), which are essentially the extension of the &epsilon;-NTU approach to simultaneous sensible and latent heat transfer, are utilized. The equation sets are switched depending on the switching criteria described below that determines the right mode based on a coil surface temperature and dew-point temperature for the air at the inlet of the coil. The transition regime between the two modes, which represents the partially-wet and partially-dry coil, is approximated by employing a fuzzy modeling approach, so-called Takagi-Sugeno fuzzy modeling (Takagi and Sugeno, 1985), which provides a continuously differentiable coil model that can cover all fully-dry, partially-dry and wet, and fully-wet regimes. </p>
<p>The switching rules are;</p>
<p>R1: If the coil surface temperature at the air inlet is lower than the dew-point temperature at inlet to the coil, then the cooling coil surface is fully-wet.</p>
<p>R2: If the surface temperature at the air outlet section is higher than the dew-point temperature of air at inlet, then the cooling coil surface is fully-dry.</p>
<p>R3: If any of the conditions in R1 or R2 is not satisfied, then the cooling coil is partially wet partially dry.</p>
<p><br>For more detailed descriptions of the fully-wet coil model and the fuzzy modeling approach, see Buildings.Fluid.HeatExchangers.BaseClasses.WetcalcsFuzzy_V3 and Buildings.Fluid.HeatExchangers.BaseClasses.DryWetCalcsFuzzy_V3.</p>
...
<p>The model uses steady-state physics. That is, no dynamics associated with water and coil materials are considered. </p>
<p>The Lewis number, which relates the mass transfer coefficient to the heat transfer coefficient, is assumed to be 1. </p>
<p>The model is not suitable for a cross-flow heat exchanger of which the number of passes is less than four. </p>
<h4>Validation </h4>
<p>Validation results can be found in Buildings.Fluid.HeatExchangers.Validation.WetCoilEffectivenessNTU, which compares with a discretized wet coil model (Buildings.Fluid.HeatExchangers.WetCoilDiscretized) for a textbook problem from Mitchell and Braun (2012b, Example SM2-1). In the problem, the absolute humidity of the air inlet increases while fixing the other boundary conditions of temperatures and flow rates of water and air, which allows investigating transition behaviors of the model from fully-dry to fully-wet modes. </p>
<h4>References </h4>
...
<p>Elmahdy, A.H. and Mitalas, G.P. 1977. &quot;A Simple Model for Cooling and Dehumidifying Coils for Use In Calculating Energy Requirements for Buildings&quot;. ASHRAE Transactions. Vol.83. Part 2. pp. 103-117. </p>
<p>Takagi, T. and Sugeno, M., 1985. Fuzzy identification of systems and its applications to modeling and control.&nbsp;IEEE transactions on systems, man, and cybernetics, (1), pp.116-132.</p>
</html>",                    revisions="<html>
<ul>
