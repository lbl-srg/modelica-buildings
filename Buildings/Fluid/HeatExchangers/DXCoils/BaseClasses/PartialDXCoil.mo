...
protected
  parameter Integer i_x(
    fixed=false)
    "Index of substance";
  Modelica.SIunits.SpecificEnthalpy hEvaIn=inStream(
    port_a.h_outflow)
  ...
  Modelica.SIunits.MassFraction XEvaIn[Medium.nXi]=inStream(
    port_a.Xi_outflow)
...
initial algorithm
  ...
  for i in 1:(nSta-1) loop
    assert(
      datCoi.sta[i].nomVal.Q_flow_nominal >= datCoi.sta[nSta].nomVal.Q_flow_nominal,
    ...
    the biggest value in magnitude. Obtained "+Modelica.Math.Vectors.toString(
        {datCoi.sta[i].nomVal.Q_flow_nominal for i in 1:nSta},
        "Q_flow_nominal"));
  end for;
  ...
  assert(
    i_x > 0,
    "Substance '"+substanceName+"' is not present in medium '"+Medium.mediumName+"'.\n"+"Change medium model to one that has '"+substanceName+"' as a substance.");
equation
