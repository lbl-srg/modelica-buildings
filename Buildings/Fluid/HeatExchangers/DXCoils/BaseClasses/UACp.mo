...
model UACp
  ...
  replaceable parameter Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Data.Generic.BaseClasses.NominalValues per
  ...
  final parameter Modelica.SIunits.SpecificHeatCapacity Cp_nominal=Medium.specificHeatCapacityCp(
    Medium.setState_pTX(
      p=per.p_nominal,
      T=per.TEvaIn_nominal,
      X=cat(1,{XEvaIn_nominal},{1-sum({XEvaIn_nominal})})))
...
initial equation
  ...
  if homotopyInitialization then
    ...
    // do not use homotopy
    psat_ADP_nominal=Buildings.Utilities.Psychrometrics.Functions.saturationPressureLiquid(
      TADP_nominal);
  end if;
  //  Taylor series
  ...
  bypass_nominal=Buildings.Utilities.Math.Functions.smoothLimit(
    ...
    u=0.999,
    deltaX=1e-4);
  UAcp=-per.m_flow_nominal*Modelica.Math.log(
    bypass_nominal);
  assert(
