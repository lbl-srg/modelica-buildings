...
algorithm
  ...
  if iSam == 1 then
    // First call, at t=0
    dT := 0;
    QL_flow := Buildings.Fluid.HeatExchangers.Ground.Boreholes.BaseClasses.exchangeValues(
   ...
   for i in 1:(iSam-1) loop
      ...
      QU_flow := Buildings.Fluid.HeatExchangers.Ground.Boreholes.BaseClasses.exchangeValues(
      ...
      dT := dT + 1/(4*Modelica.Constants.pi*k)*
        Buildings.Fluid.HeatExchangers.Ground.Boreholes.BaseClasses.powerSeries(
