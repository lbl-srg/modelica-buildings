  ...
  // Verify correctness of performance curves, and write warning if error is bigger than 10%
   for iSta in 1:nSta loop
     ...
     Buildings.Fluid.HeatExchangers.HeatPumps.BaseClasses.Functions.warnIfPerformanceOutOfBounds(
       Buildings.Fluid.Utilities.extendedPolynomial(
         x=1,
         c=heaSta[iSta].perCur.capFunFF2,
         xMin=heaSta[iSta].perCur.ff2Min,
         xMax=heaSta[iSta].perCur.ff2Min),
         msg="Capacity as a function of normalized mass flow rate ",
         curveName="heaSta[" + String(iSta) + "].perCur.capFunFF");
     Buildings.Fluid.HeatExchangers.HeatPumps.BaseClasses.Functions.warnIfPerformanceOutOfBounds(
       Buildings.Utilities.Math.Functions.biquadratic(
     ...
     Buildings.Fluid.HeatExchangers.HeatPumps.BaseClasses.Functions.warnIfPerformanceOutOfBounds(
       Buildings.Fluid.Utilities.extendedPolynomial(
         x=1,
         c=heaSta[iSta].perCur.EIRFunFF2,
         xMin=heaSta[iSta].perCur.ff2Min,
         xMax=heaSta[iSta].perCur.ff2Min),
         msg="EIR as a function of normalized mass flow rate ",
         curveName="heaSta[" + String(iSta) + "].perCur.EIRFunFF");
...
algorithm
  if mode == 1 then
    ...
    ff2[iSta]:=Buildings.Utilities.Math.Functions.smoothMax(
      x1=m2_flow,
      x2=m2_flow_small,
      deltaX=m2_flow_small/10)/heaSta[iSta].nomVal.m2_flow_nominal;
  ...
  //-------------------------Heating capacity modifiers----------------------------//
    ...
    cap_FF2[iSta] := Buildings.Fluid.Utilities.extendedPolynomial(
      ...
      c=heaSta[iSta].perCur.capFunFF2,
      xMin=heaSta[iSta].perCur.ff2Min,
      xMax=heaSta[iSta].perCur.ff2Max);
    ...
    wasHea_T[iSta] :=Buildings.Utilities.Math.Functions.smoothMax(
      x1=Buildings.Utilities.Math.Functions.biquadratic(
        a=heaSta[iSta].perCur.wasHeaFunT,
        x1=Modelica.SIunits.Conversions.to_degC(T1In),
        x2=Modelica.SIunits.Conversions.to_degC(T2In)),
      x2=0.001,
      deltaX=0.0001)
    ...
    corFac2[iSta] :=Buildings.Utilities.Math.Functions.smoothHeaviside(
       x=ff2[iSta] - heaSta[iSta].perCur.ff2Min/4,
       delta=heaSta[iSta].perCur.ff2Min/4);

    Q_flow[iSta]     := corFac1[iSta]*corFac2[iSta]*cap_T[iSta]*cap_FF1[iSta]*cap_FF2[iSta]*heaSta[iSta].nomVal.Q_flow_nominal;
    EIR[iSta]        := corFac1[iSta]*corFac2[iSta]*EIR_T[iSta]*EIR_FF1[iSta]*EIR_FF2[iSta]/heaSta[iSta].nomVal.COP_nominal;
    QRecWas_flow[iSta]  := corFac1[iSta]*corFac2[iSta]*wasHea_T[iSta]*Q_flow[iSta]*EIR[iSta]*heaSta[iSta].nomVal.wasHeaRecFra_nominal;
    end for;
