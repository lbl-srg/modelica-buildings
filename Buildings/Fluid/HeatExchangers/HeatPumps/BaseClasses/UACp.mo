...
model UACp "Calculates UA/Cp of the HX"
  extends Buildings.Fluid.HeatExchangers.HeatPumps.BaseClasses.NominalCondition;
...
//---------------------------------------Eq.1---------------------------------------//
  (XIn_nominal - XOut_nominal)*(TIn_nominal - TADP_nominal)
     =(XIn_nominal  - XADP_nominal)*(TIn_nominal - TOut_nominal);
//---------------------------------------Eq.2---------------------------------------//
...
//-----------------------------------uACp calculations-----------------------------//
  ...
  bypass_nominal=Buildings.Utilities.Math.Functions.smoothLimit(
              ...
              u=0.999,
              deltaX=1e-4);
  UAcp = -m_flow_nominal * Modelica.Math.log(bypass_nominal);
