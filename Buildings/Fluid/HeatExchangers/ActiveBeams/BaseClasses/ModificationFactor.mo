...
model ModificationFactor "Factor to modify nominal capacity"
  ...
  DerivativesCubicSpline temDif_mod
    "DerivativesCubicSpline for the temperature difference between room and water"
    annotation (Placement(transformation(extent={{20,-50},{40,-30}})));
  DerivativesCubicSpline watFlo_mod "DerivativesCubicSpline for the water flow"
    annotation (Placement(transformation(extent={{20,70},{40,90}})));
  DerivativesCubicSpline airFlo_mod "DerivativesCubicSpline for the air flow"
    annotation (Placement(transformation(extent={{20,10},{40,30}})));

  Modelica.Blocks.Interfaces.RealInput watTem
    "Actual temperature of the water entering the beam"
    annotation (Placement(transformation(extent={{-140,-50},{-100,-10}})));
  Modelica.Blocks.Interfaces.RealInput watFlo
    "Actual mass flow rate of the water entering the beam"
    annotation (Placement(transformation(extent={{-140,70},{-100,110}})));
  Modelica.Blocks.Interfaces.RealInput airFlo
    "Actual mass flow rate of the primary air entering the beam"
  ...
  Modelica.Blocks.Sources.Constant watFlo_nom "Nominal water mass flow rate"
    annotation (Placement(transformation(extent={{-70,50},{-50,70}})));
  Modelica.Blocks.Sources.Constant airFlo_nom "Nominal water mass flow rate"
...
equation
  connect(airFlo, pro_1.u1) annotation (Line(points={{-120,30},{-72,30},{-20,30},
  ...
  connect(watFlo, pro_2.u1) annotation (Line(points={{-120,90},{-72,90},{-20,90},
  ...
  connect(temDif_mod.y, mulPro.u[1]) annotation (Line(points={{41,-40},{60,-40},
          {60,2.8},{64,2.8}},   color={0,0,127}));
  connect(pro_3.y, temDif_mod.u)
    annotation (Line(points={{11,-40},{14.5,-40},{18,-40}}, color={0,0,127}));
  connect(airFlo_mod.y, mulPro.u[2]) annotation (Line(points={{41,20},{58,20},{
          58,4.44089e-016},{64,4.44089e-016}}, color={0,0,127}));
  connect(watFlo_mod.y, mulPro.u[3]) annotation (Line(points={{41,80},{58,80},{
          58,-2.8},{64,-2.8}}, color={0,0,127}));
