...
model WetCoilEffectivenessNTU_WithRated
  "Model that validates the wet coil effectiveness-NTU model with Rated condition"
  ...
  parameter Real X_a2_nominal_wet(min=0,max=1) = 0.0173
...
equation
  ...
  connect(souWat1.ports[1], hexWetNTU.port_a1) annotation (Line(points={{-160,20},
          {-40,20},{-40,12},{-28,12}},       color={0,127,255}));
  connect(hexWetNTU.port_b1, sinWat.ports[1]) annotation (Line(points={{-8,12},{
  ...
  connect(hexWetNTU.port_b2, TDryBulOut.port_a) annotation (Line(points={{-28,0},
          {-50,0}},                     color={0,127,255}));
  connect(hexWetNTU.port_a2, RelHumIn.port_b) annotation (Line(points={{-8,0},{0,
          0},{0,-80},{20,-80}},   color={0,127,255}));
...
end WetCoilEffectivenessNTU_WithRated;
