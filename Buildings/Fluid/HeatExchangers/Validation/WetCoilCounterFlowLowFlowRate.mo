...
model WetCoilCounterFlowLowFlowRate
  ...
  Controls.OBC.CDL.Continuous.Sources.Ramp ram_mWat(
  ...
  Controls.OBC.CDL.Continuous.Sources.Ramp ram_mAir(
    height=-mAir_flow_nominal,
    duration=90,
    offset=mAir_flow_nominal,
    startTime=2000)
    annotation (Placement(transformation(extent={{40,-90},{60,-70}})));
  Controls.OBC.CDL.Continuous.Sources.Ramp ram_TAir(
    height=-10,
    duration=1800,
    offset=273.15 + 30,
    startTime=800)
...
equation
  connect(sou_w.m_flow_in, ram_mWat.y)
  ...
  connect(sou_a.ports[1], senTemAirIn.port_a)
    annotation (Line(points={{66,-40},{60,-40}}, color={0,127,255}));
  ...
  connect(sou_a.m_flow_in, ram_mAir.y) annotation (Line(points={{88,-32},{94,-32},
          {94,-80},{62,-80}}, color={0,0,127}));
  connect(sou_a.T_in, ram_TAir.y) annotation (Line(points={{88,-36},{96,-36},{96,
  ...
  annotation (
    ...
    experiment(
      StopTime=3600,
      Tolerance=1e-06),
    __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/Validation/WetCoilCounterFlowLowFlowRate.mos"
        "Simulate and plot"),
    Documentation(info="<html>
<p>
This is a validation case in which the water mass flow rate and the air mass flow rate are both ramped to zero.
...
Making the air mass flow rate to be ramped to zero after the water mass flow is equal to 0;<br/>
The temperature of the inlet air is also ramped;<br/>
</li>
<li>
...
end WetCoilCounterFlowLowFlowRate;
