...
model UnitTestWetCoilCoolingFuzzy_V2_2_2
  ...
  WetEffectivenessNTU_Fuzzy_V2_2_2                         hexWetNtu(
    ...
    each final allowFlowReversal2=false) annotation (Placement(transformation(extent={{18,8},{
            38,-12}})));
  ...
  Sensors.Temperature TAout(redeclare package Medium = Medium2)
    annotation (Placement(transformation(extent={{-82,44},{-62,64}})));
  Sensors.Temperature TAin(redeclare package Medium = Medium2)
    annotation (Placement(transformation(extent={{40,42},{60,62}})));
equation
  connect(sup2.ports[1], hexWetNtu.port_a2)
    annotation (Line(points={{80,40},{60,40},{60,4},{38,4}}, color={0,127,255}));
  connect(hexWetNtu.port_b1, sin1.ports[1])
    annotation (Line(points={{38,-8},{60,-8},{60,-40},{80,-40}}, color={0,127,255}));
  connect(hexWetNtu.port_b2, sin2.ports[1])
    annotation (Line(points={{18,4},{0,4},{0,40},{-140,40}},    color={0,127,255}));
  connect(sup1.ports[1], hexWetNtu.port_a1)
    annotation (Line(points={{-8,-40},{0,-40},{0,-8},{18,-8}},color={0,127,255}));
  ...
  connect(TAin.port, hexWetNtu.port_a2) annotation (Line(points={{50,42},{44,42},
          {44,4},{38,4}}, color={0,127,255}));
  connect(TAout.port, hexWetNtu.port_b2) annotation (Line(points={{-72,44},{-28,
          44},{-28,4},{18,4}}, color={0,127,255}));
  annotation (
  experiment(
    StopTime=2000),
    Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-180,-120},{120,60}})),
    __Dymola_Commands(file=
          "Resources/Scripts/Dymola/Fluid/HeatExchangers/Validation/UnitTestWetCoilCoolingFuzzy_V2_1.mos"));
end UnitTestWetCoilCoolingFuzzy_V2_2_2;
