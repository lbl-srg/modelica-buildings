...
model DryCalcsFuzzy_V2_2_2_1
  ...
  Modelica.SIunits.ThermalConductance CMin_flow=min(CAir_flow,CWat_flow)
    "Minimum capacity rate";
  Modelica.SIunits.ThermalConductance CMin_flow_nominal=min(mAir_flow_nominal*cpAir,mWat_flow_nominal*cpWat)
    "Minimum capacity rate";
  Modelica.SIunits.ThermalConductance CMax_flow_nominal=max(mAir_flow_nominal*cpAir,mWat_flow_nominal*cpWat)
...
equation
  dryfraNonZero = Buildings.Utilities.Math.Functions.spliceFunction(
    ...
    x =   dryfra-delta,
    deltax = delta/2);
  UA = 1/ (1 / UAAir + 1 / UAWat) *dryfraNonZero "UA is for the overall coil (i.e., both sides)";
