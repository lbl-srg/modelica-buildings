...
protected
  parameter Medium.ThermodynamicState sta_nominal=Medium.setState_pTX(
      ...
      p=Medium.p_default,
      X=Medium.X_default);
  parameter Modelica.SIunits.Density rho=if perPar.Fluid > Buildings.Fluid.SolarCollector.Types.DesignFluid.Air then 1000 else 1.225
    "Density, used to compute mass flow rate";
  parameter Buildings.Fluid.SolarCollector.Data.PartialSolarCollector
                                                              perPar
...
public
  ...
  MixingVolumes.MixingVolume vol[nSeg](
    ...
    each V=perPar.V) annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=180,
        origin={48,-16})));
