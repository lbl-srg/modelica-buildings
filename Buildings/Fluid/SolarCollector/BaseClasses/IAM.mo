...
algorithm
  // E+ Equ (555)
  incAngMod := 1 + B0*(1/Buildings.Utilities.Math.Functions.smoothMax(Modelica.Math.cos(incAng), Modelica.Math.cos(incAngMin), delta) - 1) + B1*(1/Buildings.Utilities.Math.Functions.smoothMax(Modelica.Math.cos(incAng), Modelica.Math.cos(incAngMin), delta) - 1)^2; // fixme: avoid division by zero
