...
model EquationFitReversible_EnergyPlus "Validation with EnergyPlus model"
  ...
  Modelica.Blocks.Math.UnitConversions.From_degC TLoaEnt
    "Block that converts entering water temperature of the load side"
      annotation (Placement(transformation(extent={{-60,60},{-40,80}})));
  Modelica.Blocks.Math.UnitConversions.From_degC TSouEnt
    "Block that converts entering water temperature of the source side"
      annotation (Placement(transformation(extent={{-60,-40},{-40,-20}})));
  Modelica.Blocks.Math.UnitConversions.From_degC TSetCoo
...
equation
  connect(heaPum.port_a1,loaPum. ports[1])
    annotation (Line(points={{34,6},{28,6},{28,74},{-1.77636e-15,74}},color={0,127,255}));
  connect(souPum.ports[1], heaPum.port_a2)
    annotation (Line(points={{60,-50},{54,-50},{54,-6}},color={0,127,255}));
  connect(heaPum.uMod, reaToInt.y)
    annotation (Line(points={{33,0},{-39,0}},color={255,127,0}));
  connect(cooVol.ports[1], heaPum.port_b2)
    annotation (Line(points={{0,-70},{28,-70},{28,-6},{34,-6}}, color={0,127,255}));
  connect(heaPum.port_b1, heaVol.ports[1])
    annotation (Line(points={{54,6},{62,6},{62,70},{72,70}}, color={0,127,255}));
  connect(reaToInt.u, uMod.y)
    annotation (Line(points={{-62,0},{-79,0}}, color={0,0,127}));
  connect(loaPum.T_in, TLoaEnt.y)
    annotation (Line(points={{-22,70},{-39,70}}, color={0,0,127}));
  connect(TSouEnt.y, souPum.T_in)
    annotation (Line(points={{-39,-30},{92,-30},{92,-54},{82,-54}}, color={0,0,127}));
  connect(TSetCoo.y, heaPum.TSet) annotation (Line(points={{-39,40},{20,40},{20,
          9},{32.6,9}}, color={0,0,127}));
  ...
  connect(datRea.y[14], TSouEnt.u)
    annotation (Line(points={{-79,70},{-70,70},{
