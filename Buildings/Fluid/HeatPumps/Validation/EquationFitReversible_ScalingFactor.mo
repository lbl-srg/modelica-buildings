...
equation
  connect(heaPum.port_a2, sou.ports[1])
    annotation (Line(points={{10,44},{36,44}},           color={0,127,255}));
  connect(heaPum.port_b1, sin1.ports[1]) annotation (Line(points={{10,56},{20,56},
          {20,2},{40,2}},   color={0,127,255}));
  connect(heaPum.port_a1, loa.ports[1])
    annotation (Line(points={{-10,56},{-24,56},{-24,42},{-40,42}},
                                                         color={0,127,255}));
  connect(heaPum.port_b2, sin2.ports[1]) annotation (Line(points={{-10,44},{-20,
          44},{-20,2},{-40,2}},     color={0,127,255}));
  ...
  connect(TSou.y, sou1.T_in) annotation (Line(points={{79,-70},{70,-70},{70,-56},
          {62,-56}}, color={0,0,127}));
  connect(sou.T_in, sou1.T_in) annotation (Line(points={{58,48},{66,48},{66,-56},
          {62,-56}}, color={0,0,127}));
  connect(TLoa.y, loa1.T_in) annotation (Line(points={{-79,-70},{-72,-70},{-72,-56},
          {-62,-56}}, color={0,0,127}));
  connect(TLoa.y, loa.T_in) annotation (Line(points={{-79,-70},{-72,-70},{-72,46},
          {-62,46}}, color={0,0,127}));
  connect(loa1.ports[1], heaPum1.port_a1) annotation (Line(points={{-40,-60},{-26,
          -60},{-26,-48},{-10,-48}},
                               color={0,127,255}));
  connect(heaPum1.port_b1, sin1.ports[2]) annotation (Line(points={{10,-48},{20,
          -48},{20,-2},{40,-2}}, color={0,127,255}));
  connect(mLoa.y, loa.m_flow_in) annotation (Line(points={{-77,50},{-60,50}},
                         color={0,0,127}));
  connect(mSou.y, sou.m_flow_in)
    annotation (Line(points={{77,52},{56,52}},           color={0,0,127}));
  connect(TLoaEntHea.y, heaPum.TSet) annotation (Line(points={{-32,80},{-32,46},
          {-11.4,46},{-11.4,59}}, color={0,0,127}));
  connect(reaToInt.y, heaPum.uMod) annotation (Line(points={{-21,-84},{-16,-84},
          {-16,50},{-11,50}}, color={255,127,0}));
  connect(reaToInt.y, heaPum1.uMod) annotation (Line(points={{-21,-84},{-16,-84},
          {-16,-54},{-11,-54}}, color={255,127,0}));
  connect(reaToInt.u, uMod.y)
    annotation (Line(points={{-44,-84},{-51,-84}}, color={0,0,127}));
  connect(TLoaEntHea.y, heaPum1.TSet) annotation (Line(points={{-32,80},{-32,
          -45},{-11.4,-45}}, color={0,0,127}));
  connect(sou1.m_flow_in, mSou1.y)
    annotation (Line(points={{60,-52},{68,-52},{68,-40},{79,-40}},
                                                 color={0,0,127}));
  connect(loa1.m_flow_in, mLoa1.y)
    annotation (Line(points={{-60,-52},{-70,-52},{-70,-40},{-79,-40}},
                                                   color={0,0,127}));
...
<p>
The capacity of the scaled heat pump model is compared to the
