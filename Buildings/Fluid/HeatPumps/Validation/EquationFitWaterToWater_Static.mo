...
model EquationFitWaterToWater_Static "example"
 ...
 package Medium = Buildings.Media.Water "Medium model";
   ...
   Buildings.Fluid.HeatPumps.EquationFitWaterToWater heaPum(
    ...
    "Water to Water heatpump" annotation (Placement(transformation(extent={{38,-12},{58,8}})));
    ...
    parameter Data.EquationFitWaterToWater.Trane_Axiom_EXW240 per
       "Water to water heat pump performance data " annotation (Placement(transformation(extent={{74,24},{94,44}})));
    ...
    offset=20 + 273.15,
    startTime=0)
    "Condesner entering water temperature" annotation (Placement(transformation(extent={{-116,76},{-96,96}})));
    Controls.OBC.CDL.Continuous.Sources.Ramp TEva_ent(
    height=4,
    duration(displayUnit="h") = 14400,
    offset=12 + 273.15,
    startTime=0)
    "Evaporator entering water temperature" annotation (Placement(transformation(extent={{72,-96},{92,-76}})));
    FixedResistances.PressureDrop  res1(
    redeclare package Medium = Medium,
    m_flow_nominal=mCon_flow_nominal,
    dp_nominal=6000)
    "Flow resistance" annotation (Placement(transformation(extent={{66,66},{86,86}})));
    FixedResistances.PressureDrop  res2(
    redeclare package Medium = Medium,
    m_flow_nominal=mEva_flow_nominal,
    dp_nominal=6000)
    "Flow resistance" annotation (Placement(transformation(extent={{-4,-96},{16,-76}})));
    ...
    "Volume for cooling load" annotation (Placement(transformation(extent={{-54,-96},{-34,-76}})));
    Controls.OBC.CDL.Continuous.Sources.Ramp TSetCoo(
    height=4,
    duration(displayUnit="h") = 14400,
    offset=6 + 273.15,
    startTime=0) "Evaporator setpoint water temperature" annotation (Placement(transformation(extent={{-14,-62},{6,-42}})));
    Controls.OBC.CDL.Continuous.Sources.Ramp TSetHea(
    height=20,
    duration(displayUnit="h") = 14400,
    offset=40 + 273.15,
    startTime=0) "Condenser setpoint water temperature" annotation (Placement(transformation(extent={{-20,60},{0,80}})));
    Controls.OBC.CDL.Continuous.Sources.Ramp uMod(
    height=10,
    duration(displayUnit="h") = 14400,
    offset=-5,
    startTime=0) "HeatPump operational mode input signal" annotation (Placement(transformation(extent={{-120,-12},{-100,8}})));
...
equation
  ...
  connect(res2.port_b, heaPum.port_b2)
   annotation (Line(points={{16,-86},{30,
          -86},{30,-8},{38,-8}}, color={0,127,255}));
  connect(TSetCoo.y, heaPum.TEvaSet)
   annotation (Line(points={{7,-52},{22,-52},{
          22,-11},{36.6,-11}},color={0,0,127}));
  connect(uMod.y,greEquThr. u)
   annotation (Line(points={{-99,-2},{-94,-2},{-94,
  ...
  connect(addInt.y, heaPum.uMod)
   annotation (Line(points={{19,-2},{26,-2},{26,-2},{36.6,-2}},
                                               color={255,127,0}));
  connect(TEva_ent.y,evaPum. T_in)
   annotation (Line(points={{93,-86},{104,-86},
          {104,-53.4},{92.2,-53.4}},
                                   color={0,0,127}));
  connect(evaPum.ports[1], heaPum.port_a2)
   annotation (Line(points={{68,-49},{
          62,-49},{62,-8},{58,-8}}, color={0,127,255}));
  connect(heaPum.port_a1, conPum.ports[1])
   annotation (Line(points={{38,4},{30,
          4},{30,90},{-40,90}}, color={0,127,255}));
  connect(TSetHea.y, heaPum.TConSet)
   annotation (Line(points={{1,70},{22,70},{22,7},{36.6,7}},color={0,0,127}));
