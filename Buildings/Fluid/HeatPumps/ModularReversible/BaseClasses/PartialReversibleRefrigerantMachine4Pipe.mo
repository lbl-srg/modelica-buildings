...
partial model PartialReversibleRefrigerantMachine4Pipe
  ...
  replaceable parameter
    ...
    group="Safety control"),
      ...
      Placement(transformation(extent={{-118,-78},{-102,-62}})));
  ...
  //Condenser1
  parameter Modelica.Units.SI.Time tauCon1=30
    "Air coil heat transfer time constant at nominal flow"
    annotation (Dialog(tab="Air Coil", group="Dynamics"));
  parameter Modelica.Units.SI.TemperatureDifference dTCon1_nominal
    "Nominal temperature difference in air coil medium, used to calculate mass flow rate"
    annotation (Dialog(group="Nominal condition - Pressure losses"));
  parameter Modelica.Units.SI.MassFlowRate mCon1_flow_nominal
    "Nominal mass flow rate of the air coil medium"
    annotation (Dialog(group="Nominal condition - Pressure losses"));

  parameter Modelica.Units.SI.PressureDifference dpCon1_nominal(displayUnit="Pa")
    "Pressure drop at nominal mass flow rate"
    annotation (Dialog(group="Nominal condition - Pressure losses"));
  parameter Real deltaMCon1=0.1
    "Fraction of nominal mass flow rate where transition to turbulent occurs"
    annotation (Dialog(tab="Air Coil", group="Flow resistance"));
  parameter Boolean use_con1Cap=true
    "=true if using capacitor model for air coil heat loss estimation"
    annotation (Dialog(group="Heat Losses", tab="Air Coil"),
                                          choices(checkBox=true));
  parameter Modelica.Units.SI.HeatCapacity CCon1=0
    "Heat capacity of the air coil"
    annotation (Dialog(
      group="Heat Losses",
      tab="Air Coil",
      enable=use_conCap));
  parameter Modelica.Units.SI.ThermalConductance GCon1Out=0
    "Outer thermal conductance for air coil heat loss calculations"
    annotation (Dialog(
      group="Heat Losses",
      tab="Air Coil",
      enable=use_conCap));
  parameter Modelica.Units.SI.ThermalConductance GCon1Ins=0
    "Inner thermal conductance for air coil heat loss calculations"
    annotation (Dialog(
      group="Heat Losses",
      tab="Air Coil",
      enable=use_conCap));

  final parameter Modelica.Units.SI.Density rhoCon1=MediumCon1.density(staCon1_nominal)
    "Air coil medium density";
  final parameter Modelica.Units.SI.SpecificHeatCapacity cpCon1=
      MediumCon1.specificHeatCapacityCp(staCon1_nominal)
    "Air coil medium specific heat capacity";
...
//Assumptions
  ...
  parameter Boolean allowFlowReversalCon1=true
    "= false to simplify equations, assuming, but not enforcing, no flow reversal"
    annotation (Dialog(group="Condenser1", tab="Assumptions"));
...
//Initialization
  ...
  parameter Modelica.Media.Interfaces.Types.MassFraction XCon_start[MediumCon.nX]=
    ...
    parameter Modelica.Media.Interfaces.Types.AbsolutePressure pCon1_start=
      MediumCon1.p_default "Start value of pressure"
    annotation (Dialog(tab="Initialization", group="Condenser1"));
  parameter Modelica.Media.Interfaces.Types.Temperature TCon1_start=
    MediumCon1.T_default
    "Start value of temperature"
    annotation (Dialog(tab="Initialization", group="Condenser1"));
  parameter Modelica.Units.SI.Temperature TCon1Cap_start=MediumCon1.T_default
    "Initial temperature of heat capacity of condenser" annotation (Dialog(
      tab="Initialization",
      group="Condenser1",
      enable=use_con1Cap));
  parameter Modelica.Media.Interfaces.Types.MassFraction XCon1_start[MediumCon1.nX]=
     MediumCon1.X_default "Start value of mass fractions m_i/m"
    annotation (Dialog(tab="Initialization", group="Condenser1"));
...
//Advanced
  ...
  Modelica.Units.SI.HeatFlowRate Q3_flow = QCon1_flow
    "Heat transferred into the medium 3";
  ...
  Buildings.Fluid.HeatPumps.ModularReversible.BaseClasses.EvaporatorCondenserWithCapacity eva(
    ...
    Buildings.Fluid.HeatPumps.ModularReversible.BaseClasses.EvaporatorCondenserWithCapacity con1(
    redeclare final package Medium = MediumCon1,
    final allowFlowReversal=allowFlowReversalCon1,
    final m_flow_small=1E-4*abs(mCon1_flow_nominal),
    final show_T=show_T,
    ...
    final isCon=true,
    final C=CCon1,
    final TCap_start=TCon1Cap_start,
    final GOut=GCon1Out,
  ...
  Buildings.HeatTransfer.Sources.PrescribedTemperature varTOutEva if
       use_evaCap "Forces heat losses according to ambient temperature"
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-50,-130})));
  Buildings.HeatTransfer.Sources.PrescribedTemperature varTOutCon if
       use_conCap "Forces heat losses according to ambient temperature"
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-50,130})));
  Buildings.HeatTransfer.Sources.PrescribedTemperature varTOutCon1 if
       use_con1Cap "Forces heat losses according to ambient temperature"
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-50,152})));
  ...
  Modelica.Blocks.Interfaces.RealInput TEvaAmb(final unit="K", displayUnit="degC") if
       use_evaCap and not use_busConOnl
    "Ambient temperature on the evaporator side" annotation (Placement(
        ...
        origin={-152,-130}),iconTransformation(extent={{9,-9},{-9,9}},
          origin={-111,-91},
        rotation=180)));
  Modelica.Blocks.Interfaces.RealInput TConAmb(final unit="K", displayUnit="degC") if
       use_conCap and not use_busConOnl
    "Ambient temperature on the condenser side" annotation (Placement(
        ...
        rotation=0,
        origin={-111,71})));
  Modelica.Blocks.Interfaces.RealInput TCon1Amb(final unit="K", displayUnit="degC") if
       use_con1Cap and not use_busConOnl
    "Ambient temperature on the condenser side" annotation (Placement(
        transformation(
        extent={{-12,12},{12,-12}},
        rotation=0,
        origin={-154,148}),iconTransformation(
        extent={{-9,9},{9,-9}},
        rotation=0,
        origin={-111,89})));
  ...
  Buildings.Fluid.Sensors.MassFlowRate mCon_flow(final allowFlowReversal=
    ...
    "Mass flow sensor at the evaporator" annotation (Placement(transformation(
        ...
        extent={{-10,10},{10,-10}},
        rotation=0)));
  Buildings.Fluid.Sensors.MassFlowRate mCon1_flow(final allowFlowReversal=
        allowFlowReversalEva, redeclare final package Medium = MediumCon1)
    "Mass flow sensor at the evaporator" annotation (Placement(transformation(
        origin={80,46},
        extent={{-10,10},{10,-10}},
        rotation=-90)));
  ...
  RefrigerantCycleInertia refCycIneCon1 "Inertia model for condenser side"
      annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={52,-24})));
...
//@modelica_select_end
  ...
  Modelica.Blocks.Interfaces.RealOutput QCon1_flow(final quantity="HeatFlowRate",
      final unit="W") "Actual heating heat flow rate added to fluid 1"
...
//@modelica_select_end
  ...
  parameter MediumCon1.ThermodynamicState staCon1_nominal=MediumCon1.setState_pTX(
      T=MediumCon1.T_default, p=MediumCon1.p_default, X=MediumCon1.X_default)
      "Nominal state of condenser1 medium";
...
equation
  ...
  connect(safCtr.sigBus, sigBus) annotation (Line(
      points={{-111.917,-26.0833},{-111.917,-16},{-116,-16},{-116,-40},{-140,
          -40},{-140,-41},{-141,-41}},
      color={255,204,51},
      thickness=0.5));
  connect(safCtr.yOut, sigBus.yMea) annotation (Line(points={{-91.1667,-20},{
          -84,-20},{-84,-40},{-138,-40},{-138,-42},{-140,-42},{-140,-41},{-141,
          -41}},                                    color={0,0,127}));
  ...
  connect(mEva_flow.port_a, port_a2)
    annotation (Line(points={{50,-100},{100,-100}},
                                                  color={0,127,255}));
  connect(port_a1,mCon_flow. port_a)
    annotation (Line(points={{-100,100},{-68,100},{-68,100},{-60,100}},
                                                  color={0,127,255}));
  connect(mEva_flow.port_b, eva.port_a) annotation (Line(points={{30,-100},{20,-100}},
                              color={0,127,255}));
  ...
  connect(mEva_flow.m_flow, sigBus.mEvaMea_flow) annotation (Line(points={{40,-89},
          {40,-40},{26,-40},{26,-30},{-20,-30},{-20,-41},{-141,-41}},
                                                                color={0,0,127}),
      Text(
      string="%second",
      index=1,
  ...
  connect(refCyc.sigBus, sigBus) annotation (Line(
      ...
      index=1,
      extent={{6,3},{6,3}}));
  connect(refCyc.PEle, sigBus.PEleMea) annotation (Line(points={{19.89,-8.91},{
          26,-8.91},{26,-30},{-20,-30},{-20,-40},{-138,-40},{-138,-41},{-141,
          -41}},
        color={0,0,127}), Text(
      string="%second",
      index=1,
  ...
  connect(TConAmb, sigBus.TConAmbMea) annotation (Line(
      points={{-152,120},{-128,120},{-128,86},{-76,86},{-76,-6},{-78,-6},{-78,-41},
          {-141,-41}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  ...
  connect(con.T, sigBus.TConOutMea) annotation (Line(points={{22.4,90},{38,90},{
          38,32},{-76,32},{-76,-40},{-140,-40},{-140,-41},{-141,-41}},
                                                 color={0,0,127}));
  connect(senTConIn.y, sigBus.TConInMea) annotation (Line(points={{-87,60},{-76,
          60},{-76,-40},{-140,-40},{-140,-41},{-141,-41}},
                                                       color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-3,-6},{-3,-6}},
  ...
  connect(eva.T, sigBus.TEvaOutMea) annotation (Line(points={{-22.4,-90},{-76,-90},
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(senTEvaIn.y, sigBus.TEvaInMea) annotation (Line(points={{-87,40},{-76,
          40},{-76,-40},{-148,-40},{-148,-41},{-141,-41}},              color={0,
          0,127}));
  ...
  connect(ySet, sigBus.ySet) annotation (Line(points={{-152,20},{-120,20},{-120,
          -40},{-136,-40},{-136,-41},{-141,-41}},
                     color={0,0,127}));
  connect(refCyc.PEle, P) annotation (Line(points={{19.89,-8.91},{26,-8.91},{26,
          0},{150,0}},
                   color={0,0,127}));
  connect(refCycIneEva.y, QEva_flow) annotation (Line(points={{-1.9984e-15,-61},{-1.9984e-15,
          -68},{50,-68},{50,-130},{150,-130}}, color={0,0,127}));
  connect(mCon1_flow.port_a, port_a3)
    annotation (Line(points={{80,56},{80,88},{80,120},{81,120}},
                                                color={0,127,255}));
  connect(mCon1_flow.port_b, con1.port_a)
    annotation (Line(points={{80,36},{80,-20},{106,-20}},
                                                        color={0,127,255}));
  connect(con1.port_b, port_b3) annotation (Line(points={{106,-60},{106,-76},{
          80,-76},{80,-120}},
                           color={0,127,255}));
  connect(TCon1Amb, varTOutCon1.T) annotation (Line(
      points={{-154,148},{-110,148},{-110,152},{-62,152}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(varTOutCon1.port, con1.port_out) annotation (Line(points={{-40,152},{
          132,152},{132,-40},{126,-40}},
                                     color={191,0,0}));
  connect(TCon1Amb, sigBus.TConAmb1Mea) annotation (Line(
      points={{-154,148},{-126,148},{-126,-41},{-141,-41}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(refCycIneCon1.y, con1.Q_flow)
    annotation (Line(points={{63,-24},{68,-24},{68,-40},{82,-40}},
                                                 color={0,0,127}));
  connect(refCycIneCon1.y, QCon1_flow) annotation (Line(points={{63,-24},{72,-24},
          {72,20},{150,20}}, color={0,0,127}));
  connect(refCyc.QCon1_flow, refCycIneCon1.u) annotation (Line(points={{19.8,9},
          {32,9},{32,-24},{40,-24}}, color={0,0,127}));
  connect(QCon_flow, refCycIneCon.y) annotation (Line(points={{150,130},{122,
          130},{122,68},{0,68},{0,61}}, color={0,0,127}));
...
end PartialReversibleRefrigerantMachine4Pipe;
