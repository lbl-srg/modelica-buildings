...
protected
  final parameter Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TConEnt_nominal_degC=Modelica.SIunits.Conversions.to_degC(
    per.TConEnt_nominal)
...
initial equation
  ...
  Buildings.Fluid.Chillers.BaseClasses.warnIfPerformanceOutOfBounds(
    ...
    "per.capFunT");
equation
  TConEnt_degC=Modelica.SIunits.Conversions.to_degC(
    TConEnt);
  if on then
