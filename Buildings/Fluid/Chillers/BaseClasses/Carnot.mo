...
partial model Carnot
  ...
  Real yPL(
    ...
    if COP_is_for_cooling then
      QEva_flow/QEva_flow_nominal
    else
      QCon_flow/QCon_flow_nominal
    "Part load ratio";
  Real etaPL(
    final unit="1")=
    if evaluate_etaPL then
  ...
  Modelica.SIunits.Temperature TConAct(
    start=TCon_nominal+TAppCon_nominal)=Medium1.temperature(
    staB1)+QCon_flow/QCon_flow_nominal*TAppCon_nominal
  ...
  Modelica.SIunits.Temperature TEvaAct(
    start=TEva_nominal-TAppEva_nominal)=Medium2.temperature(
    staB2)-QEva_flow/QEva_flow_nominal*TAppEva_nominal
...
protected
  ...
  final parameter Boolean evaluate_etaPL=not((size(
    a,
    1) == 1 and abs(
    a[1]-1) < Modelica.Constants.eps))
  ...
  Modelica.SIunits.Temperature TUseAct=
    if COP_is_for_cooling then
      TEvaAct
    else
      TConAct
  ...
  final parameter Modelica.SIunits.SpecificHeatCapacity cp1_default=Medium1.specificHeatCapacityCp(
    Medium1.setState_pTX(
      p=Medium1.p_default,
      T=Medium1.T_default,
      X=Medium1.X_default))
    "Specific heat capacity of medium 1 at default medium state";
  final parameter Modelica.SIunits.SpecificHeatCapacity cp2_default=Medium2.specificHeatCapacityCp(
...
initial equation
  ...
  assert(
    etaCarnot_nominal_internal < 1,
    "Parameters lead to etaCarnot_nominal > 1. Check parameters.");
  assert(
