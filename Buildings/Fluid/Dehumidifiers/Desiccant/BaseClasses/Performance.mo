...
model Performance
  ...
  Modelica.Blocks.Interfaces.RealOutput VReg_flow(
    ...
    extent={{100,-50},{120,-30}}), iconTransformation(extent={{100,-50},{120,-30}})));
  Modelica.Blocks.Interfaces.RealOutput yQReg(
    final unit="1")
    "Regeneration heating output ratio" annotation (Placement(transformation(
    extent={{100,-90},{120,-70}}), iconTransformation(extent={{100,-90},{120,-70}})));
protected
...
equation
  if uRot then
    ...
    assert(X_w_ProEnt <= per.X_w_ProEnt_max and X_w_ProEnt >= per.X_w_ProEnt_min,
       ...
       the range that is defined in the performance curve.",
       level=AssertionLevel.error);
    VReg_flow = Buildings.Fluid.Dehumidifiers.Desiccant.BaseClasses.performanceCurve(
       TProEnt=TProEnt,
...
<ul>
<li>
If the dehumidification signal <code>uRot=true</code>,
<ul>
<li>
