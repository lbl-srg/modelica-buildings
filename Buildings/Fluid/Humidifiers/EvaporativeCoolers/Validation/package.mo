within Buildings.Fluid.Humidifiers.EvaporativeCoolers;
package Validation "Collection of models that validate the evaporative cooler models"

  extends Modelica.Icons.ExamplesPackage;

  annotation (Documentation(info="<html>
<p>
This package contains models that validate the models in 
<a href=\"modelica://Buildings.Fluid.Humidifiers.EvaporativeCoolers\">
Buildings.Fluid.Humidifiers.EvaporativeCoolers</a>. The examples plot various
outputs. These model outputs are stored as reference data to allow continuous
validation whenever models in the library change. </p>
</html>"));
end Validation;
