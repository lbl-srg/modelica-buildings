...
model Direct_DryWetSwitch
  ...
  Buildings.Fluid.Humidifiers.EvaporativePads.Direct dirEvaCoo1(
    redeclare final package Medium = MediumA,
    final padAre=0.6,
    redeclare Data.Generic per)
    "Direct evaporative cooler" annotation (Placement(
        transformation(origin={-50,0},
                                     extent={{-10,-10},{10,10}})));
  Buildings.Fluid.Humidifiers.EvaporativePads.Direct dirEvaCoo2(
    redeclare final package Medium = MediumA,
    final padAre=0.6,
    redeclare Buildings.Fluid.Humidifiers.EvaporativePads.Data.Generic per)
    "Direct evaporative cooler" annotation (Placement(transformation(origin={70,0},
                extent={{-10,-10},{10,10}})));
  Buildings.Fluid.Sources.Boundary_pT sin(
    redeclare final package Medium = MediumA,
    final nPorts=1)
    "Sink"
    annotation (Placement(transformation(origin={190,0}, extent={{10,-10},{-10,10}})));
  Buildings.Fluid.Sources.MassFlowSource_WeatherData sou(
    ...
    annotation (Placement(transformation(origin={-170,0},extent={{-10,-10},{10,10}})));


  Buildings.Controls.OBC.CDL.Logical.Sources.Pulse evaCooAct(
    width=0.5,
    period=86400,
    shift=0)      "Boolean pulse signal for active evaporative cooling"
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort senTemIn(
    ...
    final T_start=293.15) "Inlet air temperature sensor" annotation (Placement(
        transformation(origin={-130,0},extent={{-10,-10},{10,10}})));

  Buildings.Fluid.Sensors.TemperatureTwoPort senTemMid(
    redeclare final package Medium = MediumA,
    final initType=Modelica.Blocks.Types.Init.InitialOutput,
    final m_flow_nominal=m_flow_nominal,
    final T_start=293.15) "Middle air temperature sensor" annotation (Placement(
        transformation(origin={-10,0},extent={{-10,-10},{10,10}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senTemOut(
    redeclare final package Medium = MediumA,
    final initType=Modelica.Blocks.Types.Init.InitialOutput,
    final m_flow_nominal=m_flow_nominal,
    final T_start=293.15) "Outlet air temperature sensor" annotation (Placement(
        transformation(origin={110,0},extent={{-10,-10},{10,10}})));
  Buildings.Fluid.Sensors.MassFractionTwoPort senMasFraIn(redeclare final package Medium =
        MediumA, final m_flow_nominal=m_flow_nominal)
    "Inlet air water mass fraction sensor" annotation (Placement(transformation(
          origin={-90,0}, extent={{-10,-10},{10,10}})));
  Buildings.Fluid.Sensors.MassFractionTwoPort senMasFraMid(redeclare final package Medium =
        MediumA, final m_flow_nominal=m_flow_nominal)
    "Middle air water mass fraction sensor" annotation (Placement(
        transformation(origin={30,0}, extent={{-10,-10},{10,10}})));
  Buildings.Fluid.Sensors.MassFractionTwoPort senMasFraOut(redeclare final
      package Medium = MediumA, final m_flow_nominal=m_flow_nominal)
    "Outlet air water mass fraction sensor" annotation (Placement(
        transformation(origin={150,0}, extent={{-10,-10},{10,10}})));
equation
  ...
  connect(evaCooAct.y, dirEvaCoo2.evaCooAct) annotation (Line(points={{-158,-70},
          {50,-70},{50,-4},{61,-4}}, color={255,0,255}));
  ...
  connect(evaCooAct.y, dirEvaCoo1.evaCooAct) annotation (Line(points={{-158,-70},
          {-70,-70},{-70,-4},{-59,-4}}, color={255,0,255}));
  connect(weaDat.weaBus, sou.weaBus) annotation (Line(
      points={{-200,0},{-190,0},{-190,0.2},{-180,0.2}},
      color={255,204,51},
      thickness=0.5));
annotation (
