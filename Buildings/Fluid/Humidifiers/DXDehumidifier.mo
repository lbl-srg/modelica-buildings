...
model DXDehumidifier "DX dehumidifier"
  ...
  Buildings.Fluid.Sensors.MassFractionTwoPort senMasFra(
    redeclare package Medium = Medium,
    m_flow_nominal=mAir_flow_nominal) "Inlet air mass fraction"
  ...
  Buildings.Fluid.Sensors.RelativeHumidityTwoPort senRelHum(
    redeclare package Medium = Medium,
    m_flow_nominal=mAir_flow_nominal) "Inlet air relative humidity"
...
equation
  ...
  connect(PDeh.y, P)
    annotation (Line(points={{-59,-60},{40,-60},{40,-30},{120,-30}},
      color={0,0,127}));
  connect(u.y, deHum.u)
  ...
  connect(QHea.y, preHeaFlo.Q_flow)
    annotation (Line(points={{-71,50},{-20,50}}, color={0,0,127}));
  else
  connect(con.y, preHeaFlo.Q_flow) annotation (Line(points={{-38,80},{-30,80},{-30,
