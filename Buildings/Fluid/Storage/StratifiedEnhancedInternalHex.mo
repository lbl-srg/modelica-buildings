...
model StratifiedEnhancedInternalHex
  ...
  parameter Modelica.Fluid.Types.Dynamics energyDynamicsHexSolid=energyDynamicsHex
    ...
    annotation (Evaluate=true,Dialog(tab="Dynamics heat exchanger",group="Equations"));
  parameter Modelica.SIunits.Length lHex=rTan*abs(
    segHex_a-segHex_b)*Modelica.Constants.pi
...
protected
  ...
  final parameter Modelica.SIunits.Length dHHex=abs(
    hHex_a-hHex_b)
...
initial equation
  assert(
    hHex_a >= 0 and hHex_a <= hTan,
    "The parameter hHex_a is outside its valid range.");
  assert(
    hHex_b >= 0 and hHex_b <= hTan,
...
equation
  for j in 0:nSegHexTan-1 loop
    for i in 1:hexSegMult loop
      connect(indTanHex.port[j*hexSegMult+i],heaPorVol[segHex_a+(
        if hHex_a > hHex_b then
          j
        else
          -j)])
