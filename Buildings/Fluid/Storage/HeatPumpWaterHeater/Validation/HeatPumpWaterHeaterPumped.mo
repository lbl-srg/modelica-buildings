...
equation
  connect(heaPumWatHeaPum.port_b1, sin.ports[1])
    annotation (Line(points={{20,8},{32,8},{32,24},{50,24}},  color={0,127,255}));

  connect(TBCSid.port,heaPumWatHeaPum. heaPorSid)
    annotation (Line(points={{-36,-50},{16,-50},{16,2}},color={191,0,0}));

  connect(TBCTop.port,heaPumWatHeaPum. heaPorTop)
    annotation (Line(points={{-36,-68},{-6,-68},{-6,10},{10,10}},color={191,0,0}));
  ...
  connect(senTemIn.port_a,heaPumWatHeaPum. port_a2)
    annotation (Line(points={{28,-4},{20,-4}},color={0,127,255}));
  ...
  connect(heaPumWatHeaPum.port_a1, bouAir.ports[1])
    annotation (Line(points={{0,8},{-8,8},{-8,24},{-16,24}},color={0,127,255}));
  ...
  connect(datRea.y[8], toTotAirIn.XiDry)
      annotation (Line(points={{-89,70},{-82,70},{-82,20},{-71,20}},color={0,0,127}));
  ...
  connect(datRea.y[6], TEvaIn_K.Celsius)
    annotation (Line(points={{-89,70},{-80,70},{-80,49.6},{-72,49.6}},color={0,0,127}));

  connect(TEvaIn_K.Kelvin, bouAir.T_in)
    annotation (Line(points={{-49,49.8},{-48,49.8},{-48,50},{-44,50},{-44,28},{-38,28}}, color={0,0,127}));

  connect(TBCBot.port,heaPumWatHeaPum. heaPorBot)
    annotation (Line(points={{-36,-88},{-4,-88},{-4,-6},{10,-6}},color={191,0,0}));

  connect(TOut_K.Celsius, datRea.y[1])
    annotation (Line(points={{-80,-70.4},{-80,-70},{-84,-70},{-84,70},{-89,70}},color={0,0,127}));

  connect(TOut_K.Kelvin, TBCSid.T)
    annotation (Line(points={{-57,-70.2},{-58,-70.2},{-58,-70},{-54,-70},{-54,-50},{-49.2,-50}},color={0,0,127}));

  connect(TOut_K.Kelvin, TBCTop.T)
    annotation (Line(points={{-57,-70.2},{-54,-70.2},{-54,-68},{-49.2,-68}},color={0,0,127}));

  connect(TOut_K.Kelvin, TBCBot.T)
    annotation (Line(points={{-57,-70.2},{-58,
          -70.2},{-58,-70},{-54,-70},{-54,-88},{-49.2,-88}}, color={0,0,127}));

  connect(TWat_K.Kelvin, boundary.T_in)
    annotation (Line(points={{91,49.8},{112,49.8},{112,0},{102,0}},color={0,0,127}));


  connect(TWat_K.Celsius, datRea.y[2])
    annotation (Line(points={{68,49.6},{68,50},{62,50},{62,70},{-89,70}},color={0,0,127}));

  connect(boundary.m_flow_in, datRea.y[3])
    annotation (Line(points={{102,4},{110,4},{110,70},{-89,70}}, color={0,0,127}));

  connect(realToBoolean.y, heaPumWatHeaPum.on)
    annotation (Line(points={{-49,-30},{-14,-30},{-14,2},{-2,2}}, color={255,0,255}));

  connect(realToBoolean.u, datRea.y[7])
    annotation (Line(points={{-72,-30},{-84,-30},{-84,70},{-89,70}},color={0,0,127}));
