...
model ElectricalFollowing
  ...
  replaceable parameter Buildings.Fluid.CHPs.Data.Generic per annotation (
      choicesAllMatching=true, Placement(transformation(extent={{-78,-78},{-62,-62}})));
  ...
  Modelica.Blocks.Interfaces.RealInput PEleDem(unit="W")
    "Electric power demand" annotation (Placement(transformation(extent={{-140,130},
            {-100,170}}), iconTransformation(extent={{-140,20},{-100,60}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput mWatSet(unit="kg/s") if  per.coolingWaterControl
    "Water flow rate set point based on internal control" annotation (Placement(
        transformation(extent={{100,160},{140,200}}), iconTransformation(extent={{100,80},
            {120,100}})));
  Modelica.Blocks.Interfaces.RealOutput PCon(unit="W")
    "Power consumption during stand-by and cool-down modes" annotation (
      Placement(transformation(extent={{100,120},{140,160}}),iconTransformation(
          extent={{100,50},{120,70}})));
  Modelica.Blocks.Interfaces.RealOutput PEleNet(unit="W")
    "Electric power generation" annotation (Placement(transformation(extent={{100,80},
            {140,120}}),    iconTransformation(extent={{100,20},{120,40}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput QWat(unit="W")
    "Heat transfer to the water control volume" annotation (Placement(
        transformation(extent={{100,0},{140,40}}), iconTransformation(extent={{100,
            -90},{120,-70}})));
  CHPs.BaseClasses.Controller opeMod(per=per, waitTime=waitTime)
...
equation
  ...
  connect(mWat_flow.y, eneCon.mWat_flow)
    annotation (Line(points={{-79,100},{-74,100},{-74,86},{-52,86}},
                                                 color={0,0,127}));
  ...
  connect(conWat.mWatSet, mWatSet)
    annotation (Line(points={{95,180},{120,180}}, color={0,0,127}));
  ...
  connect(fil.PEleDem, PEleDem)
    annotation (Line(points={{-92,150},{-120,150}}, color={0,0,127}));
  ...
  connect(TWatOut.T, assWatTem.TWat)
    annotation (Line(points={{30,-50},{38,-50}}, color={0,0,127}));
  ...
  connect(QWat1.Q_flow, QWat)
    annotation (Line(points={{-10,20},{120,20}},
                                               color={0,0,127}));
  connect(powCon.opeMod, opeMod.opeMod) annotation (Line(points={{70,140},{56,140},
          {56,180},{51,180}}, color={0,127,0}));
  connect(powCon.PCon, PCon)
    annotation (Line(points={{93,140},{100,140},{100,138},{106,138},{106,140},{120,
          140}},                                  color={0,0,127}));
  connect(TRoo, QLos.port_b)
    annotation (Line(points={{-106,-40},{-90,-40}}, color={191,0,0}));
  connect(eng.TRoo, QLos.port_a) annotation (Line(points={{0,55.8},{-66,55.8},{-66,
          -40},{-70,-40}}, color={191,0,0}));
  connect(vol.heatPort, QWat1.port_b)
    annotation (Line(points={{-9,-10},{-20,-10},{-20,10}}, color={191,0,0}));
  connect(QWat1.port_a,eng.TWat)
    annotation (Line(points={{-20,30},{-20,44},{0,44}},  color={191,0,0}));
