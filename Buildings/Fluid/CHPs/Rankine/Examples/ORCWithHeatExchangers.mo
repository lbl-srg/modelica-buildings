...
/*  parameter Modelica.Units.SI.AbsoluteTemperature TWasHea(
  ...
  Buildings.Fluid.Sources.Boundary_pT sinEva(
    ...
    annotation (Placement(transformation(extent={{80,40},{60,60}})));
  Buildings.Fluid.CHPs.Rankine.BaseClasses.HeatInjector con(
    ...
    annotation (Placement(transformation(
        ...
        rotation=180,
        origin={0,-50})));
  Buildings.Fluid.Sources.Boundary_pT sinCon(redeclare final package Medium = MediumCon, nPorts
      =1) "Sink on the condenser side"
...
equation
  ...
  connect(sinEva.ports[1], eva.port_b)
    annotation (Line(points={{60,50},{10,50}}, color={0,127,255}));
  connect(cyc.port_b, con.heaPor) annotation (Line(points={{0,-10},{0,-27},{1.27676e-15,
          -27},{1.27676e-15,-44}}, color={191,0,0}));
  connect(sinCon.ports[1], con.port_b)
    annotation (Line(points={{-60,-50},{-10,-50}}, color={0,127,255}));
  connect(souCon.ports[1], con.port_a)
    annotation (Line(points={{60,-50},{10,-50}}, color={0,127,255}));
