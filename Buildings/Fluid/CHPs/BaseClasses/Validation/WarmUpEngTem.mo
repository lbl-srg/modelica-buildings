...
model WarmUpEngTem "Validate model WarmUp if warm-up by engine temperature"
  ...
  Modelica.Blocks.Sources.Ramp TEng(
    ...
    duration=360,
    offset=273.15,
    startTime=0) "Engine temperature"
    annotation (Placement(transformation(extent={{-80,60},{-60,80}})));
  Modelica.Blocks.Sources.BooleanTable runSig(startValue=true, table={600})
    "Plant run signal"
...
for defining the warm-up operating mode. The example is for the warm-up period dependent on the engine temperature (e.g. CHPs with Stirling engines). 
</p>
</html>", revisions="<html>
<ul>
<li>
