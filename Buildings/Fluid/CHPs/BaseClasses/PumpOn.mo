...
model PumpOn "Operating mode in which the water pump starts to run"
  ...
  Modelica.StateGraph.Step pumpOnState "Pump on state"
    annotation (Placement(transformation(extent={{-16,-14},{12,14}})));

equation
  connect(inPort, pumpOnState.inPort[1]) annotation (Line(points={{-160,0},
          {-17.4,0}}, color={0,0,0}));
...
CHP will transition from the pump-on mode to the warm-up mode after the time delay 
and if the water flow rate is higher than the minimum defined by the manufacturer.
</p>
</html>", revisions="<html>
<ul>
<li>
