...
protected
  Modelica.Blocks.Sources.BooleanExpression warUpEngTem(
    ...
    "Check if warm-up mode is active and warm-up is based on engine temperature"
...
equation
  connect(elePow.y, mWatIntCon.u1) annotation (Line(points={{-18,20},{0,20},{0,
          -14},{18,-14}},
                   color={0,0,127}));
  connect(maxElePow.y, elePow.u1) annotation (Line(points={{-58,60},{-50,60},{-50,
          28},{-42,28}}, color={0,0,127}));
  connect(mWatIntCon.u2, TWatIn) annotation (Line(points={{18,-26},{0,-26},{0,
          -60},{-120,-60}},
                       color={0,0,127}));
  connect(elePow.u3, PEle) annotation (Line(points={{-42,12},{-50,12},{-50,0},{
          -120,0}},  color={0,0,127}));
  connect(mWatIntCon.y, watFloSet.u3) annotation (Line(points={{41,-20},{50,-20},
          {50,12},{58,12}}, color={0,0,127}));
  connect(watFloSet.y, mWatSet_flow) annotation (Line(points={{82,20},{90,20},{
          90,0},{120,0}}, color={0,0,127}));
  connect(const.y, watFloSet.u1) annotation (Line(points={{42,60},{50,60},{50,
          28},{58,28}},
                    color={0,0,127}));
  connect(warUpEngTem.y, elePow.u2) annotation (Line(points={{-59,20},{-42,20}},
          color={255,0,255}));
  connect(offStaBy.y, watFloSet.u2) annotation (Line(points={{41,20},{58,20}},
          color={255,0,255}));
