...
model Empirical "Example for using the empirical air filter model"
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Sin vocSou(
    ...
    annotation(Placement(transformation(origin = {-150, -40}, extent = {{-10, -10}, {10, 10}})));
equation
  connect(senTraSubVOCIn.port_b, airFil.port_a) annotation(
    Line(points = {{-20, 0}, {0, 0}}, color = {0, 127, 255}));
  connect(senTraSubParIn.port_b, senTraSubVOCIn.port_a) annotation(
    Line(points = {{-50, 0}, {-40, 0}}, color = {0, 127, 255}));
  connect(sou.ports[1], senTraSubParIn.port_a) annotation(
    Line(points = {{-80, 0}, {-70, 0}}, color = {0, 127, 255}));
  connect(airFil.port_b, senTraSubParOut.port_a) annotation(
    Line(points = {{20, 0}, {40, 0}}, color = {0, 127, 255}));
  connect(senTraSubParOut.port_b, senTraSubVOCOut.port_a) annotation(
    Line(points = {{60, 0}, {70, 0}}, color = {0, 127, 255}));
  connect(senTraSubVOCOut.port_b, sin.ports[1]) annotation(
    Line(points = {{90, 0}, {110, 0}}, color = {0, 127, 255}));
  ...
  connect(parSou.y, sou.C_in[2]) annotation(
    Line(points = {{-138, 40}, {-120, 40}, {-120, -8}, {-102, -8}}, color = {0, 0, 127}));
  connect(airFil.yRep, airFil.uRep) annotation(
    Line(points = {{22, 8}, {30, 8}, {30, 16}, {-6, 16}, {-6, 6}, {-2, 6}}, color = {255, 0, 255}));
