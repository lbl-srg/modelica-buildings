...
model MassAccumulation "Mass of the contaminants capatured by the filter"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput mCon(
    ...
    annotation (Placement(transformation(origin = {0, -20}, extent = {{100, 0}, {140, 40}}),
...
protected
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant mStart(
    final k = mCon_start) "Constant for start value of contaminants" annotation(
  ...
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant mMax(
    final k = mCon_max) "Constant for maximum filter concentration" annotation(
...
equation
  connect(mStart.y, intWitRes.y_reset_in) annotation(
    Line(points = {{-58, -40}, {-40, -40}, {-40, -8}, {-22, -8}}, color = {0, 0, 127}));
  connect(notFul.u2, intWitRes.y) annotation(
    Line(points = {{18, 62}, {10, 62}, {10, 0}, {2, 0}}, color = {0, 0, 127}));
  connect(mMax.y, notFul.u1) annotation(
    Line(points = {{-18, 50}, {0, 50}, {0, 70}, {18, 70}}, color = {0, 0, 127}));
  connect(mulSum.y, intWitRes.u) annotation(
    Line(points = {{-58, 0}, {-22, 0}}, color = {0, 0, 127}));
  connect(mulSum.u, mCon_flow) annotation(
    Line(points = {{-82, 0}, {-120, 0}}, color = {0, 0, 127}));
  connect(notFul.y, ful.u) annotation(
    Line(points = {{42, 70}, {58, 70}}, color = {255, 0, 255}));
  connect(ful.y, yRep) annotation(
    Line(points = {{82, 70}, {120, 70}}, color = {255, 0, 255}));
  connect(edg.u, uRep) annotation(
    Line(points = {{-62, -80}, {-120, -80}}, color = {255, 0, 255}));
  connect(edg.y, intWitRes.trigger) annotation(
    Line(points = {{-38, -80}, {-10, -80}, {-10, -12}}, color = {255, 0, 255}));
  connect(intWitRes.y, mCon) annotation(
    Line(points = {{2, 0}, {120, 0}}, color = {0, 0, 127}));
