...
model MassAccumulation
  ...
  Modelica.Blocks.Interfaces.BooleanInput triRep
    "replacing the filter when trigger becomes true" annotation (Placement(
        transformation(
        extent={{20,-20},{-20,20}},
        rotation=180,
  ...
  Modelica.Blocks.Interfaces.RealInput mCon_flow
    "The contaminant mass flow rate" annotation (Placement(transformation(
        extent={{20,-20},{-20,20}},
        rotation=180,
        origin={-120,60}), iconTransformation(
        extent={{-20,-20},{20,20}},
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput mCon
    "mass of the contaminant held by the filter" annotation (Placement(
  ...
  Modelica.Blocks.Logical.Greater greater "check if the filter is full"
...
equation
  ...
  connect(assMes.u, greater.y)
    annotation (Line(points={{70,-40},{66,-40},{66,-38},{61,-38}},
                                                 color={255,0,255}));
