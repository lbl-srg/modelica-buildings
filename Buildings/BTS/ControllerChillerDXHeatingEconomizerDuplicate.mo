...
model ControllerChillerDXHeatingEconomizerDuplicate
  ...
  Air.Systems.SingleZone.VAV.BaseClasses.ControllerHeatingFan conSup(
    ...
    final TiFan=TiFan,
    final TdFan=TdFan,
    final minAirFlo=minAirFlo) "Heating coil and fan controller"
    annotation (Placement(transformation(extent={{-60,80},{-40,100}})));
  Air.Systems.SingleZone.VAV.BaseClasses.ControllerEconomizer conEco(
    final controllerTypeEco=controllerTypeEco,
    final kEco=kEco,
    final TiEco=TiEco,
    final TdEco=TdEco) "Economizer control"
...
equation
  ...
  connect(chiOnTRoo.y, orFan.u[3]) annotation (Line(points={{42,-70},{50,-70},{
          50,40},{34,40},{34,106.333},{38,106.333}}, color={255,0,255}));
  connect(yOutAirFra, con1.y)
    annotation (Line(points={{110,20},{92,20}}, color={0,0,127}));
  annotation (
  defaultComponentName="conChiDXHeaEco",
  Icon(graphics={Line(points={{-100,-100},{0,2},{-100,100}}, color=
              {0,0,0})}), Documentation(info="<html>
<p>
[fixme: for testing only]<br/>
Duplicate of Buildings.Air.Systems.SingleZone.VAV.BaseClasses.ControllerChillerDXHeatingEconomizer.
Replaced OA damper control signal with constant zero.
</p>
</html>", revisions="<html>
<ul>
<li>
Febuary 9, 2024 by Hongxiang Fu:<br/>
...
end ControllerChillerDXHeatingEconomizerDuplicate;
