within Buildings.Applications.DataCenters.DataHalls.CDUs.BaseClasses;
model PartialCDU "Partial model for a CDU"
  extends Buildings.Applications.DataCenters.DataHalls.CDUs.BaseClasses.PartialFourPortInterface(
    final mPla_flow_nominal=dat.mPla_flow_nominal,
    final mRac_flow_nominal=dat.mRac_flow_nominal);

  parameter Buildings.Applications.DataCenters.DataHalls.CDUs.LiquidToLiquid.Data.Generic_epsNTU dat
...
initial equation
  if checkMedia then
    ...
    assert(
      (dat.medPla == Buildings.Applications.DataCenters.DataHalls.Types.Media.Water and
        Modelica.Utilities.Strings.find(MediumPla.mediumName, "Water") > 0) or
      (dat.medPla == Buildings.Applications.DataCenters.DataHalls.Types.Media.EthyleneGlycol and
        Modelica.Utilities.Strings.find(MediumPla.mediumName, "EthyleneGlycol") > 0) or
      (dat.medPla == Buildings.Applications.DataCenters.DataHalls.Types.Media.PropyleneGlycol and
    ...
    assert(
      (dat.medRac == Buildings.Applications.DataCenters.DataHalls.Types.Media.Water and
        Modelica.Utilities.Strings.find(MediumRac.mediumName, "Water") > 0) or
      (dat.medRac == Buildings.Applications.DataCenters.DataHalls.Types.Media.EthyleneGlycol and
        Modelica.Utilities.Strings.find(MediumRac.mediumName, "EthyleneGlycol") > 0) or
      (dat.medRac == Buildings.Applications.DataCenters.DataHalls.Types.Media.PropyleneGlycol and
...
<p align=\"center\">
    <img src=\"modelica://Buildings/Resources/Images/Applications/DataCenters/DataHalls/CDUs/LiquidToLiquid/CDU_epsNTU.png\"
