...
model CouplingRCWetCoil "Example illustrating the coupling of a RC building model to a fluid loop"
  ...
  Buildings.Applications.DHC.Loads.Examples.BaseClasses.RCBuildingWetCoil bui(redeclare package Medium1 = Medium1)
  ...
  Buildings.Fluid.Sources.Boundary_pT sinHea(
    redeclare package Medium = Medium1,
    p=300000,
    T=bui.couHea.T1_b_nominal,
    nPorts=1) "Sink for heating water"
                                      annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={110,30})));
  Modelica.Blocks.Sources.RealExpression THeaInlVal(y=bui.couHea.T1_a_nominal)
