...
model CouplingTimeSeries_Hospital
  ...
  Buildings.Applications.DHC.Loads.Examples.BaseClasses.BuildingTimeSeriesModified
    bui(
    have_watHea=true,
    have_watCoo=true,
    filNam=
...
equation
  ...
  connect(bui.ports_bHeaWat[1], sinHeaWat.ports[1]) annotation (Line(points={{30,
          4},{60,4},{60,20},{120,20}}, color={0,127,255}));
  connect(bui.ports_aHeaWat[1], supHeaWat.ports[1]) annotation (Line(points={{10,
          4},{0,4},{0,40},{-40,40}}, color={0,127,255}));
  connect(supChiWat.ports[1], bui.ports_aChiWat[1]) annotation (Line(points={{-40,
          -20},{0,-20},{0,0},{10,0}}, color={0,127,255}));
  connect(bui.ports_bChiWat[1], sinChiWat.ports[1]) annotation (Line(points={{30,
          0},{60,0},{60,-20},{120,-20}}, color={0,127,255}));
annotation (
      Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-160,-140},{160,140}})),
      __Dymola_Commands(
file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DHC/Loads/Examples/CouplingTimeSeries_Hospital.mos"
     "Simulate and plot"),
      experiment(Tolerance=1e-6,
                 StopTime=86400),
Documentation(info="<html>
<p>
This example demonstartes the hydraluic connection between a hospital's heating and cooling loads
