...
model Terminal4PipesHeatPorts
  extends Buildings.Applications.DHC.Loads.BaseClasses.PartialTerminalUnit(
    ...
    final mHeaWat_flow_nominal=abs(QHea_flow_nominal/cp1Hea_nominal/(
        T_aHeaWat_nominal - T_bHeaWat_nominal)),
    final mChiWat_flow_nominal=abs(QCoo_flow_nominal/cp1Coo_nominal/(
  ...
  final parameter Modelica.SIunits.ThermalConductance CMin_nominal[nPorts1]=
    {mHeaWat_flow_nominal,mChiWat_flow_nominal} .* {cp1Hea_nominal,cp1Coo_nominal}
...
equation
  ...
  connect(TSetCoo, conTInd[2].u_s) annotation (Line(points={{-220,180},{-116,180},
          {-116,100},{-12,100}}, color={0,0,127}));
  connect(gaiFloNom1[1].y, mReqHeaWat_flow)
    annotation (Line(points={{182,100},{220,100}}, color={0,0,127}));
  connect(gaiFloNom1[2].y, mReqChiWat_flow) annotation (Line(points={{182,100},{192,
