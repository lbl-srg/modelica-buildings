...
model BuildingTimeSeriesModified
  ...
  Modelica.Blocks.Interfaces.RealOutput QReqHea_flow(
    ...
    "Heating load"
     annotation (Placement(transformation(extent={{300,20},{340,
...
protected
  ...
  parameter Modelica.SIunits.SpecificHeatCapacity cpHeaWat_nominal=
    Medium.specificHeatCapacityCp(
      Medium.setState_pTX(Medium.p_default, T_aHeaWat_nominal))
  ...
  parameter Modelica.SIunits.SpecificHeatCapacity cpChiWat_nominal=
    Medium.specificHeatCapacityCp(
      Medium.setState_pTX(Medium.p_default, T_aChiWat_nominal))
...
equation
  ...
  connect(maxTSet.y, terUniCoo.TSetCoo)
    annotation (Line(points={{-258,220},{0,220},{0,39.3333},{69.1667,39.3333}},
                                               color={0,0,127}));
  ...
  connect(loa.y[2], QReqHea_flow)
    annotation (Line(points={{21,0},{280,0},{280,
          40},{320,40}}, color={0,0,127}));
  connect(disFloHea.PPum, addPPum.u1)
    annotation (Line(points={{141,-78},{170,-78},
          {170,86},{220,86}}, color={0,0,127}));
  connect(disFloCoo.PPum, addPPum.u2)
    annotation (Line(points={{141,-268},{200,-268},
          {200,74},{220,74}},       color={0,0,127}));
  connect(addPPum.y, PPum)
    annotation (Line(points={{244,80},{320,80}}, color={0,0,127}));
  connect(noHea.y, addPPum.u1)
    annotation (Line(points={{102,130},{170,130},{170,
          86},{220,86}}, color={0,0,127}));
  connect(noCoo.y, addPPum.u2)
    annotation (Line(points={{102,88},{200,88},{200,74},
          {220,74}},     color={0,0,127}));
  connect(addPFan.y, PFan)
    annotation (Line(points={{244,130},{282,130},{282,120},{320,120}},
                                                   color={0,0,127}));
  connect(noHea.y, addPFan.u1)
    annotation (Line(points={{102,130},{200,130},{200,
          136},{220,136}}, color={0,0,127}));
  connect(noCoo.y, addPFan.u2)
    annotation (Line(points={{102,88},{200,88},{200,124},
          {220,124}},      color={0,0,127}));
  connect(terUniCoo.PFan, addPFan.u2)
    annotation (Line(points={{90.8333,36},{160,36},{160,124},{220,124}},
                                        color={0,0,127}));
  connect(terUniHea.PFan, addPFan.u1)
    annotation (Line(points={{90.8333,-24},{180,-24},{180,136},{220,136}},
                                         color={0,0,127}));
 annotation (
Documentation(info="
<html>
...
<code>QCoo_flow_nominal</code> respectively. The default value of <code>facScaHeac</code>
and <code>facScaCoo</code> is assigned to 1.
...
First implementation.
            ...
            {300,300}})));
end BuildingTimeSeriesModified;
