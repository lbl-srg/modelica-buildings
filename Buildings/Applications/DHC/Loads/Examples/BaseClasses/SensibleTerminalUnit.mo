...
model SensibleTerminalUnit
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort THexInlMes(
    ...
    final allowFlowReversal=allowFlowReversal) annotation (Placement(transformation(extent={{150,30},{130,50}})));
  Buildings.Fluid.HeatExchangers.DryCoilEffectivenessNTU hexHea(
    ...
    final allowFlowReversal2=allowFlowReversal) annotation (Placement(transformation(extent={{-80,20},{-60,0}})));
  ...
  Fluid.HeatExchangers.DryCoilEffectivenessNTU hexCoo(
    ...
    final allowFlowReversal1=allowFlowReversal,
    final allowFlowReversal2=allowFlowReversal) annotation (Placement(transformation(extent={{0,20},{20,0}})));
...
equation
  connect(THexInlMes.T, conPIDMinT.u_m)
    annotation (Line(points={{140,51},{140,180},{-30,180},{-30,188}}, color={0,0,127}));
  connect(m_flow2.y, fan.m_flow_in) annotation (Line(points={{-19,104},{80,104},{80,52}}, color={0,0,127}));
  connect(fan.P, PFanPum) annotation (Line(points={{69,49},{60,49},{60,130},{220,130}}, color={0,0,127}));
  connect(ports_a1[1], hexHea.port_a1)
    annotation (Line(points={{-200,-220},{-100,-220},{-100,4},{-80,4}}, color={0,127,255}));
  ...
  connect(hexCoo.port_b2, hexHea.port_a2) annotation (Line(points={{0,16},{-60,16}},   color={0,127,255}));
  connect(hexHea.port_b1, ports_b1[1])
    annotation (Line(points={{-60,4},{-40,4},{-40,-220},{200,-220}}, color={0,127,255}));
  connect(ports_a1[2], hexCoo.port_a1)
    annotation (Line(points={{-200,-180},{-20,-180},{-20,4},{0,4}},
                                                            color={0,127,255}));
  connect(hexCoo.port_b1, ports_b1[2])
    annotation (Line(points={{20,4},{40,4},{40,-180},{200,-180}},
                                                               color={0,127,255}));
  connect(hexHea.port_b2, port_b2)
    annotation (Line(points={{-80,16},{-100,16},{-100,0},{-200,0}},   color={0,127,255}));
  ...
  connect(THexInlMes.T, conPIDMaxT.u_m)
    annotation (Line(points={{140,51},{140,140},{-30,140},{-30,148}}, color={0,0,127}));
  connect(fan.port_a, THexInlMes.port_b) annotation (Line(points={{90,40},{130,40}}, color={0,127,255}));
  connect(port_a2, THexInlMes.port_a) annotation (Line(points={{200,0},{176,0},{176,40},{150,40}}, color={0,127,255}));
  connect(fan.port_b, hexCoo.port_a2) annotation (Line(points={{70,40},{40,40},{40,16},{20,16}},color={0,127,255}));
  connect(uSet[1], conPIDMinT.u_s)
    annotation (Line(points={{-220,200},{-132,200},{-132,200},{-42,200}}, color={0,0,127}));
