...
model Terminal4PipesFluidPorts
  ...
  Buildings.Controls.OBC.CDL.Continuous.LimPID conTInd[nPorts1](
    ...
    each yMin=0)
    "PI controller tracking indoor temperature"
  ...
  Buildings.Controls.OBC.CDL.Routing.RealReplicator T2InlVec(nout=nPorts1)
    annotation (Placement(transformation(
        ...
        origin={120,110})));
equation
  connect(fan.P, PFanPum)
    annotation (Line(points={{69,9},{60,9},{60,140},{220,140}},   color={0,0,127}));
  connect(ports_a1[1], hexHea.port_a1)
    annotation (Line(points={{-200,-220},{-100,-220},{-100,-12},{-80,-12}},
                                                                        color={0,127,255}));
  connect(gaiFloNom1.y, m_flow1Req)
    annotation (Line(points={{184,220},{220,220}},
                                color={0,0,127}));
  ...
  connect(gaiFloNom2.u, sigFlo2.y)
    annotation (Line(points={{18,170},{10,170},{0,170},{-18,170}},
                                                color={0,0,127}));
  connect(gaiFloNom2.y, fan.m_flow_in)
    annotation (Line(points={{42,170},{80,170},{80,12}},
                                                       color={0,0,127}));
  connect(THexInlMes.T, T2InlVec.u)
    annotation (Line(points={{120,11},{120,98}}, color={0,0,127}));
  connect(T2InlVec.y, conTInd.u_m)
    annotation (Line(points={{120,122},{120,200},
  ...
  connect(Q_flowHea.y, Q_flow2Act[1])
    annotation (Line(points={{181,180},{200,
          180},{200,170},{220,170}}, color={0,0,127}));
  connect(Q_flowCoo.y, Q_flow2Act[2])
    annotation (Line(points={{181,160},{192,
