...
model CoolingPlant "District cooling plant model"
  ...
  parameter Integer numChi(min=1, max=2)=2 "Number of chillers";
  ...
  parameter Modelica.SIunits.Temperature TCHWSet "Chilled water setpoint"
    annotation (Dialog(group="Control Settings"));
  ...
  Modelica.Blocks.Sources.RealExpression mSet_flow(
    y=if chiOn[numChi] then numChi*mMin_flow else mMin_flow)
    "Chilled water bypass valve mass flow rate"
...
equation
  ...
  connect(valByp.port_b, port_a)
    annotation (Line(points={{80,10},{80,50},{140,50}}, color={0,127,255}));
